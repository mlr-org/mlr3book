...
The former is the heart of the `Tuner`.
It must fulfill the following requirements:

- Run the `PerformanceEvaluator` for the parameter value `x`.
- Evaluate the `Learner` using the given `Resampling` object and add the `ResampleResult` to the `BenchmarkResult` object of the `PerformanceEvaluator`.
...
In this example we replace the private `.$tune_step()` method from `mlr3tuning::TunerGenSA` with our new objective function which we defined above.
