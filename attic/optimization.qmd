...
We will examine the optimization of a simple classification tree on the `r ref("mlr_tasks_pima")` data set as an introductory example here.
...
Information on what they do can be found in the documentation of the learner.
...
For this, we need to choose a `r ref("Resampling")` and a `r ref("Measure")`.
...
`r mlr3book::mlr_pkg("mlr3")` allows to specify complex termination criteria by selecting one of the available `r ref("Terminator")`s:
...
The tuning process is identical to the previous example, however, this time we will specify two `r ref("Measure")`s, classification error and time to train the model (`time_train`).
...
See the [in-depth `r mlr_pkg("paradox")` chapter](#paradox) for more details on using `r ref("ParamSet")`s to  define parameter ranges for use-cases besides tuning.
