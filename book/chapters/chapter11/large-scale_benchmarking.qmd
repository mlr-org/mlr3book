...
OpenML's design ensures that all data on the platform is 'FAIR' (**F**indability, **A**ccessibility, **I**nteroperability and **R**eusability), which ensures the data is easily discoverable and reusable.
...
The single algorithm in the registry is because `r ref_pkg("mlr3batchmark")` specifies a single algorithm that is parametrized with the learner IDs.
...
For this example, we will use `r ref("batchtools::chunk()", aside = TRUE)` to `r index('chunk')` the jobs such that five iterations of one resample experiment are run sequentially in one computational job -- in practice the optimal grouping will be highly dependent on your experiment (@sec-parallelization).
...
When calling `r ref("mlr3batchmark::batchmark()")` again, the new experiments will be added to the registry on top of the existing jobs.

```{r}
#| cache: false
#| output: false
extra_design = benchmark_grid(lrns("classif.debug", error_train = 0.5),
  tasks, resamplings, paired = TRUE)
