...
If we want to access the actual data, then accessing `$data` will download the data, import it into R, and then store the `data.frame` in the `OMLData` object:
...
In this example we will set the number of CPUs per job to 1, the walltime (time limit before jobs stopped by scheduler) to 1 hour (3600 seconds), and the RAM limit (memory limit before jobs stopped by scheduler) to 8000 megabytes.
