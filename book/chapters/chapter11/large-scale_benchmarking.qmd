...
We also set a featureless baseline as a fallback learner (@sec-fallback) and set `"evaluate"` as our encapsulation method, which logs errors/warnings to an external file that can be read by `batchtools` (we will return to this in @sec-batchtools-monitoring).
...
lrn_lr$encapsulate = c(train = "evaluate", predict = "evaluate")
...
lrn_rf$encapsulate = c(train = "evaluate", predict = "evaluate")
...
Note that `odatasets()` returns a `data.table` with many more meta-features than shown here; this table can itself be used to filter further.
...
Note that this differs from `mlr3` `Task` objects, also contain information about the target, but not about the resampling procedure.
...
We can either use the `list_oml_task()` argument to request the number of classes to be `2`, or we can make use of the fact that the result of `list_oml_tasks()` is a `data.table` and subset the resulting table.
