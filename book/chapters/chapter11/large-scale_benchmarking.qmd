...
# create a lost of created learners
...
adult_tasks = list_oml_tasks(data_id = 1590, limit = 5)
```
...
We now define the tasks, resamplings, and learners for the experiment.
...
Note that by default the "Interactive" cluster function (see `r ref("batchtools::makeClusterFunctionsInteractive()")`) is used - this is the abstraction for the scheduling system, and "interactive" here means to not use a real scheduler but instead to use the interactive R session for computation.
...
However, results depend on the data generating process or data selection critia.
Be aware that you might get the opposite picture for, e.g., data from a specific domain.
