...
if (dir.exists(here::here("experiments"))) {
  unlink(here::here("experiments"), recursive = TRUE)
}
```
...
```{r include = FALSE}
if (dir.exists(here::here("experiments-custom"))) {
    unlink(here::here("experiments-custom"), recursive = TRUE)
}
```

```{r large_benchmarking-046, message = FALSE}
reg = makeExperimentRegistry(
  file.dir = here("experiments-custom"),
  seed = 1,
  packages = "mlr3verse"
)
```
