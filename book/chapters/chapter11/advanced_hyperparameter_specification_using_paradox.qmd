...
Another way to get this effect, using `$extra-trafo` during construction, would be:
```{r}
psexp = ps(
  par = p_dbl(0, 1),
...
```
It is also possible to set `$extra-trafo` after construction of the `ParamSet`-object.
...
## Defining a Tuning Spaces {#sec-paradox-tuning-spaces}
