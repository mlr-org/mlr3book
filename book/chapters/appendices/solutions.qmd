...
1. Manually `$train()` a lightgbm classifier from `r ref_pkg("mlr3extralearners")` on the pima task using $1/3$ of the training data for validation.
   Because the pima task has missing values, select some a methdo from `r ref_pkg("mlr3pipelines")` to impute them.
   Explicitly set the evaluation metric to logloss, the maximum number of boosting iterations to 1000, the patience parameter to 10, and the step size to 0.01.
   After training the learner show the final validation scores as well as the early stopped number of iterations.
...
Belowm we see that the task has three features with missing values.

```{r}
tsk_pima$missings()
```

Next, we create the lightgbm classifier, but we don't specify the validation data yet.

```{r}
# don't specify the validation data yet!
...
This sets `$validate` for `lrn_lgbm` to `"predefined"` and for the `glrn` itself to `0.3`.

```{r}
set_validate(glrn, validate = 0.3, ids = "classif.lightgbm")
glrn$validate
glrn$pipeops$classif.lightgbm$validate
```

Finally, we train the learner and inspect the validation scores and internally tuned values.
...
1. Wrap the learner from exercise 1) in an `AutoTuner` using a three-fold CV for the tuning.
   ...
   As the tuning measure us the internal validation metric.
...
Note that the input to the aggregation function is a list of integer values (the early stopped values for the different resampling iterations), so we need to first convert it into an atomic vector before taking the maximum. 
...
Because we use the internal validation scores for tuning, no predictions are required, so we can set the learner's predict sets to `NULL`.
...
Now we change the validation data from `0.3` to `"test"` where we can omit specifying `ids` as lightgbm is the base learner.

```{r}
set_validate(glrn, validate = "test")
```

Next we create the auto tuner using the configuration given in the instructions.
Because the internal validation measures are computed by lightgbm and not `mlr3` we need to specify whether the metric should be minimized.
...
at_lgbm = auto_tuner(
  ...
  tuner = tnr("internal"),
  resampling = rsmp("cv", folds = 3),
  measure = msr("internal_valid_score", select = "binary_logloss", minimize = TRUE)
)
at$id = "at_lgbm"
```

Finally, we set up the benchmark design, run it and evaluate it using classification accuracy, where the autotuner performs better than the decision tree.
...
The outer resampling already removes 20 observations from the data so only 80 datapoints are left for the inner resampling.
Then, 16 (0.2 * 20; the test set of the inner holdout resampling) observations are used to evaluate the hyperparameter configurations.
This leaves 64 (80 - 16) observations for training.
For XGBoost, the 16 observations that comprise the inner test set are also used for validation.
Therefore, both the random forest and the XGBoost model use 64 observations for training.
...
This is because during the final model fit no validation data is present.

1. How would the answers to the last two questions change if we had set the `$validate` field of the graph-learner to `0.25` instead of `"test"`?

In this case, the validation data is **not** identical to the test set of the inner resampling anymore.
Instead, it is split off the 64 observations that comprise the inner training set.
Because this happens before the task enters the `GraphLearner`, both the XGBoost model *and* the random forest only get access to 48 ((1 - 0.25) * 64) observations and the remaining 16 are used to create the validation data.
Note that the random forest will ignore the validation data, as it does not have the `"validation"` property and hence cannot make use of it.
Furthermore, the autotuner would now use a different validation set to tune and the boosting iterations.
Therefore, we would usually recommend setting the validation data to `"test"` when used in an autotuner, unless there are specific reasons not to.

4. Consider the (erroring) code below:
...
Can you explain *why* the code errors?
Hint: Should the data that xgboost uses for validation be preprocessed according to the *train* or *predict* logic?

If we would set the `$validate` field of the xgboost classifier to `0.3`, the validation data would be created from the (primary) task of `PipeOpOpPCA`.
However, this data was preprocessed using the the train logic.
Because validation performance is supposed to measure how well a model would perform during prediction, the validation should be preprocessed according to the predict logic.
Therefore it is not possible to set the `$validate` field of `PipeOp`s to values other than `"predefined"` or `NULL`.
