...
1. Train a classification model with the `classif.rpart` learner on the synthetic "diabetes" dataset.
  Do this without using `tsk("diabetes")`, and instead by constructing a task from the data frame that ships with `mlr3`: `readRDS(system.file("extdata", "diabetes.rds", package = "mlr3"))`.
...
diabetes = readRDS(system.file("extdata", "diabetes.rds", package = "mlr3"))
task = as_task_classif(diabetes, target = "diabetes", positive = "pos")
...
With a false negative rate of 0.9, we miss a lot of people who have diabetes but are predicted to not have it.
