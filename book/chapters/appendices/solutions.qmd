...
my_roc_plot = function(task, learner, train_indices, test_indices) {
  ...
  # Set grid of probabilities
  prob_grid = seq(0, 1, 0.001)

  # For each possible threshold, calculate TPR, FPR + aggregate to data.table
  grid = data.table::rbindlist(lapply(prob_grid, \(thresh) {
    ...
    data.table::data.table(
      ...
      tpr = mlr3measures::tpr(truth = pred$truth, response = pred$response, positive = task$positive),
      # x axis == 1 - specificity == 1 - TNR == FPR
      fpr = mlr3measures::fpr(truth = pred$truth, response = pred$response, positive = task$positive)
    )
  }))
...
# Compare with prebuilt version in mlr3:
