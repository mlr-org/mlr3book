...
2. Use `tsk("spam")` and five-fold CV to benchmark `lrn("classif.ranger")`, `lrn("classif.log_reg")`, and `lrn("classif.xgboost")` with respect to AUC.
...
First we set a seed, instantiate our learners with their initial parameters, setting the `predict_type = "prob"` once for all of them. 
...
```{r solutions-007a, warning=FALSE}
...
In this example, `ranger` outperforms logistic regression, and both outperform XGBoost.
...
Given that we used all three learners with their initial parameters, we should not assume these results to be representative.
While logistic regression does not have any hyperparameters to tune, and Random Forest learners generally perform well without much tuning, the XGBoost learner does always require tuning to achieve good results.
Tuning is even more important for this particular learner, as the `nrounds` parameter is initialized to 1, which effectively reduces this learner to a single decision tree.
