...
instance$archive$data[, iteration := seq_len(.N)]
instance$archive$data[, best_rmse := cummin(regr.rmse)]
instance$archive$data[, method := "Random Search"]
...
plot_data = rbind(instance$archive$data[, c("iteration", "best_rmse", "method")],
...
plot_data_sampling = rbind(
  instance$archive$data[, c("mtry", "sample.fraction", "num.trees", "iteration", "method")],
  instance_mbo$archive$data[, c("mtry", "sample.fraction", "num.trees", "iteration", "method")])

ggplot(aes(x = num.trees, y = sample.fraction, shape = method, colour = iteration), data = plot_data_sampling) +
  scale_colour_gradient(low = "black", high = "darkgrey") +
  geom_point(size = 3) +
  labs(colour = "Number of Evaluations", shape = "Method") +
  facet_wrap(~ mtry, nrow = 2, ncol = 5, labeller = "label_both") +
  theme_minimal() +
...
optimizer = opt("mbo",
  ...
  surrogate = surrogate, acq_function =
  acq_function,
  acq_optimizer = acq_optimizer
)
