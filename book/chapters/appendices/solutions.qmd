...
instance_mbo$archive$data[, iteration := seq_len(.N)]
instance_mbo$archive$data[, best_rmse := cummin(regr.rmse)]
instance_mbo$archive$data[, method := "BO"]

plot_data = rbind(instance_rs$archive$data[, c("iteration", "best_rmse", "method")],
  instance_mbo$archive$data[, c("iteration", "best_rmse", "method")])
...
plot_data_sampling = rbind(
  instance_rs$archive$data[, c("mtry", "sample.fraction", "num.trees", "iteration", "method")],
  instance_mbo$archive$data[, c("mtry", "sample.fraction", "num.trees", "iteration", "method")])

ggplot(aes(x = num.trees, y = sample.fraction, colour = method, label = mtry), data = plot_data_sampling) +
  scale_colour_manual(values = viridis(2, end = 0.8)) +
  geom_point(size = 0) +
  geom_text() +
...
This is also the region where the final result returned by the BO is located.
...
As expected, we observe that the BO algorithm witht he Gaussian Process surrogate appears to outperform the random forest surrogate counterpart.
However, ideally we would replicate running each version using different random seeds and visualize the average performance along with some dispersion measure to properly take randomness of the optimization process into account.
...
We will use the same acquisition function optimizer as earlier.
