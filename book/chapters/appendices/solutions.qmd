...
3. Apply a backward selection to the `penguins` task with `lrn("classif.rpart")` and holdout resampling by the measure classification accuracy. Compare the results with those in @sec-fs-wrapper-example.
...
4. Automate the feature selection as in @sec-autofselect with the `sonar` task and `lrn("classif.log_reg")`.
