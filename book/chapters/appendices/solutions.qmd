...
We can instantiate a temporary resampling on the task to illustrate how it assigns observations across the 5 repeats (`rep`) and 3 folds:
...
Note that `resample()` automatically instantiates the resampling for us, so we pass an new object to it that has not been instantiated yet:
...
Now we can `$score()` the resampling and visualize the results, which we can do either manually or using `autoplot()`:
...
ggplot(scores, aes(y = regr.mse)) +
  geom_boxplot() +
  scale_x_continuous(breaks = 0, labels = NULL) +
  theme_minimal()
```

The `autoplot()` function provides some defaults for plots.  
Note that it internally scores the resampling using the MSE for regression:
...
First we set a seed, instantiate our learners with their initial parameters, setting the `predict_type = "prob"` once for all of them and setting the `nrounds` parameter for XGBoost to 100.
We also construct a resampling object for 5-fold CV:
...
We could have alternatively instantiated the learners like this, but would have to repeat the `predict_type = "prop"` argument multiple times.
...
Next we can construct a benchmark design grid:
...
In this example, XGBoost outperforms `ranger`, and both outperform logistic regression.
...
The overall trend remains the same, however.
While logistic regression does not have any hyperparameters to tune, and Random Forest learners generally perform well without much tuning, the XGBoost learner does generally require tuning to achieve good results.
Here, we applied ranger with its initial parameters and only evaluated one configuration for XGBoost, so we can not make any broad generalizations based on these results.
Tuning is even more important for XGBoost, as the `nrounds` parameter is initialized to 1, which effectively reduces this learner to a single decision tree.
See @sec-optimization for more details on hyperparameter tuning.
...
We try our function using the `sonar` task and the `ranger` learner with 100 trees.
