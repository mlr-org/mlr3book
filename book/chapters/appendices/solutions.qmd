...
Note that `resample()` automatically instantiates the resampling for us, so we pass a new resampling to it that has not been instantiated yet:
...
We could manually calculate these scores of course, as `rr` contains all the individual predictions.
The `$predictions()` method contains a list of predictions for each iteration, which we can use to calculate the MSE for the first iteration like this:
...
Alternatively, the `autoplot()` function provides some defaults for plots for the `ResampleResult` object.  
