...
The rules can easily be translated to R code where we first convert the truth vector into 0-1 encoded matrix, calculate the absolute different with the predicted probabilities, and then average the results.

```{r solutions-023}
mae_prob = function(truth, prob) {
  # convert truth to 0-1 encoded matrix
  truth_mat = model.matrix(~ truth - 1)
  # set the column names correctly
  colnames(truth_mat) = levels(truth)
  # ensure same order of columns and
  prob = prob[, levels(truth)]
  # average the mean absolute difference
  mean(abs(truth_mat - prob))
}
```
...
MeasureMaeProb = R6::R6Class("MeasureMaeProb",
  ...
  private = list(
    .score = function(prediction, ...) { # define score as private method
      # call loss function
      mae_prob(prediction$truth, prediction$prob)
    }
  )
)
```
...
To try this measure, we resample a logistic regression on the sonar task using 3-fold cross-validation.

```{r}

learner = lrn("classif.log_reg", predict_type = "prob") # otherwise our meausre does not work
task = tsk("sonar")
...
```{r}
rr$score(list(msr_mae_prob, msr("classif.acc")))
```

In this case, there is a clear relationship between the accuracy and the mean absolute error of the probabilities.
...
We proceed with tuning the classification accuracy of the learner on the iris task.
...
The evaluations with a higher `error_train` value tend to have a higher classification accuracy.

```{r}
archive[, .(accuracy = mean(classif.acc)), by = errors]
```


While this might be surprising at first, the explanation is that the fallback learner `lrn("classif.rpart")` is superior to the debug learner that simply predicts a random label.
This means that calculating the best hyperparameter configuration will tend to have high values for the `error_train` parameter.
The evaluated parameter configurations therefore not only have a direct influence through controlling the primary learner's behavior, but also an indirect effect through affecting the primary learner's error probability during training.
...
The effect is now more subtle, but still present.

We can learn various things from these experiments.
First, the relationship between hyperparameters and performance measures can be diluted by errors.
While this might be less of a problem for tuning methods like random or grid search, this can be a problem for more sophisticated methods like Bayesian optimization that rely on this relationship to propose new configurations.
Second, the fallback learner should be chosen carefully and tend to give worse results than the learner to tune.
Third, it is always a good idea to inspect the tuning archive after the optimization has finished.
