...
1. Train a classification model with the `classif.rpart` learner on the synthetic "diabetes" dataset.
  Do this without using `tsk("diabetes")`, and instead by constructing a task from the data set in the `mlbench`-package: `data("SynthDiabetes2", package = "mlbench")`.
...
task = as_task_classif(SynthDiabetes2, target = "diabetes", positive = "pos")
...
With a false negative rate of 0.37, we miss a lot of people who have diabetes but are predicted to not have it.
...
```{r solutions-052}
graph_learner$train(tsk("diabetes"))
...
# apply the graph to the diabetes task
graph_result = graph$train(tsk("diabetes"))

# we get the following features
graph_result[[1]]$feature_names

# Compare with feature-columns of tsk("diabetes") with missing values:
selector_missing()(tsk("diabetes"))
```
...
4. We choose to use the following options for imputation, factor encoding, and model training.
  We pass the `PipeOp`s as named lists, because `ppl("branch")` uses the list names as values of the branch selection hyperparameters.
  `lrns()` already returns a named list of `Learner` objects.
```{r solutions-062}
imputing = list(imputeoor = po("imputeoor"), imputesample = po("imputesample"))

factor_encoding = list(encode = po("encode"), encodeimpact = po("encodeimpact"))
...
1. Manually `$train()` a LightGBM classifier from `r ref_pkg("mlr3extralearners")` on the diabetes task using $1/3$ of the training data for validation.
   As the diabetes task has missing values, select a method from `r ref_pkg("mlr3pipelines")` to impute them.
