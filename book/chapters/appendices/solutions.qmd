...
```{r}
#| include: false
path_automl_suite = normalizePath(file.path("..", "..", "openml", "manual", "automl_suite.rds"))
```
...
```{r}
list_oml_data(automl_suite$data_ids)
```

The `r ref("mlr3oml::list_oml_tasks()")` allows us to filter the tasks by their characteristics.

```{r}
#| include: false
path_automl_table = normalizePath(file.path("..", "..", "openml", "manual", "automl_table.rds"))
```
...
The resulting table contains matching tasks from the AutoML benchmark suite.
