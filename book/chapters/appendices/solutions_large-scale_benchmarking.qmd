...
#| include: false
path_data_tbl = here::here("book", "openml", "manual", "data_tbl.rds")
```

```{r solutions_large-scale_benchmarking-006, eval = !file.exists(path_data_tbl)}
data_tbl = list_oml_data(automl_suite$data_ids)
```

```{r solutions_large-scale_benchmarking-007}
#| include: false
if (file.exists(path_data_tbl)) {
  data_tbl = readRDS(path_data_tbl)
} else {
  saveRDS(data_tbl, path_data_tbl)
}
```

```{r solutions_large-scale_benchmarking-008}
...
```{r solutions_large-scale_benchmarking-011, eval = !file.exists(path_automl_table)}
...
```{r solutions_large-scale_benchmarking-020}
...
```{r solutions_large-scale_benchmarking-021}
autoplot(bma)
```
...
```{r solutions_large-scale_benchmarking-022}
bma$friedman_test()
```
