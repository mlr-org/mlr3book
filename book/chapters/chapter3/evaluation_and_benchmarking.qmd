...
Whilst `$score()` returns the performance in each evaluation, `r index('$aggregate()', parent = "Learner", aside = TRUE, code = TRUE)`, returns the aggregated score across all resampling iterations.
...
To visualize the resampling results, you can use the `r ref("mlr3viz::autoplot.ResampleResult()")` function to plot scores across folds as boxplots (@fig-resamp-viz-1) or histograms (@fig-resamp-viz-2).
...
```{r performance-035, output = FALSE}
rr = resample(tsk_penguins, lrn_rpart, rsmp("cv", folds = 10))
autoplot(rr, measure = msr("classif.acc"), type = "boxplot")
```

```{r}
#| label: fig-resamp-viz-1
#| fig-cap: "Boxplot of accuracy scores."
#| message: false
#| fig-alt: "A boxplot ranging from 0.875 to 1.0 and the interquartile range between 0.925 and 0.7."
#| echo: false
plt = .Last.value
plt$layers[[1]]$aes_params$fill = "white"
print(plt)
```
...
```{r}
#| label: fig-resamp-viz-2
#| fig-cap: "Histogram of accuracy scores."
#| message: false
#| fig-alt: "A histogram with five bars in a roughly normal distribution with mean 0.95, minimum 0.875 and maximum 1.0."
#| echo: false
plt = .Last.value
plt$layers[[1]]$aes_params$fill = "white"
print(plt)
...
By default, the intermediate models produced at each resampling iteration are discarded after the prediction step to reduce memory consumption of the `ResampleResult` object (only the predictions are required to calculate the performance measure).
...
```{r performance-046, output = FALSE}
...
#| echo: false
library(ggplot2)
plt = plt + ggplot2::scale_fill_grey()
print(plt)
```
...
We will first return to the confusion matrix and discuss measures that can be derived from it and then will look at `r index('ROC', lower = FALSE)` analysis which incorporates these measures.
...
```{r, plt$layers[[1]]$aes_params$fill}
...
```{r}
#| label: fig-basics-fpracc-ranger-1
#| fig-cap: 'Comparing threshold and FPR for the random forest trained on `tsk("german_credit")`.'
#| fig-alt: 'A line graph with "Probability Threshold" on x-axis from 0-1 and "classif.fpr" on the y-axis. Line slowly decreases from (0,1) to (1,0)."'
#| echo: false
plt = .Last.value
...
```{r}
#| label: fig-basics-fpracc-ranger-2
#| fig-cap: 'Comparing threshold and accuracy or the random forest trained on `tsk("german_credit")`.'
#| fig-alt: 'A line grap hwith "Probability Threshold" on x-axis from 0-1 and "classif.acc" on the y-axis. Line travels from (0,0.7) to (0.25,0.7) to (0.4,0.75) to (1, 0.3).'
#| echo: false
plt = .Last.value
plt$layers[[1]]$aes_params$colour = "grey30"
print(plt)
...
```{r}
#| label: fig-basics-rocpr-ranger-1
#| fig-cap: 'A ROC curve for a random forest trained on `tsk("german_credit")`.'
#| fig-alt: 'A line graph. There is a ROC curve with the "best" point close to (0.25, 0.75).'
#| echo: false
plt = .Last.value
plt = plt + ggplot2::scale_fill_grey() + ggplot2::scale_color_grey()
print(plt)
```
...
```{r}
#| label: fig-basics-rocpr-ranger-2
#| fig-cap: 'A PR curve for a random forest trained on `tsk("german_credit")`.'
#| fig-alt: 'A line graph. There is a PR curve ending at 0.74.'
#| echo: false
plt = .Last.value
plt = plt + ggplot2::scale_fill_grey() + ggplot2::scale_color_grey()
print(plt)
