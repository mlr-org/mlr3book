...
For linear or polynomial regression models, LOO-CV with mean squared error as the performance measure can be computed efficiently via a closed-form formula without the need to fit so many models, yet this does not apply in the general case (@james_introduction_2014).
...
In the above example where we used `tsk("penguins")`, there is a difference in the classification error between micro and macro methods because the dataset has 344 rows, which is not divisible by three (the number of folds), hence the test sets are not of an equal size.
:::
...
In the example below, we first construct a custom holdout resampling strategy by manually assigning row indices to the `$train` and `$test` fields, then construct a resampling strategy with two iterations by passing row indices as list elements:

```{r performance-023}
rsmp_custom = rsmp("custom")

# custom holdout strategy
...
If we don't do this, we might overestimate the model's generalization capability to unseen individuals, because observations of the same individuals might simultaneously be in the train and test set.
...
In this case, strata would be formed out of each combination of the stratified features, e.g., for two stratified features A and B with levels Aa, Ab, Ba, Bb respectively then the created stratum would be AaBa, AaBb, AbBa, AbBb.
...
This would conclude a basic benchmark experiment where you can draw tentative conclusions about model performance, in this case we would possibly conclude that the random forest is the best of all three models.
...
| ---- | ----- | -------- |
| `r ref("PredictionClassif")` | `classif_lrn$predict()` | `r ref("mlr3measures::confusion_matrix()")`; `r ref("mlr3viz::autoplot.PredictionClassif", "autoplot(type = \"roc\")")` |
| - | `r ref("partition()")` | - |
| `r ref("Resampling")` | `r ref("rsmp()")` | `$instantiate()` |
| `r ref("ResampleResult")` | `r ref("resample()")` | `$score()`; `$aggregate()`; `$predictions()`; `as_benchmark_result()`; `r ref("mlr3viz::autoplot.ResampleResult", "autoplot(type = \"roc\")")` |
| - | `r ref("benchmark_grid()")` | - |
| `r ref("BenchmarkResult")` | `r ref("benchmark()")` | `$aggregate()`; `$resample_result()`; `$score()`; `r ref("mlr3viz::autoplot.BenchmarkResult", "autoplot(type = \"roc\")")` |
