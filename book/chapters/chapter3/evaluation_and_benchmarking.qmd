...
In this case, stratum would be formed out of each combination of the stratified features, e.g., for two stratified features A and B with levels Aa, Ab, Ba, Bb respectively then the created strata would be AaBa, AaBb, AbBa, AbBb.
...
rbind("Fold 1" = prop.table(table(tsk_str$data(rows = rsmp_cv10$test_set(1), cols = "species"))),
"Fold 2" = prop.table(table(tsk_str$data(rows = rsmp_cv10$test_set(2), cols = "species"))))
```
...
This is a mistake as averaging the performance will miss out on important insights into how learners compare on 'easier' or more 'difficult' predictive problems.
...
#| fig-cap: 'Boxplots of accuracy scores for each learner across resampling iterations and the three tasks. Random forests (`lrn("classif.ranger")`) consistently performs outperforms the other learners.'
...
Using different thresholds to cut off predicted probabilities and assign them to the positive and negative class will lead to different confusion matrices, as seen previously.
...
A natural performance measure that can be derived from the ROC curve is the `r index('area under the curve', aside = TRUE)` (AUC), implemented in `msr("classif.auc")`.
