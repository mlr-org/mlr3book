...
::: {.callout-warning}
## Resampling does not stop model overfitting
...
In `tsk("penguins")`, the data is ordered such that the first 152 rows all have the label 'Adelie', the next 68 have the label 'Chinstrap', and the final 124 have the label 'Gentoo'; so if we did not split the data randomly we could end up with a model that is only trained on one or two species.
...
Many performance measures are based on 'decomposable' losses, which means they compute the differences between the predicted values and ground truth values first on an observation level and then aggregate the individual loss values into a single numeric score, which is the composite loss.
...
The `$score()` method summarizes these individual loss values into a composite loss by counting the fraction of observations where the decomposable loss is `1`.
...
By repeating the data splitting process often enough, more data points can be used for both training and testing, allowing more efficient use of all available data for performance estimation.
...
LOO-CV reduces variance in the performance estimate but is computationally very expensive due to the need to fit one model per observation.
For linear or polynomial regression models, LOO-CV with mean squared error as the performance measure can be computed efficiently via a closed-form formula without the need to fit so many models, yet this does not apply in the general case (@james2013introduction).
Furthermore, LOO-CV is also problematic in imbalanced binary classification tasks as concepts such as stratification (@sec-strat-group) cannot be applied to LOO-CV.
...
When a `Resampling` object is first constructed, the objects acts as a definition for how the data splitting process will be performed on the task when running the resampling strategy.
...
So carrying on our `tsk("penguins")` example, we can instantiate the 3-fold CV object and then view the row indices of the data selected for training and testing each fold using `$train_set()` and `$test_set()` respectively:
...
`resample()` repeatedly fits a model on training sets and stores predictions in a `r ref("ResampleResult", aside = TRUE)` object, which contains all the information needed to estimate the generalization performance.
...
Whilst `$score()` returns the performance in each evaluation, `r cdefine('$aggregate()')`, returns the aggregated score across all resampling iterations.
...
In the above example where we used the tsk("`penguins`"), there is a difference in the classification error between micro and macro methods because the dataset has 344 rows, which is not divisible by three, hence the test sets are not of equal size.
:::
...
This can be used to analyze the predictions of individual intermediate models from each resampling iteration and, e.g., to manually compute a macro averaged performance estimate.
...
In the example below, we use a smaller version of tsk("`penguins`") and instantiate a custom 2-fold CV strategy using a `factor` variable called `folds` where the first and third rows are used as the test set in Fold 1, and the second and fourth rows are used as the test set in Fold 2:

```{r performance-025}
tsk_small = tsk("penguins")$filter(c(1, 100, 200, 300))
rsmp_customcv = rsmp("custom_cv")
folds = as.factor(c(1, 2, 1, 2))
rsmp_customcv$instantiate(tsk_small, f = as.factor(folds))
...
Grouping ensures that the model is tested on data from each *person*, and not each observation, thereby ensuring that data in the training set is not correlated with data in the test set.
...
#| fig-alt: "Three images, each shows a blue box with text 'Train' and white space around it with text 'Test'. The left image shows a blue box with green and red dots inside it and yellow dots outside it, the caption says 'Iteration 1'. The middle image shows a blue box with green and yellow dots inside it and red dots outside it, the caption says 'Iteration 2'. The right image shows a blue box with yellow and red dots inside it and green dots outside it, the caption says 'Iteration 3'."
...
Since this process is random, it is necessary to set a seed **before** calling `benchmark_grid()` to ensure reproducibility of the data splits.
:::
...
Analysis of benchmark experiments is covered in more detail in @sec-benchmark-analysis.
...
The contents of a `BenchmarkResult` and `ResampleResult` (@sec-resampling-inspect) are almost identical and the stored `ResampleResult`s can be extracted via the `$resample_result(i)` method, where `i` is the index of the performed benchmark experiment.
...
In addition, `BenchmarkResult`s also have the `r ref('as_benchmark_result()')` method, which can be used to convert objects from `ResampleResult` to `BenchmarkResult`.
Alongside `c.BenchmarkResult`, which combines multiple `BenchmarkResult` objects, this is most useful when conducting experiments across multiple machines:
...
In @sec-basics-classif-learner we touched on the concept of a confusion matrix and how it can be used to break down binary classification predictions in more detail.
...
We will first return to the confusion matrix and discuss measures that can be derived from it and then will look at `r index('ROC', parent = "measures")` analysis which incorporates these measures.
...
For example, the `spam` classification task is concerned with classifying if mail is spam (positive class) or not (negative class).
...
Instead, several robust measures can be derived that mainly quantify the discrimination performance of a classifier, i.e., the ability of a classifier to separate the two classes (@fig-confusion):

* **True Positive Rate\index{measures!true positive rate}\index{sensitivity|see{measures, true positive rate}}\index{recall|see{measures, true positive rate}} (TPR)**, **Sensitivity** or **Recall**: How many of the true positives did we predict as positive?
* **True Negative Rate\index{measures!true negative rate}\index{specificity|see{measures, true negative rate}} (TNR)** or **Specificity**: How many of the true negatives did we predict as negative?
* **False Positive Rate\index{measures!false positive rate} (FPR)**, or $1 -$ **Specificity**: How many of the true negatives did we predict as positive?
* **Positive Predictive Value\index{measures!positive predictive value}\index{precision|see{measures, positive predictive value}} (PPV)** or **Precision**: If we predict positive how likely is it a true positive?
* **Negative Predictive Value\index{measures!negative predictive value} (NPV)**: If we predict negative how likely is it a true negative?
* **Accuracy (ACC)\index{measures!accuracy}**: The proportion of correctly classified instances out of the total number of instances.
* **F1-score\index{measures!F1}**: The harmonic mean of precision and recall, which balances the trade-off between precision and recall. It is calculated as $2 \times \frac{Precision \times Recall}{Precision + Recall}$.
...
When a binary classifier predicts probabilities instead of discrete classes (`predict_type = "prob"`), we could set a threshold to cut off the probabilities to change how we assign observations to the positive/negative class (see @sec-thresholding).
...
ROC\index{measures!ROC} (Receiver Operating Characteristic) analysis is widely used to evaluate binary classifiers by visualizing the trade-off between the TPR and the FPR.

The ROC curve is a line graph with TPR on the y-axis and the FPR on the x-axis.
To understand the usefulness of this curve, first consider the simple case of a 'hard' classifier, which predicts discrete labels (`predict_type = "response"`)  that classify observations as either positive or negative.
This classifier would be represented as a single point in the ROC space (see @fig-roc, panel (a)) as its TPR/FPR would be static (the thresholds can never change).
...
A natural performance measure that can be derived from the ROC curve is the `r index('area under the curve', "AUC", parent = "measures", lower = FALSE, aside = TRUE)` (AUC), implemented in `msr("classif.auc")`.
...
We can also plot the `r index('precision-recall curve', parent = "measures", aside = TRUE)` (PRC) which visualizes the PPV/precision\index{measures!positive predictive value} vs. TPR/recall\index{measures!true positive rate}.
The main difference between ROC curves and PR curves is that the number of true-negatives are ignored in the latter, which can be useful in imbalanced populations where the positive class is rare and hence the FPR will be low even for a random classifier.
...
```
```{r performance-059-evalT, echo = FALSE}
...
@tbl-api-performance summarizes the most important classes, functions, and methods seen in this chapter.
...
| `r ref("BenchmarkResult")` | `r ref("benchmark()")` | `$aggregate()`/`$resample_result()` |

: Important classes and functions covered in this chapter with underlying class (if applicable), class constructor or function, and important class methods (if applicable). {#tbl-api-performance}
...
2. Use `tsk("spam")` and 5-fold CV to benchmark Random Forest (`lrn("classif.ranger")`), Logistic Regression (`lrn("classif.log_reg")`), and XGBoost (`lrn("classif.xgboost")`) with respect to AUC.
