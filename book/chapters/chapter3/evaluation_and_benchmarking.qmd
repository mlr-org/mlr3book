...
In @sec-basics-partition we introduced `r ref("partition()")`, which splits data into data for training the model and data for testing the model and estimating the generalization performance, this is known as the holdout strategy (@sec-holdout-scoring) and is where we will begin this chapter.
...
To recap by example let us split `tsk("penguins")` with a 2/3 holdout (default split):
