...
To recap by example let us split the `tsk("penguins")` task with a 2/3 holdout, which is the default in `mlr3`:
...
In the `tsk("penguins")` task, the data is ordered such that the first 152 rows all have the label 'Adelie', the next 68 have label 'Chinstrap', and the final 124 have label 'Gentoo'; so if we did not split the data randomly we could end up with a model that is only trained on one or two species.
...
`r index("Subsampling", aside = TRUE)` and `r index("bootstrapping", aside = TRUE)` are two related resampling strategies.
...
In the above example where we used the `tsk("penguins")` data, there is a difference in the classification error between micro and macro methods because the dataset has 344 rows, which is not divisible by three, hence the test sets are not of an equal size.
:::
...
In the example below, we use a smaller version of the `tsk("penguins")` task and instantiate a custom 2-fold CV strategy using a `factor` variable called `folds` where the first and third rows are used as the test set in Fold 1, and the second and fourth rows are used as the test set in Fold 2:
...
In the following code we construct a leave-one-out resampling strategy, assign the `"group"` role to the 'year' column of the `penguins` dataset, instantiate the resampling strategy, and finally show how the years are nicely separated in the first fold.
...
The `tsk("penguins")` task displays imbalance in the `species` column, as can be seen in the output below:
...
```{r performance-055}
#| fig-cap: "ROC-curve based on the `german_credit` dataset and the `lrn('classif.ranger')` Random Forest learner. Recall FPR = 1 - Specificity and TPR = Sensitivity."
...
Evaluating our random forest on the `tsk("german_credit")` task results in an AUC of around `r round(x, 2)`, which is acceptable but could be better.
...
```{r performance-056}
#| fig-cap: "Precision-Recall curve based on the `german_credit` dataset and the `lrn('classif.ranger')` Random Forest learner."
...
#| layout-ncol: 2
#| fig-subcap:
#|   - "Threshold vs. FPR plot on `german_credit` with `lrn('classif.ranger')`."
#|   - "Threshold vs. accuracy plot on `german_credit` with `lrn('classif.ranger')`."
...
2. Use the `tsk("spam")` task and five-fold CV to benchmark Random Forest (`lrn("classif.ranger")`), Logistic Regression (`lrn("classif.log_reg")`), and XGBoost (`lrn("classif.xgboost")`) with regards to AUC.
