...
During training, the PCA transforms incoming data by rotating it in such a way that features become uncorrelated and are ordered by their contribution to the total variance.
...
`Learner` objects can be converted to `PipeOp`s with `r ref("as_pipeop()")`, however, this is only necessary if you choose to manually create a graph instead of using `%>>%`, internally `Learner`s are wrapped in the `"learner"` `PipeOp`.
