...
result = gr$train(tsk_small)
result
result[[1]]$data()[1:3]
result = gr$predict(tsk_onepenguin)
result[[1]]$head()
```

## Sequential Learner-Pipelines {#sec-pipelines-sequential}

Possibly the most common application for `mlr3pipelines` is to use it to perform `r index('preprocessing')` tasks, such as missing value `r index('imputation')` or `r index('factor encoding')`, and to then feed the resulting data into a `Learner` -- we will see this in practice in @sec-preprocessing.
...
glrn_sample = as_learner(gr)
glrn_mode = as_learner(po("imputemode") %>>% lrn_logreg)

grid = benchmark_grid(tsk("pima"), list(glrn_sample, glrn_mode),
  rsmp("cv", folds = 3))
bmr = benchmark(grid)
...
You may want to inspect pipelines and the flow of data to learn more about your pipeline or to debug\index{debugging} any processes.
...
trained_p_logreg = glrn_sample$graph_model$pipeops$classif.log_reg
...
glrn = as_learner(gr)
glrn$param_set$values$classif.rpart.minsplit = 2
unlist(glrn$param_set$values)
```
...
@tbl-api-pipelines-seq summarizes the most important classes, functions, and methods seen in this chapter.
