...
| `r ref("Graph")` | `%>>%` | `$add_pipeop()`; `$add_edge()`; `$pipeops`; `$edges`; `$keep_results`; `$plot()`; `$train()`; `$predict()` |
...
2. Train the Graph created in the previous exercise on `tsk("pima")` task and display the coefficients of the resulting model.
