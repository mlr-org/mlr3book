...
In this section we will train xgboost on the sonar classification task and keep track of its performance on a validation set.
Below, we see that the xgboost classifier has both of these properties.
...
In order to track the performance, we need to configure how the required validation data is constructed.
...
* `"test"` to use the test set as validation data, which only works in combination with resampling. 
...
Furthermore, `mlr3` provides a standardized acccessor for the final validation performance via `$internal_valid_scores`.
...
In many cases, however, one does not only train an individual learner, but combines it with other (preprocessing) steps in a `r GraphLearner`.
...
We can now train the graphlearner just as before and inspect the final validation metric, which is now prefixed by the ID of the respective `PipeOp`.
...
Below, we tune xgboost on the sonar task using a simple grid search with 10 evaluations and 5-fold cross-validation.
As the evaluation measure, we will use classification error.
However, it is important to note that the internal tuning will not be influenced by this and is configured via the `eval_metric` parameter that we have set to "logloss" earlier.
...
This analogously for the `r ref("AutoTuner")`, which will use the internally optimized `nrounds`, as well as the explicitly tuned `eta` for the final model fit.
...
::: {.callout-warning}
## Specifying `$validate` correctly
When configuring which validation data to use in an `r ref("AutoTuner")` it is possible set the `$validate` field for the tuned `Learner` and for the `AutoTuner` itself.
In almost cases, the `$validate` field should only be set for the tuned learner (here `lrn_xgb`), as all available data should be used for the final fit.
:::
