...
We see that training was already stopped after `r lrn_xgb$internal_tuned_values$nrounds`.
...
When configuring which validation data to use in an `r ref("AutoTuner")` it is possible to both set the `$validate` field of the tuned `Learner` as well as of the `AutoTuner` itself.
In almost cases, the `$validate` field should only be set for the tuned learner, as no validation should be performed during the final model fit.
:::
