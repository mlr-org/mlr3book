...
When employing the validation technique in a `GraphLearner` it is best to first construct the learner without specifying the validation data and to then use the `set_validate()` function.
:::
...
The metric that is used for early stopping is the first value that we passed to `eval_metric`, which was the classification error.
Below, we set it to 5.
...
When we now train the learner, we can access the internall optimized `nrounds` through the `$internal_tuned_values` field
...
Below, we visualize the validation loss over time.
...
In such scenarios, what one might oten want is to use the same dataset to tune `eta` and to tune `nrounds`.
This is possible by activating the "test" option of the `validate` field.
In each resampling iteration, the validation data will be set to the test set, i.e. the same data that is then also used to score the parameter configuration.
...
If we not inspect the tuning archive, we find a column `internal_tuned_values` which contains the nrounds optimized via early stopping:

```{r}
as.integer(ti$archive$data$internal_tuned_values)
```
...
We also find the internally optimized `nrounds` in the result of the tuning instance.
...
In this chapter, we have learned how to track the performance of an iterative learning procedure on a validation set.
This technique also works seemlessly in a graphlearner, with the only difference that one not only has to specify how to create the validation data, but also which PipeOps should use it.
