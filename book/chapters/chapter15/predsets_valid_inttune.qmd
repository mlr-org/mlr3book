...
:::{.callout-warning}
When working with a `r GraphLearner`, the names of the internal validation scores are prefixed by the ID of the corresponding `r PipeOp`, so the `select` parameter needs to be set to `"<pipeop id>.<measure id>"`.
:::
