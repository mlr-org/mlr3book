...
library(ggplot2)
set.seed(1)
ggplot(attributes(lrn_xgb$model)$evaluation_log, aes(x = iter, y = test_logloss)) +
  geom_line() +
  labs(
...
Because the validation performance is tracked over the whole training run, the score of the *final* model is not necessarily the *best* score that was observed.
For this reason there is a second accessor, `$best_valid_scores`, which returns the validation scores of the best iteration.

```{r predsets_valid_inttune-012a}
lrn_xgb$best_valid_scores
```

Whether the two differ depends on the learner:

* XGBoost also *predicts* with the best `nrounds` when early stopping is enabled, so the final model *is* the best model and both fields agree.
* Learners that keep training after the optimum and return the last state -- most notably neural networks trained with `r ref_pkg("mlr3torch")`, where the returned network is the one after the last epoch -- report different values.

This distinction matters because `$internal_tuned_values` reports the *best* iteration, i.e. the one written into the tuning archive.
Whenever the learner does not roll back to that iteration, `$internal_valid_scores` refers to a different model than the internally tuned value does.
We come back to this when we choose a tuning measure in @sec-internal-tuning.

::: {.callout-note}
Not every learner tracks the best score.
Learners only report `$best_valid_scores` if they implement the private `$.extract_best_valid_scores()` method, see `r ref("Learner")`.
For learners that do not, `msr("best_valid_score")` returns `NA`.
:::
...
data = attributes(lrn_xgb$model)$evaluation_log
...
1.  Use `msr("best_valid_score", select = <id>)`, i.e. the validation score of the best iteration, as the tuning measure. As a learner can have multiple internal valid scores, the measure allows us to select one by specifying the `select` argument. If this is not specified, the first validation measure will be used. We also need to specify whether the measure should be minimized.
...
We recommend `msr("best_valid_score")` over `msr("internal_valid_score")` here.
The internally tuned value that is written into the archive is the *best* iteration, so the best validation score is the score that actually belongs to the configuration being recorded.
`msr("internal_valid_score")` reports the score of the final model instead, which for learners that do not roll back to the best iteration evaluates a different model than the one the archive describes -- systematically pessimistic, since the final iteration is by definition no better than the best one.
For XGBoost the two coincide, because it predicts with the best `nrounds` anyway, but the choice matters as soon as you swap in a learner that does not, such as a `r ref_pkg("mlr3torch")` network.
Use `msr("internal_valid_score")` when you deliberately want to score the final model, e.g. because you will refit without early stopping.
...
1.  Manually `$train()` a LightGBM classifier from `r ref_pkg("mlr3extralearners")` on the diabetes task using $1/3$ of the training data for validation. As the diabetes task has missing values, select a method from `r ref_pkg("mlr3pipelines")` to impute them. Explicitly set the evaluation metric to logloss (`"binary_logloss"`), the maximum number of boosting iterations to 1000, the patience parameter to 10, and the step size to 0.01. After training the learner, inspect the final validation scores as well as the early stopped number of iterations.
