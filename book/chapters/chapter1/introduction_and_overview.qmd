...
@sec-interpretation will discuss different packages that are compatible with `mlr3` to provide model-agnostic interpretability regarding feature importances, evvect and local explainability of individual predictions.
...
We have marked sections that are particularly complex with respect to either technical or methodological detail and could be skipped on a first read with the following information box that indicates their advanced nature:
...
@fig-mlr3verse shows the packages in the `r mlr3verse` that extend `r mlr3` with capabilities for preprocessing, pipelining, visualizations, additional learners, additional task types, and much more.
...
We use `data.table`, which has behavior that is more consistent than some popular base R methods (e.g., indexing `data.frame`s when the `drop` argument is omitted).
