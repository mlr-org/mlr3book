...
Throughout this book, we interchangeably refer to the package `r mlr3` and the ecosystem `mlr3`, which also encompasses all extension packages.
The core functionality to all packages in the ecosystem is provided by `r mlr3` but in almost all use cases users will use more packages in the ecosystem, and usually at least the packages in `r mlr3verse`.
...
```{r intro-001, echo = FALSE, out.width = "70%"}
#| label: fig-mlr3verse
