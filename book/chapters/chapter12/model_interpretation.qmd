...
As a running example throughout this chapter, we will consider a gradient boosting machine (GBM) fit on half the features in the `tsk("german_credit")` task:
...
Let us put this into practice by considering how the feature `amount` influences the predictions in the `tsk("german_credit")` task.
...
The predicted class labels for the binary classification `tsk("german_credit")` task are plotted separately.
...
We demonstrated on the `tsk("german_credit")` task how these packages offer an in-depth analysis of a GBM model fitted with `mlr3`.
...
The `tsk("german_credit")` task is low-dimensional with a limited number of observations.
