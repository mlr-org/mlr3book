...
As we see good agreement between the local and black-box model (on average the predictions of Charlie's 10 nearest neighbors differ only by `r round(mean(abs(gbm_pred_10nn - local_pred_10nn)), 3)`), we can move on to look at the most influential features for Charlie's predictions:
