...
Many models are considered as '`r index('black-boxes')`', where the decision-making process behind their predictions is often not immediately interpretable.
...
In recent years, many interpretation methods have been developed that allow to 'peek' inside these models and produce explanations to, for example, understand how data features are being used to make predictions (@guidotti2018survey).
...
As a running example throughout this chapter, we will consider an gradient boosting machine (GBM).
For the sake of simplicity, we have not tuned the GBM and trained it on half the features in the `"german_credit"` task.
...
However, for the sake of simplicity, we opted to utilize an untuned GBM for the `"german_credit"` task, considering that it exhibited satisfactory performance even without fine-tuning, saving the reader valuable time and resources when running the example code below:
...
Performance-based interpretation methods such as PFI (@sec-feat-importance) rely on measuring the generalization performance. Hence, they should be computed on an independent test set to decrease bias in estimation (see @sec-performance).
...
By default, the permutation is repeated five times to keep computation time low (this can be changed with `n.repetitions` when calling the constructor `$new()`) and in each repetition, the importance value corresponding to the change in the classification error is calculated.
...
Obviously, the error bars are most useful if the number of repetitions is high, e.g., at least 100 as they refer to the 5% and 95% quantiles of the importance values.
:::
...
@fig-iml-pdice shows that if the `amount` is smaller than roughly 10,000 then on average there is a high chance that the predicted creditworthiness will be `good`. Furthermore, the ICE curves are more or less parallel lines, meaning that there do not seem to be strong interactions present where `amount` is involved.
...
Hence, conclusions drawn from the surrogate model are valid if the surrogate model approximates the black-box model very well (i.e., if the model fidelity is high). 
It is therefore important to measure and report also the approximation error of the surrogate model.
...
Having trained a black-box model that captures the relationships in the original data very well, enables us to generate new labeled data in specific regions of the input space with which we can augment the original data.
<!-- During this 'surrogate data generation' process the predictions of the black-box model serve as labels. -->
...
<!-- Surrogate data generation allows for the benefits of both the accuracy of the black-box model and the interpretability of the resulting interpretable model. -->
<!-- The black-box model acts as a powerful predictor, providing labels for the artificially generated data, while the interpretable model can offer a better understanding in the targeted regions of the input space. -->
...
confusion = mlr3measures::confusion_matrix(pred_surrogate, pred_gbm, positive = "good")
print(confusion)
```
...
If the prediction of the local model and the prediction of the black-box GBM model greatly differ, then you might want to experiment with changing the `k` and `gower.power` parameters, which can be considered hyperparameters of the local surrogate model that should be tuned to achieve an accurate local surrogate.
We can first check if the predictions for Charlie match:

```{r}
c(gbm = gbm_predict[[1]], local = local_surrogate$predict()[[1]])
```

Ideally, we should assess the fidelity of the surrogate model in the local neighborhood of Charlie, i.e., how well the local surrogate model approximates the predictions of the black-box GBM model in the vicinity of Charlie.
For example, one can generate artificial data points in the local neighborhood of Charlie or select the $k$ nearest neighbors from the underlying original data (and potentially weight the data points according to their distance to Charlie) to assess the local model fidelity.
Below, we measure the mean absolute error of the 10 nearest neighbors according to the Gower distance (@gower1971general):
...
mean(abs(global_pred_10nn - local_pred_10nn))
```
...
shapley = Shapley$new(predictor, x.interest = Charlie, sample.size = 1000)
shapley$plot()
```
...
Obviously, if we do not restrict ourselves to a fixed dataset, many possible counterfactual data points can exist, as there are many ways to achieve a desired prediction.
However, the approach by @Wexler2019 only produces a single counterfactual explanation, as many other early counterfactual methods (see @guidotti2022counterfactual for a comprehensive overview).
...
Conversely, several other counterfactual methods rely on single-objective optimization methods, where multiple objectives are combined into a single objective, e.g., using a weighted sum. 
