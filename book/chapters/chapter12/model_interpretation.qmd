...
This implementation is very closely related Local Interpretable Model-agnostic Explanations (`r index('LIME')`) [@Ribeiro2016lime], the differences are outlined in the documentation of `r ref("iml::LocalModel")`.
...
Continuing our previous example, we saw that the GBM classifies Steve as having '`r predictor$class`' credit with a predicted probability of `r round(max(predictor$predict(steve))*100, 1)`%.
...
The `$find_counterfactuals()` method generates a counterfactual of class  `r ref("counterfactuals::Counterfactuals")`, where we are assuming a 0.75 threshold for landing in the `good` class.
...
Here we can see that Steve would have to be 3 years younger and the duration of credit would have to be reduced by 3 months.
