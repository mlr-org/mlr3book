...
By default, the permutation is repeated five times (this can be changed with `n.repetitions` when calling the constructor `$new()`) and in each repetition, the importance value corresponding to the change in the classification error is calculated.
...
```{r iml-crosstable}
pred_surrogate = treesurrogate$predict(credit_x, type = "class")$.class
