...
Many models are a '`r index('black-box')`' and the 'decisions' that influence predictions are rarely immediately interpretable.
This lack of explanation can decrease trust in ML and may create barriers to the adoption of predictive models, especially in critical applications such as medicine, engineering, and finance.

Interpretation methods can be used to 'peek' inside these models to understand how data features are being used to make model predictions.
These methods can be valuable from multiple perspectives:

1. To gain global insights into a model, for example, to identify which features were the most important overall.
...
We will focus on methods implemented in three packages that interface with `mlr3`: `r ref_pkg("iml")` (@sec-iml), `r ref_pkg("counterfactuals")` (@sec-counterfactuals), and `r ref_pkg("DALEX")` (@sec-dalex).
...
`counterfactuals` also uses `R6`.
In contrast to `iml` and `counterfactuals`, `DALEX` focuses on comparing multiple predictive models, usually of different types, which is a model comparison-oriented approach often referred to as the `r index('Rashomon', lower = FALSE)` perspective.
...
::: {.callout-tip}
## Interpretation Methods Require Test Data
Performance-based interpretation methods should be computed on an independent test set to decrease bias in estimation (see @sec-performance).
...
In theory, `iml` supports machine learning models (for classification or regression) fitted by *any* R package, and in particular all `mlr3` models are supported by wrapping learners in an `r ref("iml::Predictor")` object, which unifies the input-output behavior of the trained models.
...
This can be useful to better understand the problem at hand and the relationship between the features and the target to be predicted.
...
One of the most popular feature importance methods is `r index('permutation feature importance', aside = TRUE)` (PFI), originally introduced by @breiman2001random for random forests\index{random forest} and adapted by @Fisher2019pfi as a model-agnostic feature importance measure (originally termed, 'model reliance').
...
Conversely, we would expect a greater change in model performance if the feature is more important.
It is generally recommended to repeat the permutation process and aggregate performance change over multiple repetitions to decrease randomness in results.
...
By default, the permutation is repeated five times (this can be changed with `n.repetitions` when calling `$new()`) and in each repetition, the importance value corresponding to the change in the classification error is calculated.
The `$plot()` method shows the median of the five resulting importance values (as a point) and the boundaries of the error bar in the plot refer to the 5% and 95% quantiles of the importance values (@fig-iml-pfi).
...
#| fig-alt: "x-axis says 'Feature Importance (loss: ce)' and ranges from just below 0.95 to just above 1.15. y-axis lists the features in the data. Plot shows 10 error bars with solid black circles in the middle and horizontal black lines on each row. Top row is 'duration' with a CE of around 1.11 (1.08-1.17)."
#| echo: false
#| warning: false
#| message: false
...
Let us put this into practice by considering how the feature `amount` influences the predictions in `tsk("german_credit")`.
...
@fig-iml-pdice shows that if the `amount` is smaller than roughly 10,000 then on average there is a high chance that the predicted creditworthiness will be `good`.

### Surrogate Models

Interpretable models such as decision trees or linear models can be used as `r index("surrogate models", "surrogate model")` to approximate or mimic a (often very complex) black-box model.
...
This shows an accuracy of around `r round(mean(pred_gbm == pred_surrogate$.class)*100)`% in predictions from the surrogate compared to the black-box model, which is good enough for us to use our surrogate for further interpretation, for example by plotting the splits in the terminal node:
...
To illustrate this using the `iml` package, we will select a random data point to explain.
...
Charlie = credit_x[35, ]
global_predict = predictor$predict(Charlie)
global_predict
```
...
localsurrogate = LocalModel$new(predictor, Charlie, gower.power = 0.01,
  k = 2)
```
...
```{r}
c(global = global_predict[[1]], local = localsurrogate$predict()[[1]])
```

As we see good agreement between the local and black-box model we can move on to look at the most influential features for Charlie's predictions:

```{r, eval = FALSE}
localsurrogate$results[, c("feature.value", "effect")]
```
```{r, echo = FALSE}
x = localsurrogate$results[, c("feature.value", "effect")]
rownames(x) = NULL
x
```

In this case, 'duration' and 'status' were most important and both decreased the predictive value.
...
In contrast, the 'status' variable leads to a decrease in the predicted probability of over 5%.
...
A simple counterfactual method is the `r index('What-If', aside = TRUE)` approach (@Wexler2019) where, for a given prediction to explain, the counterfactual is the closest data point in the dataset with the desired prediction -- so if the underlying dataset is changed then the counterfactual can be quite different.
...
The returned set is a set of non-dominated and, therefore equally good, counterfactuals with respect to the four objectives (similarly to the `r index('Pareto front', lower = FALSE)` we saw in @sec-multi-metrics-tuning).
...
Continuing our previous example, we saw that the random forest classifies Charlie as having `r predictor$class` credit with a predicted probability of `r round(max(predictor$predict(Charlie))*100, 1)`%.
...
@tbl-interpretation-api summarizes the most important classes, functions, and methods seen in this chapter.
...
| `r ref("DALEX::explainer")` | `r ref("DALEXtra::explain_mlr3()")` | `model_performance()`; `model_parts()`; `predict_parts()` |
...
Model explanation allows us to confront our expert knowledge related to the problem with relations learned by the model. The following tasks are based on predictions of the value of football players based on data from the FIFA game. It is a graceful example, as most people have some intuition about how a footballer's age or skill can affect their value. The 2020 FIFA data can be found in `r ref("DALEX::fifa")`. The following exercises can be performed in both the `iml` and `DALEX` packages, we provide solutions for both.

1.  Prepare an `mlr3` regression task for the `fifa` data. Select only features describing the age and skills of footballers. Train a predictive model of your own choice on this task, e.g. `lrn("regr.ranger")`, to predict the value of a footballer.
