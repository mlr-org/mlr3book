...
ggplot(data_ranger_quantiles) +
  ...
  geom_line(aes(x = x, y = predicted_value, color = quantile), linewidth = 0.3) +
...
loss = mlr3misc::map_dtr(qs, function(q) {
  ...
  score_ranger = mlr3measures::pinball(truth = prds_ranger$truth, response = prds_ranger$quantiles[, q_str, drop = FALSE], alpha = q)
  score_qgam = mlr3measures::pinball(truth = prds_qgam$truth, response = prds_qgam$quantiles[, q_str, drop = FALSE], alpha = q)
