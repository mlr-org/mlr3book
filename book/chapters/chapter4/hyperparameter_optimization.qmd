...
The most commonly used terminators are those that stop the tuning after a certain time  (`trm("run_time")`) or number of evaluations (`trm("evals")`).
...
The `$result$learner_param_vals` field lists the optimal hyperparameters from tuning, as well as the values of any other hyperparameters that were set, this is useful for onward model use (@sec-analyzing-result).
...
Once we found good parameters for our learner through tuning, we can use them to train a final model on the whole data.
...
Note that in many cases, even nested resampling will overestimate the generalization error of the learner.
This is because the models fit during tuning use only a subset of the data, whereas the final model uses all available observations.`
