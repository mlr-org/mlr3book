...
The `trm("stagnation")` terminator stops when no progress is made for a set number of iterations.
...
A `r index("tuning instance", aside = TRUE)` can be constructed manually with the `r ref("ti()")` function, or automated (@sec-autotuner) with the `r ref("tune()")` function.
...
Adaptive algorithms learn from previously evaluated configurations to find good configurations more quickly, examples in `r mlr3` include Bayesian optimization (also called model-based optimization), Covariance Matrix Adaptation Evolution Strategy (CMA-ES), Iterative Racing, and Hyperband.

`r index('Bayesian optimization', lower = FALSE)` (e.g., @Snoek2012) describes a family of iterative optimization algorithms that use a surrogate model to approximate the unknown function that is to be optimized -- in HPO the mapping from a hyperparameter configuration to the estimated generalization performance.
...
However, grid search is generally not recommended due to the curse of dimensionality -- the grid size 'blows up' very quickly as the number of parameters to tune increases -- and insufficient coverage of numeric search spaces.
...
In @sec-defining-search-spaces we will look at how to implement more complex, custom transformations to any hyperparameter or combination of hyperparameters.
...
#| fig-align: "center"
#| fig-alt: "The figure shows the auto tuner."
...
Nested resampling is computationally expensive, three outer folds and four inner folds with a grid search of resolution five used to tune two parameters, results in $3*4*5*5 = 300$ iterations of model training/testing.
...
If you are interested in identifying optimal configurations, then use `r ref("tune()")`/`r ref("ti()")` or `r ref("auto_tuner()")` with `$train()`.

### Resampling an `AutoTuner`

Whilst the theory of nested resampling may seem complicated, it is all automated in `mlr3tuning` by simply passing an `r ref("AutoTuner")` to `r ref("resample()")` or `r ref("benchmark()")`.
...
```{r}
lrn("classif.svm", kernel = to_tune(c("radial", "linear")))$param_set$
  search_space()
```
...
```{r}
lrn("classif.ranger", num.trees = to_tune(c(100, 200, 400)))$param_set$
  search_space()
```
...
As we have seen, `r ref("to_tune()")` is a helper function that creates a parameter set that will go on to be used by `r ref("tune()")`/`r ref("ti()")`/`r ref("auto_tuner()")` during the tuning process.
...
| `r ref("paradox::TuneToken")` | `r ref("paradox::to_tune()")` | - |
...
| `r ref("paradox::ParamSet")` | `r ref("paradox::ps()")` | - |
| `r ref("mlr3tuningspaces::TuningSpace")` | `r ref("mlr3tuningspaces::lts()")` | `$values` |

: Important classes and functions covered in this chapter with underlying class (if applicable), class constructor or function, and important class methods (if applicable). {#tbl-api-optimization}
