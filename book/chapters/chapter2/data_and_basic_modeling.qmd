...
A `r index('model', aside = TRUE)` is formally a mapping from a feature vector to predictions, such models are induced by passing `r index('training data')` to machine learning algorithms, including `r index('decision trees', "decision tree")`, `r index('support vector machines', "support vector machine")`, `r index('neural networks', "neural network")`, and many more.
...
```{r basics-010}
c(Feature = tsk_mtcars$feature_names,
...
Predicting from trained models is as simple as passing your task to the `r index("$predict()", parent = "Learner", aside = TRUE, code = TRUE)` method of the trained `Learner`.
...
To set multiple values at once we recommend either setting these in construction or using `r index("param_set$set_values()", parent = "Learner", aside = TRUE, code = TRUE)`, which updates the given hyperparameters (argument names) with the respective values.
