...
The true values are `NA`, because we did not include an `mpg` (target) column in the generated data.
...
We can view the set hyperparameters by using `$param_set$values`:
...
The `Average: macro` attribute will be addressed when we cover resampling methods in @sec-resampling-exec.
...
In `r mlr3`, this is achieved by passing a named list to `$set_threshold()`.
