...
In contrast, the `weights_measure` column role is only used during performance evaluation and not during model training.
If this role is set, supported measures use these observation-specific weights when aggregating observation-level metrics, so some predictions contribute more than others to the final score.
This is useful when only evaluation should emphasize certain observations (e.g., costly mistakes or high-priority cases).
...
| `r ref("Learner")` | `r ref("lrn()")`/`r ref("lrns()")` | `$train()`; `$predict()`; `$predict_newdata()`; `$model()`; `$configure()` |
