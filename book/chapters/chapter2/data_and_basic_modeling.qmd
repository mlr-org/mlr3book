...
1. Train a classification model with the `classif.rpart` learner on the "Pima Indians Diabetes" dataset.
  ...
  You can either rely on `rpart`'s capability to handle them internally -- the so-called surrogate splits -- or remove them from the initial `data.frame` by using `na.omit`).
