...
@sec-special also discusses other tasks, such `r index('cost-sensitive classification', "cost-sensitive", parent = "classification")` or `r index('unsupervised learning')`.
...
A prediction can take many forms depending on the task; for classification, it can be a predicted label, or a set of predicted probabilities or scores.
...
In the next few sections we will look at the building blocks of `r mlr3` using regression as an example, we will then consider how to extend this to classification in @sec-classif.
...
You can bypass this check by setting `options(mlr3.allow_utf8_names = TRUE)` (but do not be surprised if errors occur later, especially when passing objects down to other packages).
:::
...
Hyperparameters can be optimized automatically (@sec-optimization), however, in this chapter, we show how to set them manually.
However, for this chapter, we will only discuss manual setting of hyperparameters.
...
In rare cases (`mlr3` tries to minimize it as much as possible), hyperparameters are initialized to values which deviate from the default in the underlying package.
...
Before starting the experiment we load the `r mlr3` library and set a seed.
...
We loaded the featureless classification baseline (using the default which always predicts the most common class in the training data, but which also has the option of predicting random response values) and a classification decision tree, then the accuracy measure (number of correct predictions divided by total number of predictions), trained our models and finally made and scored predictions.
...
There are two types of classification tasks supported in `r mlr3`: binary classification\index{classification!binary}, in which the outcome can be one of two categories, and multiclass classification\index{classification!multiclass}, where the outcome can be one of three or more categories.
...
If no positive class is specified then `r mlr3` assumes the first level in the `target` column is the positive class, which can lead to misleading results.
...
The final big difference with regression to discuss is `r index('thresholding', aside = TRUE)`.
...
Whilst this model may appear to have good performance on the surface, in fact, it just ignores all 'bad' customers -- this can also create very big problems here, or in healthcare tasks and other settings where `r index('false positives')` cost more than `r index('false negatives')` (see @sec-cost-sens for cost-sensitive classification).
