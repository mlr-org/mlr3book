...
In the simplest definition, `r index('machine learning')` (ML) is the process of learning computer models of relationships from data.[Machine Learning/Supervised Learning]{.aside}
...
Other tasks are also encompassed by supervised learning, and we return to them in @sec-special, which also considers `r index('unsupervised learning')` tasks.
...
A `r index('model', aside = TRUE)` is formally a mapping from a feature vector to predictions, such models are induced by passing `r index('training data')` to machine learning algorithms and include `r index('decision trees', "decision tree")`, `r index('support vector machines', "support vector machine")`, `r index('neural networks', "neural network")`, and many more.
...
This information is extracted automatically when required, so the user does not have to specify the prediction target every time a model is trained, for example.
...
As `R6` uses reference semantics (@sec-r6), you need to use `$clone()` if you want to modify a task whilst keeping the original object intact.
...
```{r basics-030}
predictions = lrn_rpart$predict(tsk_mtcars, row_ids = splits$test)
```
...
library(mlr3viz)
predictions = lrn_rpart$predict(tsk_mtcars, splits$test)
...
The output above is a `r ref("ParamSet", aside = TRUE)` object.
These objects provide information on hyperparameters including their name (`id`), data type (`class`), acceptable ranges for hyperparameter values (`lower`, `upper`), the number of levels possible if the data type is categorical (`nlevels`), the default value from the underlying package (`default`), and finally the set value (`value`).
...
In rare cases (we try to minimize it as much as possible), we alter hyperparameter values when constructing a learner.
When we do this, the reason will always be given in the learner help page.
In the case of `lrn("regr.rpart")`, we changed the `xval` default to `0` because `xval` controls internal cross-validations and if a user accidentally leaves this at the default `10`, model training can take a long time.
...
The `cond` field tells us what the condition is, which will either mean that `id` can be set if `on` equals a single value (`r ref("CondEqual")`) or any value in the listed set (`r ref("CondAnyOf")`).
...
Baselines are useful in model comparison (@sec-performance), as fallback learners (@sec-encapsulation-fallback, @sec-fallback), to be 'composed' into more complex models (@sec-surv-comp), and can be used by sophisticated models (e.g., random forests\index{random forest}) during training and/or prediction.
...
For example, we could score our decision tree to see how long it took to train the model and make predictions:
...
predictions$score(msr_sf, task = tsk_mtcars, learner = lrn_rpart)
```
...
Before starting the experiment we load the `mlr3` library and set a seed (in an exercise below you will be asked to think about why setting a seed is essential for reproducibility in this experiment).
...
We loaded the featureless classification baseline (using the default which always predicts the most common class in the training data) and a classification decision tree, then the accuracy measure (number of correct predictions divided by total number of predictions), trained our models and finally made and scored predictions.
...
There are two types of classification task supported in `mlr3`: binary classification\index{classification!binary}, in which the outcome can be one of two categories, and multiclass classification\index{classification!multiclass}, where the outcome can be one of three or more categories.
...
It is arbitrary which is which, though often the more 'important' class is set as the positive class.
You can set the positive class during or after construction.
If no positive class is specified then `mlr3` assumes the first level in the `target` column is the positive class, which can lead to misleading results, as shown in the example below.
...
tsk_classif = as_task_classif(data, target = "y")
tsk_classif$positive

# changing after construction
tsk_classif$positive = "pos"
tsk_classif$positive
```
...
However, a key difference is that the possible prediction types in classification are either `"response"` -- predicting an observation's class (a penguin's species in our example) -- or `"prob"` -- predicting the probability of an observation belonging to each class in addition.
...
lrn_rpart$train(tsk_penguins, splits$train)
predictions = lrn_rpart$predict(tsk_penguins, splits$test)
predictions
```
...
The `r index('accuracy')` measure evaluates the `"response"` predictions whereas the `r index('brier score')` (`"classif.mbrier"`) (squared difference between predicted probabilities and the truth) and `r index('logloss')` (`"classif.logloss"`) (negative logarithm of the predicted probability for the true class) are evaluating the probability predictions.
...
predictions = lrn_featureless$predict(task_credit, split$test)
predictions$score(msr("classif.acc"))
autoplot(predictions)
```

Whilst this model may appear to have good performance on the surface, in fact, it just ignores all 'bad' customers -- this can create very big problems in healthcare and other settings where there are data biases, or for insurance companies if `r index('false positives')` cost more than `r index('false negatives')` (see @sec-cost-sens for cost-sensitive classification).
...
predictions = lrn_rpart$predict(tsk_zoo, splits$test)
before = autoplot(predictions) + ggtitle("Default thresholds")
new_thresh = proportions(table(tsk_zoo$truth(splits$train)))
new_thresh
predictions$set_threshold(new_thresh)
after = autoplot(predictions) + ggtitle("Inverse weighting thresholds")
before + after + plot_layout(guides = "collect")
```

Again we see that the model better represents all classes after thresholding.
In this example we set the new thresholds to be the proportions of each class in the training set, known as `r index('inverse weighting')`.
Doing so effectively sets the thresholds as the inverse probability of occurring, which means that more common classes are will have higher thresholds and vice versa.
...
Furthermore, learners will make use of weights in different ways so it is important to read the implementation's documentation to understand how weights are used.
...
* Penalized Generalized Linear Models (`"regr.glmnet"`/`"classif.glmnet"`)\index{generalized linear model} and with built-in optimization of the penalization parameter (`"regr.cv_glmnet"`/`"classif.cv_glmnet"`).
* Weighted $k$-Nearest Neighbors\index{k-nearest neighbors} regression (`"regr.kknn"`/`"classif.kknn"`).
