...
Internally, the `fselect()` function creates an `r ref('FSelectInstanceBatchSingleCrit')` object and executes the feature selection with an `r ref('FSelector', index = TRUE)` object, based on the selected method, in this example an `r ref("FSelectorBatchSequential")` object.
...
To start the feature selection, we pass the `FSelectInstanceBatchSingleCrit` object to the `$optimize()` method of the initialized `FSelector` object:
...
We now create an `r ref("FSelector")` and call the `$optimize()` function of the `FSelector` with the `FSelectInstanceBatchMultiCrit` object, to search for the subset of features with the best TPR and FPR.
...
| `r ref("FSelectInstanceBatchSingleCrit")` or `r ref("FSelectInstanceBatchMultiCrit")` | `r ref("fselect()")` | - |
