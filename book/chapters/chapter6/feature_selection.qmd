...
As noted above, the correlation filter can only be calculated for regression tasks with numeric features and can thus not be used with `tsk("penguins")`.
...
tsk_pen = tsk("penguins")
tsk_pen$filter(which(complete.cases(tsk_pen$data())))
```
...
For the `"selected_features"` filter described in embedded methods (@sec-fs-embedded-methods), this step is straight-forward since the methods assign either a value of `1` for a feature to be kept or `0` for a feature to be dropped. Below, we find the names of features with a value of `1` and select those features with `task$select()`:
...
```{r optimization-003, out.width = "80%", echo = FALSE}
#| label: fig-sequential-forward-selection
#| fig-cap: A binary representation of sequential forward selection with four features. We start at the bottom with no selected features (all are '0'). In the next iteration all features are separately tested (each is '1' separately) and the best configuration (darkest in row two) is selected. This continues for selecting the second, third, and fourth features.
...
We start with the simple example from above and do sequential forward selection with `tsk("penguins")`, in contrast to HPO (@sec-optimization), `r ref('fselect()', aside = TRUE)` directly starts the optimization and selects features.
...
Note that `r ref("fselect")` directly starts the optimization and selects features.
...
```{r feature-selection-018-5, output = FALSE, cache = FALSE}
...
To create an `FSelectInstanceSingleCrit` object, we use the sugar function `r ref("fsi()", aside = TRUE)`:
...
Predicting observations present in the data used for feature selection should be avoided.
...
Instead, to get unbiased performance estimates for the current tsk_pen, nested resampling (see @sec-autofselect and @sec-nested-resampling) is required.
:::
...
However, these two subsets will generally not coincide, i.e. the subset with the highest classification accuracy will probably be another subset than that with the lowest training time.
With `mlr3fselect`, the result is the Pareto-optimal\index{Pareto optimality} solution, i.e. the best feature subset for each of the criteria that is not dominated by another subset.
...
The `r ref('AutoFSelector', index = TRUE)` class wraps a learner and augments it with an automatic feature selection for a given task.
...
1. Calculate a correlation filter on `tsk("mtcars")`.
2. Use the filter from the first exercise to select the five best features in `tsk("mtcars")`.
3. Apply a backward selection to `tsk("penguins")` with `lrn("classif.rpart")` and holdout resampling by the measure classification accuracy. Compare the results with those in @sec-fs-wrapper-example. Answer the following questions:
...
4. Automate the feature selection as in @sec-autofselect with `tsk("spam")` `spam` task and `lrn("classif.log_reg")`.
