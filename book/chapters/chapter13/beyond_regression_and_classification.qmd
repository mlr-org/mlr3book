...
However all costs are positive, which means a loss is made, so let us now see if we can improve these models by using thresholding.
...
In fact, a false positive results in a cost of +5 whereas a false negative only results in a cost of 0, hence we would prefer a model with higher false negative rates and lower false positive rates.
This is highlighted in the `"threshold"` `autoplot` (@fig-costsens-threshold).
...
As expected, the optimal threshold is greater than 0.5 which leads the model to predict 'bad' credit more often.
...
```{r, warning=FALSE, message=FALSE, output = FALSE, cache = FALSE}
autoplot(tsk_rats)
```
...
`mlr3proba` has a different predict interface to `mlr3` as all possible types of prediction ('predict types') are returned when possible for all survival models -- i.e., if a model can compute a particular predict type then it is returned in `PredictionSurv`.
...
In this chunk we are partitioning the `rats` dataset and training a `r index("Cox Proportional Hazards", aside = TRUE)` model (`lrn("surv.coxph")`) on the training set and making predictions for the test set.
For this model, all predict types except `response` can be computed.

```{r special-015}
tsk_rats = tsk("rats")
split = partition(tsk_rats)
prediction = lrn("surv.coxph")$train(tsk_rats, split$train)$
...
data.frame(pred = prediction$response[1:3],
  truth = prediction$truth[1:3])
```
...
```{r special-020}
as.data.table(mlr_measures)[1:5,
...
In the code below we use the recommended measures and find this model's performance seems okay as the RCLL and DCalib are relatively low 0 and the C-index is greater than 0.5.
...
Below we construct a C-Means clustering learner with `"prob"` prediction type and three clusters, train it on the `ruspini` dataset and then return the cluster assignments (`$assignments`) for each observation.
...
We can see that K-means clustering gives us a higher average silhouette score and might assume that K-means learner with two centroids is a better choice than DBSCAN method.
...
p = autoplot(pred_kmeans, tsk_spirals)
spirals_data = tsk_spirals$data()

df_kmeans = cbind(spirals_data, clust = as.factor(pred_kmeans$partition))
df_dbscan = cbind(spirals_data, clust = as.factor(pred_dbscan$partition))

p_kmeans = ggplot(df_kmeans, aes(x = V1, y = V2, color = clust)) +
  geom_point() + theme_minimal() + ggtitle("K-means")
p_dbscan = ggplot(df_dbscan, aes(x = V1, y = V2, color = clust)) +
  geom_point() + theme_minimal() +
  theme(axis.text.y = element_blank(), axis.title.y = element_blank(),
    axis.ticks.y = element_blank()) + ggtitle("DBSCAN")
...
By plotting the result of PCA (@fig-beyond-clust-usarrests), we see that our model has separated observations into two clusters along the first two principal components.
...
```{r special-043, warning=FALSE, echo=FALSE, message=FALSE, fig.height = 5}
...
@tbl-beyond-api summarizes the most important classes, functions, and methods seen in this chapter.
...
3. Train and predict a survival forest using `rfsrc` (from `r mlr3extralearners`). Run this experiment using `task = tsk("rats"); split = partition(task)`. Evaluate your model with the RCLL measure.
4. Estimate the density of the `tsk("precip")` data using `lrn("clust.kmeans")` (from `mlr3extralearners`). Run this experiment using `task = tsk("precip"); split = partition(task)`. Evaluate your model with the logloss measure.
...
6. Run a (spatially) unbiased classification benchmark experiment on `tsk("ecuador")` with a featureless learner and xgboost, evaluate with the binary Brier score.
