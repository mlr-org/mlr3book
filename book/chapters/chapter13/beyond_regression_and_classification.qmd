...
`r index('Functional data analysis')` deals with data where each observation is a curve or function rather than a single number, for example a spectrum measured across wavelengths or a signal recorded over time.
...
* `r ref("PipeOp")`s for preprocessing: smoothing (`po("fda.smooth")`), interpolation to a common grid (`po("fda.interpol")`), and domain scaling (`po("fda.scalerange")`).
* `r ref("PipeOp")`s for feature extraction: summary statistics (`po("fda.extract")`), functional PCA (`po("fda.fpca")`), flattening (`po("fda.flatten")`), discrete wavelet transform (`po("fda.wavelets")`), and others.
...
po_interpol = po("fda.interpol", grid = "union", method = "linear")
...
```{r fda-007, message=FALSE, warning=FALSE, results='hide'}
...
In general, all standard `mlr3`-workflows, i.e. resampling, benchmarking, tuning, and the use of pipelines, can be applied to functional data tasks through the `PipeOp`-based feature extraction provided by `r mlr3fda`.
