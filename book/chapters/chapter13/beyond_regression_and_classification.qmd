...
As expected, the optimal threshold is greater than 0.5 which leads the model to predicting 'bad' credit more often.

To automate the process of optimizing the threshold, we can make use of `r mlr3tuning` (@sec-optimization) and `r mlr3pipelines` (@sec-pipelines) by creating a graph with `po("tunethreshold")`.
Continuing the same example:

```{r special-005}
graph = po("learner_cv", lrn("classif.log_reg", predict_type = "prob")) %>>%
...
Note how `as_task_surv()` coerces the target columns into a `r ref("survival::Surv")` object.
...
#| fig-cap: Kaplan-Meier plot of `tsk("rats")`. x-axis is time variable and y-axis is survival function, S(T), defined by 1 - F(T) where F is the cumulative distribution function. Red crosses indicate points where censoring takes place.
...
Similarly to these settings, survival learners are constructed with `r ref("lrn()")`; available learners are listed in @sec-appendix-overview-tables.

`mlr3proba` has a different predict interface to `mlr3` as all possible types of prediction ('predict types') are returned when possible for all survival models -- i.e., if a model can compute a predict type then it is returned in `PredictionSurv`.
...
In this chunk we are partitioning the `rats` dataset and training a `r index("Cox Proportional Hazards", aside = TRUE)` model (`lrn("surv.coxph")`) on the training set and making predictions for the predict set.
For this model, all predict types except `response` can be computed.

```{r special-015}
tsk_rats = tsk("rats")
split = partition(tsk_rats)
prediction = lrn("surv.coxph")$train(tsk_rats, split$train)$predict(tsk_rats, split$test)
prediction
```
...
prediction = lrn("surv.svm", type = "regression", gamma.mu = 1e-3)$
  train(tsk_rats, split$train)$predict(tsk_rats, split$test)
data.frame(pred = prediction$response[1:3], truth = prediction$truth[1:3])
```
...
split = partition(tsk_rats)
prediction = lrn("surv.coxph")$train(tsk_rats, split$train)$predict(tsk_rats, split$test)
prediction$distr[1:3]$survival(77)
```

The output indicates that there is a `r paste0(round(prediction$distr[1:3]$survival(77)*100, 1), "%")`, chance of the first three predicted rats being alive at time 77 respectively.
...
Survival models in `mlr3proba` are evaluated with `r ref("MeasureSurv")` objects, which are constructed in the usual way with `msr()`; measures currently implemented are listed in see @sec-appendix-overview-tables.
...
```{r}
prediction$score(msrs(c("surv.rcll", "surv.cindex", "surv.dcalib")))
```
...
In `mlr3proba` we make use of composition internally to return a `"crank"` prediction for every learner.
...
# now with distr
graph_learner$train(task, split$train)$predict(task, split$test)
```
...
In this small experiment, Coxnet and Cox PH have the best discrimination, Cox PH has the best calibration, and Coxnet and Cox PH have similar overall predictive accuracy (with lowest RCLL).
...
prediction = learner$train(task, 1:900)$predict(task, 901:1000)
x = seq.int(-2, 2, 0.01)
ggplot(data.frame(x = x, y = prediction$distr$pdf(x)), aes(x = x, y = y)) +
...
```{r special-025}
learner = lrn("dens.hist")
prediction = learner$train(task, 1:10)$predict(task, 11:13)
prediction
cbind(prediction$distr$cdf(task$data()$x[11:13]), prediction$cdf[1:3])
```
...
`r index("Cluster analysis", aside = TRUE)` is another unsupervised task implemented in `mlr3`.
...
Currently we have two clustering tasks shipped with `mlr3cluster`:
...
Below we construct a C-Means clustering learner with `"prob"` prediction type and 3 clusters, train it on the `ruspini` dataset and then return the cluster assignments (`$assignments`) for each observation.
...
For consistency, `mlr3cluster` offers `predict` method for hierarchical clusters but with a warning:
...
prediction$score(measures, task = task)
```
...
pred_kmeans = bmr$resample_results$resample_result[[1]]$prediction()
pred_dbscan = bmr$resample_results$resample_result[[2]]$prediction()
autoplot(pred_kmeans, task)
autoplot(pred_dbscan, task)
```
...
The C-means and K-means algorithms are both considerably better than the featureless baseline.
...
Visually this can be seen using built-in `r ref("mlr3spatiotempcv::autoplot")` methods.
...
See the `mlr3spatiotempcv` visualization `r link("https://mlr3spatiotempcv.mlr-org.com/articles/spatiotemp-viz.html", "vignette")` for specific details about 3D spatiotemporal visualization.
:::
...
```{r special-049, cache.lazy=FALSE, cache=FALSE}
learner = lrn("classif.ranger")$train(tsk_leipzig)
prediction = predict_spatial(leipzig_raster, learner, format = "terra")
prediction
```
...
In this chapter, we explored going beyond deterministic regression and classification to see how functions in `mlr3` can be used to implement other machine learning tasks.
Cost-sensitive classification extends the 'normal' classification setting by assuming the cost associated with `r index('false negatives')` and `r index('false positives')` are unequal.
...
The `mlr3` interface is highly extensible, which means future machine learning tasks can (and will) be added to our universe and we will add these to this chapter of the book in future editions.

## Exercises

We will run `set.seed(11)` before each of our solutions so you can reproduce our results.

1. Run a benchmark experiment on `tsk("german_credit")` with algorithms: `lrn("classif.featureless")`, `lrn("classif.log_reg")`, `lrn("classif.ranger")`. Tune the `lrn("classif.featureless")` model using `tunetreshold` and `learner_cv`. Use 2-fold CV and evaluate with `msr("classif.costs", costs = costs)` where you should make the parameter `costs` so that the cost of a true positive is -10, the cost of a true negative is -1, the cost of a false positive is 2, and the cost of a false negative is 3. Are your results surprising?
2. Train and predict a survival forest using `rfsrc` (from `r mlr3extralearners`). Run this experiment using `task = tsk("rats"); split = partition(task)`. Evaluate your model with the RCLL measure.
3. Estimate the density of the `tsk("precip")` data using `lrn("clust.kmeans")` (from `mlr3extralearners`). Run this experiment using `task = tsk("precip"); split = partition(task)`. Evaluate your model with the logloss measure.
...
5. Run a (spatially) unbiased classification benchmark experiment on `tsk("ecuador")` with a featureless learner and xgboost, evaluate with the binary Brier score.
