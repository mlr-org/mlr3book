...
We will use the `tsk("german_credit")` task as a running example.
...
#| fig-cap: Kaplan-Meier plot of the `tsk("rats")` task. x-axis is time variable and y-axis is survival function, S(T), defined by 1 - F(T) where F is the cumulative distribution function. Red crosses indicate points where censoring takes place.
...
prediction = lrn("surv.coxph")$train(t, split$train)$predict(tsk_rats, split$test)
prediction
```
...
data.frame(pred = prediction$response[1:3], truth = pred$truth[1:3])
```
...
5. Run a (spatially) unbiased classification benchmark experiment on the `tsk("ecuador")` task with a featureless learner and xgboost, evaluate with the binary Brier score.
