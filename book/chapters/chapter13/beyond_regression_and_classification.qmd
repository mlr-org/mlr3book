...
However, this may not be a sensible approach as the cost of a false positive and `r index('false negative')` is not equal.
In fact, a `r index('false positive')` results in a cost of +5 whereas a false negative only results in a cost of 0, hence we would prefer a model with higher false negative rates and lower false positive rates.
...
By using `po("learner_cv")` for internal resampling and `po("tunethreshold")` to find the optimal threshold we have improved our model performance considerable can now even expect a profit.
...
```{r special-043, warning=FALSE, echo=FALSE, message=FALSE, out.width = "60%", out.height = "60%", fig.height = 5}
...
```{r special-050, message = FALSE, cache.lazy=FALSE, cache=FALSE, out.width = "50%", out.height = "50%"}
