...
| Functional data analysis | `r mlr3fda`  | Analysis and modeling of data where each observation is a function, often varying over time or space. |
...
The package `pendensity` was removed from CRAN in 2026.
The following code chunks will therefore fail.
See https://github.com/mlr-org/mlr3book/issues/935 for more details.
We will replace `dens.pen` soon.
...
## Functional Data Analysis (FDA) {#sec-fda}

`r define("Functional data analysis")` deals with data where each observation is a curve or function rather than a single number, for example a spectrum measured across wavelengths or a signal recorded over time.
Because the measurements within one curve are ordered and dependent, standard ML methods that treat each measurement point as an independent feature tend to perform poorly.
The package `r mlr3fda` extends `mlr3` with support for functional data by providing `r ref("PipeOp")`s that preprocess functional columns and extract scalar features from them, so that any standard `mlr3` learner can be applied afterward.

Functional data is represented using the `tfd_reg` (regular) and `tfd_irreg` (irregular) types from the `r ref_pkg("tf")` package.
Regular functional data means all observations are measured at the same set of input points (e.g. the same wavelengths), while irregular data allows each observation to have its own set of input points.

The package provides:

* `PipeOp`s for preprocessing: smoothing (`po("fda.smooth")`), interpolation to a common grid (`po("fda.interpol")`), and domain scaling (`po("fda.scalerange")`).
* `PipeOp`s for feature extraction: summary statistics (`po("fda.extract")`), functional PCA (`po("fda.fpca")`), flattening (`po("fda.flatten")`), discrete wavelet transform (`po("fda.wavelets")`), and others.

### Tasks

`mlr3fda` ships with three example tasks.
The `"fuel"` task contains spectral data of fossil fuels with two regular functional features (`NIR` and `UVVIS`) and one scalar feature (`h2o`), with the goal of predicting the heat value (`heatan`).
The `"dti"` task contains diffusion tensor imaging data with two irregular functional features (`cca` and `rcst`), and the `"phoneme"` task is a classification task with a single functional feature.

```{r beyond_regression_and_classification-048}
library(mlr3fda)
...
We can see that the task has two functional features of type `tfr` (regular).
Let us look at all tasks shipped with `mlr3fda`:

```{r beyond_regression_and_classification-049}
as.data.table(mlr_tasks)[tfr > 0 | tfi > 0, c(1:2, 4:5)]
```

### Preprocessing

Before extracting features, functional data often needs preprocessing.
Two common steps are smoothing noisy curves and interpolating irregular data to a common grid.

Smoothing uses `po("fda.smooth")`, which applies methods like LOWESS or rolling means to reduce noise in the functional observations:

```{r beyond_regression_and_classification-050}
po_smooth = po("fda.smooth", method = "rollmean", args = list(k = 5))
task_smoothed = po_smooth$train(list(task_fuel))[[1L]]
```

For irregular functional data (like in the `"dti"` task), observations are measured at different input points.
The `po("fda.interpol")` operator interpolates all observations to a common grid, which is required by some downstream operations like FPCA.
With `grid = "minmax"`, we restrict each functional feature to the range shared by all its curves, so linear interpolation does not require extrapolation beyond their observed ranges:

```{r beyond_regression_and_classification-051}
task_dti = tsk("dti")
task_dti$data(cols = "cca", rows = 1:2)

po_interpol = po("fda.interpol", grid = "minmax", method = "linear")
task_dti_reg = po_interpol$train(list(task_dti))[[1L]]
```

### Feature Extraction

Since there are currently no learners that directly operate on functional data in `mlr3fda`, the functional columns must be transformed into scalar features before applying a standard learner.
The simplest approach is `po("fda.extract")`, which computes summary statistics (mean, min, max, slope, median, variance) from each functional column:

```{r beyond_regression_and_classification-052}
po_extract = po("fda.extract",
  features = c("mean", "min", "max", "slope"),
  drop = TRUE
)
task_extracted = po_extract$train(list(task_fuel))[[1L]]
task_extracted$head()
```

Setting `drop = TRUE` removes the original functional columns and keeps only the extracted numeric features.

A more sophisticated approach is functional principal component analysis (FPCA) via `po("fda.fpca")`, which extracts principal component scores that capture the main modes of variation in the functional data.
Note that FPCA requires regular functional data, so irregular data must be interpolated first:

```{r beyond_regression_and_classification-053}
po_fpca = po("fda.fpca", n_components = 3L)
task_fpca = po_fpca$train(list(task_fuel))[[1L]]
task_fpca$head()
```

### Putting It All Together

Because the feature extraction `PipeOp`s produce standard numeric features, they can be combined with any learner in a `r ref("GraphLearner")`.
This makes it straightforward to benchmark different feature extraction strategies.
Let us compare three approaches on the `"fuel"` task: summary statistics extraction, FPCA, and flattening.

```{r beyond_regression_and_classification-054, message=FALSE, warning=FALSE}
glrn_extract = as_learner(
  po("fda.extract", features = c("mean", "min", "max", "slope"), drop = TRUE) %>>%
    lrn("regr.rpart")
)
glrn_extract$id = "extract+rpart"

glrn_fpca = as_learner(
  po("fda.fpca", n_components = 5L) %>>%
    lrn("regr.rpart")
)
glrn_fpca$id = "fpca+rpart"

glrn_flatten = as_learner(
  po("fda.flatten") %>>%
    lrn("regr.rpart")
)
glrn_flatten$id = "flatten+rpart"

learners = list(glrn_extract, glrn_fpca, glrn_flatten)
bmr = benchmark(benchmark_grid(task_fuel, learners, rsmp("cv", folds = 3)))
bmr$aggregate(msr("regr.rmse"))[, c("learner_id", "regr.rmse")]
```

In this small experiment, we can compare how different representations of the functional features affect predictive performance.
The FPCA approach provides a compact representation of the curves that often works well in practice, while simple summary statistics offer an easy-to-interpret baseline.
The flatten approach, which treats each observed point as a separate feature, can capture fine-grained patterns but may suffer from the curse of dimensionality.

In general, all standard `mlr3`-workflows, i.e. resampling, benchmarking, tuning, and the use of pipelines, can be applied to functional data tasks through the `PipeOp`-based feature extraction provided by `mlr3fda`.
...
We saw how we can use the predict type `"quantiles"` to predict conditional quantiles and construct prediction intervals.
Finally, functional data analysis with `r mlr3fda` enables working with curve-valued observations by extracting scalar features through preprocessing `PipeOp`s.
...
| `r ref("mlr3pipelines::PipeOp")` | `po("fda.extract")`, `po("fda.fpca")`, `po("fda.flatten")` | - |
