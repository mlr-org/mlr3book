...
It is very important for predictive tasks that you keep track of missing data as it is common for the reasons behind missing data to be informative.
To preserve information about missing data, imputation should be tracked by adding binary indicator features (one for each imputed feature) that are `1` if the feature was missing for an observation and `0` if it was present (`po("missind")`).
It is important to note that recording this information may not prevent problems in model interpretation.
