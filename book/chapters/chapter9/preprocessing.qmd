...
To preserve the information about the missingness for the model, imputation should be tracked by adding binary indicator features (one for each imputed feature) that are `1` if the feature was missing for an observation and `0` if it was present (`po("missind")`).
But also problems in model interpretation can arise, as in this real-world example: medical data are typically collected more extensively for white communities than for racially minoritized communities.
...
Nevertheless, these methods have their justification: Not for the predictive performance, but for finding imputations that fit well to the distribution of the observed values, so that a model can be fitted that can be interpreted and analyzed in a second step.
