...
Common difficulties in data that can be solved with feature engineering include features with (high) skew distributions, high cardinality categorical features, missing observations, high dimensionality and imbalanced classes in classification tasks.
...
Typically it can work well with natural language processing and computer vision problems, while standard tabular data lacks the structure for deep learning models to extract meaningful features automatically.
...
Hence, manual feature engineering is often required but with `mlr3pipelines`, we can simplify the process as much as possible.

As we work through this chapter we will use an adapted version of the Ames housing data (@de2011ames).
We changed the data slightly and introduced some additional (artificial) problems to showcase as many aspects of preprocessing as possible on a single dataset, the code to recreate this version of the data from the original raw data can be found at `r link("https://github.com/mlr-org/mlr3book/data")`.
This dataset was collected as an alternative to the Boston Housing data and is commonly used to demonstrate feature engineering in ML.
...
ames = data.table::fread(url, stringsAsFactors = TRUE)
```
...
This is typically called dummy\index{dummy encoding|see{encoding, treatment}} or treatment encoding\index{encoding!treatment} and is required if the learner is a generalized linear (GLM) or additive model (GAM) model.
...
Impact encoding is a good approach for handling high-cardinality features.
`r index('Impact encoding', 'impact', parent = 'encoding', aside = TRUE)` converts categorical features into numeric values based on the impact of the feature on the target.
...
As well as encoding features, other basic preprocessing steps for categorical features include removing constant features (which only have one level and may have been removed as part of data cleaning), and collapsing levels that occur very rarely as these may be missed during resampling, though stratification can be used to mitigate this (@sec-strat-group).
...
#| fig-cap: Mean imputation of missing values using existing values.
...
In the code below we create a pipeline from the `PipeOp`s listed above as well as making use of `po("featureunion")` to combine multiple `PipeOp`s acting on the `"integer"` columns.
...
`mlr3pipelines` offers a simple and reusable pipeline for (among other things) `r index('imputation')` and factor `r index('encoding')` called `index('ppl("robustify")', aside = TRUE, code = TRUE)`, which includes sensible defaults that can be used most of the time when encoding or imputing data.
The pipeline includes the following `PipeOp`s (some are applied multiple times and most use selectors):
...
Normalization of features may also be necessary to ensure features with a larger scale do not have a higher impact, which is especially important for distance-based methods such as `r index('K-nearest neighbor')` models or regularized parametric models such as Lasso or Elastic net.
...
With the target transformation and the `ppl("robustify")` pipeline, the simple linear regression now appears to be the best-performing model.
...
In @sec-feature-selection we look at automated `r index('feature selection')` and how automated approaches with filters and wrappers can be used to reduce a dataset to the optimal set of features.
...
energy_data = data.table::fread(url, stringsAsFactors = TRUE)
```
...
bmr_final = benchmark(benchmark_grid(c(tsk_ames_ext, tsk_ames), learners,
  rsmp_cv3))
bmr_final$aggregate(measure = msr_mae)
```
...
We focused primarily on feature engineering, which can make use of `r mlr3pipelines` to automate preprocessing as much as possible whilst still ensuring user control.
...
Preprocessing is almost always required in machine learning experiments, and applying the `ppl("robustify")` pipeline will help in many cases to simplify this process by applying the most common preprocessing steps, we will see this in use in @sec-large-benchmarking.
