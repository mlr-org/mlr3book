...
Hence, manual feature engineering is still often required but with `mlr3pipelines`, we can simplify the process as much as possible.
...
For all three problems, simply removing the problematic features (or feature in a pair) might the best course of action.
...
Unfortunately, these types of problems can occur as artifacts in resampling as dataset size is further reduced here.
...
A common problem in real-world data is `r index('missing values')` in features.
...
9. `po("collapsefactors")` -- Categorical features levels are collapsed (starting from the rarest factors in the training data) until there are less than a certan number of levels.
  This number is controlled by the `max_cardinality` argument and defaults to a conservative 1000.
...
If these are provided, then the resulting pipeline will be set up to handle the given task and learner specifically: it will e.g. not impute missing values if the learner has the `"missings"` property, or if there are no missing values in the task to begin with.
...
```{r preprocessing-027, warning=FALSE, R.options = list(datatable.print.nrows = 13)}
...
4. Apply `pipeline_robustify` to glmnet and compare the results with your pipeline.
