...
`mlr3` uses custom `r index('condition classes')` (defined in `r ref_pkg("mlr3misc")`) to categorize errors and warnings.
...
The corresponding warning classes are:

* `r ref("mlr3misc::warning_mlr3()")` -- Base warning class for all mlr3-related warnings.
* `r ref("mlr3misc::warning_config()")` -- Warns about potential user misconfiguration.
* `r ref("mlr3misc::warning_input()")` -- Warns about potentially invalid input.

By default, errors of class `Mlr3ErrorConfig` always propagate immediately, even when encapsulation is active -- configuration errors indicate a bug in the setup that should be fixed rather than silently handled by a fallback learner.
All other errors (e.g., `Mlr3ErrorLearnerTrain` or `Mlr3ErrorTimeout`) are caught and the fallback learner is used instead.
