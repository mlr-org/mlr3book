...
The `r ref_pkg("mirai")` package is an alternative parallelization backend that starts persistent R sessions called daemons.
Compared to the `r ref_pkg("future")` package, `mirai` has significantly lower overhead per task, making it especially advantageous when training many fast-fitting models.
...
To parallelize computations, start daemons with `mirai::daemons()`.
...
With only daemons configured in the main R session, outer loop runs in parallel while the inner loop runs sequentially on each daemon.
...
To parallelize both outer and inner loops, use `mirai::everywhere()` to configure daemons for the inner loop.
...
```{r advanced_technical_aspects_of_mlr3-030a, error = TRUE}
lrn_debug = lrn("classif.debug", error_train = 1)
