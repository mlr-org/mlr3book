...
For these objects, e.g., certain learners, filters, and pipeops, `mlr3` simply provides a unified interface to control the execution.
...
By default, all CPUs of your machine are used unless you specify the argument `workers` in `future::plan()` (see previous section with issues that might cause).
...
%%| fig-cap: CPU utilization for 4 CPUs while parallelizing the outer five-fold CV with a sequential two-fold CV inside. Jobs are labeled as [iteration outer]-[iteration inner].
gantt
...
%%| fig-cap: CPU utilization for 4 CPUs while parallelizing the inner 2-fold CV with a sequential five-fold CV outside. Jobs are labeled as [iteration outer]-[iteration inner].
gantt
    title CPU Utilization
    dateFormat  s
    axisFormat %S
...
learner$param_set$values$error_train = 1
...
```{r technical-024}
learner = lrn("classif.debug", error_train = 0.3)
...
To demonstrate the `r ref("mlr3db::DataBackendDplyr")` we use the NYC flights dataset from the `r ref_pkg("nycflights13")` package and move it into a `r index("SQLite")` database.
...
To use this measure in `mlr3`, we need to create a new `r ref("R6::R6Class")`, which will inherit from `Measure` and in this case specifically inheriting from `r ref("MeasureRegr")`.
