...
* `r index('Parallelization')` with the `r ref_pkg("future")` and `r ref_pkg("mirai")` frameworks (@sec-parallelization);
...
* The `r index("parallelization backend", aside = TRUE)` is the hardware to parallelize with a respective interface provided by an R package.
  ...
  Alternatively, `mlr3` also provides direct support for the `r ref_pkg("mirai")` package, which offers a lightweight backend with lower overhead per task (see @sec-parallel-mirai).
...
For example, we could reduce the overhead of parallelizing a `for`-loop with 10000 iterations on two CPU cores by `r index('chunking')` the work of the 10000 jobs into two computational jobs performing 5000 iterations each, resulting in two big jobs and not 10000 small ones.
...
```{r advanced_technical_aspects_of_mlr3-007, echo = FALSE}
...
The `mirai` package is an alternative parallelization backend that starts persistent R sessions called `r index("daemons")`.
Compared to the `future` package, `mirai` has significantly lower overhead per task, making it especially advantageous when training many fast-fitting models.
Manual chunking is no longer necessary, because `mirai`'s dispatcher automatically distributes tasks across available daemons.

To parallelize computations, start daemons with `r ref("mirai::daemons()")`.
The compute profile must be set to `"mlr3_parallelization"`.
To make results reproducible with `mirai`, set a `seed` when creating daemons.
For local daemons, you can redirect log messages to the main session with `output = TRUE`.
...
`mirai` also supports nested setups.
With only daemons configured in the main R session, the outer loop runs in parallel while the inner loop runs sequentially on each daemon.

```{r advanced_technical_aspects_of_mlr3-021}
...
To parallelize both outer and inner loops, use `r ref("mirai::everywhere()")` to configure daemons for the inner loop.

```{r advanced_technical_aspects_of_mlr3-022, eval = FALSE}
mirai::daemons(5, .compute = "mlr3_parallelization")
...
```{r advanced_technical_aspects_of_mlr3-024, error = TRUE}
...
```{r advanced_technical_aspects_of_mlr3-026}
lrn_debug$log
lrn_debug$warnings
lrn_debug$errors
```
...
```{r advanced_technical_aspects_of_mlr3-029, error = TRUE}
...
```{r advanced_technical_aspects_of_mlr3-031}
lrn_debug$log
lrn_debug$model
...
```{r advanced_technical_aspects_of_mlr3-032}
...
```{r advanced_technical_aspects_of_mlr3-033}
...
### `r index('Condition Classes', "condition classes")` {#sec-condition-classes}

`mlr3` uses custom `r index('condition classes')` to categorize errors and warnings.
These classes determine how encapsulation and fallback learners behave.

The following error classes are available (see `r ref("mlr3misc::mlr_conditions")` for full details):

* `r ref("mlr3misc::error_mlr3()")` -- Base error class for all mlr3-related errors.
* `r ref("mlr3misc::error_config()")` -- Signals user misconfiguration, e.g., an invalid learner setup.
* `r ref("mlr3misc::error_input()")` -- Signals invalid input.
* `r ref("mlr3misc::error_timeout()")` -- Signals a timeout during encapsulation.
* `r ref("mlr3misc::error_learner()")` -- Signals a general learner error.
* `r ref("mlr3misc::error_learner_train()")` -- Signals that a learner failed during training.
* `r ref("mlr3misc::error_learner_predict()")` -- Signals that a learner failed during prediction.

By default, errors of class `Mlr3ErrorConfig` always propagate immediately, even when encapsulation is active.
Configuration errors indicate a setup bug that should be fixed, not silently handled by a fallback learner.
All other errors (for example, `Mlr3ErrorLearnerTrain` or `Mlr3ErrorTimeout`) are caught, and the fallback learner is used instead.

This default behavior can be customized by passing a `when` function to `$encapsulate()`.
This function receives the condition object (`cond`) and the stage (`"train"` or `"predict"`) and returns `TRUE` if the fallback should be used, or `FALSE` if the error should propagate.
For example, to also let timeout errors propagate instead of falling back:

```{r advanced_technical_aspects_of_mlr3-034, error = TRUE}
lrn_debug = lrn("classif.debug", error_train = 1)
...
`r mlr3` uses the `r ref_pkg("lgr")` package to control the verbosity of the output, i.e., to decide how much output is shown when `mlr3` operations are run, from suppression of all non-critical messages to detailed messaging for debugging.
...
### Logger Hierarchy {#sec-logger-hierarchy}

The `mlr3` ecosystem uses a hierarchical logging system.
The base logger is named `"mlr3"` and serves as the parent for all loggers in the ecosystem.
Each package registers its own child logger under this base logger:

* `"mlr3"` -- The base logger for the entire mlr3 ecosystem.
* `"mlr3/core"` -- Child logger for the core `mlr3` package.
* `"mlr3/bbotk"` -- Child logger for `r bbotk`, also used by `r mlr3tuning` and `r mlr3fselect`.
* `"mlr3/mlr3pipelines"` -- Child logger for `r mlr3pipelines`.

Setting the threshold on the base `"mlr3"` logger controls the logging level for the entire ecosystem.
Child loggers inherit the threshold of their parent, but can also be configured independently.
This makes it easy to silence all logging at once or to selectively enable logging for specific packages.

### Changing Log Levels {#sec-log-levels}
...
To change the logging threshold you need to retrieve the `R6` logger object from `lgr`, and then call `$set_threshold()`.
To change the threshold for the entire ecosystem, set it on the base logger:

```{r advanced_technical_aspects_of_mlr3-035, eval = FALSE}
lgr::get_logger("mlr3")$set_threshold("debug")
```

Or to suppress all messaging except warnings:

```{r advanced_technical_aspects_of_mlr3-036, eval = FALSE}
lgr::get_logger("mlr3")$set_threshold("warn")
```

Because child loggers inherit the threshold of their parent, this silences all packages in the ecosystem.
To selectively control logging, set the threshold on individual child loggers.
For example, to disable logging from the core `mlr3` package but keep logging from `r mlr3tuning`:

```{r advanced_technical_aspects_of_mlr3-037, eval=FALSE}
lgr::get_logger("mlr3/core")$set_threshold("warn")
lgr::get_logger("mlr3/bbotk")$set_threshold("info")
```


### Redirecting Log Output {#sec-log-output}
...
```{r advanced_technical_aspects_of_mlr3-039, message = FALSE}
# load data
...
```{r advanced_technical_aspects_of_mlr3-044, message=FALSE}
...
```{r advanced_technical_aspects_of_mlr3-046}
...
```{r advanced_technical_aspects_of_mlr3-047}
...
```{r advanced_technical_aspects_of_mlr3-048}
...
```{r advanced_technical_aspects_of_mlr3-049}
...
```{r advanced_technical_aspects_of_mlr3-050, eval = FALSE}
tsk_penguins = tsk("penguins")
