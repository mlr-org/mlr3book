...
So far, we have covered machine learning concepts without going into a lot of technical detail, which can be important for more advanced uses of `r mlr3`.
...
* The `r index("parallelization backend", aside = TRUE)` is the hardware to parallelize with a respective interface provided by an R package.
  ...
  From a user perspective, `r mlr3` interfaces with `r ref_pkg("future")` directly so all you will need to do is configure the backend before starting any computations.
...
For example, we could reduce the overhead of parallelizing a `for`-loop with 1000 iterations on four CPU cores by chunking the work of the 1000 jobs into four computational jobs performing 250 iterations each, resulting in four bigger jobs and not 1000 small ones.
...
For these objects, e.g., certain learners, filters, and `PipeOp`s, `mlr3` simply provides a unified interface to control the execution.
...
system.time({resample(tsk_spam, lrn_rpart, rsmp_cv3)})
```
...
* Option `mlr3.exec_chunk_size` can be used to control how many jobs are mapped to a single `future` and defaults to one.
...
This approach makes the computation fine-grained and allows the `r ref_pkg("future")` backend to group the jobs into chunks of suitable size (depending on the number of workers), it also makes the procedure identical to parallelizing resampling:
...
lrn_rpart_tuned = auto_tuner(tnr("mbo"), lrn_rpart, rsmp("cv", folds = 2),
  msr("classif.ce"), 20)

resample(tsk("penguins"), lrn_rpart_tuned, rsmp("cv", folds = 5))
```
...
This example would run on eight cores on the local machine, parallelizing the outer resampling on two, and the inner resampling on four workers.
...
For illustration (and internal testing) of error handling, `mlr3` ships with the learners `lrn("classif.debug")` and `lrn("regr.debug")`.
...
With the learner's default settings, the learner will remember a random label and constantly predict this label, so let us change that and tell the learner to signal an error during the training step:
...
This means that models can be used for fitting and predicting and any conditions can be analyzed post hoc, however, the result for our experiment will be a missing model and/or predictions, depending on where the error occurs, in @sec-fallback we will discuss fallback learners to replace missing models and/or predictions.
...
# set error_train and remove segfault_train
lrn_debug$param_set$values = list(error_train = 1)
# no saved model as there was an error during training
lrn_debug$train(tsk_penguins)$model

# set error_predict and remove error_train
lrn_debug$param_set$values = list(error_predict = 1)
...
`mlr3` includes two baseline learners: `lrn("classif.featureless")`, which always predicts the majority class, and `lrn("regr.featureless")`, which always predicts the mean response (extension packages include task-specific baselines).
...
In the next example, we add a classification baseline to our debug learner, so that when the debug learner errors and encapsulation is enabled, `mlr3` falls back to the predictions of the featureless learner internally.
...
Finally, `mlr3` uses `r ref_pkg("future")` and encapsulation (@sec-encapsulation) to make evaluations fast, stable, and reproducible.
...
To demonstrate the `r ref("mlr3db::DataBackendDplyr")` we use the (pretty big) NYC flights dataset from the `r ref_pkg("nycflights13")` package and move it into a `r index("SQLite")` database.
...
By example, below we first query the location of an example dataset in a Parquet file shipped with `r ref_pkg("mlr3db")` and then convert the resulting `r ref("DataBackendDuckDB")` object into a classification task, all without loading the dataset into memory:
...
Whilst many classes could be extended, all have a similar design interface and so, will only demonstrate how to create a custom `r ref("Measure")`.
...
So now we will turn to extending the `Measure` class to implement new metrics.
...
To find out more about logging, then have a read of the vignettes in `ref_pkg("lgr")`, which cover everything from logging to JSON files to retrieving logged objects for debugging.
