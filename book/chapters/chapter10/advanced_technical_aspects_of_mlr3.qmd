...
# predict
lrn_ranger$predict(tsk_sonar)$score()
```
...
Each `Learner` contains the field `$encapsulate` to control how the train or predict steps are wrapped.
...
To make this procedure convenient during resampling and benchmarking, we support fitting a baseline (though in theory you could use any `Learner`) as a `r index('fallback learner')`.
