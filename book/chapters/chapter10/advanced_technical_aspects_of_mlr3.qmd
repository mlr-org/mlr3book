...
In the previous chapters, we demonstrated how to turn machine learning concepts and methods into code without going into a lot of technical detail.
In this chapter we will turn to those technical details that can be important for more advanced uses of `mlr3`, including:
...
Larger computational sites rely on scheduling systems to orchestrate the computation for multiple users and offer a shared network file system all machines can access.
...
* The `r index("parallelization backend", aside = TRUE)` is the hardware to parallelize with a respective interface provided by an R package.
  ...
  From a user perspective, `mlr3` interfaces with `r ref_pkg("future")` directly so all you will need to do is configure the backend before starting any computations.
...
An important step in parallel programming involves the identification of sections of the program flow that are both time-consuming ('bottlenecks') and can run independently of a different section, i.e., section A's jobs are not dependent on the results of section B's jobs, and vice versa.
...
1. Training of a learning algorithm (or other computationally intensive parts of a machine learning pipeline) *may* contain independent sections which can run in parallel, e.g.
    ...
    * Many feature `r index('filters')` work in a univariate fashion, i.e., calculate a numeric score for each feature independently.
...
3. Tuning (@sec-optimization) is repeated benchmarking (nested resampling), embedded in a sequential procedure that determines the hyperparameter configuration to try next. In addition, while many tuning algorithms are inherently sequential, there are some (e.g., random search) that can propose multiple configurations in parallel to be evaluated independently, providing another level for parallelization (@sec-nested-resampling-parallelization).
...
If you can write a computation into a function that can be passed to map-like functions such as `r ref("lapply()")`, then you have an embarrassingly parallel problem.
...
For example, we could reduce the overhead of parallelizing a `for`-loop with 1000 iterations on four CPU cores by `r index('chunking')` the work of the 1000 jobs into four computational jobs performing 250 iterations each, resulting in four bigger jobs and not 1000 small ones.
...
We can never guarantee complete reproducibility as this depends on the computational accuracy of the hardware, the processor instructions used, compiler versions and optimization flags, and/or the `r index("BLAS")` library for linear algebra.
:::
...
These threads can be parallelized by the scheduler of the operating system as there is no need for communication between the threads.
...
To enable the parallelization for this learner, `mlr3` provides the helper function `r ref("set_threads()")`:
```{r technical-003}
# use four CPUs
...
* You have linked R to a threaded `r index('BLAS')` implementation like OpenBLAS and your learners make heavy use of linear algebra.
...
`mlr3` makes use of `r ref_pkg("future")` to enable parallelization over resampling iterations using the parallel backend configured by the user via the `r ref("future::plan()")` function.
...
All learners are then passed to `benchmark()` together with the resampling strategy of the tuning instance.

For many optimizers, you can adjust the `batch_size`, i.e., control how many hyperparameter configurations are evaluated in parallel during a call of `benchmark()`.
...
If you run this terminator with a batch size of 10 and the first configuration of the batch yields better performance than the required performance threshold, the remaining nine configurations are evaluated nevertheless.
For this reason, the `batch_size` parameter defaults to `1` so that all `Terminator`s work as intended, i.e., you cannot exceed the computational budget and draw wrong conclusions while comparing tuning algorithms.
...
By example, let us tune the `minsplit` argument of a classification tree using an `r ref("AutoTuner")` (@sec-autotuner) and a racing algorithm (which is dependent on previous iterations), we will use two-fold CV for our inner resampling strategy and five-fold CV for our outer resampling strategy; first running sequentially without parallelization:
...
lrn_rpart_tuned = auto_tuner(tnr("mbo"), lrn_rpart,
  rsmp("cv", folds = 2), msr("classif.ce"), 20)

resample(tsk("penguins"), lrn_rpart_tuned,
  rsmp("cv", folds = 5))$aggregate()
```

To parallelize this experiment, we could either parallelize the outer CV or the inner CV.
Let us assume we have four cores available and each job takes four seconds, and consider each option in turn.

If we parallelize the outer five-fold CV (@fig-parallel-outer), all four cores would be utilized first with the computation of the first four resampling iterations and the computation of the fifth iteration has to wait.
...
Assume that each fit during the inner resampling takes four seconds to compute and that there is no other significant overhead.
Then, each of the four workers starts with the computation of an inner two-fold CV.
As there are more jobs than workers, the remaining fifth iteration of the outer resampling is queued on CPU1 **after** the first four iterations are finished after eight seconds.
During the computation of the fifth outer resampling iteration, only CPU1 is utilized, the other three CPUs are idling.
Even if we had set up the parallelization with `"multisession"` in both inner and outer CV, it would have the same result, the outer loop is parallelized while all subsequent loops default to sequential execution.

In contrast, if we parallelize the inner two-fold CV (@fig-parallel-inner) then the outer loop would run sequentially and distribute the two computations for the inner resampling on two CPUs; meanwhile, CPU3 and CPU4 are idling.
...
%%| fig-cap: CPU utilization for four CPUs while parallelizing the inner two-fold CV with a sequential five-fold CV outside. Jobs are labeled as [iteration outer]-[iteration inner].
gantt
    title CPU Utilization
    dateFormat  s
    axisFormat %S
...
Both possibilities for parallelization are not exploiting the full potential of the four CPUs.
With parallelization of the outer loop, all results are computed after 16 seconds, in contrast to parallelization of the inner loop where the results are only available after 20 seconds.

It is possible to enable parallelization for both loops for nested parallelization, even on different parallelization backends, which can be useful in some distributed computing setups.
In this case, the number of workers must be manually tweaked with `r ref("future::tweak")` so that the system does not get overburdened:
...
This example would run on eight cores on the local machine, parallelizing the outer resampling on two workers, and the inner resampling on four.
...
To predict in parallel, the test data is first split into multiple groups and the predict method of the learner is applied to each group in parallel using an active backend configured via `future::plan()`.
...
lrn_ranger$parallel_predict = TRUE

# predict
lrn_ranger$predict(tsk_sonar)$score(msr("classif.tpr"))
```
...
Learners behave quite differently when encountering such problems: some models signal a warning during the training step that they failed to fit but return a baseline model, while other models stop the execution.
...
Each `Learner` contains the field `r cdefine("$encapsulate", parent = "Learner")` to control how the train or predict steps are wrapped.
The easiest way to encapsulate the execution is provided by the package `r ref_pkg("evaluate")`, which evaluates R expressions and captures and tracks conditions (outputs, messages, warnings or errors) without letting them crash the process (see documentation of `r ref("mlr3misc::encapsulate()")` for full details):
...
Missing learners and/or predictions are particularly problematic during automated processes such as resampling, benchmarking, or tuning (@sec-encapsulation-fallback), as results cannot be aggregated across iterations.
...
2. Penalize failing learners -- Instead of ignoring failed iterations, we could impute the worst possible score (as defined by a given `Measure`) and thereby heavily penalize the learner for failing.
...
`mlr3` includes two baseline learners: `lrn("classif.featureless")`, which always predicts the majority class, and `lrn("regr.featureless")`, which always predicts the average response.

To make this procedure convenient during resampling and benchmarking, we support fitting a baseline (though in theory you could use any `Learner`) as a `r index('fallback learner')` by passing a `Learner` to `r cdefine('$fallback', parent = "Learner")`.
...
```{r technical-024}
lrn_debug = lrn("classif.debug", error_train = 0.3)
lrn_debug$fallback = lrn("classif.featureless")
...
This means you could disable logging from the `mlr3` logger, but keep the output from `mlr3tuning`:
...
By default, output from `lgr` is printed in the console, however, you could choose to redirect this to another file format, for example to JSON:
...
1. Although only a small proportion of the data is required, the complete data set sits in, and consumes, memory.
  This is especially a problem if you work with large tasks or many tasks simultaneously, e.g., for `r index('benchmarking')`.
...
2. `r ref("DataBackendDuckDB")` for the impressive *`r index('DuckDB')`* database connected via `r ref_pkg("duckdb")`, which is a fast, zero-configuration alternative to `r index('SQLite')`.
...
To demonstrate `r ref("mlr3db::DataBackendDplyr")` we use the (pretty big) NYC flights dataset from the `r ref_pkg("nycflights13")` package and move it into a `r index("SQLite")` database.
...
Note that the `DataBackendDplyr` can only operate on the data we provided, so does not 'know' about the rows and columns we already filtered out (this is in contrast to using `$filter` and `$subset` as in @sec-tasks-mutators).
...
`r index('Parquet')` is a popular column-oriented data storage format supporting efficient compression, making it far superior to other popular data exchange formats such as CSV.
...
To find out more about logging, then have a read of the vignettes in `lgr`, which cover everything from logging to JSON files to retrieving logged objects for debugging.
...
|  | `r ref("set_threads()")` | |
|  | `r ref("future::tweak()")` | |
| `Learner` | `lrn()` | `$encapsulate`; `$fallback`; `$log` |
| `r ref("lgr::Logger")` | `r ref("get_logger")` | `$set_threshold()` |
