...
In each iteration, a tuner proposes a batch of hyperparameter configurations -- in the worst case of size 1 -- which can then be evaluated in parallel.
After each iteration, a typical tuner adapts itself in some way based on the obtained performance values.
Random and grid search are exceptions, since they do not choose configurations based on past results.
For them, all evaluations are therefore independent and can in principle be fully parallelized.
Otherwise, tuning can be regarded as iterative benchmarks, which is also how `mlr3` implements it:
The `r ref("Tuner")` proposes a batch of learners, each with a different configuration in its `$param_set$values`.
The size of this batch can often be controlled with a tuner's `batch_size` configuration parameter.
This batch is then passed to `r ref("benchmark()")` together with the resampling strategy of the tuning instance.

Since each `benchmark()`-call depends on previous results, it is generally not possible to parallelize tuning at a higher "level" than individual benchmarks.
Instead, the individual `benchmark()` evaluations are parallelized by `mlr3` in the same way as when benchmarks are run manually, without tuning.
This has the benefit that the individual resampling iterations of each configuration being evaluated are all parallelized at the same time.
Therefore, to get full parallelization, make sure that the `batch_size`, multiplied by the number of resampling iterations, is at least equal to the number of available workers.
...
Note, however, that termination criteria are only checked between evaluations of batches.
This is the case even if parallelization is not used.
...
The following code shows a parallelized execution of random search with the termination criterion set to 20 iterations and a moderate batch size, where 36 resampling splits -- 12 configurations of 3 splits each -- are evaluated in parallel on 4 workers.
...
In this example, we could have increased the batch size to 20 (not having a multiple of 4, the number of workers, would not matter here, since we want to terminate after 20 configurations and are not worried about sub-optimal usage of workers).
Due to the embarrassingly parallel nature of random search, this would have made use of available resources in the most efficient way while stopping exactly at the point we configured, i.e. after 20 evaluations.
However, for other termination criteria, such as `trm("perf_reached")`, which checks whether a sufficient performance value has already been reached, we would not know the number of evaluations beforehand and could not set the batch size accordingly.
For example, with a batch size of 12, if the first configuration of the batch would already yield better performance than the given threshold, the remaining 11 configurations would still be evaluated.
...
This is also shown in Algorithm 3 from @hpo_practical, which is, in slightly simplified and conceptual form:

```{r, eval = FALSE}
# outer resampling, level 1:
for (i in seq_len(n_outer_splits)) {
  tuning_inst = ti(...)  # tuning instance
  ...
  while (!tuning_inst$is_terminated) {
    ...
    for (configuration in proposed_points) {
      ...
      for (j in seq_len(n_inner_splits)) {
        split_performances[j] = evaluate_performance(
          learner, configuration, inner_task, inner_splits[[j]]
        )
      }
  ...
  evaluate_performance(learner, tuning_inst$result, task, outer_splits[[i]])
}
```

The pseudocode above is implemented in `mlr3` in a slightly more efficient variant.
At the second level (the evaluation of configurations), it exploits the functionality of `benchmark()`: a learner is created for each proposed hyperparameter configuration and all learners are resampled in a benchmark experiment in the innermost for-loop, effectively executing the second level along with the third level on a finer granularity (number of proposed points times number of inner resampling iterations).
Hence, when parallelizing nested resampling in `mlr3`, the user has to choose between two options: parallelizing the outer resampling or the inner benchmarking.
...
lrn_rpart_tuned = auto_tuner(tnr("random_search", batch_size = 2), lrn_rpart,
  rsmp("holdout"), msr("classif.ce"), 2)
...
Each of the four workers starts with the computation of a different inner benchmark, each of which runs sequentially and therefore takes 8 seconds on one worker.
...
%%| fig-cap: CPU utilization for four cores while parallelizing the outer five-fold CV with a sequentially executed inner benchmark inside. Jobs are labeled as [iteration outer]-[iteration inner].
%%| fig-alt: A diagram visualizing the utilization of four different cores (on the y axis). The x represents the time. C1 runs for all 16 seconds and evaluates two outer resampling iterations.
The remaining three cores each evaluate one outer resampling iteration and idle afterwards. Each evaluation of an outer resampling consists of two sequentially executed evaluations from the inner benchmark.
gantt
...
%%| fig-alt: A diagram visualizing the utilization of four different cores (on the y axis). The x axis represents the time. C1 and C2 are each running five evaluations from the inner benchmark and are therefore utilized for 20 seconds. C3 and C4 are idling during the whole computation.
gantt
    title CPU Utilization
    dateFormat  s
    axisFormat %S
