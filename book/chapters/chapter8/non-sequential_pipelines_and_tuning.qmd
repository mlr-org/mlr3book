...
library(mlr3oml)
dir.create(here::here("book", "openml", "manual"), showWarnings = FALSE, recursive = TRUE)
...
When applied to the first three rows of the `"diabetes"` task we can see how this imputes missing data and adds a column indicating where values were missing.

```{r non-sequential_pipelines_and_tuning-004, eval = TRUE}
tsk_diabetes_head = tsk("diabetes")$filter(1:3)
tsk_diabetes_head$data(cols = c("diabetes", "glucose", "triceps"))
result = graph$train(tsk_diabetes_head)[[1]]
result$data(cols = c("diabetes", "glucose", "missing_glucose", "triceps",
  "missing_triceps"))
```
...
```{r non-sequential_pipelines_and_tuning-032a}
#| include: false
path_mnist = here::here("book", "openml", "manual", "tsk_mnist.rds")
```

```{r non-sequential_pipelines_and_tuning-032, eval = !file.exists(path_mnist)}
library(mlr3oml)
...
```{r non-sequential_pipelines_and_tuning-032b}
#| include: false
if (file.exists(path_mnist)) {
  tsk_mnist = readRDS(path_mnist)
} else {
  saveRDS(tsk_mnist, path_mnist)
}
```
...
1. Create a graph that replaces all numeric columns that do not contain missing values with their PCA transform.
  ...
  Train the graph on `tsk("diabetes")` to check your result.
