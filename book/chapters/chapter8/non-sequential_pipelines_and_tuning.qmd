...
A simple combination can be a linear model (maybe regularized if you have very many level 0 models), since a weighted sum of level 0 models is often plausible and good enough.
However, non-linear level 1 models are also possible, and it is also an option for the level 1 model to access the input features besides the level 0 predictions.
Stacking can be built with more than two levels (both conceptually, and in `mlr3`) but we will limit ourselves to this simpler setup here, which also often works well enough.
...
For a stacking pipeline to work, it is necessary that the level 0 learners return predictions during the `$train()` phase.
This is possible in `mlr3pipelines` with `r ref("PipeOpLearnerCV", index = TRUE)`.
During training, it performs a cross-validation and returns the out-of-sample predictions into the level 1 model.
Using cross-validated predictions is also recommended in stacking since it reduces the risk of overfitting.
...
2. Tuning not only the hyperparameters of a pipeline, whose structure is not completely fixed in terms of its included operations, but also which concrete `r ref("PipeOp")`s should be applied to data.
  This allows us to select these operations (e.g. which learner to use, which preprocessing to perform) in a data-driven manner and this is known as "`r index('Combined Algorithm Selection and Hyperparameter optimization')`"\index{CASH|see{combined algorithm selection and hyperparameter optimization}} (@Thornton2013).
...
As the optimal setting of the `rank.` hyperparameter of our PCA `r ref("PipeOp")` can very realistically depend on the value of the `k` hyperparameter of the KNN model, jointly tuning them seems reasonable.
...
Therefore, parallelization is often appropriate (@sec-parallelization) and/or more efficient tuning methods for searching large tuning spaces such as `r index('Bayesian optimization', lower = FALSE)` (@sec-bayesian-optimization) become more attractive.
