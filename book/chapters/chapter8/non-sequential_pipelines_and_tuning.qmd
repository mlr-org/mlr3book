...
Next we use `r ref("pipeline_greplicate")` to copy the graph `single_pred` 10 times (`n = 10`) and finally `po("classifavg")` to take the majority vote of all predictions, note that we pass `innum = 10` to `"classifavg"` to tell the `r ref("PipeOp")` to expect 10 inputs.
