...
In `mlr3pipelines` this can either (@fig-pipelines-select-affect) be achieved by passing the column subset to the `affect_columns` hyperparameter of a `r ref("PipeOp")` (assuming it has that hyperparameter), which controls which columns should be affected by the `PipeOp`.
...
However, tuning pipelines is not straightforward as not only do you have to choose which `Learner` `r index('hyperparameters')` to tune, but you also have to consider which `r ref("PipeOp")`s to include in the pipeline and which of their hyperparameters to tune.
For example, we saw how to create different complexities of bagging pipelines in @sec-pipelines-bagging, if we wanted to tune this we would have to decide: 1) which `r ref("Learner")` to use as our weak learner; 2) the hyperparameters to tune for the weak learner and their ranges; 3) if we should tune the replications of `ppl("greplicate")`; 4) if we should tune or select the averaging method; 5) should we add a custom feature selection and tune this?; 6) what sampling method should we use and should the sampling fraction be tuned?
...
This concept makes it possible to use tuning to decide which `r ref("PipeOp")`s and `r ref("Learner")`s to include in the pipeline, whilst also allowing all options in every path to be tuned.
