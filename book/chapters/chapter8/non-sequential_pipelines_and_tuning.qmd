...
In the following example, we create a `Graph` that centers its inputs and then copies the scaled data to two parallel streams: one replaces the data with columns that indicate whether data is missing, and the other imputes missing data using the median (we will return to this in @sec-preprocessing-missing).
...
When applied to the first three rows of the `tsk("pima")` task we can see how this imputes missing data and adds a column indicating where values were missing.
...
Alternatively, one can use the `r ref("PipeOpSelect", aside = TRUE)` operator to create operations in parallel on specified feature subsets, and then unite the result using `r ref("PipeOpFeatureUnion", aside = TRUE)`.
...
Now you have the tools to create sequential and non-sequential pipelines, you can now create an infinite number of transformations on `r ref("Task")`, `r ref("Learner")`, and `r ref("Prediction")` objects.
...
However, many common problems in ML can be well solved by the same pipelines, and so to make your life easier we have implemented and saved these pipelines in the `r ref("mlr_graphs", aside = TRUE)` dictionary; pipelines in the dictionary can be accessed with the `r ref("ppl()", aside = TRUE)` sugar function.

At the time of writing, this dictionary includes seven `r ref("Graph")`s (required arguments included):

* `ppl("bagging", graph)`: In `mlr3pipelines`, `r index('bagging')`, described in detail in @sec-pipelines-bagging, is the process of running a `graph` multiple times on different data samples and then averaging the results. The `iterations` argument controls the number of bagging iterations, `frac` controls the sampling fraction, and the averaging method can be adjusted through the `averager` parameter.
...
Next, we use `r ref("pipeline_greplicate")` to copy the graph `gr_single_pred` 10 times (`n = 10`) and finally `po("classifavg")` to take the majority vote of all predictions, note that we pass `innum = 10` to `"classifavg"` to tell the `r ref("PipeOp")` to expect 10 inputs.
...
Now let us see how well our bagging pipeline compares to the single decision tree and a random forest, we will test this on the `tsk("sonar")` task.
...
grid = benchmark_grid(tsk("sonar"),
  c(glrn_bagging, lrn_rpart, lrn("classif.ranger")),
  rsmp("cv", folds = 3))
bmr = benchmark(grid)
...
The steps above are saved as an accessible graph in `r ref("pipeline_bagging()")`, to construct the above we would simply construct the `ppl("bagging")` pipeline specifying the learner to 'bag', number of iterations, fraction of data to sample, and the `r ref("PipeOp")` to average the predictions, note we set `collect_multiplicity = TRUE` which collects the predictions across paths, which technically use the `r ref("Multiplicity")` method that we will not discuss here but refer the reader to the documentation.
...
Each learner is wrapped in `po("learner_cv")` which trains and predicts with the given learner, returning the model predictions in a new `r ref("Task")` as its output.
...
In this example the original features were removed as each `r ref("PipeOp")` only returns the predictions made by the respective learners.
...
However, tuning pipelines is not straightforward as not only do you have to choose which `r ref("Learner")` hyperparameters to tune, but you also have to consider which `r ref("PipeOp")`s to include in the pipeline and which of their hyperparameters to tune.
...
We can see how the pipeline's `$param_set` includes the tune tokens for all elements of the pipeline so when we tune the pipeline these are combined into different hyperparameter configurations.
...
The resulting `r ref("Graph")` then looks as follows:
...
# use the "PCA" path
graph$param_set$values$branch_PO.selection = "pca"
head(graph$train(tsk_mnist)[[1]]$feature_names)
# use the "No-Op" path
graph$param_set$values$branch_PO.selection = "nop"
head(graph$train(tsk_mnist)[[1]]$feature_names)
```
...
Here we see that in combination with the decision tree, `pca` was the best performing `r ref("PipeOp")`.
...
graph_learner$plot()
```
```{r 05-pipelines-branch-02-evalT, fig.width = 8, fig.height = 6, echo = FALSE, out.width = "80%"}
...
instance$result[,
  .(branch_PO.selection, classif.kknn.k, branch.selection, classif.ce)]
```
...
`po("proxy")` is a meta-operator that performs the operation that is stored in its `content` hyperparameter, which could be another `r ref("PipeOp")` or `r ref("Graph")`, it can therefore be used to tune over and select different `PipeOp`s or `Graph`s that could be passed to this hyperparameter (@fig-pipelines-alternatives).
...
Now that you have learned about pipelines and how to tune them, in this short section we will briefly return to Hyperband to showcase how we can put together everything we have learned in this chapter to allow Hyperband to be used with any `r ref("Learner")`.
...
In this example, we will optimize the SVM\index{support vector machine}} hyperparameters, `cost` and `gamma`, on the `sonar` dataset:
...
1. Consider the `po("select")` in @sec-pipelines-stack that is used to only keep the columns ending in "`M`".
  ...
  (Note: The `lrn("classif.log_reg")` learner used in @sec-pipelines-stack cannot handle more than two classes. To build the entire stack, you will need to use a different learner).
...
3. Use the `po("imputelearner")` PipeOp to impute missing values in the `r ref("mlr_tasks_penguins", "tsk(\"penguins\")")` task using learners based on `r ref("ranger::ranger")`.
