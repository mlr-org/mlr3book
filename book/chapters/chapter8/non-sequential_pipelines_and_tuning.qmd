...
autoplot = function(...) {
  eval.parent(substitute({plt = ggplot2::autoplot(...)}))
  invisible(NULL)
}
```
...
```{r 05-pipelines-branch-03-post, echo = FALSE}
library("ggplot2")
for (i in seq_along(plt)) {
  ## Turn plot into black and white
  plt[[i]]$scales$scales[[1]] = scale_fill_grey()
...
autoplot = ggplot2::autoplot # reset to original function
print(plt)
```

As we can see in the plot, the KNN-learner with `k` set to `r instance$result$classif.kknn.k` was selected, which performs best in combination with the Yeo-Johnson transform.
