...
As we work through this chapter we will use the `"adult_train"` and `"adult_test"` datasets from `r mlr3fairness`, which contain a subset of the `Adult` dataset (@uci).
...
Questionnaires for machine learning models and datasets have been previously proposed in literature and are available in `r mlr3fairness` from automated report templates (`r ref("report_modelcard()")` and `r ref("report_datasheet()")`) using R markdown for datasets and machine learning models.
