...
ggplot(plot_data, aes(x = x, y = loss, color = tau)) +
  ...
  theme_minimal() +
  theme(axis.title = element_text(size = 6), legend.key = element_text(size = 6))
```
...
```{r, message=FALSE, warining=FALSE, results='hide'}
...
print(bmr_agg[, -c(1, 2, 5, 6)])
```

In general, all standard `ml3`-workflows, i.e. resampling, benchmarking, tuning, and the use of pipelines, can be applied to quantile regression learners just as they are applied to regression learners with other predict types.
