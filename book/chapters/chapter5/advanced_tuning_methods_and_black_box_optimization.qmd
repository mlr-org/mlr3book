...
Other common models that have fidelity parameters include `r index('neural networks', "neural network")` (number of epochs) and `r index('random forests', 'random forest')` (number of trees).
...
Typical choices of regression learners used as surrogate models include `r index('Gaussian processes', "Gaussian process", lower = FALSE)` (`lrn("regr.km")`) for low dimensional numeric search spaces and `r index('random forests', 'random forest')` (e.g., `lrn("regr.ranger")`) for higher dimensional mixed (and/or hierarchical) search spaces.
Note that `r ref("default_gp()")` and `r ref("default_rf()")` also grant convenient access to pre-configured Gaussian process and random forest learners, respectively.
These learners come equipped with sensible default hyperparameters (e.g., choice of kernel), allowing them to be readily used as surrogate models without any additional configuration.
