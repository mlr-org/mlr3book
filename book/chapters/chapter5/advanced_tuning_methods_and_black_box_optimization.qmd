...
We use `trm("none")` and set the `repetitions` control parameter to `1`, causing Hyperband to terminates itself after all brackets have been evaluated a single time.
...
In the real world, black-box optimization problems are encountered quite often in engineering, for example, modeling experiments like crash tests or chemical reactions.
...
Bayesian optimization (BO) is a global optimization algorithm that usually follows the following process, displayed in @fig-optimization-loop:
