...
`r ref("default_gp()")` and `r ref("default_rf()")` further provide access to 
...
Use either a Gaussian process with Matérn 5/2 kernel (`lrn("regr.km")`, similarly as above) or a random forest (`lrn("regr.ranger")`) as the surrogate model and compare the anytime performance of these two BO algorithms.
...
```
To construct the different surrogate models you can use:
```{r}
