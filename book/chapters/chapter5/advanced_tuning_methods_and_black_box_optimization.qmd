...
So far we have considered optimizing a model with respect to one metric, but multi-criteria, or `r index("multi-objective", parent = "HPO", lower = FALSE, aside = TRUE)` optimization, is also possible.
...
`r index('Multi-fidelity HPO', "multi-fidelity", parent = "HPO", lower = FALSE, aside = TRUE)` is motivated by the idea that the performance of a lower-fidelity model is indicative of the full-fidelity model, which can be used to make HPO more efficient (as we will soon see with Hyperband).
...
On the other hand, higher fidelity levels result in high-fidelity models that are slower to train but may better predict the full-fidelity model's performance.
...
Analogously to tuners, `Optimizer`s in `r bbotk` are stored in the `r ref('mlr_optimizers')` dictionary and can be constructed with `r ref('opt()', aside = TRUE)`.
