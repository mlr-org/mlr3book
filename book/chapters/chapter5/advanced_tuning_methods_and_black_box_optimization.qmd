...
@sec-hyperband looks in detail at multi-fidelity tuning and the Hyperband tuner, considers some theory behind this method and then demonstrates it in practice with `r mlr3hyperband`.
...
Note by passing `"evaluate"` to both `train` and `predict`, we are telling the learner to set up encapsulation in both the training and predicting stages, however, we could have only set it for one stage.
...
We can prevent this from happening by setting the `timeout` field in a learner, which signals the learner to stop if it has been running for that much time, again this can be set for training and predicting individually:
...
The metric values corresponding to the Pareto set are referred to as the `r index("Pareto front", aside = TRUE)`.
...
We refer to the model with 1000 rounds as the model at `r index('full-fidelity', aside = TRUE)` and we want to approximate this model's performance using models at different fidelity levels.
...
Other common models that have fidelity parameters include neural networks (number of epochs) and random forests (number of trees).
...
A big disadvantage of this method is that it is unclear if it is better to start with many configurations (large $m$) and a small budget or fewer configurations (small $m$) but a larger budget.
...
Each bracket starts with a different budget, $r_0$, where smaller values mean that more configurations can be evaluated and so the most exploratory bracket (i.e., the one with the most number of stages) is allocated the minimum budget $r_{min}$.
...
: Hyperparameter configurations in each stage and bracket from the schedule in @tbl-hyperband. Entries are unique identifiers for tested hyperparameter configurations (HPCs) (all HPCs are assigned unique identifiers although theoretically multiple HPCs could contain the same values). $HPC^*_s$ is the optimal hyperparameter configuration in bracket $s$ and $HPC^*$ is the optimal hyperparameter configuration across all brackets. {#tbl-hyperband-eg}
...
By example we will optimize XGBoost on `tsk("sonar")` and use the number of boosting iterations as the fidelity parameter, this is a suitable choice as increasing iterations increases model training time but generally also improves performance.
...
In the real world, black-box optimization problems are encountered all the time, for example, modeling experiments like crash tests or chemical reactions.
...
Similarly to how we can use `r ref("tune()")` to construct a tuning instance, here we can use `r ref("bb_optimize()", aside = TRUE)`, which returns a list with elements `"par"` (best found parameters), `"val"` (optimal outcome), and `"instance"` (the tuning instance); `"par"` and `"val"` are equivalent to the `instance$result`:
...
However, even when employing a Gaussian process as a surrogate model, allowing for the calculation of gradients of the acquisition function with respect to the input variables, the issue of vanishing gradients in the mean and standard deviation predictions concerning the input variables presents a significant challenge [@garnett_2022].
...
Below we use a non-linear optimization with the DIRECT algorithm and we terminate the acquisition function if there is no improvement of at least `1e-5` for `100` iterations.
...
As an example, below we will tune the `cost` and `gamma` parameters of a classification SVM with a radial kernel on the `sonar` task with three-fold CV for the inner and outer resamplings.
...
We will tune three parameters of a decision tree with respect to the true positive and true negative rates, the Pareto front is visualized in @fig-pareto-bayesopt.
...
For example, if you were modeling a machine in a factory to estimate the rate of production, even if all parameters of the machine were controlled, we would still expect different performance at different times due to uncontrollable background facts such as environmental conditions.
...
For example, in the case of a purely numeric search space, `mlr3mbo` will by default use a Gaussian process as the surrogate model and a random forest as the fallback learner and additionally encapsulates (@sec-encapsulation-fallback) the learner via the `r ref_pkg("evaluate")` package.
...
@tbl-api-advanced-tuning summarizes the most important classes, functions, and methods seen in this chapter.
...
Use either a Gaussian process with Matérn 5/2 kernel (`lrn("regr.km")`, similarly as above) or a random forest (`lrn("regr.ranger")`) as the surrogate model and compare the anytime performance (similarly as in @fig-bayesian-sinusoidal_bo_rs) of these two BO algorithms.
