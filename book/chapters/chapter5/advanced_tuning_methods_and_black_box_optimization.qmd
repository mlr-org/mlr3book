...
Note by passing `"evaluate"` to both `train` and `predict`, we are telling the learner to setup encapsulation in both the training and prediction stages, however we could have only set it for one stage.
...
Unfortunately, if predictions are not made, then our hyperparameter optimization experiment will still fail as for any resampling iteration with errors, the result will be `NA`, and so we are unable to aggregate results across resampling iterations.
...
The goal of multi-objective hyperparameter optimization is to find a set of non-dominated solutions so that their corresponding metric values approximate the true Pareto front.
...
Methodological details on multi-objective hyperparameter optimization can be found in @hpo_multi and @Morales2022.
...
In the real-world, black-box optimization problems are encountered all the time, for example modelling experiments like crash tests or chemical reactions.
...
We refer to these elements as the 'building blocks' of BO as it is a highly modular algorithm; as long as the above structure is in place, then the surrogate models, acquisition functions, and acquisition function optimizers are all interchangeable.
...
`r mlr3mbo` allows you to either construct the initial design manually, or let a `r ref("loop_function")` construct and evaluate this for you.
...
We can see that the expected improvement is high in regions where the mean prediction (blue dotted lines) of the Gaussian process is low.
...
We have now manually shown how to run a single iteration of the BO algorithm loop.
...
`r ref("OptimizerMbo")` can be used to assemble the building blocks described above into a single object that can then be used for optimzation.
The other benefit of this method is that you do not need to pass any of these building blocks to each other as the optimizer will do this for you internally prior to optimization:
...
::: {.callout-tip}
Additional arguments for customizing certain loop functions can be passed through with the `args` parameter of `OptimizerMbo`.
:::
...
We will tune three parameters of a decision tree with respect to true positive rate (maximize) and false positive rate (minimize), the Pareto front is visualized in @fig-pareto-bayesopt.
...
Noisy objectives can be treated in BO different ways:
...
Finally, `r mlr3mbo` allows for explicitly specifying how the final result after optimization is assigned to the instance (i.e., what will be saved in `instance$result`) with a `r index('result assigner', aside = TRUE)`, which can be specified during the construction of an `OptimizerMbo` or `TunerMbo`.
...
We then looked at multi-fidelity tuning, in which the Hyberband tuner can be used to efficiently tune algorithms by making use of lower fidelity evaluations.
We will return to Hyperband in @sec-pipelines-nonseq where we will learn how to make use of pipelines in order to tune algorithms with Hyperband.
...
| `r ref('mlr3mbo::OptimizerMbo')` | `opt("mbo")` | |
