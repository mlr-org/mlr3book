...
Note by passing `"evaluate"` to both `train` and `predict`, we are telling the learner to set up encapsulation in both the training and prediction stages, however, we could have only set it for one stage (see @sec-error-handling for other encapsulation options).
...
When this happens, our hyperparameter optimization experiment will fail as we cannot aggregate results across resampling iterations (@sec-fallback discusses why you cannot just ignore the `NA`s).
