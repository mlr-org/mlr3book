...
Note by passing `"evaluate"` to both `train` and `predict`, we are telling the learner to set up encapsulation in both the training and prediction stages, however, we could have only set it for one stage.
...
We can prevent this from happening by setting the `timeout` field in a learner, which signals the learner to stop if it has been running for that much time, again this can be set for training and prediction individually:
...
A common approach is to use a featureless baseline (`lrn("regr.featureless")` or `lrn("classif.featureless")`).
...
#| label: fig-pareto
#| fig-cap: Pareto front of selected features and classification error. White dots represent tested configurations, each black dot individually represents a Pareto-optimal configuration and all black dots together represent the Pareto front.
...
We can see that the expected improvement is high in regions where the mean prediction (gray dashed lines) of the Gaussian process is low.

```{r}
#| label: fig-bayesian-optimization-ei
#| fig-cap: Expected improvement (darkgray) based on the mean and uncertainty prediction (gray) of the Gaussian process surrogate model trained on an initial design of four points (black). Ribbons represent the mean plus minus the standard deviation prediction.
...
ggplot(xydt, mapping = aes(x = x, y = y)) +
  ...
  theme_minimal()
```
<!-- FIXME UPDATE ABOVE COLORS AND CHECK -->
...
bayesopt_ego = function(
  ...
  # if initial design has not been manually constructed use Sobol method
...
We will tune three parameters of a decision tree with respect to the true positive (maximize) and false positive (minimize) rates, the Pareto front is visualized in @fig-pareto-bayesopt.
...
#| fig-alt: Scatter plot with classif.tpr on x-axis (between 0.75 and 1.00) and classif.fpr on y-axis (between 0.2 and 1.0). The Pareto front is shown as the set of points at roughly (0.85, 0.23), (0.88, 0.24), (0.91, 0.25), (0.93, 0.3), (0.95, 0.35), (0.96, 0.4), (1.00, 0.8).
...
| `r ref("mlr3mbo::SurrogateLearner")` | `r ref("srlrn()")`|  |
| `r ref("mlr3mbo::AcqFunction")` | `r ref("acqf()")` |  |
| `r ref("mlr3mbo::AcqOptimizer")` | `r ref("acqo()")` |  |
| - | `r ref("mlr3mbo::loop_function")` | - |
| `r ref('mlr3mbo::OptimizerMbo')` | `bbotk::opt("mbo")` | |
