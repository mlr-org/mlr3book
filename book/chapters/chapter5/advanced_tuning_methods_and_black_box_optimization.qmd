...
To unpack what these terms mean and to motivate multi-fidelity tuning, say that we think a gradient boosting algorithm with 1000 rounds will be a very good fit to our training data.
...
In this example, the hyperparameter controlling the number of rounds is a *fidelity parameter*, as it controls the tradeoff between model performance and complexity.
...
Other common models that have fidelity parameters include `r index('neural networks', "neural network")` (number of epochs) and `r index('random forests')` (number of trees).
The proportion of data to sample before running any algorithm can also be viewed as a model-agnostic fidelity parameter, we will return to this in @sec-hyperband-example-svm.
...
The design of BO is different from what we have seen so far in other tuning methods so to help motivate this we will spend a little more time in this section on theory and methodology.
...
Similarly to how we can use `r ref("tune()")` to construct a tuning instance, here we can use `r ref("bb_optimize()")`, which returns a list with elements `"par"` (best found parameters), `"val"` (optimal outcome), and `"instance"` (the tuning instance); `"par"` and `"val"` are equivalent to the `instance$result`:
...
Typical choices of regression learners used as surrogate models include `r index('Gaussian processes')` (`lrn("regr.km")`) for low dimensional numeric search spaces and `r index('random forests')` (e.g., `lrn("regr.ranger")`) for higher dimensional mixed (and/or hierarchical) search spaces.
...
Widely used approaches for optimizing acquisition functions include derivative-free global optimization methods like branch and bound algorithms, such as the `r index('DIRECT')` algorithm [@jones_1993_lipschitzian], as well as multi-start local optimization methods, such as running the `r index('L-BFGS-B')` algorithm (@byrd1995limited) or a local search multiple times from various starting points [@kim_2021].
...
However, even when employing a `r index('Gaussian process')` as a surrogate model, allowing for the calculation of gradients of the acquisition function with respect to the input variables, the issue of vanishing gradients in the mean and standard deviation predictions concerning the input variables presents a significant challenge [@garnett_2022].
...
A common choice of loop function is the `r index('Efficient Global Optimization')` (EGO) algorithm [@jones_1998] (`r ref('bayesopt_ego()')`), a simplified version of this code is shown at the end of this section to help demonstrate the algorithm and its purpose.
...
`r mlr3mbo` can be used for HPO by making use of `r ref("TunerMbo", aside = TRUE)`, which is a wrapper around `r ref("OptimizerMbo")` and works in the exact same way.
As an example, below we will tune the `cost` and `gamma` parameters of `lrn("classif.svm")` with a radial kernel on `tsk("sonar")` with three-fold CV for the inner and outer resamplings.
...
#| fig-cap: Pareto front of TPR and FPR obtained via ParEGO. White dots represent tested configurations, each black dot individually represents a Pareto-optimal configuration and all blue dots together represent the Pareto front.
...
In the first case, instead of using an interpolating `r index('Gaussian process')`, we could instead use Gaussian process regression that estimates the measurement error by setting `nugget.estim = TRUE`:
...
The value of the acquisition function is also always logged in the archive of the optimization instance so inspecting this is a good idea to ensure the algorithm behaved as expected and that the acquisition function column is populated as expected (`instance$archive$data`).
