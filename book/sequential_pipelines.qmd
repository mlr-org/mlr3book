...
The `r ref("Graph")` class represents an object similar to a `r index('directed acyclic graph')` (DAG), since the input of a `PipeOp` cannot depend on its output and hence cycles are not allowed.
