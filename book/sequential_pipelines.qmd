...
The following example uses the `"mutate"` `PipeOp` to add a new feature to the task, and the `"scale"` `PipeOp` to then scale and center all numeric features.
...
To save memory, intermediate results in a `Graph` are not saved by default.
See the `$keep_results` field of a `Graph` object to `TRUE` to enable inspection of a `Graph`, which will be most useful when debugging.
...
Possibly the most common application for `r mlr3pipelines` is to use it to perform preprocessing tasks, such as missing value imputation or factor encoding, and to then feed the resulting data into a `r ref("Learner")`.
...
### `Learner`s as `PipeOp`s and `Graph`s as `Learner`s

In @fig-pipelines-pipeline the final `PipeOp` is a `Learner`.
`Learner` objects can be converted to `PipeOp`s with `as_pipeop`, however this is only necessary if you choose to manually created a graph instead of using `%>>%`, internally `Learner`s are wrapped in the `"learner"` `PipeOp`.
The following code creates a `Graph` that uses the `"imputesample"` `PipeOp` to impute missing values by sampling from observed values  (@sec-preprocessing-missing) then fits a logistic regression on the transformed task.
...
#| fig-cap: '`"mutate"` and `"learner"` PipeOps in a sequential pipeline.'
#| fig-alt: 'Four boxes in a straight line connected by arrows: "<INPUT> -> imputesample -> classif.rpart -> <OUTPUT>".'
learner_logreg = lrn("classif.log_reg")
gr = po("imputesample") %>>% learner_logreg
...
### Inspecting `Graph`s and their hyperparameters {#sec-pipelines-inspect}
...
We first need to set the `$keep_results` flag to be `TRUE` so that intermediate results are retained.
...
In @sec-preprocessing we will look specifically at pipelines for data preprocessing.
...
| `r ref("Graph")` | `%>>%` | `$pipeops`/`$edges`/`$train()`/`$predict()` |
