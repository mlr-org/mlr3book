...
```{r 04-optimization-tuning-001, echo = FALSE}
...
`r mlr_pkg("mlr3")` allows to specify complex termination criteria by selecting one of the available `r ref("Terminator", text = "Terminators")`:
...
We can automate this entire process in `r mlr_pkg("mlr3")` so that learners are tuned transparently, without the need to extract information on the best hyperparameter settings at the end.
