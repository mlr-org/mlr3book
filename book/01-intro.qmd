...
Since `r mlr_pkg("mlr3")` makes use of the `R6` system for object oriented programming, some basic familiarity with R6 is beneficial, and a [quick intro](#r6} is given in the next Chapter. It is followed by a [short introduction](#data.table) for `r cran_pkg("data.table")`, which is used by `r mlr_pkg("mlr3")` as a frequent return type.
...
This book concerns itself with machine learning ("ML"), and some preliminary knowledge about the subject is assumed. A good first introduction to machine learning is ["An Introduction to Statistical Learning"](https://www.statlearning.com/) by James, Witten, Hastie, and Tibshirani; it fits particularly well since it has code examples in R (albeit not using `mlr3`).

Another option that covers fewer ML algorithms per se, and instead focuses more on the learning problem itself, is ["Learning From Data"](https://amlbook.com/) by Abu-Mostafa, Magdon-Ismail, and Lin, which is the acompanying book of the [free Caltech "Learning from Data" online course](https://work.caltech.edu/telecourse).
...
Other popular works that give very thorough introductions to machine learning, including many algorithms, are "Pattern Recognition and Machine Learning" by Bishop, and "The Elements of Statistical Learning" by Hastie, Tibshirani and Friedman. The latter is more statistical in nature and may require more knowledge of statistics as a prerequisite.

There is also a free online [Introduction to Machine Learning](https://slds-lmu.github.io/i2ml/) undergraduate level course by the authors of this book. Because of this fact, its use of terminology and concepts overlap with this book and complement it well.
