...
```{r 07-extending-001, echo = FALSE, message = FALSE}
library("data.table")
library("mlr3")
library("mlr3book")
...
```{r 07-extending-003, eval = FALSE, tidy = FALSE}
...
```{r 07-extending-004, eval = FALSE, tidy = FALSE}
...
```{r 07-extending-019, eval = -1}
source("measure_root_mse.R")
msr("root_mse")
```

```{r 07-extending-020, include = FALSE}
...
```{r 07-extending-023, eval = FALSE, tidy = FALSE}
...
```{r 07-extending-025, eval = FALSE}
.train = function(inputs) {
  self$state = list()
  c(inputs, inputs)
}
```
```{r 07-extending-026, eval = FALSE}
.predict = function(inputs) {
  c(inputs, inputs)
}
```
