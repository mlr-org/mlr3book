...
task = as_task_classif_st(data_sf, id = "ecuador",
  target = "slides", positive = "TRUE")
```

You can also use a plain `data.frame`.
The following example shows how to use a `data.frame` and the `$new()` method to construct the same task as above.
In this case, the `DataBackend` needs to be create beforehand and `crs` and `coordinate_names` need to be passed alongside:
...
Here, the performance of the classification tree learner is around 0.05 percentage points worse when using Spatial Cross-Validation (SpCV) compared to Non-Spatial Cross-Validation (NSpCV).
...
:::{.callout-note}
The use of `range = 1000L` is arbitrary here and should not be c/p into a real application.
:::
