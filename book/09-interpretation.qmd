...
```{r 09-interpretation-001, echo = FALSE, message = FALSE}
library("data.table")
library("mlr3")
library("mlr3book")
...
In principle, all generic frameworks for model interpretation are applicable on the models fitted with `r mlr_pkg("mlr3")` by just extracting the fitted models from the `r ref("Learner")` objects.

However, two of the most popular frameworks,

* `r cran_pkg("iml")` in @sec-iml,
* `r cran_pkg("DALEX")` in @sec-dalex, and

additionally come with some convenience for `r mlr_pkg("mlr3")`.

## iml {#sec-iml}

Author: Shawn Storm

`r cran_pkg("iml")` is an R package that interprets the behavior and explains predictions of machine learning models.
The functions provided in the `r cran_pkg("iml")` package are model-agnostic which gives the flexibility to use any machine learning model.

This chapter provides examples of how to use `r cran_pkg("iml")` with `r mlr_pkg("mlr3")`.
...
To understand what `r cran_pkg("iml")` can offer, we start off with a thorough example.
...
```{r 09-interpretation-002, message=FALSE, warning=FALSE}
...
```{r 09-interpretation-003, message=FALSE, warning=FALSE}
library("iml")
library("mlr3")
...
```{r 09-interpretation-004, message=FALSE, warning=FALSE}
...
```{r 09-interpretation-005, message=FALSE, warning=FALSE}
...
All interpretation methods in `r cran_pkg("iml")` need the machine learning model and the data to be wrapped in the `Predictor` object.

Next is the core functionality of `r cran_pkg("iml")`.
...
```{r 09-interpretation-006, message=FALSE, warning=FALSE, fig.cap='Plot of the results from FeatureEffects. FeatureEffects computes and plots feature effects of prediction models',  fig.align='center'}
...
```{r 09-interpretation-007, message=FALSE, warning=FALSE, fig.cap='Plot of the results from Shapley. $\\phi$ gives the increase or decrease in probability given the values on the vertical axis',  fig.align='center'}
x = penguins[which(names(penguins) != "species")]
...
```{r 09-interpretation-008, message=FALSE, warning=FALSE, fig.cap='Plot of the results from FeatureImp. FeatureImp visualizes the importance of features given the prediction model',  fig.align='center'}
...
```{r 09-interpretation-009, message=FALSE, warning=FALSE}
...
```{r 09-interpretation-010, message=FALSE, warning=FALSE, fig.cap='FeatImp on train (left) and test (right)',  fig.align='center'}
# plot on training
...
```{r 09-interpretation-011, message=FALSE, warning=FALSE, fig.cap='FeatEffect train data set', fig.align='center'}
model = Predictor$new(learner, data = penguins[train_set, ], y = "species")
effect = FeatureEffects$new(model)
plot(effect, features = num_features)
```

```{r 09-interpretation-012, message=FALSE, warning=FALSE, fig.cap='FeatEffect test data set',  fig.align='center'}
model = Predictor$new(learner, data = penguins[test_set, ], y = "species")
effect = FeatureEffects$new(model)
plot(effect, features = num_features)
```
...
The `r cran_pkg("DALEX")` package X-rays any predictive model and helps to explore, explain and visualize its behaviour.
...
In the following sections, we will present subsequent methods available in the `r cran_pkg("DALEX")` package based on a random forest model trained for football players worth prediction on the FIFA 20 data.
...
* In @sec-interpretability-data-fifa we introduce the FIFA 20 dataset and then in @sec-interpretability-train-ranger we train a random regression forest using the `r cran_pkg("ranger")` package.
* @sec-interpretability-architecture introduces general logic beyond `r cran_pkg("DALEX")` explainers.
...
```{r 09-interpretation-013, echo=FALSE, fig.cap='Taxonomy of methods for model exploration presented in this chapter. Left part overview methods for instance level exploration while right part is related to dataset level model exploration.', out.width = '92%', fig.align='center'}
...
After some basic data cleaning, the processed data for the top 5000 football players is available in the `r cran_pkg("DALEX")` package under the name `fifa`.

```{r 09-interpretation-014, message=FALSE, warning=FALSE}
library("DALEX")
...
```{r 09-interpretation-015, message=FALSE, warning=FALSE}
dim(fifa)
```
...
```{r 09-interpretation-016, message=FALSE, warning=FALSE}
...
The `r cran_pkg("DALEX")` package works for any model regardless of its internal structure. Examples of how this package works are shown on a random forest model implemented in the `r cran_pkg("ranger")` package.

We use the `r mlr_pkg("mlr3")` package to build a predictive model.
First, let's load the required packages.

```{r 09-interpretation-017, message=FALSE, warning=FALSE}
library("mlr3")
library("mlr3learners")
```
...
```{r 09-interpretation-018, message=FALSE, warning=FALSE}
fifa_task = as_task_regr(fifa, target = "value_eur")
```

Finally, we train mlr3's `r ref("mlr_learners_regr.ranger", text = "ranger learner")` with 250 trees.
...
```{r 09-interpretation-019, message=FALSE, warning=FALSE}
...
Working with explanations in the `r cran_pkg("DALEX")` package always consists of three steps schematically shown in the pipe below.
...
1. All functions in the `r cran_pkg("DALEX")` package can work for models with any structure.
  ...
  In general, such an adapter is created with `r ref("DALEX::explain.default()")` function, but for models created in the `r mlr_pkg("mlr3")` package it is more convenient to use the `r ref("DALEXtra::explain_mlr3()")`.
...
1. Explanations can be visualized with the generic function `plot` or summarised with the generic function `"print()"`.
  ...
  The `plot` function creates graphs using the `r cran_pkg("ggplot2")` package, so they can be easily modified with usual `r cran_pkg("ggplot2")` decorators.
...
In the `r cran_pkg("DALEXtra")` package there generic versions for the most common ML frameworks.
Among them the `r ref("DALEXtra::explain_mlr3()")` function works for `r mlr_pkg("mlr3")` models.
...
```{r 09-interpretation-020, message=FALSE, warning=FALSE}
library("DALEX")
library("DALEXtra")
...
```{r 09-interpretation-021, message=FALSE, warning=FALSE}
...
```{r 09-interpretation-022, message=FALSE, warning=FALSE, echo=FALSE}
tmp = ingredients:::plot.feature_importance_explainer(fifa_vi, max_vars = 12, show_boxplots = FALSE)
```

```{r 09-interpretation-023, message=FALSE, warning=FALSE, fig.width=8, fig.height=4.5}
...
```{r 09-interpretation-024, message=FALSE, warning=FALSE}
...
```{r 09-interpretation-025, message=FALSE, warning=FALSE, fig.width=8, fig.height=8}
library("ggplot2")
plot(fifa_pd) +
...
```{r 09-interpretation-026, message=FALSE, warning=FALSE}
...
```{r 09-interpretation-027, message=FALSE, warning=FALSE, echo=FALSE}
tmp = iBreakDown:::plot.break_down(ronaldo_bd_ranger)
```

```{r 09-interpretation-028, message=FALSE, warning=FALSE, fig.width=8, fig.height=4.5}
...
```{r 09-interpretation-029, message=FALSE, warning=FALSE, fig.width=8, fig.height=4.5}
...
```{r 09-interpretation-030, message=FALSE, warning=FALSE, fig.width=8, fig.height=8}
selected_variables = c("age", "movement_reactions",
  "skill_ball_control", "skill_dribbling")
