...
```{r 06-technical-error-handling-001, include = F}
library(mlr3)
library(mlr3book)
```

To demonstrate how to properly deal with misbehaving learners, `r mlr3book::mlr_pkg("mlr3")` ships with the learner `r ref("mlr_learners_classif.debug", "classif.debug")`:
...
```{r 06-technical-error-handling-004}
task = tsk("iris")
...
By default, `r mlr3book::gh_pkg("mlr-org/mlr3")` does not catch conditions such as warnings or errors raised by third-party code like learners:

```{r 06-technical-error-handling-005, error = TRUE}
...
One way to encapsulate the execution is provided by the package `r mlr3book::cran_pkg("evaluate")` (see `r ref("encapsulate()")` for more details):
...
Another method for encapsulation is implemented in the `r mlr3book::cran_pkg("callr")` package.
`r mlr3book::cran_pkg("callr")` spawns a new R process to execute the respective step, and thus even guards the current session from segfaults.
On the downside, starting new processes comes with a computational overhead.

```{r 06-technical-error-handling-008}
...
```{r 06-technical-error-handling-009, error = TRUE}
...
So whenever the debug learner fails (which is every time with the given parametrization) and encapsulation in enabled, `r mlr3book::mlr_pkg("mlr3")` falls back to the predictions of the featureless learner internally:
...
```{r 06-technical-error-handling-014}
task = tsk("iris")
learner = lrn("classif.debug")
