...
`r authors("Algorithmic Fairness")`

In this chapter, we will explore `r index('algorithmic fairness')` in automated decision making and how we can build fair and unbiased (or at least less biased) predictive models.
...
<!-- FIXME: FIX REFERENCE This chapter heavily borrows from the paper accompanying the pacakge [@mlr3fairness], the interested reader is referred to this manuscript for a more in-depth introduction. -->
...
As we work through this chapter we will use the `"adult_train"` dataset from `r mlr3fairness`, which contains a subset of the `Adult` data set [@uci].
This is a binary classification task to predict if an individual earns more than $50,000 per year.
...
```{r}
sex_salary = table(task$data(cols = c("sex", "target")))
...
In this chapter we will only focus on group fairness, however one could also consider auditing `r index('individual fairness')`, which assesses fairness at an individual level, `r index('and causal fairness')`, which incorporate causal relationships in the data and propose metrics based on a directed acyclic graph [@fairmlbook;@mitchell21].
Whilst we will only focus on metrics for binary classification here, most metrics discussed naturally extend to more complex scenarios, such as multi-class classification, regression, or survival analysis [@Sonabend2022a].
...
This group includes the concept of "Demographic Parity", which tests if the the proportion of positive predictions (PPV) is equal across all groups.
Bias transforming methods (i.e., those that test for independence) do not depend on labels and can help detect biases arising from different base rates across populations, but enforcing them might induce a shift in the data distribution, possibly leading to feedback loops.
...
Several metrics fall under this category, such as "equalized odds", which tests if the TPR and FPR is equal across groups.
Bias preserving metrics (which test for separation) test if errors made by a model are equal across groups but might not account for bias in the labels.
...
Whichever metric is chosen, we are essentially condensing systemic biases and prejudices into a single number, and therefore all metrics are limited and none can identify all biases that may exist in the data.
...
If we used TPR as our metric $M$ then if $|\Delta_{M}| > \epsilon$ (e.g., $\epsilon = 0.05$) then we would conclude predictions from our model violate the equality of opportunity metric.

In `r mlr3fairness` we can construct a fairness metric from any `Measure` by constructing the `msr("fairness")` measure and passing our metric of choice to `base_measure` as well as the possible range the metric can take (i.e., the range in differences possible based on the base measure):
...
prediction = lrn_rpart$predict(test_task)
...
In this example, we can example see the model is more likely to predict 'Female' observations as being more likely to have a lower salary.
...
1. Preprocessing data -- The underlying data is preprocessed (@sec-preprocessing) in some way to address bias in the data before it is passed to the `Learner`;
...
#| fig-alt: "Scatterplot with dots and crosses. x-axis is 'classif.acc' between 0.725 and 0.825, y-axis is 'fairness.equalized_odds' between 0 and 0.4. Plot results described in below text."
#| label: fig-fairness-tradeoff
fairness_accuracy_tradeoff(bmr, fairness_measure = msr("fairness.eod"), accuracy_measure = msr("classif.ce")) +
  ggplot2::scale_color_viridis_d("Learner") +
  ggplot2::theme_minimal()
```

In this experiment, the 'fair' algorithm actually performs worst with respect to both accuracy and equalized odds.
The `EOd` post-processing strategy has the lowest equalized odds (which is intuitive given this is what the method hopes to achieve) but at the cost of the second lowest accuracy.
The decision tree without any pre- or post-processing has the best accuracy but second highest bias; the reweighting preprocessing method has negligible effect on either fairness or accuracy.
