...
This chapter heavily borrows from the paper accompanying the package [@mlr3fairness], the interested reader is referred to this manuscript for a more in-depth introduction.
...
#| fig-cap: Fairness prediction density plot (left) showing the density of predictions for the positive class split into "Male" and "Female" individuals. The metrics comparison plot (right) depicts the model's scores across the specified metrics using bars.
...
The `r mlr3fairness` package comes with several options to address biases in models, which broadly fall into three categories:
...
Employing the `EOd` post-processing strategy improves the model for the  (equalized odds difference) metric at the cost of some accuracy. Employing the reweighing strategy however seems to have a negligible effect.
...
Questionnaires for ML models and data sets have been previously proposed in literature and are easily available from `r mlr3fairness`.
...
2. Assume our goal is to achieve parity in *false omission rates*.
  Construct a fairness metric that encodes this and ageing; evaluate your model.
