...
As we work through this chapter we will use the `adult_train` and `adult_test` datasets from `mlr3fairness`, which contain a subset of the `Adult` data set [@uci].
...
This group includes the concept of "`r index('Demographic Parity')`", which tests if the the proportion of positive predictions (`r index('PPV')`) is equal across all groups.
...
We have implemented several `Measure`s in `mlr3fairness` that simplify this step for you, these are named as `fairness.<base_measure>`, for example for TPR: `msr("fairness.tpr")` would run the same code as above.
...
Questionnaires for machine learning models and data sets have been previously proposed in literature and are available in `mlr3fairness` from automated report templates (`r ref("report_modelcard()")` and `r ref("report_datasheet()")`) using R markdown for data sets and machine learning models.
