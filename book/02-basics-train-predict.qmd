...
The concept is demonstrated on a supervised classification task using the `"mlr_tasks_penguins", text = "penguins")` dataset and the `"mlr_learners_classif.rpart", text = "rpart")` learner, which builds a single classification tree.
...
More precisely, a `"LearnerClassif")` returns a `"PredictionClassif")` object.
...
The simplest way to extract this information is by converting the `"Prediction")` object to a `data.table()`:
...
To predict these probabilities, the `predict_type` field of a `"LearnerClassif")` must be changed from `"response"` (the default) to `"prob"` before training:
...
Similarly to predicting probabilities for classification, many `"LearnerRegr", text = "regression learners")` support the extraction of standard error estimates for predictions by setting the predict type to `"se"`.


## Plotting Predictions {#autoplot-prediction}

Similarly to [plotting tasks](#autoplot-task), `r mlr3book::mlr_pkg("mlr3viz")` provides an `"ggplot2::autoplot()", text = "autoplot()"` method for `"Prediction")` objects.
All available types are listed in the manual pages for `"autoplot.PredictionClassif()"`, `"autoplot.PredictionRegr()"` and the other prediction types (defined by extension packages).
...
We choose accuracy (`"mlr_measures_classif.acc", text = "classif.acc")`) as our specific performance measure here and call the method `$score()` of the `prediction` object to quantify the predictive performance of our model.
...
Note that if no measure is specified, classification defaults to classification error (`"mlr_measures_classif.ce", text = "classif.ce")`, the inverse of accuracy) and regression to the mean squared error (`"mlr_measures_regr.mse", text = "regr.mse")`).
