...
```{r 08-special-costsens-001, include = F}
library(mlr3)
library(mlr3book)
```
...
For our first model, we choose an ordinary logistic regression (implemented in the add-on package `r mlr3book::mlr_pkg("mlr3learners")`).
...
Fortunately, there already is a predefined measure `r ref("Measure")` for this purpose: `"MeasureClassifCosts")`:

```{r 08-special-costsens-009}
...
We compare the logistic regression to a simple featureless learner and to a random forest from package `r mlr3book::cran_pkg("ranger")` :

```{r 08-special-costsens-010}
learners = list(
  lrn("classif.log_reg"),
...
Currently `r mlr3book::mlr_pkg("mlr3pipelines")` offers two main strategies towards adjusting `classification thresholds`.
...
`PipeOpThreshold` can be put directly after a `r ref("Learner")`.

A simple example would be:

```{r 08-special-costsens-014}
...
```{r 08-special-costsens-016}
library("paradox")
ps = ps(threshold.thresholds = p_dbl(lower = 0, upper = 1))
```

We now create a `AutoTuner`, which automatically tunes the supplied learner over the `r ref("ParamSet")` we supplied above.

```{r 08-special-costsens-017}
at = AutoTuner$new(
  learner = l,
...
Again, we create a `AutoTuner`, which automatically tunes the supplied learner over the `r ref("ParamSet")` we supplied above.
