...
In this practical example, we will optimize the hyperparameters of XGBoost on the `tsk("spam")` dataset.
...
In this example, we will optimize the hyperparameters of an SVM on the `tsk("sonar")` dataset.
...
In `r mlr3mbo`, the EGO algorithm is implemented via the `bayesopt_ego()` loop function.
