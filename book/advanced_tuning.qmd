...
1. Generate an `r index('initial design', aside = TRUE)`
2. Loop:
    a. Fit a `r index('surrogate model', aside = TRUE)` on available data  to model the unknown black-box function.
    b. Use `r index('acquisition function', aside = TRUE)` to determine which points of the search space are promising candidate(s) that should be evaluated next.
