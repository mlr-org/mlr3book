...
Due to the nonconvex nature of most commonly used acquisition functions \citep[p.~208]{garnett_2022}, it is typical to employ global optimization techniques for acquisition function optimization.
Widely used approaches for optimizing acquisition functions include derivative-free global optimization methods like branch and bound algorithms, such as the DIRECT algorithm \citep{jones1993lipschitzian}, as well as multi-start local optimization methods, such as running the L-BFGS-B algorithm \citep{byrd1995limited} or a local search multiple times from various starting points \citep{kim_2021}.
...
However, even when employing a Gaussian Process as surrogate model, the issue of vanishing gradients in the mean and standard deviation predictions concerning the input variables presents a significant challenge \citep[p.~208]{garnett_2022}.
