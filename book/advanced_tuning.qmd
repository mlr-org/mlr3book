...
Therefore it is essential to also select a fallback learner (@sec-fallback), which is a learner that will be fitted if the primary learner fails.
...
Similarly to how we can use `r ref("tune()")` to construct a tuning instance, here we can use `r ref("bb_optimize()", aside = TRUE)`, which returns a list with elements `"par"` (best found parameters), `"val"` (optimal outcome), and `"instance"` (the optimization instance); `"par"` and `"val"` are the equivalent to the `instance$result`:
...
We will tune three parameters of a decision tree with respect to true positive and false positive rate, the Pareto front is visualized in @fig-pareto-bayesopt.
