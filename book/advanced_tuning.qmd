...
`r authors("Advanced Tuning Methods and Black Box Optimization")`
...
## Error handling and memory management Tuning {#sec-tuning-errors}

In this section we will look at how to use `mlr3` to ensure that tuning workflows are efficient and robust.
In particular, we will consider how to enable features that prevent fatal errors leading to irrecoverable data loss in the middle of an experiment.

### Encapsulation and Fallback Learner {#sec-encapsulation-fallback}

Until now, the book has focused on non-breaking examples, but to demonstrate encapsulation we will now consider 'broken' examples, e.g., where learners do not converge, run out of memory, or terminate with an error.
...
The Pareto front is visualized in @fig-bo-pareto.
Note that the number of selected features can be fractional, as in this example, it is determined through resampling and calculated as an average across the number of selected features per cross-validation fold.
