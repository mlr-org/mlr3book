...
HPO is therefore a prime example for `r index('black-box optimization', aside = TRUE)`, which considers the optimization of a function whose structure and analytical description is unknown, unexploitable, or non-existent.
...
`OptimInstanceSingleCrit` relies on an `r ref("Objective", aside = TRUE)` function that wraps the actual mapping from a domain (all possible function inputs) to a codomain (all possiple function outputs).
...
* `r ref("ObjectiveRFun")` wraps a function that takes a list describing a *single configuration* as input. It is suitable when the underlying function evaluation mechanism is given by evaluating a single configuration at a time.
* `r ref("ObjectiveRFunMany")` wraps a function that takes a list of *multiple configurations* as input. It is useful when the function evaluation of multiple configurations can be parallelized.
...
Similarly to HPO, transformation are often used to efficiently search the space (@sec-logarithmic-transformations).
...
::: {.callout-tip}
`r ref("SurrogateLearnerCollection")` should be used instead of `r ref("SurrogateLearner")` if the optimization problem and choice of loop function means that multiple optimization targets are modelled by multiple surrogates.
:::
...
An `r index('acquisition function optimizer')` of class `r ref("AcqOptimizer", aside = TRUE)` is used to solve this optimization problem by efficiently searching the space of potential candidates within a limited computational budget.
Due to the nonconvex nature of most commonly used acquisition functions [@garnett_2022] it is typical to employ global optimization techniques for acquisition function optimization.
...
Consequently, in most cases, the optimization problem of the acquisition function can be regarded as a black box optimization problem itself.
...
The `$optimize()` method optimizes the acquisiton function and returns the next candidate.
...
A common choice of loop function is the Efficient Global Optimization (EGO) algorithm [@jones_1998] (`r ref('bayesopt_ego()')`), a simplified version of this code is shown at the end of this section to help demonstrate the algorithm and its purpose.
In short, the code gets the relevant components discussed above and then loops the steps above:  1) update the surrogate model 2) update the acquisition function 3) optimize the acquisition function to yield a new candidate 4) evaluate the candidate and add it to the archive.
...
`r mlr3mbo` can be used for HPO by making use `r ref("TunerMbo", aside = TRUE)`, which is a wrapper around `r ref("OptimizerMbo")` and therefore works in the examt same way.
...
Multi-objective tuning is also possible with BO however algorithms may handling this differently with many different design choices, for example whether they use a scalarization approach of objectives and only rely on a single surrogate model, or fit a surrogate model for each objective.
...
Real world black-box functions, however, are often noisy meaning that repeatedly evaluating the same point will return different objective function values due to background noise augmenting the black-box function.
...
1. A surrogate model can be used to model the noise
...
In the worst-case scennario, if all iterations errored, the BO algorithm will simply perform a random search.
...
<!-- FIXME: HOW CAN A USER DO THIS WITH TRM? -->
