...
learner = lrn("classif.xgboost", eta = to_tune(1e-4, 1))

tune(rnd_srch, task_pim, learner, cv3, msr_ce, 10)
```
```{r, echo = FALSE}
task_pim = tsk("pima")
cv3 = rsmp("cv", folds = 3)
...
Below we set `classif.featureless`, which always predicts the majority class, by passing this learner to the `$fallback` field.
...
```{r optimization-038}
instance = tune(rnd_srch, task_pim, learner, cv3, msr_ce, 10)
...
Increasingly large datasets and search spaces and increasingly complex models make hyperparameter optimization a time-consuming task.
To tackle this, some HPO methods make use of evaluating a configuration at multiple fidelity levels.
`r index('Multi-fidelity HPO', aside = TRUE)` is motivated by the idea that the performance of a lower-fidelity model is predictive of the full-fidelity model, which can be used to make HPO more efficient (as we will soon see with Hyperband).

To unpick what these terms mean and to motivate multi-fidelity tuning, say that we think a gradient boosting algorithm with 1000 rounds will be a very good fit to our training data however we are concerned this model will take too long to run.
Therefore, we want to predict the performance of this model using a less complex model that is quicker to train by setting a smaller number of rounds.
...
Lower fidelity levels result in `r index('low-fidelity models', aside = TRUE)` that are lower complexity and quicker to train but may poorly predict the full-fidelity model's performance.
On the other hand, higher fidelity levels result in `r index('high-fidelity models', aside = TRUE)` that are higher complexity and slower to train but may better predict the full-fidelity model's performance.
...
Successive Halving is initialized with the number of starting configurations $n$, the proportion of configurations discarded in each stage $\eta$, and the minimum, $r{_{min}}$, and maximum, $r{_{max}}$, budget (fidelity) of a single evaluation.
...
A big disadvantage of this method is that it is unclear if it is better to start with many configurations (large $n$) and a small budget or less configurations (small $n$) but a larger budget.
...
The algorithm is initialized with the same $\eta$, $r{_{min}}$, and $r{_{max}}$ parameters (but not $n$).
Each bracket starts with a different budget, $r{_{0}}$, where smaller values mean that more configurations can be evaluated and so the most exploratory bracket (i.e., most number of stages) is allocated the minimum budget $r{_{min}}$.
In each bracket, the starting budget increases by a factor of $\eta$ until the last bracket ($s = 0$) essentially performs a random search with the full budget $r{_{max}}$ -- the minimum budget, $r{_{min}}$, may have to be adjusted so the last bracket does not exceed $r{_{max}}$.
The total number of brackets, $s{_{max}} + 1$, is calculated as $s{_{max}} = {\log_\eta \frac{r{_{max}} }{r{_{min}}}}$.
...
We specify a range from 16 ($r{_{min}}$) to 128 ($r{_{max}}$) boosting iterations and tag the parameter with `"budget"`to identify it as a fidelity parameter.
