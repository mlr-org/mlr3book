...
Bayesian optimization (BO) is a global optimization algorithm that usually follows a simple iterative loop:

1. Fit a `r index('surrogate model', aside = TRUE)` on an `r index('initial design', aside = TRUE)` to model the unknown black-box function.
2. Train the surrogate model on all data points observed so far.
3. Use `r index('acquisition function', aside = TRUE)` to determine which points of the search space are promising candidate(s) that should be evaluated next.
4. Evaluate the next candidate(s) and update the archive of all observations made so far.
5. Repeat 2-4 until a given termination criteria is met.
