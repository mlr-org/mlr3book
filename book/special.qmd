...
In 'regular' classification, the aim is to optimize a metric (usually the misclassification rate) whilst assuming all misclassification errors are deemed equally severe.
...
In this experiment the logistic regression performs best as it minimizes the expected costs (and maximizes expected profits); the featureless learner performs the worst.
...
As with probabilistic regression, we implement these predictions using the `r ref_pkg("alan-turing-institute/distr6")` interface, which allows visualization and evaluation of survival curves.
...
Below we benchmark a decision tree on the `r ref("mlr_tasks_ecuador")` task using two different repeated cross-validation resampling methods, the first ("NSpCV") is *not* optimized for spatial data, the second ("SpCV") *is* optimized for spatial data.
