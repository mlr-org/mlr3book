...
The chapter title reflects that up until now we have only considered two tasks.
...
| Probabilistic regression | `r mlr3` and `r mlr3proba`  | Probability distribution prediction of a continuous variable. |
| Deterministic single-label classification | `r mlr3`  | Prediction of a single class an observation falls into. |
...
This is a deterministic classification problem where we are classifying if someone will be in class 'Good' or 'Bad'.
...
If you think they have bad credit then there is no profit or loss, the loan is not provided.
If they have good credit and return the loan then you make a \$1K profit, but if they default then you lose \$5K.
...
However all costs are positive, which actually means a loss is made, so let's now see if we can improve these models by using thresholding.
...
For example, instead of just predicting someone's height, $\mu$, we could also predict how confident we are about our prediction, usually estimated as a standard error, $\sigma$.
...
Predicting a `r index("confidence interval")` is possible using methods in the `r mlr3` core package (no extension packages are needed).
...
An example is in @fig-probregrconfint where we assume a symmetric Normal 95% confidence interval by setting `quantile = 1.96` which calculates `response` $\pm$ 1.96 `se`.
...
`r mlr3proba` ('mlr3 **proba**bilistic') includes a pipeline `r ref("pipeline_probregr()")` that incorporates the key steps of distribution prediction for probabilistic regression, which are:
...
In the example below we train a Cox PH model on the `rats` dataset and then plot the survival function of the first three predictions and evaluate this function for all three predictions at $t = 77$.
...
The output indicates that there is a 90.2\%, 90.0\%, and 94.6\% chance of the first three predicted rats being alive at time 77 respectively.
...
Density estimation is the learning task to estimate the unknown distribution from which a univariate dataset is generated, or put more simply to estimate the probability density (or mass) function for a single variable.
...
We then use the `probregr` pipeline (@sec-probregr) and train, predict, and score a random forest learner (`regr.ranger`).
Note the key difference in these outputs is that whilst both make distribution predictions, the former (density) *estimates* a single probability distribution (of class `r ref("distr6::Distribution")`) for the variable of interest, whereas the latter (regression) *predicts* a vector of distributions (`r ref("distr6::VectorDistribution")`), where each distribution in the vector is a prediction for a respective observation.
...
Instead we provide new resampling methods in `r mlr3spatiotempcv` to account for the similarity in the train and test sets during resampling that originates from spatial autocorrelation.
...
Below we benchmark a decision tree on the `r ref("mlr_tasks_ecuador")` task using two different repeated cross-validation resampling methods, the first ("NSpCV" (non-spatial cross-validation)) is a standard resampling method from `r mlr3`, the second ("SpCV" (spatial cross-validation)) is from `r mlr3spatiotempcv` and is optimized for spatial data.
...
Now we have seen why spatial resampling matter we can take a look at what is implemented in `r mlr3spatiotempcv`.
