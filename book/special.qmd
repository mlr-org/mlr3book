...
Besides performance estimation, prediction on spatial data is another challenging task, which we turn to next.

#### Spatio*temporal* resampling

Throughout this section we have focused on spatial analysis but referred to "spatiotemporal" and "spatiotemp".
The resampling methods discussed in this section extend trivially to temporal analysis (or spatial and temporal analysis combined) by setting a `col_role` in the task (@sec-performance), as demonstrated in the example below.

```{r special-044}
# load spatiotemporal task
task = tsk("cookfarm_mlr3")
task$col_roles[c("target", "space", "time")]

# load spatiotemporal resampling strategy
rsmp_cstf_time = rsmp("sptcv_cstf", folds = 5)

# visualise partitions
plot = autoplot(rsmp_cstf_time,
  fold_id = 5, task = task, plot3D = TRUE,
  sample_fold_n = 3000L
)
