...
Spatial data has additional complexity due to `r define("autocorrelation")` which violates one of the fundamental assumptions in machine learning - all observations in the dataset are independent [@hastie2001].

To illustrate autocorrelation, let us start with a classic example of regression, predicting the height of a person.
Imagine we could sample the entire world's population and we pick out three people at random: Rose, Lily, and Juniper.
Given no other information, the best prediction we could make is to predict that each of them is 169cm (roughly average human height).
Now as a spatial example, imagine we are predicting the number of cases of a disease in three countries: Singapore, Jamaica, Lebanon.
Given no other information, naively we might think the best prediction is around 14 degrees Celsius.
...
The nature of spatial data means that observations are naturally grouped together according to distance in space.
For example, say a disease has broken out in Australia and an epidemiologist is trying to predict the number of cases worldwide.
Say the epidemiologist has no data other than latitude, longitude, and number of cases at the start of the outbreak in Australia.
If there was no spatial autocorrelation in the data then they might predict something like we see in Fig XXX (top).
However this is of course not true to real-life where diseases will spread outwards from the point of origin (Fig XXX (bottom)), meaning that there is information contained with the spatial coordinates.
In simpler terms then, spatial autocorrelation is the degree of correlation between two values of the same variable.
In our example we can visualise the autocorrelation as the number of cases decreases as distance in space increases.

```{r, warning=FALSE}
world_coordinates <- map_data("world")
world_coordinates$cases <- pmax(0, rnorm(99338, 0, 375))
p = ggplot() +
...
p1 = p + geom_map(
    ...
    aes(long, lat, map_id = region, fill = cases)
  )
world_coordinates$cases <- -(abs(world_coordinates$lat + 20) + abs(world_coordinates$lon - 120))
world_coordinates$cases <- (world_coordinates$cases + abs(min(world_coordinates$cases)))
