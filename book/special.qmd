...
To visualise this, silhouette plots include a dotted line at average silhouette coefficient value and a bar for the score of each data point coloured by cluster.
In our particular case, the average silhouette index is `r round(msr("clust.silhouette")$score(prediction, task), 2)`.
If many or all data points in a cluster are scored below the average silhouette coefficient line or are near zero then the these observations are considered to be poorly clustered whereas observations with higher silhouette values are well clustered.
...
1. Run a benchmark experiment on the `wine` dataset without a label column. Compare the performance of `k-means` learner with `k` equal to 2, 3, 4 using the silhouette measure. Use insample resampling technique. What value of `k` would you choose based on the silhouette scores?
