...
abstract: Previous chapters have considered the core functionality of functions and features in the mlr3 universe and other helper packages, which altogether create a modular ecosystem. This chapter uses that modularity to discuss tasks beyond deterministic regression and single-label classifation. The chapter starts with cost-sensitive classification, which makes use of mlr3, mlr3tuning, and mlr3pipelines to implement a classification task in which different types of errors have different costs. Next, the chapter looks at probabilistic regression, which is a regression problem where the prediction is a distribution and not a single point prediction. We first discuss how this is implemented in mlr3 and how this is extended in mlr3proba. We then discuss survival analysis and density estimation, which are both implemented in mlr3proba and make use of the distr6 package for distribution predictions. After these tasks, we look at unsupervised learning in the form of clustering with mlr3cluster, and then finally spatial analysis with mlr3spatial and mlr3spatiotempcv. As some or all of these tasks may be new to some readers, we include an overview to the theory behind each, before looking at the extensions to common mlr3 objects (learners, predictions, tasks, and measures), and then finally consider task-specific extensions.
---
...
This chapter title is a mouthful, but it highlights the limited number of tasks we have considered up until now.
...
In many cases, to implement a new machine learning paradigm, all you need to do is create a class that extends the above four building blocks and then implement algorithms that inherit from these (read @sec-extending if you are interested in extending new tasks or adding new learners and/or measures).

@tbl-alltasks summarises available extension tasks, including the package(s) they are implemented in, the stability of the task implementation, and a brief description.
...
In 'regular' classification, the aim is to optimise a metric (usually the misclassification rate) whilst assuming all misclassification errors are deemed equally severe.
...
As we discuss this task we will work with `r ref("mlr_tasks_german_credit")` (@sec-tasks) as a running example.

Let us imagine in this task that an insurance company, InsureU, provides fixed loans of \$5K over a one year period and customers are expected to pay back \$6K at the end of this period.
...
Note that cost-sensitive classification is a minimisation problem where we assume lower costs mean higher profits/positive outcomes, hence above we wrote the profit and loss as `-1` and `+5` respectively.
...
To automate the process of optimising the threshold, we can make use of `r mlr3tuning` (@sec-optimization) and `r mlr3pipelines` (@sec-pipelines) by creating a graph with `r ref("PipeOpTuneThreshold")`.
...
This 'confidence' could be expressed as a confidence interval, for example $\mu \pm \sigma$, or by assuming a distribution, for example $Norm(\mu, \sigma)$.
...
For example the bottom prediction in @fig-probregrconfint has wide error bars and is further from the truth than the second prediction from the bottom, which has narrower error bars and is closer to the truth.
Analogously to classification, we might think of the bottom prediction as "P(Y = 14) = 0.4" versus the third prediction as "P(Y = 16) = 0.8" (numbers not exact).
...
Instead we could have plugged the predicted `reponse` and `se` into a `r index("probability distribution")` as an estimate of the distribution mean and standard deviation.
...
4. Transform predictions to parameterise the chosen distribution
5. Return prediction

This pipeline is a wrapper around `r ref("PipeOpProbregr")`, and is showcased in the code below where we specify three inputs: 1) a random forest as the main learner, which predicts `response`; 2) a featureless learner to estimate the standard deviation, which simply predicts the future standard error as the sample standard deviation in the training set; and 3) we assume a Normal distribution for the predicted distribution. We visualise the quality of predictions in @fig-probregrdist.
...
Similarly to these settings, survival learners are constructed with `"lrn"`; available learners are listed below.
...
`r mlr3proba` has a different design interface to `r mlr3` as all possible types of prediction ('predict types') are returned for all survival models.
...
In the example below we train and predict from a survival support vector machine (`r ref("mlr_learners_surv.svm")`), note we use `type = "regression"` to select the algorithm that optimises survival time predictions.
...
The `crank` prediction type is informative and common in practice because it allows identifying observations at lower/higher risk to each other, which is useful for resource allocation and prioritisation (e.g., which patient should be given an expensive treatment), and clinical trials (e.g., are people in a treatment arm at lower risk of disease X than people in the control arm.).
...
Below we construct a C-Means clustering learner with `prob` prediction type and 3 clusters, train it on the `r ref("Ruspini")` dataset and then return the cluster assignments (`$assignments`) for each observation.
...
For consistency, `r mlr3cluster` offers `predict` method for hierarchical clusterers but with a warning:
...
In this case, `predict` method simply cuts the dendrogram into number of clusters specified by `k` parameter of the learner.
...
Two common measures are the within sum of squares (WSS) measure, `r ref("mlr_measures_clust.wss")`, and silhouette coefficient, `r ref("mlr_measures_clust.silhouette")`.
...
Silhouette coefficient quantifies how well each point belongs to its assigned cluster versus neighboring clusters, where scores closer to `1` indicate well clustered and scores closer to `-1` indicate poorly clustered.
Note that `r mlr3cluster`'s silhouette measure outputs mean Silhouette score across all points and when there is only a single cluster, the measure simply outputs 0.
...
Often reducing an unsupervised task to a quantitative measure may not be useful (given no ground truth) and instead visualisation may be a more effective tool for assessing the quality of the clusters.

#### Measure Limitations
It is easy to rely on clustering measures to assess the quality of clustering.
However this should be done with some care.
...
Now we will attempt to perform cluster analysis.
...
We observe that K-means clustering gives us a higher average silhouette score.
Therefore we might assume that K-means learner with 2 centroids is a a better choice than DBSCAN method.
However, take a look at the cluster assignment plots.
...
We see that learners came up with different results -- K-means assigned points that are part of the same line into two different cluster while DBSCAN assigned each line into its own cluster.
Which one of these approaches is correct?
The answer is it depends on your specific task and the goal of cluster analysis.
However just relying on the silhouette score would not have revealed the details of how exactly the clustering was done and whether or not it was appropriate for the task.

### Visualization

As with other tasks, we use `r mlr3viz` to visualise performance of cluster learners. The two most important plots are `r index("principal components analysis")` (PCA) and silhouette plots.

PCA is a commonly used dimension reduction method in ML to reduce the number of variables in a dataset or to visualise the most important 'components', which are linear transformations of the dataset features.
Components are considered more important if they have higher variance (and therefore more predictive power).
In the context of clustering, by plotting observations against the first two components, and then colouring them by cluster, we could visualise our high dimensional dataset and we would expect to see observations in distinct groups.

Since our running examples already have only two features, it does not make sense for us to perform PCA and then visualize the data.
So let us use a task based on `USArrests` dataset instead.
By plotting result of PCA we see that our model has separated observations into two clusters along the first two principal components.
...
Silhouette plots visually assess the quality of the estimated clusters by visualising if observations in a cluster are well-placed both individually and as a group.
The average silhouette index is 0.57, which means observations that are scored below this line are considered to be poor estimations whereas observations above are well clustered, the silhouette coefficient is just the average across all observations.
To visualise this, silhouette plots include a dotted line at 0.57 and a bar for the score of each data point coloured by cluster.

Continuing with our new example, we find that a lot of observations are actually below the average line, and therefore the quality of our prediction is not very good, meaning that many observations are likely assigned to the wrong cluster.
...
Finally, we conduct a small benchmark study using the `r ref("ruspini")` data and with a few integrated cluster learners:
...
Spatial analysis can be a subset of any other machine learning task (e.g., regression or classification) andis defined by the presence of spatial information in a dataset, usually stored as coordinates that are often named "x" and "y" or "lat" and "lon".
...
Outbreaks radiate outwards from an epicentre and therefore countries closer to Germany will have higher numbers of cases and countries further away will have lower numbers (Fig @fig-autocorrelation, bottom), thus
there is autocorrelation in the 'country'.

```{r special-042, warning=FALSE, echo=FALSE, message=FALSE}
...
Below we benchmark a decision tree on the `r ref("mlr_tasks_ecuador")` task using two different repeated cross-validation resampling methods, the first ("NSpCV") is *not* optimised for spatial data, the second ("SpCV") *is* optimised for spatial data.
...
The choice of method may depend on specific characteristics of the dataset and there is no easy rule to pick one method over another, full details of different methods can be found in @Schratz2021.
...
See the `r mlr3spatiotempcv` visualisation `r link("https://mlr3spatiotempcv.mlr-org.com/articles/spatiotemp-viz.html", "vignette")` for specific details about 3D spatiotemporal visualisation.
:::
...
```{r special-047, warning=FALSE, message=FALSE}
library(sf)
...
```{r special-049, message = FALSE, cache.lazy=FALSE, cache=FALSE}
...
Survival analysis, also available in `r mlr3proba`, is a special form of probabilistic regression when the outcome is censored, which means it may never be observed within a given timeframe.
...
See below for a list gallery posts and vignettes for the tasks discussed in this chapter:

1. `r link("https://mlr-org.com/gallery/optimization/2020-10-14-threshold-tuning/", "Threshold tuning")`
1. `r link("https://mlr-org.com/gallery/technical/2023-02-27-land-cover-classification/", "Spatial analysis")`
1. `r link("https://mlr-org.com/posts/2020-08-26-introducing-mlr3cluster-cluster-analysis-package/", "Introducing mlr3cluster")`
1. `r link("https://mlr3spatial.mlr-org.com/articles/benchmark.html", "Benchmarking parallel spatial predictions")`
1. `r link("https://mlr3spatiotempcv.mlr-org.com/articles/spatiotemp-viz.html", "Spatiotemporal visualisation")`
