...
In the above example we used the `r ref("survival::rats")` dataset, which looks at predicting if a drug treatment was successful in 150 rats.
Note the dataset (by its own admission) is not perfect and should generally be treated as 'dummy' data which isgood for examples but not real-world analysis.
