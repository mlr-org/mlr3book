...
This chapter explores the different functions of `r ref_pkg("mlr3")` when dealing with specific data sets that require further statistical modification to undertake sensible analysis.
...
Instructions on how to deal with [multilabel classification](#multilabel) in`r ref_pkg("mlr3")`can be found in this sub-chapter.
...
This sub-chapter explains how to perform [cluster analysis](#cluster) in`r ref_pkg("mlr3")`with the help of `mlr3cluster` extension package.
...
The package `r ref_pkg("mlr3proba")` [@mlr3proba] extends `r ref_pkg("mlr3")` with the following objects for survival analysis:
...
The example below uses the `r ref("mlr_tasks_rats", text = "rats")` task shipped with `r ref_pkg("mlr3proba")`.
...
Finally we take a look at the `r ref("PipeOp")`s implemented in `r ref_pkg("mlr3proba")`, which are used for composition of predict types.
...
See `r ref_pkg("mlr3pipelines")` for full tutorials and details on `r ref("PipeOp")`s.
...
The conditional case, where a distribution is predicted conditional on covariates, is known as ‘probabilistic supervised regression’, and will be implemented in `r ref_pkg("mlr3proba")` in the near-future.
...
The package `r ref_pkg("mlr3proba")` extends `r ref_pkg("mlr3")` with the following objects for density estimation:
...
This task ships as pre-defined `r ref("TaskDens")` with `r ref_pkg("mlr3proba")`.
...
In the `r ref_pkg("mlr3")` framework, the following packages relate to this field:

- `r ref_pkg("mlr3spatiotempcv")` (biased-reduced performance estimation)
- `r ref_pkg("mlr3spatial")` (spatial prediction support)
...
`r ref_pkg("mlr3spatiotempcv")` provides access to the most frequently used spatiotemporal resampling methods.
...
To account for the spatial autocorrelation probably present in the landslide data, we will make use one of the most used spatial partitioning methods, a cluster-based k-means grouping [@brenning2012], (`r ref("mlr_resamplings_spcv_coords", text = "spcv_coords")` in `r ref_pkg("mlr3spatiotempcv")`).
...
Performance is commonly estimated via cross-validation and `r ref_pkg("mlr3spatiotempcv")` provides specialized resamplings methods for spatiotemporal data.
...
In addition, examples which show how resamplings with spatial information can be visualized using `r ref_pkg("mlr3spatiotempcv")`.
...
Have a look at the ["Getting Started"](https://mlr3spatiotempcv.mlr-org.com/dev/articles/mlr3spatiotempcv.html#resampling-methods) vignette of `r ref_pkg("mlr3spatiotempcv")` to see all available methods and choose one which **fits your data and its prediction purpose**.
:::
...
Every partitioning method in `r ref_pkg("mlr3spatiotempcv")` comes with S3 methods for `plot()` and `autoplot()` to visualize the created groups.
...
`r ref_pkg("mlr3spatiotempcv")` can also visualize non-spatial partitonings.
...
For almost all methods implemented in `r ref_pkg("mlr3spatiotempcv")`, there is a scientific publication describing the strengths and weaknesses of the respective approach (either linked in the help file of `r ref_pkg("mlr3spatiotempcv")` or its respective dependency packages).
...
:::{.callout-tip}
A list of all implemented methods in `r ref_pkg("mlr3spatiotempcv")` can be found in the [Getting Started](https://mlr3spatiotempcv.mlr-org.com/articles/mlr3spatiotempcv.html#resampling-methods) vignette of the package.
:::
...
Support for (parallel) spatial prediction with `r ref_pkg("terra")`, `r ref_pkg("raster")`, `r ref_pkg("stars")` and `r ref_pkg("sf")` objects is available via `r ref_pkg("mlr3spatial")`.

`r ref_pkg("mlr3spatial")` has two main scopes:
...
`r ref_pkg("mlr3spatial")` provides `as_task_*` helpers to directly convert these into a spatiotemporal (ST) task:
...
`r ref_pkg("mlr3spatial")` adds support for the following spatial data classes:
...
To save users from having to go the extra mile of extracting the final model and using it with the respective `predict()` function of the spatial R package of their desired outcome class, `r ref_pkg("mlr3spatial")` provides `r ref("predict_spatial()")`.
...
To use a raster image for prediction, it must be wrapped into a `r ref("TaskUnsupervised")` for internal `r ref_pkg("mlr3")` reasons.
...
For our first model, we choose an ordinary logistic regression (implemented in add-on package `r ref_pkg("mlr3learners")`).
...
The package `r ref_pkg("mlr3cluster")` extends `r ref_pkg("mlr3")` with the following objects for cluster analysis:
...
For consistency with the rest of the ecosystem, `r ref_pkg("mlr3cluster")` offers `predict` method for hierarchical clusterers but it simply assigns all points to a specified number of clusters by cutting the resulting tree at a corresponding level.
...
Cluster analysis in `r ref_pkg("mlr3")` is integrated with `r ref_pkg("mlr3viz")` which provides a number of useful plots. Some of those plots are shown below.
