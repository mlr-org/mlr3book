...
To provide an example, with ${P}\left(\hat{Y} = 1 \mid A = \star, Y = 1\right)$ denoted with ${TPR}_{A=\star}$, we calculate the difference in TPR between the two groups:
\[
...
In `r mlr3`, the sensitive attribute is identified by the column role `pta` and can be set as follows, here on the `Adult` [@uci] dataset shipped with `r mlr3fairness` which will be explained below.

```{r special-050, eval = FALSE}
task = tsk("adult_train")
task$set_col_roles("martial_status", add_to = "pta")
```
...
The task is to predict whether an individual earns more than $50.000 per year from different demographic variables.
