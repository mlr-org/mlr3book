...
* `r ref("TaskSurv")` to define (censored) survival tasks
* `r ref("LearnerSurv")` as base class for survival learners
* `r ref("PredictionSurv")` as specialized class for `r ref("Prediction")` objects
* `r ref("MeasureSurv")` as specialized class for performance measures
...
The example below uses the `r ref("mlr_tasks_rats")` task shipped with `r mlr3proba`.
...
Finally, we conduct a small benchmark study on the `r ref("mlr_tasks_rats")` task using some of the integrated survival learners:
...
* `r ref("LearnerDens")` as base class for density estimators
* `r ref("PredictionDens")` as specialized class for `r ref("Prediction")` objects
* `r ref("MeasureDens")` as specialized class for performance measures

In this example we demonstrate the basic functionality of the package on the `r ref("datasets::faithful")` data from the `r ref_pkg("datasets")` package.
...
Finally, we conduct a small benchmark study on the `r ref("mlr_tasks_precip")` task using some of the integrated survival learners:
...
The following examples use the `r ref("mlr_tasks_ecuador")` dataset created by [Jannes Muenchow](https://scholar.google.com/citations?user=Slq94Y4AAAAJ&hl=de&authuser=1&oi=ao).
...
To account for the spatial autocorrelation probably present in the landslide data, we will make use one of the most used spatial partitioning methods, a cluster-based k-means grouping [@brenning2012], (`r ref("mlr_resamplings_spcv_coords")` in `r mlr3spatiotempcv`).
...
The performance of a simple classification tree (`"classif.rpart"`) is evaluated on a random partitioning (`r ref("mlr_resamplings_repeated_cv")`) with four folds and two repetitions.
The chosen evaluation measure is "classification error" (`"classif.ce"`).

For the spatial example, `r ref("mlr_resamplings_repeated_spcv_coords")` is chosen whereas `r ref("mlr_resamplings_repeated_cv")` represents the non-spatial example.

:::{.callout-note}
The selection of `r ref("mlr_resamplings_repeated_spcv_coords")` in this example is arbitrary.
...
In this example the `r ref("mlr_tasks_ecuador")` example task is taken to estimate the performance of an `rpart` learner with fixed parameters on it.
...
This examples showcases another SpCV method: `r ref("mlr_resamplings_spcv_block")`.
...
In the following we use the `r ref("mlr_resamplings_sptcv_cstf")` method after @meyer2018.
...
This is because `r ref("mlr_resamplings_sptcv_cstf")` can also be visualized in 2D (which only makes sense if the "space" column role is used for partitioning).
...
If both space and time are used for partitioning in `r ref("mlr_resamplings_sptcv_cstf")`, the visualization becomes even more powerful as it allows to also show the observations which are omitted, i.e., not being used in either train and test sets for a specific fold.
...
While the example in this section made use of the `r ref("mlr_resamplings_spcv_coords")` method, this should by no means infer that this method is the best or only method suitable for this task.
Even though this method is quite popular, it was mainly chosen because of the clear visual grouping differences when being applied on the `r ref("mlr_tasks_ecuador")` task when compared to random partitioning.
...
An exemplary data set for such a problem is the `r ref("mlr_tasks_german_credit")` task:
...
In the following example, `r ref("mlr_learners_clust.kmeans")` is applied on the `r ref("mlr_tasks_usarrests")` task.
...
One of them is `r ref("mlr_measures_clust.wss")` which calculates the sum of squared differences between observations and centroids.
...
Another measure is `r ref("mlr_measures_clust.silhouette")` that quantifies how well each point belongs to its assigned cluster versus neighboring cluster.
...
The following is an example of conducting a benchmark experiment with various learners on `r ref("mlr_tasks_iris")` without target variable and assessing the quality of each learner with both within sum of squares and silhouette measures.
