...
This chapter explores the different functions of `r mlr_pkg("mlr3")` when dealing with specific data sets that require further statistical modification to undertake sensible analysis.
...
Instructions on how to deal with [multilabel classification](#multilabel) in`r mlr_pkg("mlr3")`can be found in this sub-chapter.
...
This sub-chapter explains how to perform [cluster analysis](#cluster) in`r mlr_pkg("mlr3")`with the help of `mlr3cluster` extension package.
...
The package `r mlr_pkg("mlr3proba")` [@mlr3proba] extends `r mlr_pkg("mlr3")` with the following objects for survival analysis:
...
* `distr` - Predicted survival distribution, either discrete or continuous.
  Implemented in `r cran_pkg("distr6")`.
...
Finally we take a look at the `r ref("PipeOp")`s implemented in `r mlr_pkg("mlr3proba")`, which are used for composition of predict types.
...
See `r mlr_pkg("mlr3pipelines")` for full tutorials and details on `r ref("PipeOp")`s.
...
Support for (parallel) spatial prediction with `r cran_pkg("terra")`, `r cran_pkg("raster")`, `r cran_pkg("stars")` and `r cran_pkg("sf")` objects is available via `r cran_pkg("mlr3spatial")`.

`r cran_pkg("mlr3spatial")` has two main scopes:
...
`r cran_pkg("mlr3spatial")` provides `as_task_*` helpers to directly convert these into a spatiotemporal (ST) task:
...
`r cran_pkg("mlr3spatial")` adds support for the following spatial data classes:

- `stars` (from package `r cran_pkg("stars")`)
- `SpatRaster` (from package `r cran_pkg("terra")`)
- `RasterLayer` (from package `r cran_pkg("raster")`)
- `RasterStack` (from package `r cran_pkg("raster")`)
...
To save users from having to go the extra mile of extracting the final model and using it with the respective `predict()` function of the spatial R package of their desired outcome class, `r cran_pkg("mlr3spatial")` provides `r ref("predict_spatial()")`.
`r ref("predict_spatial()")` let's users

- choose the output class of the resulting raster image
- optionally predict in parallel via the `r cran_pkg("future")` package, using a parallel backend of their choice

To use a raster image for prediction, it must be wrapped into a `r ref("TaskUnsupervised")` for internal `r cran_pkg("mlr3")` reasons.
...
Thanks to the improved handling of factor variables in `r cran_pkg("terra")`, the raster image contains the correct labeled classes from the prediction right away.
...
```{r, include=FALSE}
#| fig-align: center
#| fig-alt: '`r cran_pkg("mlr3spatial")`'
...
For our first model, we choose an ordinary logistic regression (implemented in add-on package `r mlr_pkg("mlr3learners")`).
