...
A more general setting is cost-sensitive classification, in which costs caused by different kinds of errors are equal.
...
If a customer has good credit and returns the loan then InsureU makes a \$1K profit, but if they default then InsureU loses \$5K - note above this is shown as `-1` and `-5` as cost-sensitive classification is a minimisation problem where we assuming lower costs are associated with more positive outcomes.
...
This measure takes one argument, which is a matrix with row and column names corresponding to the class labels.

Continuing our insurance example (we have already named the matrix as required):
...
With our cost-sensitive measure we find that the logistic regression minimise the expected costs (and maximise expected profits) and the featureless learner performs the worst.
