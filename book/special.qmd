...
Below we construct a C-Means clustering learner with `prob` prediction type and 3 clusters, train it on the `r ref("USArrests")` datasets and then return the cluster assignments (`$assignments`) for each observation.
...
Two common measures are the within sum of squares (WSS) measure, `r ref("mlr_measures_clust.wss")`, and silhouette quality index (SQI), `r ref("mlr_measures_clust.silhouette")`.
...
SQI quantifies how well each point belongs to its assigned cluster versus neighboring clusters, where scores closer to `1` indicate well clustered and scores closer to `-1` indicate poorly clustered.
...
The very high WSS and middling SQI indicate that our clusters could do with a bit more work.
...
Continuing our running example, by plotting a PCA we see that our model has created cleanly separated clusters along the first two principal components, indicating our clusters are well-separated.
...
The average silhouette index is 0.5, which means observations that are scored below this line are considered to be poor estimations whereas observations above are well clustered, the SQI is just the average across all observations.
To visualise this, silhouette plots include a dotted line at 0.5 and a bar for the SQI of each data point coloured by cluster.
...
Finally, we conduct a small benchmark study using the `r ref("USArrests")` data and with a few integrated cluster learners:
...
The experiment shows that using the C-means algorithm with three centers has the best cluster cohesion (lowest within sum of squares). However, the featureless learner has the best cluster separation (lowest silhouette).
