...
The most popular method are partial dependence plots (PD) plots.
...
However, they cannot only convey linear effects but also more complex ones (similar to splines in generative additive models).
:::

::: {.callout-warning}
As for the permutation feature importance, feature effect methods suffer form the extrapolation issue if feature are correlated. 
...
### Shapley Values
Shapley values is originally an approach from cooperative game theory. Shapley values answer how the payout from a game can be fairly distributed among the players that form a team. This principle can be transferred to IML to receive *local* explanations for a single instance: The game is defined as the process of making a prediction for a single observations; the overall payout is the prediction of the single observation; the players are the features.
...
::: {.callout-warning}
The exact Shapley value computation is problematic for high-dimensional feature spaces.
