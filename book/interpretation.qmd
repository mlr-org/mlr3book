...
The `r ref_pkg("counterfactuals")` package is a subpackage of the `r ref_pkg("iml")` package it only encludes methods to generate counterfactual explanations, or short counterfactuals.
Counterfactuals explanations are a local model-agnostic interpretation technique that explain single predictions using statements in the form of "what if". 
An example for the `r ref("palmerpenguins::penguins")` data would be: "If the penguin has a bill length of 45 mm instead of 35 mm and a flipper length of 210 mm instead of 180 mm, the penguin would have been classified as Gentoo with a probability of > 80 \%".

Many methods were proposed in previous years to generate counterfactual explanations. That is why counterfactual explanations were outsourced into a separated package instead of integrating it into the `r ref_pkg("iml")` package. Currently, three methods are implemented in the R package but the R6-based interface makes it easy to add other counterfactual explanation methods in the future.
Since the package relies on the `r ref("iml::Predictor()")` class, it can explain any prediction model fitted with the `r ref_pkg("mlr3")` package as well as with other frameworks in `R` using a wrapper function.

### The What-If method

In the following, we focus on the simplest method among the three implemented ones: the What-If approach [@Wexler2019]. For a data point, whose prediction should be explained, the approach simply returns as a counterfactual the closest neighbor of a given data set with the desired prediction (e.g., Gentoo with a probability of > 80\%). For the other two implemented methods, the process to generate counterfactuals is very similar to the process for the What-If approach.
