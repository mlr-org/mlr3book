...
Appropriate surrogate models are, for example, decision trees or linear models whose tree structure or coefficients can be easily interpreted. The surrogate model is either trained on the same data as the black box model or on a new data set with the same distribution. For local surrogate models, the data is additionally weighted by the closeness to a specific point.
As response variable for the surrogate, not the true outcome (if any exists in the data set) but the predictions obtained from the black-box model are used.
