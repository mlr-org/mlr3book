...
By default, the plot method of `r ref("iml::TreeSurrogate")` extracts the decision rules found by the tree surrogate and visualizes the distribution of the predicted outcomes for each leaf node.
...
Since the surrogate model does not see or use the real outcomes of the underlying data used to train the black-box model (here, the random forest), general conclusions can not be drawn.
