...
### Feature Importance
Feature importance methods globally quantify the relevance of features w.r.t. prediction performance as a single number per feature.
The most popular method is the permutation feature importance: 
the importance of a feature is calculated as the increase in the model's prediction error after permuting the feature. 
The method requires a labeled dataset (for which both the feature and true outcome are given) and a trained model. 
...
Since such feature combinations are also very unlikely part of the training data, the model extrapolates in these areas. 
This distorts the interpretation of the feature importances. 
A better alternative is then, the conditional feature importance. 
:::


### Feature Effects
Feature effect methods visualize or quantify the marginal contribution of a feature of interest w.r.t. predictions. 
The most popular method are partial dependence (PD) plots.
Their building blocks are individual conditional expectation (ICE) values.
ICE values display how the prediction of a *single* observation changes when varying a feature of interest while all other features stay constant (ceteris paribus). 
...
ICE curves are a local interpretation method that only allow to assess a feature's effect on a prediction for a *single* observation. 
To receive a global effect estimate, the ICE values/curves of all observations in a given data set can be averaged. 
