...
A popular technique for assessing feature importance in a model-agnostic manner is the permutation feature importance. It was briefly introduced in the @sec-feat-importance section. More about this technique can be found in *Variable-importance Measures* chapter of [EMA book](https://ema.drwhy.ai/).
...
The bars start in loss (here 1 minus AUC) for the selected data and end in a loss for the data after the permutation of the selected variable. The more important the variable, the more the model will lose after its permutation.

#### Feature Effects

Once we know which variables are most important, we can use [Partial Dependence Plots](https://ema.drwhy.ai/partialDependenceProfiles.html) to understand its effect and show how the model, on average, changes with changes in selected variables.
...
From the figure above, we can read the interesting information that the random forest model has learned a non-monotonic relationship for the variables `amount` and `age.`
...
A popular technique for assessing the contributions of variables to model prediction is Break Down (see *Introduction to Break Down* chapter in [EMA book](https://ema.drwhy.ai/) for more information about this method).
...
Looking at the plots above, we can read that the biggest contributors to the final prediction were for Steve the variables `credit_history` and `status`.
