...
abstract: 
  ...
  Users more and more often want to know which features are important and how they influence the model's predictions. 
...
The increasing availability of data and frameworks to create models has allowed the widespread adoption of these solutions. 
However, high predictive performance of predictive models often comes at the cost of interpretability: 
The models are often too complex to be understood by humans such that questions like ''What are the most important features and how do they influence a prediction?'' cannot be directly answered. 
This lack of explanations hurts trust and creates barriers to adapt these models especially in critical areas with decision affecting human life, such as credit scoring or medical applications.

In this chapter, we focus on post-hoc interpretation methods from the field of interpretable machine learning.
Interpretation methods are valuable from multiple perspectives:

1. to gain global insights into a model, for example, to identify which features are most important. 
2. to understand and control individual predictions, for example, to identify how a given prediction
changes when changing the input.
3. to assess fairness, for example, by inspect whether the model adversely affects certain subpopulations. 
<!--TODO: reference to fairness chapter if there is one in the future --> 
4. to improve the model after flaws (in the data or model) were identified.

## Overview of Methods

In the following, we give a brief overview of the most important methods. 
More details on all details can be found in @Molnar2022.
In general, the following methods can be categorized into local and global methods: 
Global methods aim to explain the model behavior for the entire input space by considering all available observations, 
while local methods explain the model behavior for a single data instance.
If we want to understand how features affect the prediction over the *entire* population, 
we should use global methods; 
If we want to understand how feature affect the prediction of a *single* observation, 
we should use a local methods. 

### Feature Importance
Feature importance answers which features are most important for a prediction.
The importances of feature are displayed as single numbers per feature, such that the features can be ranked according to their importances.

One of the most popular methods is the permutation feature importance (PFI) (@Fisher2019pfi).
The PFI quantifies the importance of a feature w.r.t. the model performance. 
...
The worse the performance under permutation is in relation to the original model performance, the more important this feature is w.r.t. prediction performance.
<!-- TODO: Explain that we look here only at loss-based FI and that there also may be other notions of feature importance e.g. based on computing shapley values for each obs. and averaging their absolute values (SHAP feature importance)??? -->


### Feature Effects
Feature effects answers how a feature affects a model, i.e. how the predictions change 
when changing a feature.
There exist both local and global feature effect methods: 
local feature effect methods answer how a *single* prediction changes if a feature is changed, 
while global methods answer how a prediction changes *on average* if a feature is changed. 

The probably most popular local method is individual conditional expectation (ICE) curve (@Goldstein2015ice), 
the probably most popular global method is the partial dependence (PD) plot (@Friedman2001pdp). 
They are acutally pretty similar because multiple ICE cruves are the building blocks of PD plots.
...
Hence, for each observation, the values of the feature of interest is replaced by multiple other values (e.g., on an equidistant grid of the feature's value range) to inspect the changes of the model prediction.
Every ICE curve only allow to assess a feature's effect on the model prediction for a *single* observation. 
To receive an estimate for the global effect, the ICE curves of all observations in a given data set can be averaged.
...
However, PD plots cannot only convey linear effects but also more complex ones (similar to splines in generative additive models).
:::

###  Surrogate Models
Surrogate models approximates the (often) very complex machine learning model by an interpretable one.
Inspecting the interpretable surrogate model helps us to gain further insights into a model. 
Appropriate interpretable surrogate models are, for example, decision trees or linear models whose tree structure or coefficients can be easily interpreted. 

We differentiate between local surrogate models that approximate a model locally around a specific point and global surrogate models that approximate the model as a whole (@Ribeiro2016lime, @Molnar2022).
...
As target to train the surrogate model, the predictions obtained from the black-box model are used.
...
Shapley values were first proposed in cooperative game theory: They answer how the payout from a game can be fairly distributed among the players that form a team. This principle was transferred by @Trumbelj2013Shapley to IML: The game is defined as the process of making a prediction for a single observations; the overall payout is the prediction of the single observation; the players are the features.

<!-- To compute Shapley values, we need to inspect how the prediction changes if a feature values is present vs. when a feature value  -->
...
<!-- For inspecting the prediction changes, the feature values of the other features do not necessary have to stay constant (as it is demanded for ICE curves or PD plots) but they can also differ. This is necessary such that the interaction effects are also taken into account.  -->

Shapley values are often misinterpreted: a Shapley values does *not* display the difference of the predicted value after removing the feature from the model training; it is the contribution of a feature value to the difference between the actual prediction and the mean prediction, given the current set of features.
...
Such statements are valuable because we can then see which features affect a prediction and which strategies exist to obtain (a maybe more desirable) prediction (in the future).
...
More details can be found [here](https://christophm.github.io/interpretable-ml-book/counterfactual.html).

## Choice of the underlying dataset {#sec-dataset}

According to Section [Evaluation, Resampling and Benchmarking](#evaluation), performance evaluation should not be conducted on the training data but on hold-out/test data to receive unbiased estimates for the performance. 
...
This question is especially relevant when we use a performance-based IML method like the PFI compared to prediction-based methods like the other methods we saw above. 
When using training data instead of test data for computing PFI, the importance values for the features on which the model overfits can be overestimated.
<!-- TODO: reference missing!--> 

For prediction-based methods the differences in interpretation between test and train data are less pronounced. 
In the following, we focus on using hold-out/test data to assess model interpretation.\footnote{While espcially for small datasets cross-validation is recommended over hold-out spliting (see #evaluation), how to conduct cross-validation for the purpose of interpretation is an open question.} 
...
Next, we want to fit a random forest model to build a prediction model 
for a penguin's species using `r ref_pkg("mlr3")`.
Before that, we need to omit the 11 cases with missing values in the features. 
...
Next, we fit a learner to predict the probabilities for the different penguin specifies (by setting `predict_type = "prob"`). 
...
In the following, we will show how above's interpretation methods can be applied to our trained random forests. 
We utilize three packages:

-   `r ref_pkg("iml")` presented in @sec-iml, 
-   `r ref_pkg("counterfactuals")`, a subpackage of `iml` presented in @sec-counterfactuals which only considers counterfactual explanation methods, and
-   `r ref_pkg("DALEX")` presented in @sec-dalex.

`iml` and `DALEX` offer similar functionality, but they differ in design choices. 
`r ref_pkg("iml")` and `r ref_pkg("counterfactuals")` are based on the R6 class system and for this reason working with it is more similar in style to working with the `r ref_pkg("mlr3")` package. `r ref_pkg("DALEX")` is based on the S3 class system and is mainly focused on the ability to compare multiple different models on the same graph for comparison and on the explainable model analysis process.
...
One of the advantages of model-agnostic interpretation methods is that they can be applied to any machine learning model without any restriction to a specific model class.
...
Next, we inspect *how* bill length influences the penguin classification. Therefore, we compute feature effects using a partial dependence plot (PDP) and ICE curves. In general, PD plots should always be accompanied by ICE curves because showing the PD plot alone might obfuscate heterogeneous effects and interaction effects.. 
...
Again, we used the `$plot()` method to visualize the results. We see that when the bill length is smaller than roughly 45 mm, there is a high chance that the penguin is an Adelie.
...
For the probability of being a Chinstrap, the bill length has for some penguins no to a very small effect, for some penguins the effect is very large. This indicates that bill length interacts with other features. 
Unfortunately, the plot does not tell us exactly which ones. 
...
but it is much simpler if we only focus only on a small area in the feature space surrounding a specific point -- the point of interest. That is why, we want to focus on a local surrogate model next.

Therefore, we need to conduct the following steps: 
...
```
From the table we see that the three *locally* most influential features are the bill length, bill depth and the flipper length. 
...
Instead of a single counterfactual, we can also generate multiple counterfactuals with the multi-objective counterfactuals method of @Dandl2020 which is also implemented in the package. 
Compared to the WhatIf method, the counterfactuals generated by MOC are not equal to observations in a given dataset but are artifically generated. 
The generation of counterfactuals is based on a optimization problem that aims for counterfactuals that 
...
Calling the MOC method is very similar to calling the WhatIf method. Instead of a `WhatIfClassif` objective, 
we initialize a `MOCClassif` objective. 
For illustrative purpose it is enough to let the multi-objective optimization run for 30 generations. 
Furthermore, we set the epsilon parameter to 0 to penalize counterfactuals in the optimization process with predictions outside the desired range. 

<!-- ```{r interpretation-whatifmulti,echo=FALSE, message=FALSE, warning=FALSE} -->
<!-- library("counterfactuals") -->
<!-- set.seed(123L) -->
<!-- moc = MOCClassif$new(predictor, epsilon = 0) -->
<!-- ranger_cfexp_multi = moc$find_counterfactuals(steve,  -->
<!--   desired_class = "Gentoo", desired_prob = c(0.5, 1)) -->
<!-- ``` -->

<!-- ```{r interpretation-whatifmulti,eval=FALSE, message=FALSE, warning=FALSE} -->
<!-- library("counterfactuals") -->
<!-- set.seed(123L) -->
<!-- moc = MOCClassif$new(predictor, epsilon = 0) -->
<!-- ranger_cfexp_multi = moc$find_counterfactuals(steve,  -->
...
<!-- Since the multi-objective approach does not guarantee that all counterfactuals have the desired prediction, -->
<!-- we remove all counterfactuals with predictions not equal to the desired prediction via the `$subset_to_valid()` method. -->
<!-- ```{r interpretation-whatifmulti, message=FALSE, warning=FALSE} -->
<!-- ranger_cfexp_multi$subset_to_valid() -->
<!-- ranger_cfexp_multi -->
<!-- ``` -->
<!-- Overall we generate `r nrow(ranger_cfexp_multi$data)` counterfactuals.  -->
<!-- For a concise overview of the required feature changes, we can use the `plot_freq_of_feature_changes()` method.  -->
<!-- It visualizes the frequency of feature changes accross all returned counterfactuals.  -->
...
<!-- ``` -->
<!-- We see that bill length and flipper length are most often changed followed by the island.  -->
<!-- The`$parallel_plot()` method shows how the features need to be changed:  -->
...
::: {.callout-warning}
Because the results can be unreliable due to random permutations, multiple repetitions of the permutation should be conducted and the results should be averaged.
Furthermore, permuting single features despite correlation with others can lead to unrealistic combinations of the feature values.
Since such feature combinations are also very unlikely part of the training data, the model will likely extrapolate in these areas. 
This distorts the interpretation of the feature importances. 
A better alternative might be using the conditional feature importance. 
<!-- TODO: CITE pitfalls paper, christoph's conditional PDP / PFI paper or Hooker's stop permuting paper -->
:::


::: {.callout-warning}
Exact Shapley value computation is not feasible for high-dimensional feature spaces due to the enormous computational costs.
That is why in previous years more approximation methods were proposed, e.g., based on sampling procedures or kernel-based estimators (SHAP).
These methods still suffer from the extrapolation issue (similar to PFI and PD) in case of correlated features. 
For tree-based models, KernelSHAP offers a potential solution. 
More details can be found [here](https://christophm.github.io/interpretable-ml-book/shap.html).
:::

::: {.callout-warning}
As for the permutation feature importance, ICE curves and, consequently, PD plots suffer form the extrapolation issue if feature are correlated. 
A better alternative in this case is the the [Accumulated Local Effect (ALE) plot](https://christophm.github.io/interpretable-ml-book/ale.html).
:::

::: {.callout-note}
A practical challenge is that for one instance and model, usually multiple different, but equally good counterfactuals explanations can be found. This issue is also called the Rashomon effect.
How to handle this, e.g., whether to select one (danger of incomplete picture) or to report all (danger of overwhelming the user), is still unsolved.
:::
