...
#| fig-alt: Plot of the permutation feature importance values per feature of the credit data set.  The permutation process was repeated 5 times. On the plot points indicate the median and bars the 5 % and 95 % quantiles of the permutation feature importance over the 5 repetitions.
...
#| fig-cap: Distribution of the predicted outcomes for each leaf node identified by the tree surrogate. The first two leave node consist of applications with a balance of the checking account being positive (`status`is either `"0 <= ... < 200 DM"`, `"... >= 200 DM"` or `"salary for at least 1 year"`) and either a duration of less or equal than 36 months (first node), or more than 36 months (second node). The third and fourth node contain applicants that either have no checking account or a negative balance (`status`). For the third node the additional requirement was a durations of less or equal to 21 month, for the fourth node it was a duration of more than 21 months.
...
```{r iml-006}
#| fig-height: 3
#| fig-cap: Shapley values. Positive values indicate that a feature value has a positive effect on the predicted probability of being creditworthy, negative values indicate a negative effect. The actual prediction displays the prediction of the model for the observation we are interested in, the average prediction displays the average prediction over the given test data set. 
