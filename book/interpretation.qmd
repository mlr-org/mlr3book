...
The importances are displayed as single numbers per feature, such that the features can be ranked according to their importance.
...
The worse the performance under permutation is in relation to the original model performance, the more important this feature is w.r.t. the prediction performance.
...
There exist local and global feature effect methods: 
...
Every ICE curve provides a local explanation that only allows assessing a feature's effect on the model prediction for a *single* observation. 
...
As a target to train the surrogate model, the predictions obtained from the black-box model are used.
...
In the following, we focus on using hold-out/test data to assess model interpretation.\footnote{Cross-validation is recommended over hold-out splitting (see #evaluation), but how to conduct cross-validation for the purpose of interpretation is an open question.}
