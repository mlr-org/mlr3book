...
In principle, all generic frameworks for model interpretation are applicable on the models fitted with `r ref_pkg("mlr3")` by just extracting the fitted models from the `r ref("Learner")` objects.

However, two of the most popular frameworks,

* `r ref_pkg("iml")` in @sec-iml,
* `r ref_pkg("DALEX")` in @sec-dalex, and

additionally come with some convenience for `r ref_pkg("mlr3")`.
...
This chapter provides examples of how to use `r ref_pkg("iml")` with `r ref_pkg("mlr3")`.
...
To understand what `r ref_pkg("iml")` can offer, we start off with a thorough example.
...
The `r ref_pkg("DALEX")` package X-rays any predictive model and helps to explore, explain and visualize its behaviour.
...
In the following sections, we will present subsequent methods available in the `r ref_pkg("DALEX")` package based on a random forest model trained for football players worth prediction on the FIFA 20 data.
...
* In @sec-interpretability-data-fifa we introduce the FIFA 20 dataset and then in @sec-interpretability-train-ranger we train a random regression forest using the `r ref_pkg("ranger")` package.
* @sec-interpretability-architecture introduces general logic beyond `r ref_pkg("DALEX")` explainers.
...
After some basic data cleaning, the processed data for the top 5000 football players is available in the `r ref_pkg("DALEX")` package under the name `fifa`.
...
The `r ref_pkg("DALEX")` package works for any model regardless of its internal structure. Examples of how this package works are shown on a random forest model implemented in the `r ref_pkg("ranger")` package.

We use the `r ref_pkg("mlr3")` package to build a predictive model.
...
1. All functions in the `r ref_pkg("DALEX")` package can work for models with any structure.
  ...
  In general, such an adapter is created with `r ref("DALEX::explain.default()")` function, but for models created in the `r ref_pkg("mlr3")` package it is more convenient to use the `r ref("DALEXtra::explain_mlr3()")`.
...
Among them the `r ref("DALEXtra::explain_mlr3()")` function works for `r ref_pkg("mlr3")` models.
