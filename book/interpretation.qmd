...
In the following, we give a brief overview of the most important, model-agnostic methods.
Model-agnostic means that the the methods can be applied to any machine learning model without any restriction to a specific model class.
Furthermore, we distinguish between local and global methods: 
If we want to explain the model behavior over the *entire* population by taking all observations into account, we should use global methods; 
If we want to understand how the model behaves for single data instances (and the close neighborhood around them), we should use local methods. 
The following methods are also described in more detail in the introductory book for IML by @Molnar2022.

### Feature Importance

Feature importances help us to identify the features that are most important for a prediction.
The importances are displayed as single numbers per feature, such that the features can be ranked according to their importances.
...
### Feature Effects
Feature effects show how a feature affects a model, i.e. how the predictions change 
...
The probably most popular method is the partial dependence (PD) plot (@Goldstein2015ice). Its building blocks are individual conditional expectation (ICE) curves (@Friedman2001pdp). 
...
This question is especially relevant when we use a performance-based IML method like the PFI compared to prediction-based methods like ICE/PD, surrogate models, Shapley values and counterfactuals. 
...
In the following, we focus on using hold-out/test data to assess model interpretation.\footnote{Cross-validation is recommended over hold-out spliting (see #evaluation), but how to conduct cross-validation for the purpose of interpretation is an open question.}
<!-- TODO: is footnote shown properly?--> 
...
Next, we fit a learner to predict the probabilities for the penguin species (by setting `predict_type = "prob"`). 
...
`r ref_pkg("iml")` and `r ref_pkg("DALEX")` offer similar functionality, but they differ in design choices. 
...
```{r interpretation-mocfreq, message=FALSE, warning=FALSE, fig.height=4, out.width = '90%', fig.align='center'}
...
```{r interpretation-mocparallel, message=FALSE, warning=FALSE, fig.height=4, out.width = '90%', fig.align='center'}
...
```{r interpretation-moc2features,message=FALSE, warning=FALSE, fig.height=4, out.width = '90%', fig.align='center'}
...
```{r interpretation-mocsurface,message=FALSE, warning=FALSE, fig.height=4, out.width = '90%', fig.align='center'}
...
In this chapter, we learned how to gain post-hoc insights into a model trained with the `r ref_pkg("mlr3")` by using the most popular approaches from the field of interpretable machine learning.
The methods are all model-agnostic such that they do not depend on specific model classes.  
We utilized three different packages: the `r ref_pkg("iml")`, `r ref_pkg("counterfactuals")` and `r ref_pkg("DALEX")`. `iml`and `DALEX` offer a wide range of (partly) overlapping methods, while the `counterfactuals` package focuses solely on counterfactual methods.
We demonstrated on the `r ref("palmerpenguins::penguins")` data set how these packages offer an  indepth analysis of a random forest model fitted with `r ref_pkg("mlr3")`.
...
Alternative methods can help in these cases: conditional feature importance instead of PFI, accumulated local effect plots instead of PD plots and the KernelSHAP method instead of Shapley values (@Molnar2022pitfalls).
...
This phenomenon is also called the Rashomon effect (@Breimann2001).
Differing hyperparameters can be one reason, for example, local surrogate models react very sensitive to changes in the underlying weighting scheme of observations. 
...
<!-- TODO: Cite rashomon effect --> 

The `r ref("palmerpenguins::penguins")` is only a low dimensional dataset with a limited number of observations. 
Applying interpretation methods off-the-shelf to higher dimensional datasets is often not feasible due to the enormous computational costs and the high-dimensional IML output that is overwhelming for users.
That is why in previous years more efficient methods were proposed, e.g., Shapley value computations based on kernel-based estimators (SHAP) and specific methods to cope with 
high-dimensional IML outputs like grouped feature importance.
<!--TODO: Cite grouped feature importance paper --> 
