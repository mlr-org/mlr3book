...
```{r iml-localsurrogate, message=FALSE, warning=FALSE, fig.cap='Local surrogate model implemented in `iml::LocalModel` for the penguin classification task and random forest model.', fig.align = 'center'}
...
::: {.callout-tip}
Local surrogate models should be interpreted with caution: how the observations are weighted and which data is used has a huge effect on the performance.
:::
...
```{r iml-006, message=FALSE, warning=FALSE, fig.cap='Plot of the results from Shapley. $\\phi$ gives the increase or decrease in probability given the values on the vertical axis',  fig.align='center'}
x = penguins[which(names(penguins) != "species")]
