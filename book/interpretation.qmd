...
In this chapter, we focus on post-hoc interpretation methods from the field of interpretable machine learning that are applied to an already trained model.
...
In general, methods can be categorized into local and global methods:
Global methods aim to explain the model behavior for the entire input space by 
considering all available observations, while local methods explain the 
model behavior for a single data instance.

### Feature Importance
Feature importance methods globally quantify the relevance of features as a single number per feature often w.r.t. the model performance.
One of the most popular methods is the permutation feature importance (PFI).
The PFI quantifies the importance of a feature by calculating the decrease in model performance (or, equivalently the increase in the model's prediction error) after permuting a feature of interest. 
Its intuition is simple: if a feature is not important, permuting that feature should not affect the model performance.
...
More details can be found [here](https://christophm.github.io/interpretable-ml-book/feature-importance.html#feature-importance).
...
### Feature Effects
Feature effect methods visualize or quantify the marginal contribution of a feature of interest w.r.t. model predictions. 
The probably most popular method is the partial dependence (PD) plot.
Its building blocks are individual conditional expectation (ICE) curves.
...
Every ICE curve provides a local explanation that only allow to assess a feature's effect on the model prediction for a *single* observation. 
To receive an estimate for the global marginal effect, the ICE curves of all observations in a given data set can be averaged.
...
###  Surrogate Models
Surrogate models approximate the (often) very complex machine learning model by an interpretable one. 
We differentiate between local surrogate models that try to approximate a model locally around a specific point or global surrogate models that approximates the model as a whole.
...
Appropriate surrogate models are, for example, decision trees or linear models whose tree structure or coefficients can be easily interpreted. 
...
<!-- The surrogate model is trained with the predictions of the black box model as the target, using either the same data with which the black box model was trained or new data. -->
...
### Shapley Values
Shapley values are an approach from cooperative game theory. They answer how the payout from a game can be fairly distributed among the players that form a team. This principle can be transferred to IML to receive *local* explanations for a single instance: The game is defined as the process of making a prediction for a single observations; the overall payout is the prediction of the single observation; the players are the features.

The Shapley value for a single feature shows how a feature value contributed to the *difference* between the prediction of an observation of interest and the average prediction (over the whole dataset).
More details can be found [here](https://christophm.github.io/interpretable-ml-book/shapley.html).
...
### Counterfactuals
Counterfactual explanations are a local interpretation method. They explain the prediction of single observations by presenting which minimal changes in the point are sufficient to receive a different, desired outcome. This allows for statements such as:
''If you had the following feature values instead of the present ones, you would have received the desired outcome.''
...
If not omitted, there will be an error while training the random forest model because the learner cannot handle missing values. 
...
Furthermore, we need to decide whether we want to train the model on the whole data or only on a subset of the data -- the training data.
According to Section [Evaluation, Resampling and Benchmarking](#evaluation), the latter is recommended. Then, performance assessment can be conducted on unseen test data instead of training data to receive unbiased estimates for the performance. An overfitted model may perfectly fit the data on which it was trained, but may not generalize well to new data.

This leads us to the question whether model interpretation should be based on training or test data. 
...
For PFI, using test instead of training data is recommended.
When using training data instead of test data, the feature importance could be much higher for the features on which the model overfits.
Consequently, if we are interested in which features help the model to generalize, we have to apply PFI on test data.
For prediction-based methods the differences in interpretation between test and train data are less pronounced and more research might be required.
In the following, we focus on using hold-out/test data to assess model interpretation. 
We leave it to interested readers as an exercise to compare the results when using training data. 
...
As explained in Section [Learners](#learners), specific learners can be queried with `r ref("mlr_learners")`. In Section [Train/Predict](#train-predict) it is recommended for some classifiers to use the `predict_type` as `prob` instead of directly predicting a label. This is what is done in this example. 
...
Depending on the insights we want to get from the interpretation method or the question we want to answer with it, the data set can be the training data or test data not used for training the model to be analyzed (see  @sec-penguin-task for details). 
