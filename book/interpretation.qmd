...
As a running example throughout this chapter, we will consider a random forest fit on half the features in the `german_credit` task:
...
learner = lrn("classif.ranger", predict_type = "prob", seed = 123)
learner$train(task, row_ids = split$train)
learner$predict(task, row_ids = split$test)
learner$predict(task, row_ids = split$test)$score(msrs(c("classif.ce", "classif.bbrier")))
```
...
#| fig-alt: "x-axis says 'Feature Importance (loss: ce)' and ranges from just below 1.00 to just above 1.20. y-axis lists the features in the text. Plot shows 10 error bars with solid black circles in the middle and horizontal black lines on each row. Top row is 'status' with a CE of around 1.16 (1.15-1.21)."
...
For example, we could quantify the number of matching predictions and measure the accuracy of the surrogate in predicting the predictions of the black-box random forest:
...
This shows an accuracy of around `r round(mean(pred_rf == pred_surrogate$.class)*100)`% in predictions from the surrogate compared to the black-box model, which is good enough for us to now use our surrogate for further interpretation, for example by plotting the splits in the terminal node:

```{r iml-globalsurrogate-plot,message=FALSE}
#| fig-cap: Distribution of the predicted outcomes for each terminal node identified by the tree surrogate. The top two nodes consist of applications with a positive balance in the account (`status`is either `"0 <= ... < 200 DM"`, `"... >= 200 DM"` or `"salary for at least 1 year"`) and either a duration of less or equal than 30 months (top left), or more than 30 months (top right). The bottom nodes contain applicants that either have no checking account or a negative balance (`status`) and either a duration of less than or equal to 24 months (bottom left) or more than 24 months (bottom right).
#| fig-alt: Four barplots with 'count' on the y-axis and 'class' on the x-axis. Top left shows only 'good' credit predictions. Top right shows about 25 'good' predictions and almost no 'bad' credit predictions. Bottom left shows 100 'good' predictions and about 40 'bad' predictions. Bottom right shows about 27 'bad' predictions and around 10 'good' ones.
#| label: fig-iml-surro
treesurrogate = TreeSurrogate$new(predictor, maxdepth = 2L)
treesurrogate$plot()
```
...
#| fig-cap: Effect size of the two most important features (status and duration). Effect of status on the predicted value is positive, whereas duration is negative. Plot title indicates good agreement between the actual and surrogate prediction.
#| fig-alt: "x-axis says 'effect' and ranges from -0.1 to 0.2. Plot shows two horizontal bars, top is 'status' and ranges from 0 to 0.2, bottom is 'duration' and ranges from -0.1 to 0. Title says 'Actual prediction: 0.78, LocalModel prediction: 0.79'."
...
The output tell us that the two most influential features for Steve's predictions were 'duration' and 'status', and that 'status' increased the predictive value whereas 'duration' decreased it.
We can also see good agreement between the global and local predictions.

This implementation is very closely related Local Interpretable Model-agnostic Explanations (LIME) [@Ribeiro2016lime]\index{LIME}, the differences are outlined in the documentation of `r ref("iml::LocalModel")`.
...
```{r iml-006}
#| fig-height: 3
#| fig-cap: Shapley values for Steve. The actual prediction (0.78) displays the prediction of the model for the observation we are interested in, the average prediction (0.70) displays the average prediction over the given test data set. Each horizontal bar is the Shapley value (phi) for the given feature.
#| fig-alt: 10 bar plots of Shapley values, one for each feature. x-axis says 'phi' and ranges from -0.05 to 0.05. The strongest positive contribution is from 'status', 'other_debtors' has the smallest value, and 'purpose' is the strongest negative contribution.
...
The status variable has the most positive effect on the probability of being creditworthy, with an increase in the predicted probability of over 5%.
In contrast, the 'purpose' variable leads to a decrease in predicted probability of more than 5%.
...
The simplest method is the `r index('What-If', aside = TRUE)` approach [@Wexler2019] where, for a given prediction to explain, the counterfactual is the closest data point in the data set with the desired prediction.
Another method is the `r index('multi-objective counterfactuals', aside = TRUE)` method (MOC) introduced by @Dandl2020, which can generate multiple counterfactuals that are not equal to observations in a given dataset but are artificially generated data points.
...
### What-If Method
<!-- TODO: ADD DESCRIPTION ABOUT INFINITE CFS AND CHOICE IS ARBITRARY -->
...
We can use the What-If method to understand how the features need to change for this predicted probability to increase to 80%.
We initialize a `r ref("counterfactuals::WhatIfClassif")`  object with our `Predictor` and state that we only want to find one counterfactual (`n_counterfactuals = 1L`).
The `$find_counterfactuals()` method generates a counterfactual of class  `r ref("counterfactuals::Counterfactuals")`, which assumes a 0.8 threshold for landing in the `good` class.

```{r interpretation-whatif}
library(counterfactuals)
whatif = WhatIfClassif$new(predictor, n_counterfactuals = 1L)
cfe = whatif$find_counterfactuals(steve,
  desired_class = "good", desired_prob = c(0.85, 1))
...
For example we can see that Steve would have to be 14 years younger and the duration of credit would have to be reduced by 9 months.
...
cfe_multi = moc$find_counterfactuals(steve,
  desired_class = "good", desired_prob = c(0.90, 1))
```
...
#| fig-alt: Barplots of the relative frequency of feature changes of the counterfactuals found by MOC. x-axis says 'relative frequency' and ranges from 0 to just over 0.6. Changed features were 'purpose' (in 60% of the counterfactuals), 'employment_duration' (20%), 'savings' (10%) and 'credit_history' (10%).
#| label: fig-cf-mocfreq
cfe_multi$plot_freq_of_feature_changes()
```

We can see that only 'purpose', 'employment_duration', 'savings', and 'credit_history' were changed in the counterfactuals, with 'purpose' being changed most frequently.
...
#| fig-height: 3.5
#| fig-cap: Two-dimensional surface plot for the 'purpose' and 'employment_duration' variables, higher predictions are darker. The colors and contour lines indicate the predicted value of the model when 'purpose' and 'employment_duration' differ while all other features are set to the true (Steve's) values. The white point displays the true prediction (Steve), and the black points are the counterfactuals that only propose changes in the two features.
#| fig-alt: Surface plot that is primarily light blue in top left and gets darker as it increases towards bottom right. y-axis is the employement duration variable and x-axis is the 'purpose' variable. There is a white dot at 'employment_duration = 1 <= ... < 4 yrs' and 'purpose = retraining'. Two black dots are in a straight line above the white dot and five black dots are in a roughly straight line to the left of the white dot.
#| label: fig-cf-mocsurface

cfe_multi$plot_surface(feature_names = c("purpose", "employment_duration")) +
...
#| out-width: 90%
#| label: fig-dalex-pdp
#| fig-cap: Graphical summary of the model's partial dependence profile for three selected variables (age, amount, duration). The jumps in the response function is characteristic of a random forest model.
#| fig-alt: Left plot is PD plot of 'age' against 'average prediction', between ages 20-40 the prediction dips from 0.4 to 0.3 then increases up to 0.35 from 40-60. Middle plot is PD plot of 'amount', between amounts 0-5000 the prediction dips from 0.4 to 0.2 then increases up to above 0.5 to the end. Right plot is PD plot of 'duration', between duration 0-40 the prediction linearly increases from 0.2 to 0.45 then stays flat.
...
From the figure above, we can see that the random forest model has learned a non-monotonic relationship for the features `amount` and `age.`
...
#| fig-alt: On the x-axis are numbers from 0.0 to 0.4, and y-axis is variables from the dataset. There are five bars in red with negative number labels and thee bars in green with positive number labels. A dashed vertical lines runs through x=0.3 and there is a violet bar with text '0.223'.
#| label: fig-dalex-breakdown

plot(predict_parts(ranger_exp, new_observation = steve))
```
...
#| fig-alt: x-axis says 'contribution' and ranges from -0.1 to 0.05, y-axis is feature names. Plots show five red bars with negative contributions and five green bars making positive contributions. Longest bar is for 'status' and shortest for 'other_debtors'.
...
#| fig-alt: Plots are very similar to the PD plots from earlier except with 10 lines plotted. y-axis is 'prediction' ranging from 0.1 to 0.5. Left plot is titled 'age' and shows a dip in prediction value then increase between 20,30,70. Middle plot is titled 'amount' and shows a fairly linear increase in prediction value from 0.2 to 0.5. Right plot is titled 'duration' and show an increase in prediction value from 0.2-0.35 and then is flat. There is fairly good agreement between plotted curves.

plot(predict_profile(ranger_exp,  credit_x[240:250, ]))
```
...
We demonstrated on the `german_credit` task how these packages offer an in-depth analysis of a random forest model fitted with `r ref_pkg("mlr3")`.
