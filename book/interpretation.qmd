...
First, we analyze which features were the most important ones according to the PFI for the fitted random forest model to classify creditworthiness.
...
::: {.callout-tip}
By default, `r ref("iml::FeatureImp")` uses the ratio of the model performance before and after permutation as importance value and repeats the permutation 5 times. 
...
That is, if we permute the `r importance$results$feature[1]` column in the data, the classification error of our model increases by a factor of around `r importance$results$importance[1]`. 
In each repetition, the change in the classification error is computed and the median of these importance values is computed.
...
If the prediction of the local model and the prediction of the black box model are far away, you can change the value of `gower.power` and e.g. increase the number of non-zero coefficients `k` for a more accurate local model.
:::
...
2) are close to the observation of interest (here, the credit applicant we called Steve),
...
An example for a counterfactual for the `r ref("rchallenge::german")` data would be: "If the credit applicant would have asked for a lower credit amount, he would have been classified being of good credit risk with a probability of \> 50 %" (@fig-counterfactuals-penguins). By revealing the feature changes that alter a decision, the counterfactuals reveal which features are the key drivers for a decision.
...
ranger_exp = DALEX::explain(learner,
  data = credit_x,
  y = as.numeric(credit_y$credit_risk == "good"),
