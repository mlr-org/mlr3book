...
The `r ref_pkg("counterfactuals")` package is a subpackage of the `r ref_pkg("iml")` package for methods that generate counterfactual explanations, or short counterfactuals. Counterfactuals explanations are a local model-agnostic interpretation technique that explain single predictions using statements in the form of "what if". An example for the `r ref("palmerpenguins::penguins")` data would be: "If the penguin has a bill length of 45 mm instead of 35 mm and a flipper length of 210 mm instead of 180 mm, the penguin would have been classified as Gentoo with a probability of \> 80 %".
Counterfactuals reveal which features are the main driver for a given decision and how the decision can 
be altered.
Since the generated counterfactuals give insights into the prediction of a single data point (and maybe its close neighborhood), 
they are a local interpretation tool.

Many methods were proposed in previous years to generate counterfactual explanations. That is why counterfactual explanations were outsourced into a separated package instead of integrating these methods into the `r ref_pkg("iml")` package. Currently, three methods are implemented in the R package but the R6-based interface makes it easy to add other counterfactual explanation methods in the future.

### The What-If method

In the following, we focus on the simplest method among the three implemented ones: the What-If approach [@Wexler2019]. For a data point, whose prediction should be explained, the returned counterfactual is the closest neighbor of a given data set with the desired prediction (e.g., as in the example above, Gentoo with a probability of \> 80%).
Specifically, we want to generate counterfactuals for Steve, the first penguin in the dataset.

```{r interpretation-steve, message=FALSE, warning=FALSE}
steve = penguins[1,]
```

Since the `r ref_pkg("counterfactuals")` packages uses `r ref("iml::Predictor()")` as a model wrapper, it can explain any prediction model fitted with the `r ref_pkg("mlr3")` package including the random forest `model` we trained above.

```{r interpretation-predictsteve, message=FALSE, warning=FALSE}
model$predict(steve)
```
The random forest predicts that Steve is with 99 \% an Adelie.
