...
We initialize a `r ref("iml::FeatureImp")` object with the model and the classification error (`"ce"`) as the performance measure and visualize the results with the `$plot()` method.
...
::: {.callout-tip}
By default, `r ref("iml::FeatureImp")` uses the ratio of the model performance before and after permutation as an importance value and repeats the permutation 5 times.
...
```{r iml-globalsurrogate-tree, message=FALSE, warning=FALSE, fig.height=7, fig.align='center'}
...
```{r steve2, message=FALSE, warning=FALSE, asis='results'}
...
#| fig-cap: Illustration of a counterfactual explanation. The brown dot displays a counterfactual for a given point (blue dot) which proposed decreasing the credit amount and duration and flipper such that the prediction changes from bad to good.
...
To assess whether the proposed features changes are minimal, we can visualize the counterfactuals on a 2-dim ICE plot, also called a surface plot.
...
```{r interpretation-mocsurface,message=FALSE, warning=FALSE, fig.height=3.5, out.width = '100%', fig.align='center'}
...
The `r ref_pkg("DALEX")` [@Biecek2018] package implements a similar set of methods as the `r ref_pkg("iml")` package presented above, but the architecture of `r ref_pkg("DALEX")` is oriented towards not only analysis of a single model but also comparing different models with each other. The logic behind working with this package, assumes that the process of exploring models is an iterative process, and in successive iterations we want to compare different perspectives with each other, including perspectives presented/learned by different models. This logic is commonly referred as the Rashomon perspective, first described in The Two Cultures paper [@Breiman2001] and more extensively developed and formalized as interactive explanatory model analysis [@Baniecki2023].

You can use the `r ref_pkg("DALEX")` package with any model built with the `r ref_pkg("mlr3")` package as well as with other frameworks in `R`. The counterpart in `Python` is the library `dalex` [@Baniecki2021].
...
The global model analysis aims to understand how a model behaves on average on a set of observations. In the `r ref_pkg("DALEX")` package, functions for global analysis have names starting with the prefix `model_`.

#### Model Performance

As shown in @fig-dalex-fig-plot-01, the process starts by evaluating the performance of a model. This can be done with a variety of tools, in the `r ref_pkg("DALEX")` package the default is to use the `r ref("DALEX::model_performance")` function. Since the `explain` function checks what type of task is being analysed, it can select the appropriate performance measures for it. In our illustration, we have a binary classification model, so measures such as precision, recall, f1, accuracy or auc are calculated in the following snippet. 
...
And plot it with the `r ref_pkg("ggplot2")` package.

```{r interpretation-020b, message=FALSE, warning=FALSE, fig.width=6, fig.height=5, out.width = '60%', fig.align='center'}
library("ggplot2")
old_theme = set_theme_dalex("ema") 
plot(perf_credit, geom = "roc")
```
::: {.callout-tip}
Different visuall summary may be specified with the `geom` argument. In credit risk, the LIFT curve is also a popular graphical summary.
:::
...
The process of model analysis usually begins with understanding which features are the most important. A popular technique for this is the permutation feature importance introduced in the @sec-feat-importance section. More about this technique can be found in *Variable-importance Measures* chapter of [EMA book](https://ema.drwhy.ai/).
...
The bars start in loss (here 1 minus AUC) for the selected data and end in a loss for the data after the permutation of the selected feature. The more important the feature, the more the model will lose after its permutation.

::: {.callout-tip}
The `type` argument in the `model_parts` function allows you to specify how the importance of the features is to be calculated, at by the difference of the loss functions (`type = "difference"`) or by the quotient (`type = "ratio"`), or without any transformation (`type = "raw"`).
:::

#### Feature Effects

Once we know which features are most important, we can use [Feature Effects Plots](https://ema.drwhy.ai/partialDependenceProfiles.html) to understand its effect and show how the model, on average, changes with changes in selected features. The `r ref("DALEX::model_profile()")` function calculates effects and by default it calculates the partial dependence profiles. 

```{r interpretation-024, message=FALSE, warning=FALSE, fig.width=8, fig.height=5, out.width = '90%', fig.align='center'}
...
We will carry out the following examples using sample credit application, labeled as Steve.
...
As shown in Figure @fig-dalex-fig-plot-01, the local analysis starts with the calculation of a model prediction.  It turns out that for the selected credit application, the chances are quite high that it will not be paid.
...
A popular technique for assessing the contributions of features to model prediction is Break Down (see *Introduction to Break Down* chapter in [EMA book](https://ema.drwhy.ai/) for more information about this method).
...
```{r interpretation-027, message=FALSE, warning=FALSE, fig.width=8, fig.height=4.5, out.width = '90%', fig.align='center'}
...
By far the most popular technique for local model exploration [@Holzinger2022] is Shapley values and the most popular algorithm for estimating these values is the SHAP algorithm. Find a detailed description of the method and algorithm in the chapter *SHapley Additive exPlanations (SHAP)* of [EMA book](https://ema.drwhy.ai/).
...
::: {.callout-tip}
Shapley values can take a long time to compute. This process can be sped up at the expense of accuracy. The parameters `B` and `N` are responsible for this.
:::

#### Ceteris Paribus Effects

In the previous section, we have introduced a global explanation -- Partial Dependence plots. Ceteris Paribus plots are the local version of that explanation. Read more about this technique in the chapter *Ceteris Paribus* of [EMA book](https://ema.drwhy.ai/). They show the response of a model when only one feature is changed while others stay unchanged.
...
```{r interpretation-029, message=FALSE, warning=FALSE, fig.width=8, fig.height=5, out.width = '90%', fig.align='center'}
ranger_ceteris = predict_profile(ranger_exp, steve)
plot(ranger_ceteris) 
```

Blue dot stands for the prediction for Steve. 

::: {.callout-tip}
Note that Ceteris Paribus profiles are also known as Individual Conditional Expectations (ICE).
:::
...
We demonstrated on the `r ref("rchallenge::german")` data set how these packages offer an indepth analysis of a random forest model fitted with `r ref_pkg("mlr3")`. In the following, we show some limitations of the presented methods.
...
1.  Prepare a `mlr3` regression task for `fifa` data. Select only features describing the age and skills of footballers. Train any predictive model for this task, e.g. `regr.ranger`.

2.  Use the permutation importance method to calculate feature importance ranking. Which feature is the most important? Is it surprising?

3.  Use the Partial Dependence profile to draw the global behavior of the model for this feature. Is it aligned with your expectations?
...
6.  For the selected footballer, calculate the Ceteris Paribus / Individual Conditional Expectatons profiles to draw the local behaviour of the model for this feature. Is it different from the global behaviour?
