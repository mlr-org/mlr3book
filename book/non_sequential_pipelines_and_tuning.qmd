...
tuned_knn = auto_tuner(tnr("random_search"),
  lrn("classif.kknn", k = to_tune(1, 32)), rsmp("holdout"), term_evals = 10)
...
```{r}
tuned_pipe = auto_tuner(tnr("random_search"), pipe, rsmp("holdout"), term_evals = 10)
untuned_pipe = po("pca") %>>% lrn("classif.kknn")
design = benchmark_grid(tsk("sonar"), c(tuned_pipe, untuned_pipe), rsmp("cv", folds = 5))
...
graph_learner = graph %>>%
  ppl("branch", lrns(c("classif.rpart", "classif.kknn")))
fig <- magick::image_graph(width = 2000, height = 1000, res = 100, pointsize = 16)
graph_learner$plot()
invisible(dev.off())
magick::image_trim(fig)
```
...
In @sec-hyperband-tuner we learnt about the Hyperband tuner and how it can make use of `r index('fidelity parameters')` to efficiently tune learners.
...
As we saw earlier, `po("subsample")` samples the data and passes the subset to the next `PipeOp` in a `Graph`, the `frac` parameter controls how much data to sample.
In this example we will optimize the SVM hyperparameters, `cost` and `gamma`, on the `sonar` dataset:
