...
* `ppl("robustify")`: Perform common preprocessing steps to make any `Task` compatible with a given `Learner`.
  ...
  This pipeline is returned to in more detail in @sec-prepro-robustify.
...
* `ppl("greplicate", graph, n)`: Create a `Graph` that replicates `graph` (which can also be a single `PipeOp`) `n` times.
  The pipeline avoids ID clashes by adding a suffix to each `PipeOp`, we will see this pipeline in use in @sec-pipelines-bagging.
