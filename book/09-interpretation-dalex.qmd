...
#`r mlr3book::cran_pkg("DALEX")`{#dalex}
...
The `r mlr3book::cran_pkg("DALEX")` package X-rays any predictive model and helps to explore, explain and visualize its behaviour.
...
In the following sections, we will present subsequent methods available in the `r mlr3book::cran_pkg("DALEX")` package based on a random forest model trained for football players worth prediction on the FIFA 20 data.
...
* In @sec-interpretability-data-fifa we introduce the FIFA 20 dataset and then in section \@ref(interpretability-train-ranger) we train a random regression forest using the `r mlr3book::cran_pkg("ranger")` package.
* @sec-interpretability-architecture introduces general logic beyond `r mlr3book::cran_pkg("DALEX")` explainers.
...
After some basic data cleaning, the processed data for the top 5000 football players is available in the `r mlr3book::cran_pkg("DALEX")` package under the name `fifa`.
...
The `r mlr3book::cran_pkg("DALEX")` package works for any model regardless of its internal structure. Examples of how this package works are shown on a random forest model implemented in the `r mlr3book::cran_pkg("ranger")` package.
...
Working with explanations in the `r mlr3book::cran_pkg("DALEX")` package always consists of three steps schematically shown in the pipe below.
...
1. All functions in the `r mlr3book::cran_pkg("DALEX")` package can work for models with any structure.
  ...
  In general, such an adapter is created with `r ref("DALEX::explain.default()")` function, but for models created in the `r mlr3book::mlr_pkg("mlr3")` package it is more convenient to use the `r ref("DALEXtra::explain_mlr3()")`.
...
1. Explanations can be visualized with the generic function `plot` or summarised with the generic function `"print()"`.
  ...
  The `plot` function creates graphs using the `r mlr3book::cran_pkg("ggplot2")` package, so they can be easily modified with usual `r mlr3book::cran_pkg("ggplot2")` decorators.
...
In the `r mlr3book::cran_pkg("DALEXtra")` package there generic versions for the most common ML frameworks.
Among them the `r ref("DALEXtra::explain_mlr3()")` function works for `r mlr3book::mlr_pkg("mlr3")` models.
