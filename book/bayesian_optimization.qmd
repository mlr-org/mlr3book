...
However, if the overhead of the evaluation of the black-box function becomes large (e.g., as in HPO), sample efficiency of an optimizer becomes highly relevant.
Bayesian Optimization (BO) describes a class of sample efficient iterative black-box optimization algorithms that make use of a so-called surrogate trained on observed data modeling the black-box function.
Based on this surrogate model and a so-called acquisition function quantifying the attractiveness of each point in the search space, the next candidate point is obtained that is to be evaluated next.
...
Our running example will be to optimize the following sinusoidal function which is characterized by two local minima and one global miminum:
...
We can proceed to visualize the sinusoidal function:
...
#| fig-cap: Contour plot of the sinusoidal function.
#| fig-alt: FIXME
library(ggplot2)
...
By wrapping the objective in an `r ref("OptimInstanceSingleCrit")` we can proceed to optimize the sinusoidal function, e.g., via a simple random search.
...
Note that we speak of BO flavors, as BO is highly modular, i.e., users can choose different types of surrogate models, acquisition functions and acquisition function optimizer freely.
...
#| fig-cap: Comparing different samplers for constructing an initial design of 9 points on a domain of two numeric variables ranging from 0 to 1. Dotted horizontal and vertical lines parition the domain into equally sized bins. Histograms on the top and right visualize the marginal distribution of the generated sample.
#| fig-alt: FIXME
library(ggplot2)
library(ggExtra)
library(gridExtra)
...
A grid design will always result in points being marginally equidistant from their nearest neighbor and does not cover the domain well.
...
A Sobol design has a similar goal in mind but must not guarantee this for a small number of samples and low dimensions.
...
In sequential single-objective black-box optimization for example, the Efficient Global Optimization (EGO) algorithm [@jones_1998)] can be considered the reference algorithm.
...
1. `surrogate$update()`: update the surrogate model
...
```{r}
surrogate = srlrn(lrn("regr.km"), archive = instance$archive)
```
...
The following figures visualize the mean and variance prediction (@fig-bayesian-optimization-mean-variance) of the surrogate in lightblue.
...
When working interactivelythe `$surrogate` on which the acquisition function operates on must be specified:
...
@fig-bayesian-optimization-ei shows that the Expected Improvement is high in regions where the mean prediction is low but the variance prediction suggests uncertainty:
...
```{r}
surrogate = srlrn(lrn("regr.km", covtype = "matern5_2", optim.method = "BFGS"))
acq_function = acqf("ei")
acq_optimizer = acqo(opt("nloptr", algorithm = "NLOPT_GN_ORIG_DIRECT"),
  terminator = trm("stagnation", iters = 100, threshold = 1e-5))
optimizer = opt("mbo",
...
Visualizing the sequential decision making process of the BO algorithm (i.e., which candidate to evaluate next) shows that focus is given more and more to regions around the (as we know) global optimum (@fig-bayesian-optimization-sampling).
...
#| fig-cap: Sampling trajectory of the BO algorithm. Points that have been evaluated in later stages (i.e., batches) of the optimization process are coloured in a lighter red.
#| fig-alt: FIXME
ggplot() +
  geom_line(aes(x = x, y = y), data = xydt) +
  geom_point(aes(x = x, y = y, colour = batch_nr), size = 2, data = instance$archive$data) +
...
g = ggplot() +
  geom_line(aes(x = nr_eval, y = mean_y_min, colour = method), data = mean_results) +
  geom_ribbon(aes(x = nr_eval, min = mean_y_min - se_y_min, max = mean_y_min + se_y_min, fill = method), colour = NA, alpha = 0.1, data = mean_results) +
...
An alternative approach to multi-obective BO would be to model each objective function via a separate surrogate model and finding the next candidate for evaluation via a so-called aggregating acquistion function.
...
Using SMS-EGO for the multi-objective HPO problem above looks like the following:
...
```{r}
surrogate = srlrnc(list(lrn("regr.km", covtype = "matern5_2", optim.method = "BFGS"), lrn("regr.km", covtype = "matern5_2", optim.method = "BFGS")))
...
In `r bbotk`, noisyness of an objective function can be indicated via the `$properties` field of the objective.
...
`r mlr3mbo` allows for several ways how noisyness of objectives can be respected during BO:

1. A surrogate can be used that can model noisyness of observations
2. An acquisition function can be used that properly respects noisyness
3. The final best point(s) after optimization (i.e., the `$result` field of the instance) can be choosen in a way to reflect noisyness
...
An example for an acquisition fucntion that properly respects noisyness of observations is given by the Augmented Expected Improvement [@huang_2012] which essentially rescales the Expected Improvement taking measurement error into account:

```{r, output=FALSE}
acqf("aei")
```

Finally, `r mlr3mbo` allows for explicitly specifying how the final result after optimization is assigned to the instance via so-called result assigners.
Examples are givenn by.

## Defaults and Safety Nets
...
1. Minimize the 2D Rastrigin function ($f(\mathbf{x})=10 D+\sum_{i=1}^D\left[x_i^2-10 \cos \left(2 \pi x_i\right)\right]$) via BO (standard sequential single-objective BO via `bayesopt_ego`) using the lower confidence bound with `lambda = 1` as acquisition function and `"NLOPT_GN_ORIG_DIRECT"` via `nloptr` as acquisition function optimizer (similarly as above).
Use either a Gaussian Process with Matérn 3/2 kernel (`regr.km`, similarly as above) or a random forest (`regr.ranger`) as surrogate model and compare the anytime performance of these two BO algorithms.
