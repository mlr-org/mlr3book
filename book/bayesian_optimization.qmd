...
Moreover, Hyperparameter Optimization (HPO, see @sec-optimization) is a prime example for black-box optimization.
...
Often, using BO results in very good optimization performance, especially if the overhead of the black-box evaluation (e.g., as in HPO) becomes large and optimization budget is tight.
Therefore this chapter is of general relevance to users concerned with sample efficient black-box optimization and HPO.
...
```{r}
#| echo: true
#| label: fig-bayesian-optimization-ackley
#| fig-cap: Contour plot of the 2D Ackley function.
#| fig-alt: FIXME
library(ggplot2)
...
We probably can do better than a simple random search by using a more sophisticated optimizer (e.g., BO).
...
We will now introduce the building blocks of BO.
...
After having made some assertions and safety checks, and having evaluated the initial design, `r ref("mlr_loop_functions_ego", "bayesopt_ego")` essentially repeatedly performs the following steps:
...
FIXME: the more I read this the more I think we should have named this $learner field...
...
FIXME: maybe show closed form expression and reference exploration vs. exploitation again?
...
The following figures visualize the mean (@fig-bayesian-optimization-mean) and variance prediction (@fig-bayesian-optimization-variance of the surrogate.
...
```{r}
#| echo: true
#| label: fig-bayesian-optimization-mean
#| fig-cap: Mean prediction of the Gaussian Process surrogate model trained on an initial design of eight points (red) choosing uniformly at random
#| fig-alt: FIXME
ggplot() +
...
```{r}
#| echo: true
#| label: fig-bayesian-optimization-variance
#| fig-cap: Variance prediction of the Gaussian Process surrogate model trained on an initial design of eight points (red) choosing uniformly at random. The variance is higher the further away from points in the initial design.
#| fig-alt: FIXME
ggplot() +
...
The following figure ((@fig-bayesian-optimization-ei) shows that the Expected Improvement is high in regions where the mean prediction is low but the variance prediction suggests uncerctainty:

```{r}
#| echo: true
#| label: fig-bayesian-optimization-ei
#| fig-cap: Expected Improvement based on the mean and variance prediction of the Gaussian Process surrogate model. Points of the initial design in red.
#| fig-alt: FIXME
ggplot() +
  geom_contour_filled(aes(x = x1, y = x2, z = ei), data = xydt) +
  geom_point(aes(x = x1, y = x2), colour = "darkred", data = instance$archive$data) +
  labs(fill = "Expected Improvement") +
  theme_minimal()
```

Based on an acquisition function optimizer we now optimize the Expected Improvement (of course we also could have picked the next candidate based on the grid we used to generate the figure but in a real BO setting we probably would choose a more sophisticated optimizer, e.g., the DIRECT algorithm provided by the `nloptr` package).
FIXME: link
...
#| fig-cap: Next candidate point (black) obtained by maximizing the Expected Improvement based on the mean and variance prediction of the Gaussian Process surrogate model. Points of the initial design in red.
#| fig-alt: FIXME
ggplot() +
  geom_contour_filled(aes(x = x1, y = x2, z = ei), data = xydt) +
  geom_point(aes(x = x1, y = x2), colour = "darkred", data = instance$archive$data) +
  geom_point(aes(x = x1, y = x2), colour = "black", size = 3, data = candidate) +
  labs(fill = "Expected Improvement") +
  theme_minimal()
```
...
#| fig-cap: Sampling trajectory of the BO algorithm. Points that have been evaluated in later stages (i.e., batchers) of the optimization process are coloured in a lighter red.
#| fig-alt: FIXME
ggplot(aes(x = x1, y = x2, z = y), data = xydt) +
  geom_contour_filled() +
...
FIXME: show the parameters of the optimizer? State that loop_function, surrogate, acq_function and acq_optimizer are fields and not params
       ...
       otoh this is all explained in detail in the vignette
...
FIXME: discuss with Bernd objective of tuning instance has property noisy --> therefore mlr3mbo treats it noisy and will change its default (e.g., GP); and mostly relevant the result function
       discuss with Bernd new ResultAssigner PR in mlr3mbo
FIXME: note about defaults and safety nets and extending; separate chapters? note about extending --> maybe in a last technical subchapter? or maybe simply reference the vignette
...
Note that `r ref("TunerMbo")` is simply a light-weight wrapper around `ref("OptimizerMbo")`.
...
FIXME: some notes about noisy here and reference to the noisy chapter (if we want it or write about noisy here)
...
Multi-Objective BO algorithms can differ in many design choices, for example whether they use a scalarization approach of
...
FIXME: reference the multi-objective HPO chapter more because we do not want to reiterate all of it again
...
#| echo: true
#| label: fig-pareto
#| fig-cap: Pareto front of selected features and classification error obtained using ParEGO.
#| fig-alt: FIXME
library(viridisLite)
...
HPO is already somehwat noisy (although our resampling instance is fixed)
