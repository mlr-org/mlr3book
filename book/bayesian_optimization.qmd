...
abstract:
  ...
  We briefly discuss handling noisy objective functions and further provide practical considerations for using and implementing Bayesian optimization algorithms.
...
HPO is therefore a prime example for black-box optimization, which considers the optimization of a function whose structure and analytical description is unknown, unexploitable or non-existent.
...
Other examples for black-box optimization problems are real-life experiments, e.g., crash tests or chemical reactions, or computer simulations.
...
Bayesian optimization (BO) refers to a class of sample-efficient global black-box optimization algorithms that rely on a surrogate model trained on observed data to model the black-box function.
This surrogate model is typically a non-linear regression model that captures the unknown function using limited observed data.
To select the next candidate point for evaluation, BO algorithms use an acquisition function that quantifies the attractiveness of each point in the search space based on the surrogate model.
...
Local optima pose a significant challenge for many optimization algorithms as they can trap the algorithm, preventing it from escaping to potentially better solutions.
For example, gradient-based methods may struggle to escape local optima because gradients typically only indicate the direction towards the nearest local optimum and become zero at local optima, causing the algorithm to get stuck in sub-optimal solutions.

At the core of an `r ref("OptimInstanceSingleCrit")` lies an `r ref("Objective")` function wrapping the actual function mapping a domain to a codomain (see also #sec-defining-search-spaces).
...
We can proceed to visualizing the sinusoidal function by generating a grid of points on which we evaluate the function:
...
While the domain describes the set of all possible input values for which the function is defined and can produce a valid output, the search space is a subset of the domain and describes the set of all input values we want to optimize over.
When not specifying the `search_space`, the whole domain of the `objective` will be used by default.
In the following we use a simple random search to optimize the sinusoidal function.
...
Similarly as in tuning (@sec-simplified-tuning), we can also use the `bb_optimize()` helper function for optimization.
Internally, this creates an `r ref("OptimInstanceSingleCrit")`, starts the optimization and returns the result with the instance.
...
Note that evolutionary strategies are available within the mlr3 ecosystem via the `r link("https://github.com/mlr-org/miesmuschel", text = "miesmuschel")` package which we will not cover in this chapter.
...
Bayesian optimization (BO) is an iterative optimization algorithm that makes use of a so-called surrogate model to model the unknown black-box function.
After having observed an initial design, the surrogate model is trained on all data points observed so far.
Then a so-called acquisition function is used to determine which point of the search space is a promising candidate that should be evaluated next.
...
We want to exploit knowledge about regions where we observed that performance is good and the surrogate model has low uncertainty but also want to make sure that we do not miss crucial regions and therefore also want to explore into regions where the uncertainty of the surrogate model is high.
...
Note that in the following we will often speak of BO flavors, as BO is highly modular, i.e., users can choose different types of surrogate models, acquisition functions and acquisition function optimizers to their liking and there is no single BO algorithm.
...
In the following, we will illustrate the steps involved in a standard BO loop in more detail, before we move on to using an `r ref("OptimizerMbo")` or `r ref("TunerMbo")` directly.
...
A surrogate model wraps a regression learner that models the unknown black-box function based on observed data.
...
As a learner, any regression learner (`r ref("LearnerRegr")`) from `r mlr3` can be used, however, most acquisition functions require both a mean and a variance prediction (therefore not all learners are suitable for all scenarios).
...
@fig-bayesian-optimization-mean-variance visualizes the mean and variance prediction of the surrogate model.
...
xydt[, c("mean", "variance") := prediction]
```

```{r}
#| echo: false
#| label: fig-bayesian-optimization-mean-variance
#| fig-cap: Mean and variance prediction (lightblue) of the Gaussian Process surrogate model trained on an initial design of four points (black). Ribbons represent the mean plus minus the variance prediction.
#| fig-alt: Mean and variance prediction of the Gaussian Process surrogate model trained on an initial design of four points. The mean prediction closely follows the true sinusoidal function that is being modelled. The variance prediction is high in unexplored areas and low in areas where points were already evaluated.
ggplot() +
  ...
  geom_line(aes(x = x, y = mean), colour = "steelblue", linetype = 2,
    data = xydt) +
  geom_ribbon(aes(x = x, min = mean - variance, max = mean + variance),
...
Roughly speaking, an acquisition function relies on the predictions of a surrogate model and quantifies the attractiveness of each point of the search space if it were to be evaluated in the next iteration.
...
#| fig-alt: Expected Improvement based on the mean and variance prediction of the Gaussian Process surrogate model. The Expected Improvement is high where the mean prediction is low but the variance prediction still suggests some uncertainty.
...
In this example we use the DIRECT algorithm provided by the `r link("https://github.com/astamm/nloptr", text = "nloptr")` package to optimize the Expected Improvement.
...
#| fig-alt: Next candidate point obtained by maximizing the Expected Improvement based on the mean and variance prediction of the Gaussian Process surrogate model.
ggplot(aes(x = x, y = ei), data = xydt) +
  geom_line() +
  geom_point(aes(x = x, y = acq_ei), colour = "darkred", size = 2,
    data = candidate) +
  labs(y = "Expected Improvement") +
  theme_minimal()
```
...
<!-- FIXME: Unclear if HPO has noisy flag or not, clarify this -->
<!-- FIXME: Unclear if noisy flag is automatically respected to select defaults -->
...
This will result in the Gaussian Process not perfectly interpolating training data and the variance associated with the training data will be non-zero, reflecting the uncertainty in the observed function values due to the measurement error.
...
<!-- FIXME: Do we need parallel eval somewhere?-->
...
In this case, the surrogate model will not provide useful mean and variance predictions resulting in poor performance of the BO algorithm.
...
Besides, vanilla Gaussian Processes natively cannot handle categorical input variables or dependencies in the search space (recall that in HPO we often deal with hierarchical mixed spaces).
In contrast, a random forest is cheap to train, quite robust in the sense that it is not as sensitive to its hyperparameters as a Gaussian Process, and can easily handle hierarchical mixed spaces.
...
A more in-depth introduction to `r mlr3mbo` is given in its `r link("https://mlr3mbo.mlr-org.com/articles/mlr3mbo.html", text = "getting started vignette")`.
