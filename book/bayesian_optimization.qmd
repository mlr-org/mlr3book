...
Bayesian optimization (BO) describes a class of sample efficient iterative black box optimization algorithms that make use of a so-called surrogate trained on observed data modelling the black box function.
...
If we replicate running BO ten times (with random initial designs and varying random seeds) and compare this to a random search, we can see that BO indeed performs much better and reaches the global optimum after around 14 function evaluations (@fig-bayesian-sinusoidal_bo_rs).
...
## Robustness, Defaults and Safety Nets

FIXME:

robust behavior, safety nets, fallback learner, HPO practical paper verweisen, parallel mit multi point?
overhead von modellklassen kurz diskutieren
gp vs rf kurz diskutieren
