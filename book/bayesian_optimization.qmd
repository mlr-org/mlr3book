...
This value corresponds to the point with the lowest function value:
...
It provides various options, including BO via the key `mbo`, which is introduced in this chapter.
...
In the subsequent sections, we will provide a more detailed explanation of the individual building blocks that constitute a standard BO loop and explore their interplay: The initial design, the loop function, the acquisition function and the acquisition function optimizer.
In essence, we will show what happens inside an `r ref("OptimizerMbo")` during optimization.
Subsequently, we will progress to utilizing the `r ref("OptimizerMbo")` class directly for optimization, after having obtained a deeper understanding of the building blocks and their interplay and interaction involved during optimization.
...
<!--
#FIXME: different loop functions can have different intial designs per default
-->
If a specific initial design is desired, it needs to be evaluated on the `r ref("OptimInstance")` before optimizing it.
...
The function then enters a loop where it repeatedly performs the steps described above: Update the surrogate model $\rightarrow$ update the acquisition function $\rightarrow$ optimize the acquisition function to yield a new candidate$\rightarrow$ evaluate the candidate and add it to the archive.
...
In our running example we will now essentially perform a single iteration of this loop function.
...
In `r mlr3mbo`, the `r ref("SurrogateLearner")` is a higher-level R6 class inheriting from the base `r ref("Surrogate")`, designed to construct and manage the surrogate model.
It leverages regression learners from `r mlr3` to facilitate the modelling process.
Wrapping the learner in a `r ref("Surrogate")` is necessary to allow for automatic construction of the task the learner should be trained on at each iteration of the BO loop.
...
::: {.callout-note}
Depending on the nature of the optimization problem and choice of the loop function, multiple optimization targets must be modelled by multiple surrogates, in which case a `r ref("SurrogateLearnerCollection")` should be used instead of a `r ref("SurrogateLearner")`, see @sec-multi-objective-bayesian-optimization.
:::
...
In the next section we illustrate how the building blocks can be assembled within an `r ref("OptimizerMbo")` so that BO can be used straightforward for black-box optimization.
...
The following arguments can be provided during construction: `loop_function`, `surrogate`, `acq_function`, `acq_optimizer`.
<!--
#FIXME: args, result assigner
-->
...
Here we use `bayesopt_ego()` (`r ref("mlr_loop_functions_ego")`) loop function suitable for standard single-objective optimization.
Moreover, note that passing arguments such as `archive` to the `surrogate` is no longer required because we now work with an `r ref("OptimizerMbo")` directly instead of the building blocks.
...
Here, we use the same initial design as earlier, evaluate it and optimize the instance using our newly constructed BO algorithm:
...
As expected, the performance for the size of the initial is close to the performance of the random search.
...
<!--
#FIXME: this maybe has some overlap with robustness above and should be moved and rewritten
-->
