...
HPO is therefore a prime example for `r define("black-box optimization")`, which considers the optimization of a function whose structure and analytical description is unknown, unexploitable or non-existent.
...
At the core of an `r ref("OptimInstanceSingleCrit")` lies an `r ref("Objective")`\define{Objective}[Objective]{.aside} function wrapping the actual function mapping a domain to a codomain (see also #sec-defining-search-spaces).
...
The initial set of points that is evaluated before a surrogate model can be fit is referred to as the `r define("initial design")`.
...
The `r ref("loop_function")`\define{loop_function}[Loop Function]{.aside} determines the behavior of the BO algorithm on a global level, i.e., how the subroutine should look like that is performed at each iteration.
...
A `r define("surrogate model")` wraps a regression learner that models the unknown black-box function based on observed data.
...
Roughly speaking, an `r define("acquisition function")` relies on the predictions of a surrogate model and quantifies the expected 'utility' of each point of the search space if it were to be evaluated in the next iteration.
...
An `r define("acquisition function optimizer")` is then used to solve this optimization problem.
...
Real world black-box functions, however, are often noisy\define{noisy Objective}[Noisy Objective]{.aside}, i.e., the true signal of the black-box function is augmented by some noise and repeatedly evaluating the same point will return different objective function values.
...
Finally, `r mlr3mbo` allows for explicitly specifying how the final result after optimization is assigned to the instance (i.e. what will be written to `instance$result`) via a so-called `r define("result assigner")`.
...
(2) If the actual proposal mechanism of a BO algorithm should be parallelized in the sense that the loop proposes a *batch* of candidates \define{batch proposal}[Batch Proposal]{.aside} that should be evaluated synchronously in the next iteration, the evaluation of the objective function itself must be parallelized.
