...
abstract:
  ...
  Moreover, hyperparameter optimization can be very expensive and therefore calls for very sample efficient optimization algorithms.
  ...
  The chapter closes with comments on practical considerations regarding Bayesian Optimization algorithms.
---
...
Other examples for black-box optimization problems range from real-life experiments, e.g., crash tests or chemical reactions to computer simulations.
...
In @sec-optimization, we used a grid or random search to find the optimal hyperparameter configuration for a learner on a given task.
However, more sophisticated black-box optimizers such as evolutionary strategies or especially Bayesian Optimization allow for usually allow for finding much better performing configurations much more efficiently.
This chapter is therefore of general relevance to users concerned with sample efficient black-box optimization and HPO.
...
Indeed, in @sec-optimization, the classes `r ref("TuningInstanceSingleCrit")`, `r ref("TuningInstanceMultiCrit")` and `r ref("Tuner")` were already introduced which are special cases of the `r ref("OptimInstanceSingleCrit")`, `r ref("OptimInstanceMultiCrit")` and `r ref("Optimizer")` classes with the latter being suited for general black-box optimization and not only HPO.
...
The local optima can pose a risk for many optimization algorithms to get stuck (@fig-bayesian-optimization-sinusoidal) as for example gradient based methods will usually have a hard time escaping a local optimum due to gradients only pointing towards the local optimum.

At the core of an `r ref("OptimInstanceSingleCrit")` lies an `r ref("Objective")` function wrapping the function, domain and codomain (see also #sec-defining-search-spaces).
<!-- FIXME: we may want to discuss domain vs search space? -->
...
#| fig-alt: Visualization of the sinusoidal function. The function is defined on a domain ranging from 0 to 1 and has a sinusoidal shape. There are two local minima, one at 0 and one at around 0.35. The global minimum is located at around 0.79.
library(ggplot2)
library(viridisLite)
...
Bayesian Optimization (BO) is an iterative optimization algorithm that makes use of a so-called surrogate (i.e., a statistical model) to model the unknown black-box function.
After having observed an initial design of observations, the surrogate model is trained on all data points observed so far.
...
The initial design refers to the first set of points the black-box function was evaluated on before BO algorithms start their loop.
...
* `r ref("generate_design_lhs")`: Latin hypercube sampling
* `r ref("generate_design_sobol")`: Sobol sequence
...
We observe that a random design does not cover the domain well and simply by chance samples points close to each other leaving large areas unexplored.
A grid design will always result in points being marginally equidistant from their nearest neighbour and does not cover the domain well (areas between points are unexplored).
...
Technically, all `loop_function`s are members of the `S3` class `r ref("loop_function")` (and are actually simply `r ref("base::function")`s with some attributes set).
...
#| fig-alt: Anytime performance of BO and random search on the 1D sinusoidal function given a budget of 20 function evaluations. BO is much more sample efficient than random search and quickly converges to the global optimum after around 15 function evaluations.
...
ParEGO, implemented via the `bayesopt_parego` loop function (`r ref("mlr_loop_functions_parego")`) tackles multi-objective BO via a scalarization approach and models a single scalarized objective function via a single surrogate model and then proceeds to find the next candidate for evaluation making use of a standard single-objective acquisition function such as the Expected Improvement:
...
For example, if two points in the training data are too close to each other, fitting the Gaussian Process can fail due to singularity of the covariance matrix.
And even if the training of the surrogate model succeeds, the prediction step needed for the computation of the acquisition function can error out.
...
If set to `TRUE`, all errors that occur during the acquisition function optimization (either due to the surrogate model failing to predict or the acquisition function or acquisition function optimizer erroring out) are caught and encapsulated.
...
In contrast, a random forest is cheap to train, quite robust in the sense that it is not that sensitive to its hyperparameters as a Gaussian Process and can easily handle hierarchical mixed spaces.
...
>>>>>>> 3c2e3a199dbe8b4a81490b5097ce76bffed18698
...
If you are interested in learning more about the underlying R6 classes to gain finer control, take a look at the online API.

| S3 function | R6 Class | Summary |
| ------------------- | -------- | -------------------- |
| `r ref("srlrn")` or `r ref("srlrnc")`   | `r ref("SurrogateLearner")` or `r ref("SurrogateLearnerCollection")` | Construct a surrogate |
