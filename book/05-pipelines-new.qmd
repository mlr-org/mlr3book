...
In the following, we give a short `r mlr3pipelines` exampl to give you a quick idea what it is capable of.
...
More about PipeOps and how they are created and configured is described in @sec-pipelines-pipeops.
...
@sec-pipelines-graphs gives a more in-depth introduction into how simple Graphs are constructed; the simple sequential pipeline shown here is described in more detail in @sec-pipelines-sequential.
...
#| out.width: "70%"
#| echo: false
knitr::include_graphics("images/affect_pipe.svg")
knitr::include_graphics("images/select_pipe.svg")
```
...
Tuning ML models with `r mlr3pipelines` comes in three levels of difficulty:

1. Tuning the hyperparameters of a Learner or a PipeOp individually when it is part of a Graph.
2. Tuning the hyperparameters of both Learner and its preprocessing operations together.
3. Tuning not only the hyperparameters, but also the option of whether and which preprocessing to perform.
...
It follows the @sec-pipelines-branch, showing one way of implementing the third level.
Another way of achieving 3. is to use `PipeOpProxy`, demonstrated in the (optional) @sec-pipelines-proxy.

We will be using the "Sonar" dataset.
It contains many features -- sonar response levels in different frequency bands -- that are likely correlated but also likely not very meaningful by themselves.
In this setting, doing a principal component analysis using `po("pca")`, and selecting the highest variance components using the `rank.` hyperparameter, is a good option.
In the following, we will show how to choose this hyperparameter, and how `po("pca")` measures up against other preprocessing methods.

### Tuning Combined Spaces {#sec-pipelines-combined}

We use SVM Learner that we already tuned in @sec-optimization and combine it with `po("pca")`.
Because we found that the SVM is very sensitive to the right choice of its `gamma` hyperparameter, we will tune it together with the `rank.` hyperparameter of `po("pca")`.
...
library("mlr3learners")
glrn = po("pca") %>>%
  lrn("classif.svm", cost = 2.5e4, kernel = "radial", type = "C-classification")
glrn = as_learner(glrn)

library("paradox")
search_space = ps(
  pca.rank. = p_int(lower = 2, upper = 60, logscale = TRUE),
  classif.svm.gamma = p_dbl(lower = 1e-6, upper = 1e0, logscale = TRUE)
)
```

We tune this using the `r ref("tune")` function on a 5x5 grid, stepping through both the number of selected principal components (`pca.rank.`) and the SVM's `gamma` hyperparameter on a log-scale.
...
Note that the hyperparameter values are the log of the actual values.
On the normal scale, the optimum is at:
```{r 05-pipelines-modeling-009-2}
cbind(data.table::as.data.table(instance$result_x_domain), classif.ce = instance$result_y)
```

The observed performance values are shown in @fig-pipelines-opttrace-1.
It becomes clear that, for different values of `rank.`, different `gamma` values can be optimal, so tuning both hyperparameters together is prudent.

```{r fig.align='center', eval = TRUE}
#| label: fig-pipelines-opttrace-1
#| fig-cap: "Observed performance values when optimizing both `rank.` of `po(\"pca\")` and `gamma` of `lrn(\"svm\")` at the same time.
#|   The x-axis shows `gamma` values (log scale). Each facet (i.e. individual plot) shows the behavior for a different `rank.` value, displayed at the top."
#| fig-alt: "Plot showing performance values of pca, followed by SVM."
#| echo: false
library("ggplot2")
ggplot(instance$archive$data[, c(data.table::rbindlist(x_domain), list(classif.ce = classif.ce))], aes(x = classif.svm.gamma, y = classif.ce)) +
  geom_line() +
  facet_grid(cols = vars(pca.rank.)) +
  scale_x_log10() +
  theme_bw() +
  labs(title = "Performance of po(\"pca\") %>>% lrn(\"classif.svm\")")
```
...
We now extend this example so that both the preprocessing (PCA, scaling, or no preprocessing), as well as the model to use (SVM or decision tree) can be tuned.
For this, we add another branching pathway to our Graph:

```{r 05-pipelines-branch-02}
glrn = graph %>>%
  po("branch", c("classif.rpart", "classif.svm"), id = "branch2") %>>%
    gunion(list(
      lrn("classif.rpart"),
      lrn("classif.svm", cost = 2.5e4, kernel = "radial", type = "C-classification")
    )) %>>%
...
glrn = as_learner(glrn)
glrn$graph$plot(html = FALSE)
```

Note that it is necessary to give the two branching operations different IDs to avoid name clashes.

Finally, we would still like to tune over the `gamma` hyperparameter of the SVM, since it may again depend on the kind of preprocessing being done.
However, this hyperparameter is only active when the "`classif.svm`" path is chosen.
We therefore have to declare a dependency in the search space.
The search space that tunes over all options of both branching operators as well as the SVM's `gamma` hyperparameter therefore looks like the following:


```{r 05-pipelines-branch-03}
search_space = ps(
  branch.selection = p_fct(c("nop", "pca", "scale")),
  branch2.selection = p_fct(c("classif.rpart", "classif.svm")),
  classif.svm.gamma = p_dbl(lower = 1e-6, upper = 1e0, logscale = TRUE,
    depends = branch2.selection == "classif.svm")
)
...
We also see that the tuned SVM performs better than the untuned decision tree, "classif.rpart".
A more thorough investigation could try to tune the decision tree to check if its performance can be brought to the level of the SVM.

```{r fig.align='center', eval = TRUE}
#| label: fig-pipelines-opttrace-2
#| fig-cap: "Observed performance values when optimizing both preprocessing (between \"pca\", \"scale\", and \"nop\", which is no preprocessing) and Learner (between SVM and the decision tree \"classif.rpart\") at the same time.
#|   The x-axis shows preprocessing. Each facet (i.e. individual plot) shows the Learner being used, together with setting for \"gamma\", if applicable."
#| fig-alt: "Plot showing performance values of pca, scale, or nop, followed by SVM or decision tree."
#| echo: false

ggplot(instance$archive$data[, learner := ifelse(is.na(classif.svm.gamma), "classif.rpart", sprintf("classif.svm\ngamma = %s", format(classif.svm.gamma, digits = 2, scientific = TRUE)))],
...
knitr::include_graphics("images/proxy.svg")
```
...
search_space = ps(
  ...
  learner.content = p_fct(list(
    ...
    classif.svm = lrn("classif.svm", cost = 2.5e4, kernel = "radial", type = "C-classification")
  )),
  classif.svm.gamma = p_dbl(lower = 1e-6, upper = 1e0, logscale = TRUE,
    depends = learner.content == "classif.svm"),
  .extra_trafo = function(x, param_set) {
    if (!is.null(x$classif.svm.gamma)) {
      x$learner.content$param_set$values$gamma = x$classif.svm.gamma
      x$classif.svm.gamma = NULL
    }
    x
  }
)
...
as.data.table(instance$result)[,
  .(preproc.content, learner.content, classif.svm.gamma = exp(classif.svm.gamma), classif.ce)
]
```
