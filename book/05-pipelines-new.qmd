...
### `PipeOpLearnerCV` and Stacking {#sec-pipelines-stack}
...
It can even be used to tune which, of several learners, is most appropriate for a given dataset.
Consider the following example, where both the preprocessing (PCA or no preprocessing), as well as the model to use (multinomial linear model or shallow decision stump) is tuned over.
The `Graph` we need is the following:
...
We can now get the search space that tunes over all options of both branching operators and tune the graph.
...
`r ref("to_tune", "paradox::to_tune()")` is called with a `r ref("p_fct", "paradox::p_fct()")` that lists the PipeOps that should be inserted.
