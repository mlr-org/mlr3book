...
```
### Filter-based Feature Selection {fs-filter-based}

A common use case for filters is to conduct feature selection based on filter scores.
This can result in a sparser model, sometimes trading in model performance for simplicity.
Filter-based feature selection is implemented via `r ref("mlr_pipeops_filter", text = "PipeOpFilter")`.
This PipeOp takes as input a Task, applies the filter, and selects the features based on the calculated scores.
Four different methods are available to control how the features are selected, each corresponding to a parameter, indicate by the name in brackets:

  * The number of values (`filter.nfeat`) with the highest filter score are selected.
  * The fraction of values (`filter.frac`) with the highest filter score are selected.
  * All features with a filter score greater than the specified cutoff (`filter.cutoff`) are selected.
  * A random permutation of each feature is added to the task before applying the filter.
    All features selected before the n-th permuted feature (`filter.permuted`) is selected are kept.



We will illustrate the usage of this filter on a pipeline for the `r ref("mlr_tasks_optdigits", text = "optdigits")` task, where the goal is to classify handwritten digits.
In the first step, the features are subset based on the `r ref("mlr3filters::mlr_filters_information_gain", text = "FilterInformationGain")`.
As the selection method, we opt for choosing a fraction of features with the highest filter value and tuning this parameter with a grid search.
In the second step, a `r ref("mlr3learners::mlr_learners_classif.lda", text = "LDA")` is trained using the selected features.
Afterwards, we will visualize the classification error for the different subset sizes.
...
We could therefore obtain a 30 % sparser model without sacrificing much in accuracy.
