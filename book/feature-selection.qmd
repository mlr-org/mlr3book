...
[Feature selection](#fs) is the process of finding a subset of features that leads to improved models.
...
Dropping irrelevant features is important to obtain performantt models.
...
Other filter methods are built without a `Learner` in mind and therefore e.g. only analyse univariate relationships between a given
...
[Filter](#fs-filter) algorithms select features by asigning numeric scores to each feature, (e.g. the correlation )
Features that are assigned lower scores can then be omitted in subsequent modeling steps.
All filters are implemented via the package `r mlr_pkg("mlr3filters")`.
...
One special case of fitlers are [variable importance filters](#fs-var-imp-filters).
...
All wrapper metthods are implemented via the package `r mlr_pkg("mlr3fselect")`.
