...
`r index('Feature selection')`, also known as variable or descriptor selection\index{variable selection|see{feature selection}}\index{descriptor selection|{feature selection}}, is the process of finding a subset of features to use with a given task and learner.
...
Many filter methods are implemented in `r mlr3filters`, including:
...
#| fig-alt: Scatter and line plot with "Batch" on the x-axis and "classif.acc" on the y-axis. Line shows improving performance from ~0.76 in batch 1 to ~0.88 in batch 2 to ~0.91 in batch 3 and ~0.93 in batch 4. Points on the plot show same trend."
autoplot(instance, type = "performance") + ggplot2::scale_fill_grey() + ggplot2::scale_color_grey()
```

In the plot, we can see that adding features in each batch improves performance, which we can see in each iteration:
...
However by the fourth batch the improvement is very small and therefore we may prefer to select a marginally worse model to reduce data size.
...
Internally, the `fselect()` function creates an `r ref('FSelectInstanceSingleCrit')` object and executes the feature selection with an `r ref('FSelector', index = TRUE)` object, based on the selected method, in this example an `FSelectorSequential` object.
...
The function `r ref("fsi")` creates an instance of `r ref('FSelectInstanceMultiCrit')` if more than one measure is selected.
...
4. Automate the feature selection as in @sec-autofselect with the `r ref("mlr_tasks_sonar", text = "sonar")` dataset and a logistic regression learner (`"classif.log_reg"`). Hint: Remember to call `library("mlr3learners")` for the logistic regression learner.
