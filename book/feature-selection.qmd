...
Thus, more advanced filter methods, e.g. *multivariate* filters based on feature importance, usually perform better [@bommert2020]. In the [Filters](#fs-filter) section, it is described how to calculate univariate, multivariate and feature importance filters, how to access implicitly selected features, how to integrate filters in a machine learning pipeline and how to optimize filter thresholds.
...
To create an `r ref("FSelectInstanceSingleCrit")` object, we use the sugar function `r ref("fsi")`, which is short for `FSelectInstanceSingleCrit$new()` or `FSelectInstanceSingleCrit$new()`, depending on the selected measure(s): 

```{r feature-selection-015-001}
instance = fsi(
  task = tsk("penguins"),
  learner = lrn("classif.rpart"),
  resampling = rsmp("holdout"),
...
```{r feature-selection-021, eval=FALSE}
...
```{r feature-selection-023, eval=FALSE}
...
```{r feature-selection-024, eval=FALSE}
instance$result_feature_set
```
...
```{r feature-selection-026, eval=FALSE}
learner = lrn("classif.rpart")
...
```{r feature-selection-027, eval=FALSE}
grid = benchmark_grid(
  task = tsk("pima"),
