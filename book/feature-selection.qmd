...
Therefore, many [learners](#learners) perform implicit feature selection, e.g. via the choice of variables used for splitting in a decision tree.
...
[Filter](#fs-filter) algorithms select features by assigning numeric scores to each feature, (e.g. correlation between feature and target variables).
...
Filter methods assign a score to each feature, use these to rank the features and select a feature subset based on the ranking.
...
Some filters have hyperparameters, which can be changed similar to setting hyperparameters of a `r ref("Learner")` using `$param_set$values`. For example, to calculate `"spearman"` instead of `"pearson"` correlation with the correlation filter: 
