...
All filters are implemented via the package `r ref_pkg("mlr3filters")`.
...
All wrapper metthods are implemented via the package `r ref_pkg("mlr3fselect")`.
...
Similar to other instances in `r ref_pkg("mlr3")`, these are registered in a dictionary (`r ref("mlr_filters")`) with an associated shortcut function `r ref("flt()")`.
...
Wrapper feature selection is supported via the `r ref_pkg("mlr3fselect")` extension package.
At the heart of `r ref_pkg("mlr3fselect")` are the R6 classes:
...
The following algorithms are currently implemented in `r ref_pkg("mlr3fselect")`:
