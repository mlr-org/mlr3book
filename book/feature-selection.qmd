...
Methods and classes in `r mlr3fselect` can be mapped to those in `r mlr3tuning` (@tab-fselect-tuning) and therefore if you have read the previous chapter, the code in this section will seem very familiar to you.
...
: Comparison of mlr3tuning and mlr3fselect methods {#tab-fselect-tuning}

### `fs` and `fsi`

Let's look at the 'manual' approach first with `fs` and `fsi` to find the optimal set of features using a random search (random subset of features in each iteration), a decision train, holdout resampling in each fold, and terminating after 20 evaluations.
