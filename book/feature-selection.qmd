...
1. Calculate a correlation filter on the `tsk("mtcars")` data set.
2. Use the filter from the first exercise to select the five best features in the `tsk("mtcars")` dataset.
3. Apply a backward selection to the `tsk("penguins")` dataset with a classification tree learner `lrn("classif.rpart")` and holdout resampling by the measure classification accuracy. Compare the results with those in @sec-fs-wrapper-example. Answer the following questions:
...
4. Automate the feature selection as in @sec-autofselect with the `tsk("spam")` dataset and a logistic regression learner (`lrn("classif.log_reg")`). Hint: Remember to call `library("mlr3learners")` for the logistic regression learner.
