...
To see which feature was added, we can go back to the archive and look at the second iterations:
...
Again, we point out the similarity with hyperparameter optimization and refer to multi-objective hyperparameter optimization (@sec-multi-metrics-tuning).
...
In this example, we optimize the sensitivity, i.e. true positive rate (TPR), and specificity, i.e. true negative rate (TNR). 
...
We see different tradeoffs of sensitivity and specificity but no feature subset is dominated by another, i.e. has worse sensitivity *and* specificity. 
