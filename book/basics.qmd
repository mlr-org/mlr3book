...
Here, we retrieve the `r ref("mlr_tasks_mtcars")` task, which is provided by the package `r ref_pkg("datasets")`:
...
For example, the data for `r ref("datasets::mtcars")` is also available separately, as a `data.frame()` and not a task.
`r ref("datasets::mtcars")` contains characteristics for different types of cars, along with their fuel consumption.
...
Similar to plotting tasks, `r mlr3viz` provides an `r ref("autoplot()")` method for `r ref("Prediction")` objects.
...
We choose root mean squared error (`r ref("mlr_measures_regr.rmse")`) as our performance measure for this example.
...
Regression defaults to mean squared error (`r ref("mlr_measures_regr.mse")`).
:::

It is possible to calculate multiple measures at the same time by passing multiple measures to `$score()`.
For example, to compute both root mean squared error `r ref("mlr_measures_regr.rmse")` and mean squared error `r ref("mlr_measures_regr.mse")`:
...
Some measures have hyperparameters themselves, for example `r ref("mlr_measures_selected_features")`.
...
We will use the `r ref("datasets::penguins")` dataset as a running example:
...
Classification tasks (`r ref("TaskClassif")`) can also be plotted using `r ref("autoplot")`.
...
Just like predictions of regression models, we can plot classification predictions with `r ref("autoplot")`:
...
You can specify the *positive class* for a `r ref("TaskClassif")` object during task creation.
...
:::{.callout-tip}
Thresholds can be tuned automatically with respect to prediction performance with the `r mlr3pipelines` package using `r ref("mlr_pipeops_tunethreshold")`.
...
The `r ref("datasets::zoo")` dataset illustrates this concept nicely.
...
* Featureless classifier `r ref("mlr_learners_classif.featureless")`: Simple baseline classification learner.
...
* Featureless regressor `r ref("mlr_learners_regr.featureless")`: Simple baseline regression learner.
  Predicts the mean of the target values in the training set.
* Rpart decision tree learner `r ref("mlr_learners_classif.rpart")`: Tree learner from `r ref_pkg("rpart")`.
* Rpart regression tree learner `r ref("mlr_learners_regr.rpart")`: Tree learner from `r ref_pkg("rpart")`.

The `r mlr3learners` package contains cherry-picked implementations of the most popular machine learning methods:

* Linear (`r ref("mlr_learners_regr.lm")`) and logistic (`r ref("mlr_learners_classif.log_reg")`) regression.
* Penalized Generalized Linear Models (`r ref("mlr_learners_regr.glmnet")`, `r ref("mlr_learners_classif.glmnet")`), possibly with built-in optimization of the penalization parameter (`r ref("mlr_learners_regr.cv_glmnet")`, `r ref("mlr_learners_classif.cv_glmnet")`).
* (Kernelized) $k$-Nearest Neighbors regression (`r ref("mlr_learners_regr.kknn")`) and classification (`r ref("mlr_learners_classif.kknn")`).
* Kriging / Gaussian Process Regression (`r ref("mlr_learners_regr.km")`).
* Linear (`r ref("mlr_learners_classif.lda")`) and Quadratic (`r ref("mlr_learners_classif.qda")`) Discriminant Analysis.
* Naïve Bayes Classification (`r ref("mlr_learners_classif.naive_bayes")`).
* Support-Vector machines (`r ref("mlr_learners_regr.svm")`, `r ref("mlr_learners_classif.svm")`).
* Gradient Boosting (`r ref("mlr_learners_regr.xgboost")`, `r ref("mlr_learners_classif.xgboost")`).
* Random Forests for regression and classification (`r ref("mlr_learners_regr.ranger")`, `r ref("mlr_learners_classif.ranger")`).
...
1. Using `r ref("mlr_tasks_sonar")`, measure the classification error (`classif.ce`) of a classification tree model (`classif.rpart`) trained with default hyperparameters on 80% of the data and tested on the remaining 20%.
