...
These building blocks include the data (and the methods of creating training and testing sets), the machine learning algorithm (and it's training and prediction process), and evaluation measures for predictions.

`r mlr3` encapsulates data in [tasks](#sec-tasks), which also contain meta data like the type of machine learning task (regression, classification, etc.), which column is the target (the feature that we hope to predict with new data), and which rows are considered part of the training set and which are part of the testing set (this concept is explored further in the [resampling](#resampling) section).
`r mlr3` uses [learners](#sec-learners) to implement popular machine learning algorithms. 
These learners also have a type (regression, classification, etc.) that must match the task that is used with it.
The model a learner constructs during the training process is saved in the [learner](#sec-learners) object and can then used to produce [predictions](#sec-predicting) on the test data.
...
1. Using the `r ref("mlr_tasks_sonar", text = "Sonar dataset")`, find the classification error (`classif.ce`) of a classification tree model (`classif.rpart`) trained on 80% of the data and tested on the remaining 20%.
1. State the true positive, false positive, true negative, and false negative rates of the predictions made by the model in problem 1.
1. Change the threshold of the model from problem 1 such that the false positive rate is lower than the false negative rate. State a reason why you might do this.
