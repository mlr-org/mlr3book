...
This chapter will teach you the essential building blocks of `r ref_pkg("mlr3")`, as well as its `r ref_pkg("R6")` classes and operations used for machine learning.
...
The data, which `r ref_pkg("mlr3")` encapsulates in [tasks](#tasks), is split into non-overlapping training and test sets.
...
The training data is given to a machine learning algorithm, which we call a [learner](#learners) in `r ref_pkg("mlr3")`.
...
`r ref_pkg("mlr3")` offers a number of different [measures](#measure) to quantify how well a model performs based on the difference between predicted and actual values.
...
2. [Learners](#learners) encapsulate machine learning algorithms to train models and make predictions for a [task](#tasks).
    ...
    * access the set of [classification and regression learners](#predefined-learners) that come with `r ref_pkg("mlr3")` and retrieve a specific learner (more types of learners are covered later in the book),
...
* **Survival Task**: The target is the (right-censored) time to an event. More censoring types are currently in development → `r ref("mlr3proba::TaskSurv")` in add-on package `r ref_pkg("mlr3proba")`.

* **Density Task**: An unsupervised task to estimate the density → `r ref("mlr3proba::TaskDens")` in add-on package `r ref_pkg("mlr3proba")`.

* **Cluster Task**: An unsupervised task type; there is no target and the aim is to identify similar groups within the feature space → `r ref("mlr3cluster::TaskClust")` in add-on package `r ref_pkg("mlr3cluster")`.

* **Spatial Task**: Observations in the task have spatio-temporal information (e.g. coordinates) → `r ref("mlr3spatiotempcv::TaskRegrST")` or `r ref("mlr3spatiotempcv::TaskClassifST")` in add-on package `r ref_pkg("mlr3spatiotempcv")`.
...
1. `x`: Object to convert.
  ...
  This allows connecting to out-of-memory storage systems like SQL servers via the extension package `r ref_pkg("mlr3db")`.
...
We can also plot the task using the `r ref_pkg("mlr3viz")` package, which gives a graphical summary of its properties:
...
:::{.callout-tip}
Instead of loading multiple extension packages individually, it is often more convenient to load the `r ref_pkg("mlr3verse")` package instead.
`mlr3verse` imports the Namespace of most `r ref_pkg("mlr3")` packages and re-exports functions which are used for common machine learning and data science tasks.
:::

### Predefined tasks {#tasks-predefined}

`r ref_pkg("mlr3")` includes a few predefined machine learning tasks.
...
As getting a task from a dictionary is a very common problem, `r ref_pkg("mlr3")` provides the shortcut function `r ref("tsk()")`.
...
For example, `r ref_pkg("mlr3data")` adds some more example and toy tasks for regression and classification, and `r ref_pkg("mlr3proba")` adds survival and density estimation tasks.
:::

To get more information about a particular task, it is easiest to use the `help()` method that all `r ref_pkg("mlr3")`-objects come with:
...
The `r ref_pkg("mlr3viz")` package provides plotting facilities for many classes implemented in `r ref_pkg("mlr3")`.
...
The `r ref_pkg("mlr3")` package ships with the following set of classification and regression learners.
...
Thus, we have cherry-picked implementations of the most popular machine learning method and collected them in the `r ref_pkg("mlr3learners")` package:
...
:::{.callout-tip}
A full list of available learners across all `r ref_pkg("mlr3")` packages is hosted on our website: [list of learners](https://mlr-org.com/learners.html).
:::
...
`r ref_pkg("mlr3")` makes it easy to only train on subsets of given tasks. We first create a vector indicating on what row IDs of the task the `r ref("Learner")` should be trained, and another that indicates the remaining rows that should be used for prediction. These vectors indicate the train-test-split we are using.
This is done manually here for demonstration purposes: In @sec-resampling, we show how `r ref_pkg("mlr3")` can automatically create training and test sets based on resampling strategies that can be more elaborate.
...
:::{.callout-tip}
Thresholds can be tuned automatically with the `r ref_pkg("mlr3pipelines")` package, i.e. using `r ref("mlr_pipeops_tunethreshold", text = "PipeOpTuneThreshold")`.
:::


### Plotting Predictions {#autoplot-prediction}

Similarly to [plotting tasks](#autoplot-task), `r ref_pkg("mlr3viz")` provides an `r ref("ggplot2::autoplot()", text = "autoplot()")` method for `r ref("Prediction")` objects.
