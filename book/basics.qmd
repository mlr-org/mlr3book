...
:::{.callout-tip}
You can list all regression learners that support the "se" prediction type by converting the `mlr_learners` dictionary to a `data.table` and subsetting it:
...
:::{.callout-tip}
You can list all classification learners that support the "prob" prediction type by converting the `mlr_learners` dictionary to a `data.table` and subsetting it:
```{r basics-learners-classif-prob}
as.data.table(mlr_learners)[task_type == "classif" & sapply(predict_types, function(x) "prob" %in% x)]
```
:::
...
#### Plotting Predictions {#sec-classif-pred-plot}
