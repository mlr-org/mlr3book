...
Other tasks are also encompassed by supervised learning, and there are returned to in @sec-special - we will also consider `r index('unsupervised learning')`  tasks in that chapter.
...
This whistlestop tour of machine learning provides the basic knowledge required to use software in `mlr3` and is sumamrised in @fig-ml-abstraction.
...
Finally, after a task has been created, you may want to perform operations on the task such as filtering down to subsets of rows and columns, which is often useful for manually creating train and test splits (@train-test-splits) or to fit models on a subset of given features.
...
Subsetting by features (columns) is possible with `$select()`\index{$select()} with the desired feature names passed as a character vector.
Subsetting by observations (rows) is performed with ``r define('$filter()')`` by passing the row IDs as a numeric vector.
...
As `R6` uses reference semantics (@sec-r6), you need to use `$clone()` method if you want to copy a task and then mutate it further:
...
Learners encapsulate methods to train a model and make predictions using it given a `r ref("Task")` and provide metadata about the learners.
...
Each learner provides the following metadata:
...
* `$param_set`: the set of hyperparameters. See @sec-param-set.
...
`mlr3` manages hyperparameters through the `r ref_pkg("paradox")` package, developed by the mlr3 team.
Hyperparameters affect the way a model is fit to the given data by a learner.
For example, the `xval` hyperparameter for the `rpart` learner controls the number of internal cross-validations.
The `mlr3` universe provides information on possible hyperparameter settings for all its learners, which has the following advantages:
...
1. Hyperparameters can be tuned automatically; covered in [Hyperparameter Tuning](#tuning).

The following table lists the available hyperparameter types, all of which derive from the `r ref("Param")` base class that represents a single hyperparameter.
...
Individual hyperparameters can be combined into `r ref("ParamSet")` objects.
Every [learner](#sec-learners) has a `r ref("ParamSet")` that defines the hyperparameter values to use when training a model.
You can access this object using the `$param_set` field.
...
Most learners do not have hyperparameter values set unless explicitly specified when instantiating the learner, i.e. the *value* column is empty.
Some learners, such as `r ref("LearnerClassifRpart")`, automatically initialize some of their hyperparameters to values that differ from the default, as shown above.

In more complex hyperparameter spaces, there may be additional columns.
For example, the hyperparameter set of `r ref("LearnerClassifSVM")` from the `r mlr3learners` package has conditional dependencies:

```{r}
lrn("classif.svm")$param_set
```

The additional column *parents* indicate which other hyperparameters a `r ref("Param")` depends.
The field `$deps` lists the hyperparameters that have such dependencies, and the field `$cond` the conditional expressions:
...
For example, the hyperparameter `cost` can only be set if `type` is set to `"C-classification"`; otherwise it does not apply.
Similarly, the (polynomial) `degree` can only be specified if the `kernel` is `"polynomial"`.

#### Setting Hyperparameter Values {#sec-parameters-setting}

To change the hyperparameters of a learner, the `$values` field of the `r ref("ParamSet")` must be changed.
There are three ways to change the values of an existing hyperparameter set:
...
: Ways of setting hyperparameter values. {#tbl-parameters-configure}
...
We do not recommend using the second method as it clears all meta information (e.g. initial values; more on this here: `r ref_pkg("paradox")`).
We recommend the `$set_values()` method.

While there are different methods for changing parameter values, we recommend to use the `$set_values()` method.
...
The `r ref("lrn()")` function also accepts additional arguments to set hyperparameters when constructing it:
...
:::{.callout-note}
Details on the hyperparameters of our `rpart` learner can be found in the documentation of `r ref ("rpart::rpart.control()")`.
...
After the model has been created, we can use it to make predictions.
We can give the test partition to the `$predict()` function with the second argument being the testing set (`splits$test`) defined in `splits` by `partition()` earlier in this chapter.
This `$predict()` function takes the `row_ids` given by `splits$test` and returns predictions for those `row_ids` in the given task (first argument) and the model stored in the learner (`learner_rpart`).
...
The `r ref("Prediction")` object can easily be converted into a `data.table` or `data.frame` using the appropriate function:
```{r}
...
The learner does not need to know more metadata about this data to make predictions, as this was given when training the model.
...
The `rpart` learner we used above does not support predicting standard errors, so we use the `lm` model from the `r ref("mlr3learners")` package instead:
...
At the core of all performance measures is a quantification of the difference between the predicted value and the ground truth value (except for unsupervised tasks, see @sec-task-types-add).
...
The list of learners included in the `r mlr3` package is deliberately small to avoid large sets of dependencies for this core package:
...
* CART decision tree learner `r ref("mlr_learners_classif.rpart", text = "classif.rpart")`: Tree learner from `r ref_pkg("rpart")`.
* CART regression tree learner `r ref("mlr_learners_regr.rpart", text = "regr.rpart")`: Tree learner from `r ref_pkg("rpart")`.
...
The resulting `data.table` contains a lot of metadata that is useful for identifying learners that have particular properties.
...
1. Give the true positive, false positive, true negative, and false negative rates of the predictions made by the model in exercise 1.
1. Change the threshold of the model from exercise 1 such that the false positive rate is lower than the false negative rate.
