...
## Supported Learners {#sec-lrns-add}

`mlr3` supports many learners.
...
* Featureless learner `regr.featureless`/`classif.featureless`: Simple baseline regression/classification learner. The former predicts the mean of the target values in the training set for all new observations, the latter predicts the most frequent label. Featureless learners are commonly used as fallbacks (@sec-fallback).
* Classification and regression tree (CART) `regr.rpart`/`classif.rpart` from `r ref_pkg("rpart")`
* Debug learners `regr.debug`/`classif.debug` used to debug code (@sec-error-handling)

The `r mlr3learners` package contains cherry-picked implementations of the most popular ML methods:
