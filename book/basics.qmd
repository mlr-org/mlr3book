...
The training data is given to a machine learning algorithm, which is implemented within an object called a [learner](#sec_learners) in `r mlr3`.
...
By using the [learner](#sec_learners) with the training data, the user can model the relationship between the input features and the output target values.
This model can be saved in the [learner](#sec_learners) object and is then used to produce [predictions](#sec-predicting) on the test data, which are compared to the ground truth values to assess the quality of the model.
`r mlr3` offers a number of different measures to quantify how well a model performs based on the difference between predicted and actual values.
...
5. How to [train, predict, and assess performance](#sec-train-predict).
...
task_mtcars_smaller$cbind(data.frame(letters = letters[2:3])) # add column with letters
task_mtcars_smaller$data()
```
...
#### Plotting Tasks {#sec-regr-autoplot}
...
For regression tasks (inheriting from `r ref("TaskRegr")`), see the documentation `r ref("mlr3viz::autoplot.TaskRegr")` for the implemented plot types ("target" (default) and "pairs").
...
In this section, we explain how [tasks](#sec_tasks) and [learners](#sec_learners) can be used to train a model and predict on a new dataset. This process consists of three steps:
1. **Train** a [learner](#sec_learners) by optimizing the model parameters (i.e. m and b in y = mx + b) based on the given data (training set) to minimize error.
...
Similarly to [plotting tasks](#sec-regr-autoplot), `r mlr3viz` provides an `r ref("ggplot2::autoplot()", text = "autoplot()")` method for `r ref("Prediction")` objects.
