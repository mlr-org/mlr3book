...
Below we retrieve the task `tsk("mtcars")`, which uses the `r ref("datasets::mtcars")` dataset:
...
Running `tsk()` without any arguments lists all tasks in the dictionary, this also works for all other sugar constructors that you will encounter throughout the book.

:::{.callout-tip}
## Help pages

Usually in R, the help pages of functions can be queried with `?`.
The same is true of R6 classes, so if you want to find the help page of the `tsk("mtcars")` task you could use `?mlr_tasks_mtcars`.
...
To create your own regression task, you will need to construct a new instance of the `r ref("TaskRegr", aside = TRUE)`.
...
Printing a task provides a short summary, in this case we can see the task has `r tsk_mtcars$nrow` observations and `r tsk_mtcars$ncol` columns (32 x 3), of which `mpg` is the target, there are no special properties, and there are `r length(tsk_mtcars$feature_names)` features stored in double-precision floating point format.
...
tsk_mtcars_small$select(c("am", "carb")) # keep only these features
tsk_mtcars_small$filter(2:4) # keep only these rows
...
```{r basics-015}
# the wrong way
tsk_mtcars_small = tsk("mtcars")$filter(1:2)$select("cyl")
tsk_mtcars_wrong = tsk_mtcars_small
tsk_mtcars_wrong$data()
tsk_mtcars_wrong$filter(1)
# original data affected
tsk_mtcars_small$data()

# the right way
tsk_mtcars_small = tsk("mtcars")$filter(1:2)$select("cyl")
tsk_mtcars_right = tsk_mtcars_small$clone()
tsk_mtcars_right$data()
tsk_mtcars_right$filter(1)
...
In @sec-performance we will look at resampling and benchmark experiments, which will go into more detail about performance estimation, for now we will just discuss the simplest method of splitting data using the `r ref("partition()", aside = TRUE)` function.
...
The `lrn("regr.rpart")` learner we used above does not support predicting standard errors, so in the example below we will use a linear regression model implemented in `lrn("regr.lm")`, note how the output now includes standard errors.
...
The output above is a `r ref("paradox::ParamSet", aside = TRUE)` object from the package `r ref_pkg("paradox")`.
...
In the case of `lrn("regr.rpart")`, we change the `xval` default to `0` because `xval` controls internal cross-validations and if a user accidentally leaves this at 10 then model training can take a long time.
...
So by example:
```{r}
lrn_rpart_params = lrn("regr.rpart")
...
One such example is an SVM classifier, implemented in `lrn("classif.svm")`.
...
For regression, we have implemented the baseline `lrn("regr.featureless")`, which always predicts the mean of the target of the training data:
...
By example let us consider the mean absolute error (`msr("regr.mae")`).
...
To actually calculate model performance, we simply call the `r index("$score()", aside = TRUE, code = TRUE)` method of a `Prediction` object and pass as a single argument the measure (or measures passed as a list) that we want to compute.
Less abstractly:
...
For example, below we compute performance for mean squared error (`msr("regr.mse")`) and mean absolute error (`msr("regr.mae")`) -- note we use `r index("msrs()", aside = TRUE, code = TRUE)` to load multiple measures at once.
...
Next we loaded a featureless baseline learner (`lrn("regr.featureless")`) with the `lrn()` function.
Then loaded a decision tree (`lrn("regr.rpart")`) but changed the complexity parameter and max tree depth from their defaults.
We then used `msrs()` to load multiple measures at once, the mean squared error (MSE) (`msr("regr.mse")`) and the mean absolute error (MAE) (`msr("regr.mae")`).
...
In this experiment we loaded the predefined task `tsk("penguins")`, which is based on the `r ref("palmerpenguins::penguins")` dataset, then partitioned the data into training and test splits.
...
You can view the predefined classification tasks in `mlr3` by filtering the `mlr_tasks` dictionary, and you can create your own with `r ref("as_task_classif", aside = TRUE)`.
...
The `tsk("sonar")` task is an example of a binary classification problem, as it has two targets, in `mlr3` terminology it has the "twoclass" property:
...
In contrast, `tsk("penguins")` is a multiclass problem as there are more than two species of penguins, in `mlr3` terminology it has the "multiclass" property:
...
The accuracy measure evaluates the `"response"` predictions whereas the brier score (`msr("classif.mbrier")`) (squared difference between predicted probabilities and the truth) and logloss (`msr("classif.logloss")`) (negative logarithm of the predicted probability for the true class) are evaluating the probability predictions.

If no measure is passed to `$score()`, the default classification error (`msr("classif.ce")`) is calculated, which is the number of misclassifications divided by the number of predictions, i.e., 1 - `msr("classif.acc")`.

### `PredictionClassif`, Confusion Matrix, and Thresholding {#sec-classif-prediction}

`r ref("PredictionClassif", aside = TRUE)` objects have two important differences from the regression case.
...
You can visualize a confusion matrix with `r ref('autoplot.PredictionClassif', aside = TRUE)`.

```{r}
autoplot(prediction)
```

If we take `tsk_sonar$positive` (``r tsk_sonar$positive``) to be the positive class then the confusion matrix corresponds to `r index('true positives')` (top left), `r index('false positives')` (top right), `r index('false negatives')` (bottom left), and `r index('true negatives')` (bottom right) (see @fig-confusion):
...
As an example, let us take the `tsk("german_credit")` task in which 700 customers have good credit and 300 have bad.
...
preds = lrn_featureless$predict(task_credit, split$test)
...
Not all models can handle weights in the data so it is important to check a learner's properties to make sure this column role is being used as expected.
...
* Featureless learner (`lrn("regr.featureless")`/`lrn("classif.featureless")`), which are implemented in `r mlr3` and are baseline learners used for model comparison or as `r index('fallback learners')` (@sec-fallback). The former predicts the mean of the target values in the training set for all new observations, the latter predicts the most frequent label.
* Debug learners (`lrn("regr.debug")`/`lrn("classif.debug")`), which are implemented in `r mlr3` and used only to debug code (@sec-error-handling).
* Classification and regression trees (also known as CART) (`lrn("regr.rpart")`/`lrn("classif.rpart")`).
...
* Linear (`lrn("regr.lm"`) and logistic (`lrn("classif.log_reg"`) regression.
* Penalized Generalized Linear Models (`lrn("regr.glmnet")`/`lrn("classif.glmnet"`) and with built-in optimization of the penalization parameter (`lrn("regr.cv_glmnet")`/`lrn("classif.cv_glmnet"`).
* Weighted $k$-Nearest Neighbors regression (`lrn("regr.kknn")`/`lrn("classif.kknn"`).
* Kriging / Gaussian Process Regression (`lrn("regr.km"`).
* Linear (`lrn("classif.lda"`) and Quadratic (`lrn("classif.qda"`) Discriminant Analysis.
* Naïve Bayes Classification (`lrn("classif.naive_bayes"`).
* Support-Vector machines (`lrn("regr.svm")`/`lrn("classif.svm"`).
* Gradient Boosting (`lrn("regr.xgboost")`/`lrn("classif.xgboost"`).
* Random Forests for regression and classification (`lrn("regr.ranger")`/`lrn("classif.ranger"`).
