...
You may notice that we reference `mlr_tasks_mtcars` but run `tsk("mtcars")`.
This is because all object names in `mlr3` are standardized according to the convention: `mlr_objecttypes_key`.
Where `objecttypes` will be `tasks`, `learners`, `measures`, and others to be covered later in the book; and `key` refers to the ID of the object.
However to simplify the loading process, you only need to know the object key and the sugar function for construction.
e.g. `mlr_tasks_mtcars` / `tsk("mtcars")`;`mlr_learners_regr.rpart` / `lrn("regr.rpart")`; `mlr_measures_regr.rmse` / `msr("regr.rmse")`.
:::
...
All this information is encapsulated in the `r ref("Measure")` object.
...
`mlr3` simplifies the process of bringing these quantities together by storing the predictions and true outcomes in the `r ref("Prediction")` object as we have already seen.
...
In @sec-benchmarking we will see how to formalize comparison between models in a more efficient way using `r ref("benchmark()")`\index{benchmark()}.
...
For example, predicting the species of penguin from it's physical characteristics would be a classification problem as there are only a finite number of species.
...
As before we will start tasks, then move to learners, and finally evaluation.


### Classification Tasks {#sec-classif-tsks}

As with regression, we have several predefined classification tasks that can be loaded.
You can quickly view the available classification tasks by filtering the `r ref('mlr_tasks')` dictionary.

```{r basics-040}
as.data.table(mlr_tasks)[task_type == "classif"]
```

We will use the `r ref("datasets::penguins")` dataset as a running example, which comes with `mlr3` as `r ref('mlr_tasks_penguins')`:
...
Now we are in the classification setting, the target variable is of type factor and will have a limited number of possible classes/categories that observations can fall into:
