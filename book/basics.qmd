...
## Hyperparameters {#sec-parameters}

The `r ref_pkg("paradox")` package (developed by the mlr3 team) implements a system for representing hyperparameter spaces.
Hyperparameters are mutable values that effect the way that a [learner](#sec-learners) creates a model.
An example of a hyperparameter is the maximum depth of a decision tree.
Representing hyperparameters using the `r ref_pkg("paradox")` package provides the following advantages:

1. Available hyperparameter settings, their data types and accepted ranges can easily be listed without crawling through the respective manual pages.
1. Hyperparameters and their ranges can be defined in a search space for optimization algorithms (this subject will be covered in [Hyperparameter Tuning](#tuning)).

There are two classes within the `r ref_pkg("paradox")` package that we will cover:

* `r ref("Param")`: representing a single hyperparameter
* `r ref("ParamSet")`: which is a container object that combines multiple `r ref("Param")`s.

The following table lists the available hyperparameter types.

| Parameter Class    | Description                          |
| :-----------------: | :----------------------------------: |
| `r ref("ParamDbl")` | Real-valued (Numerical) parameters   |
...
Individual hyperparameters can be combined into `r ref("ParamSet")` objects. Every [learner](#sec-learners) has a `r ref("ParamSet")` that defines the values it uses when training a model. The user can access this object using the `$param_set` field.

To illustrate how to work with `r ref("ParamSet")`s, we will be using the hyperparameter set of `r ref("LearnerClassifSVM")` from the `r mlr3learners` package.
For simplicity, we consider only a subset of the hyperparameters of the `r ref("LearnerClassifSVM")`. 
We can obtain this subset by calling the method `$subset()` with the ids of the hyperparameters we want to keep.
```{r}
svm = lrn("classif.svm")
all_params = svm$param_set
subset = all_params$subset(c("cost", "kernel", "degree", "shrinking", "type"))
subset
```
...
The *parent* column indicates on which other hyperparameters a `r ref("Param")` depends. 
For example, the hyperparameter `cost` can only be set if `type` is set to `"C-classification"`, because the hyperparameter does otherwise not apply.
Similarly, the (polynomial) `degree` can only be specified if the `kernel` is `"polynomial"`.

The *value* column indicates to which value the hyperparameters are set.
This hyperparameter set has none of its values set after initialization.
...
When using any of these methods, the new hyperparameter setting is checked to make sure it falls in the allowed range/options.

These methods are demonstrated here:

```{r}
# Changing a single hyperparameter
subset$values$shrinking = FALSE

# Changing all hyperparameter values
subset$values = list(
  shrinking = TRUE, 
...
# Changing only the provided values
subset$set_values(cost = 3, shrinking = TRUE)
```

We do not recommend using the second method since it effectively clears all meta information (like inital values, read more about this here: `r ref_pkg("paradox")`).
