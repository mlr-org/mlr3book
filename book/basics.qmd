...
Subsetting by features (columns) is possible with `r index("$select()", code = TRUE, aside = TRUE)` with the desired feature names passed as a character vector.
Subsetting by observations (rows) is performed with `r index('$filter()', aside = TRUE, code = TRUE)` by passing the row IDs as a numeric vector.
...
To add extra rows and columns to a task, you can use `r index("$rbind()", code = TRUE, aside = TRUE)` and `r index('$cbind()', aside = TRUE, code = TRUE)` respectively:
...
As with `Learner`s we can view it's hyperparameters with `r index("$param_set", code = TRUE)`:
...
Whilst the choice of positive and negative class is arbitrary, it is essential to ensuring results from models are interpreted as expected -- this is best demonstrated when we discuss thresholding (@sec-classif-prediction).
...
Classification measures, which are of class `r ref('MeasureClassif', aside = TRUE, index = TRUE)` are almost identical to those for regression and again the key difference is that predict types are more important to pay attention to  ensure you are evaluating the 'correct' predictions.
To evaluate `"response"` predictions, then you will need to use measures with `predict_type = "response"`, or to evaluate probability predictions then you will require `predict_type = "prob"`.
...
Now we can evaluate the quality of our probability predictions and we can also simultaneously our response predictions:
...
Unlike the accuracy measure, the brier score (`classif.mbrier`) and logloss (`classif.logloss`), are hard to interpret by themselves and are therefore primarily used to compare multiple learners (@sec-performance).
...
A `r index('confusion matrix', aside = TRUE)` is a popular way to show the quality of classification (response) predictions by seeing if a model is good at (mis)classifying observations in a particular class.
...
We will return to these in greater detail in @sec-roc.

```{r}
autoplot(predictions)
```

Note that in a binary classification case, the confusion matrix corresponds to true positives (top left), false positives (top right), false negatives (bottom left), and true negatives (bottom right) (see @fig-confusion):
...
For `n` classes, with predicted probabilities $p_1,...,p_n$, this is the same as saying `response` = $max\{p_1,...,p_n\}$.
If the maximum probability is not unique, i.e., multiple classes are predicted to have the highest probability, then the response is chosen randomly from these classes.
...
This 50% value is known as the `r index('threshold', aside = TRUE)` and it can be useful to change this threshold if there is class imbalance.
`r index('Class imbalance', aside = TRUE)` occurs when one or more classes are over- or under-represented in a dataset.
For example, say 700 people have good credit and 300 have bad (the situation in the `r ref("mlr_tasks_german_credit")` task) then the 'good' class is overrepresented.
In fact, a model that always predicts a customer has good credit will achieve 70% accuracy, thus making it appear like a 'good' model on the surface, when in fact it just ignores all 'bad' customers -- this can create very big problems in healthcare and other settings where there are data biases.
Thresholding is a way to correct this class imbalance by allowing classes to be selected with a lower probability threshold, so instead of predicting a customer has bad credit if P(good) < 50%, instead we might predict bad credit if P(good) < 70% -- notice how we write this in terms of the positive class, which in this task is 'good'.
...
preds = lrn_rpart$train(task_credit, splits$train)$predict(task_credit, splits$test)
title = sprintf("Threshold = 0.5; Accuracy = %s",
  round(preds$score(msr("classif.acc")), 2))
before = autoplot(preds) + ggtitle(title)
preds$set_threshold(0.75)
title = sprintf("Threshold = 0.7; Accuracy = %s",
  round(preds$score(msr("classif.acc")), 2))
after = autoplot(preds) + ggtitle(title)
before + after + plot_layout(guides = "collect")
```

Our model now captures the relationship between classes more accurately, just by adjusting the threshold.
