...
These building blocks include the data (and the methods of creating training and test sets), the machine learning algorithm (and its training and prediction process), the configuration of a machine learning algorithm through its hyperparameters, and evaluation measures to assess the quality of predictions.
...
In addition to the parameters that are fit to the data, most learners also have hyperparameters that change how the model is fit.
The proper choice of these hyperparameters can be -- depending on the algorithm -- be almost as important as the choice of the learner itself.
In @sec-parameters we will give a brief introduction on how to configure the hyperparameters of a learner, which is done using the `r paradox` package.
...
The performance of a model, quantified by the value of the loss function when evaluated on new data, is called the estimated of the generalization error -- how well do we expect this model to do in general?
...
If no `id` is given provided, the deparsed name of the data will be used (an R way of turning data into strings).
...
While there are different methods for changing parameter values, it is recommended to use the `$set_values()` method.

learner_rpart$param_set$set_values(minsplit = 10)
learner_rpart$param_set$values


More information on how to set parameters can be found in the #sec-parameters.
...
The `r ref_pkg("paradox")` package (developed by the mlr3 team) implements a language for systematically representing parameter spaces that is used throughout the mlr3 universe.
One of the most important use cases is to handle the hyperparameter spaces for objects like learners, which we have already encountered in @sec-training.
...
This section will start with teaching you the basics of what these objects are and how to set their values.
We will also briefly cover the conceptual difference between initial parameter values and default values.
Information on defining parameter sets an be found in @sec-learner-search-space, where they are used to define search spaces for optimization.
Furthermore, chapter @sec-extending gives additional details that are relevant when adding new learner classes.

To illustrate how to work with `r ref("ParamSet")`s, we will be using the parameter set of `r ref("LearnerClassifSVM")` from the `r mlr3learners` package.
...
For more on that, see @sec-parameters-defaults.
...
Parameters have different classes, depending on the type of value they represent. 
The kernel parameter above takes categorical values, which corresponds to the `r ref("ParamFct")` class.

```{r}
class(param_set$params$kernel)
```

An overview of existing classes is in @tbl-parameters-classes.

| Parameter Class    | Description                          |
| :-----------------: | :----------------------------------: |
| `r ref("ParamDbl")` | Real valued parameter ("double")     |
| `r ref("ParamInt")` | Integer parameter                    |
| `r ref("ParamFct")` | Discrete valued parameter ("factor") |
| `r ref("ParamLgl")` | Logical / Boolean parameter          |
| `r ref("ParamUty")` | Untyped parameter                    |

: Parameter Classes and the type of value they represent. {#tbl-parameters-classes}

### Setting Parameter Values {#sec-parameters-setting}

The most common user-interaction with a `r ref("ParamSet")` is to change its `$values`, e.g. for the purpose of changing the hyperparameters.
...
| `param_set$values = list(<par1> = <value1>, ...)`           | Replace all parameter values with the given list  |
| `param_set$set_values(<par1> = <value1>, ...)`              | Changes only the provided values                    |

: Different ways of setting parameter values for an existing parameter set {#tbl-parameters-configure}
...
We can modify a single parameter as follows

```{r}
#| error: true
# Shrinking must be logical
param_set$values$shrinking = "FALSE"
param_set$values$shrinking = FALSE

# Parameter cost depends on type being "C-classification"
param_set$values$cost = 2
param_set$values$type = "C-classification"
param_set$values$cost = 2
```

If we want to change all parameter values, we can also assign a new list. 
Be aware that this will remove all previously set parameters, i.e. the parameter `cost` does not have a value assigned.

```{r}
param_set$values = list(shrinking = TRUE, type = "C-classification")
param_set$values
```

A convenient way to change multiple parameter values, while keeping previously set parameters is the `$set_values()` method.
Below, one can observe that the previously set value of parameter `type` is preserved.

```{r}
param_set$set_values(cost = 3, shrinking = TRUE)
param_set$values
```

In the next section we will show why we recommend using the third of these options.


### Initial Values vs. Defaults {#sec-parameters-defaults}

Parameter sets can have **initial values** (that are not set by the user), as well as **default values**.
An example for such a learner is the `r ref("LearnerClassifRpart")`.
Its parameter `xval` has a default of of 10, but its value is initialized to 0.
...
Because the value for parameter `xval` has been initially set to 10, the `r ref("LearnerClassifRpart")` will not run with the default values.
...
This is not the case, when using method 1 and 2 from @tbl-parameters-configure.
