...
```{r basics-010}
c(Features = task_mtcars$feature_names,
  Target = task_mtcars$target_names)
```
...
To access the hyperparameters in the decision tree, we use `r index("$param_set", "Learner!$param_set", aside = TRUE, code = TRUE)`:
...
To set multiple values at once we recommend either setting these in construction or using `r index("$set_values()", "Learner!$set_values()", aside = TRUE, code = TRUE)`, which updates the given hyperparameters (argument names) with the respective values.
...
measures = msrs(c("regr.mse", "regr.mae"))
# train learners
featureless$train(tsk_mtcars, splits$train)
rpart$train(tsk_mtcars, splits$train)
# make and score predictions
featureless$predict(tsk_mtcars, splits$test)$score(measures)
rpart$predict(tsk_mtcars, splits$test)$score(measures)
```
...
If no measure is passed to `$score()`, the default is the `r index('classification error', parent = "measures")` (`"classif.ce"`), which is the number of misclassifications divided by the number of predictions, i.e., $1 -$ `"classif.acc"`.
...
Firstly, the added field `$confusion`, and secondly the added method `r index('$set_threshold()', "PredictionClassif!$set_threshold()", code = TRUE)`.
