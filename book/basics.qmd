...
`r mlr3` encapsulates data in [tasks](#sec-tasks), which also contain meta-data like the type of machine learning task (regression, classification, etc.), which column is the target (the feature that we want to predict with new data), and which rows are considered part of the training set and which are part of the test set (this concept is explored further in the [resampling](#resampling) section).
`r mlr3` uses [learners](#sec-learners) to provide a unified interface to popular machine learning algorithms. 
...
Usually, the value of a [measure](#sec-measure) is a numeric score and is called the estimate of the generalization error -- given new data that we have not seen before, how well do we estimate the learned model to perform?
...
:::{.callout-tip}
Thousands more datasets are readily available via `r link("https://openml.org", "Openml.org")` [@openml2013] and `r ref_pkg("mlr3oml")`.
For example, to download the dataset `r link("https://www.openml.org/search?type=data&id=44994", "cars")` with data id `44994` and automatically convert it to a regression task, all you need to do is:

```{r basics-005, eval = FALSE}
library("mlr3oml")
tsk("oml", task_id = 44994)
```
:::
