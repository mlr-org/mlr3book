...
Splitting data into training and test sets, building a model, and evaluating it can be repeated several times, resampling different training and test sets from the original data each time (@sec-resampling).
Multiple resampling iterations allow us to get a better and less biased generalizable performance estimate for a particular type of model.
...
The columns (some not shown for space) `"lgl"` (`r ref("logical")`), `"int"` (`r ref("integer")`), `"dbl"` (`r ref("double")`), `"chr"` (`r ref("character")`), `"fct"` (`r ref("factor")`), `"ord"` (`r ref("ordered", text = "ordered factor")`) and `"pxc"` (`r ref("POSIXct")` time) show the number of features in the task of the respective type.
...
Fit the classification tree using the training set of the task by calling the `$train()` method of `r ref("Learner")`.
...
```{r basics-051, message = FALSE, warning = FALSE}
task = tsk("penguins")
