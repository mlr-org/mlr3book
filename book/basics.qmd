...
A `r index('model', aside = TRUE)` is formally a mapping from a feature vector to predictions,
<!-- FIXME - MAKE CLEAR DIFFERENCE BETWEEN MODEL AND LEARNER; WE LEARN A MODEL FROM DATA -->
...
<!-- FIXME - WHISTLE STOP? WHISTLE-STOP? -->
This whistlestop tour of machine learning provides the basic knowledge required to use software in `mlr3` and is summarized in @fig-ml-abstraction-basics.
...
:::{.callout-tip}
## Help pages
<!-- FIXME - CLEANUP TEXT, MAKE EASIER TO NAME -->
...
However, for the underlying class you can use more standard help methods: `?mlr_tasks_mtcars`, or `help(mlr_tasks_mtcars)`.
:::
...
In the next chapter we will look at resampling and benchmark experiments, which will go into more detail about performance estimation, but for now we will just discuss the simplest method of splitting data using the `r ref("partition()", aside = TRUE, index = TRUE)` function.
...
<!-- FIXME: Add motivational sentence about HPO -->
...
<!-- FIXME: UPDATE EVERYWHERE HYPERPARAMTERS NOT IN LEARNERS -> CONTROL PARAMETERS/SETTINGS -->
...
<!-- FIXME: Reduce to a sentence, probably under selected_features -->
...
<!-- FIXME: Add section about baselines for i) fallbacks; ii) baseline comparisons -->
...
<!-- FIXME: REMOVE BOSTON HOUSING EVERYWHERE -->
...
<!-- FIXME: Briefly explain scoring rules, don't say hard to interpret, can say more complex. -->
...
We will return to the concept of binary misclassification in greater detail in @sec-roc.
...
<!-- FIXME: Just reframe in terms of cost-sensitivity -->
...
<!-- FIXME - More openly say that we as a team selected popular algorithms and popular impleemntations and that we recommend these as a good starting point -->
...
<!-- FIXME - TABLE MISSING AS_TASK, AS.DATA.TABLE() -->
