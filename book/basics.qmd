...
We can use the function `r ref("as_task_regr()")` to convert our `data.frame()` in `data` to a regression task, specifying the target feature in an additional argument:
...
The `print()` method gives a short summary of the task: it has `r task_mtcars$nrow` observations and `r task_mtcars$ncol` columns, of which `mpg` is the target and `r length(task_mtcars$feature_names)` are features stored in double-precision floating point format.
...
For more info on connecting to data bases, see [backends](#backends).
:::
...
They are available through the `mlr_learner` dictionary.
Unlike tasks, only supported learners can be used in `r mlr3`.
The list of learners supported in `r mlr3` is deliberately small; support for additional learners is provided by the `r mlr3learners` and `r mlr3extralearners` packages.

Learners encapsulate methods to train a model and make predictions using it given a `r ref("Task")` and provide meta-information about the learners, such as their hyperparameters (which control the behavior of the learner).
...
To retrieve a `r ref("Learner")` from the `mlr_learner` dictionary, use the function `r ref("lrn()")`:
...
The `rpart` learner we used above does not support predicting standard errors, so we use the `lm` linear model instead:
...
We can access predictions and standard error directly:

```{r basics-034}
# directly access the predicted quantities
predictions$response

# directly access the standard error
predictions$se
```

### Plotting Predictions {#sec-autoplot-prediction}

Similar to [plotting tasks](#autoplot-task), `r mlr3viz` provides an `r ref("ggplot2::autoplot()", text = "autoplot()")` method for `r ref("Prediction")` objects.
...
Classification tasks (`r ref("TaskClassif")`) involve a discrete, categorical target.
...
`r ref("datasets::penguins", text = "penguins")` can be retrieved using the sugar function `tsk()`.
...
The target variable, `r penguins_task$target_names`, is of type factor and has the following three class labels (levels):

```{r basics-042}
unique(penguins_task$data(cols = "species"))
```

#### Plotting Tasks {#sec-classif-autoplot}

Classification tasks (inheriting from `r ref("TaskClassif")`) have three built-in plotting options ("target" (default), "duo", "pairs").
See the documentation of `r ref("mlr3viz::autoplot.TaskClassif")` for the implemented plot options.
For simplicity, we subset the task to only the first 3 features and show the pairs plot (requires package GGally).

```{r basics-043, warning = FALSE, message = FALSE}
library("mlr3viz")

penguins_task$select(head(penguins_task$feature_names, 3))
autoplot(penguins_task, type = "pairs")
```

### Classification Learners {#sec-classif-lrns}

The classification learner `r ref("LearnerClassif")` inherits from the base class `r ref("Learner")`.
...
For classification problems there are two predict types:

1. `response` - The prediction will return a predicted class label for each observation.

2. `prob` - The prediction will return a probability for each class label and the class label with the highest probability for each observation.

To determine what predict type the learner is use `$predict_type`. To change the learner predict type set `$predict_type` to your desired type ("response" or "prob").

```{r basics-045}
rpart_learner$predict_type = "prob"
rpart_learner$predict_type
```

When the learner is trained and tested (covered in the [Learners](#sec-learners) section) we will see the difference.

```{r basics-046}
splits = partition(penguins_task, ratio=0.8)
rpart_learner$train(penguins_task, row_ids = splits$train)
prediction = rpart_learner$predict(penguins_task, row_ids = splits$test)
prediction
```

### Classification Evaluation {#sec-classif-eval}

Evaluation measures for classification problems that are built-in to `r mlr3` can be found in `mlr_measures`.

```{r basics-047}
data.frame(measures=head(mlr_measures$keys("classif")))
```

Some of these measures require that the [predict type](#sec-classif-pred_type) be either "response" (ex. `classif.acc`) or "prob" (ex. `classif.auc`).
Check this property with `$predict_type`.
...
The confusion matrix is a method unique to classification performance evaluation. 
The rows in a confusion matrix are the predicted class and the columns are the true class.
Therefore, all off-diagonal entries are misclassifications, and all diagonal entries are correctly classified.
...
In this case, our classifier does fairly well at correctly classifying the penguins.
...
sonar_task = as_task_classif(Sonar, target = "Class", positive = "R")

# switch positive class to level 'M'
sonar_task$positive = "M"
```

### Thresholding {#sec-thresholding}

Models trained on binary classification tasks that predict the probability for the positive class usually use a simple rule to determine the predicted class label.
If the probability is more than 50%, predict the positive label; otherwise, predict the negative label.
...
splits = partition(sonar_task, ratio=0.8)
rpart_learner$train(sonar_task, row_ids = splits$train)
prediction = rpart_learner$predict(sonar_task, row_ids = splits$test)
autoplot(prediction)
...
Through the `autoplot()` visualization you can see that after the threshold was changed, the positive class M is predicted more often.
