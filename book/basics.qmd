...
```{r basics-010}
c(Feature = task_mtcars$feature_names, Target = task_mtcars$target_names)
```
...
```{r basics-016, eval = TRUE}
# select the 10th row of the task by extracting the second row_id:
task_mtcars$data(rows = task_mtcars$row_ids[10])
```
:::
...
As `R6` uses reference semantics (@sec-r6), you need to use `$clone()` if you want to modify a task but keep the original object as well.
...
learner_rpart$train(task_mtcars)
```
...
We see that the regression tree has identified features in the task that are predictive of the target (`mpg`) and used them to partition the observations.
...
Predicting from trained models is as simple as passing your task to the `r index("$predict()", aside = TRUE, code = TRUE)` method of the trained `Learner`.
...
The true values are `NA`, because we did not include an `mpg` column in the generated data.
...
Finally, to set multiple values at once we recommend either setting these in construction or using `param_set$set_values()`.
...
To view exactly what the dependency is we can use `$deps`, this returns a `data.table` which can be queried in the usual way.
...
Similarly, the `coef0` parameter should be set only if the kernel is `"polynomial"`, `"radial"`, or `"sigmoid"`.
...
For regression, we have implemented the baseline `regr.featureless`, which by by default predicts the mean of the target of the training data (but can also be configured to predict the median):
...
`r mlr3` also provides measures that do not quantify the quality of the predictions of a model, but instead provide 'meta' information about the model, in particular these include:
...
So we can now score our decision tree to see how long it took to train the model and then make predictions:
...
measures = msrs(c("regr.mse", "regr.mae"))
# train both learners on the train set
featureless$train(task, splits$train)
rpart$train(task, splits$train)
# make and score predictions on the test set
...
In this experiment we loaded the regression task `mtcars` with `tsk()` and then split this using `partition` with the default 67/33 split.
...
Finally we made predictions from our trained models and scored these, note how we use 'method chaining', which is an R6 technique to combine multiple methods (called with `$()`) in a row on the same line (this only works if the method returns the object itself).
...
As we have now seen the similarities between classification and regression, we can turn to some key differences.
...
The `sonar` task (`r ref('mlr_tasks_sonar')`) is an example of a binary classification problem, as the target can only take two different values, in `mlr3` terminology it has the "twoclass" property:
...
To evaluate probability predictions you will require `predict_type = "prob"` (which also includes the response prediction).
...
If no measure is passed to `$score()`, by default the classification error (`classif.ce`) is calculated, which is the number of misclassifications divided by the number of predictions, i.e., 1 - `classif.acc`.
...
You can visualize the distributions of true and predicted classes using `r ref('autoplot.PredictionClassif', aside = TRUE)`.

```{r}
autoplot(predictions)
```

For binary classifcation problems the entries in the confusion matrix even have designated names.
...
As an example, let us take the `german_credit` task in which 700 customers have a good and 300 a bad credit score.
