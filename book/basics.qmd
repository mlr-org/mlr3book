...
`r index('Supervised learning', aside = TRUE)` is a subfield of machine learning in which datasets consist of observations (rows in tabular data) that are labeled, which means that each data point includes `r index('features')` (columns in tabular data) and a quantity that we are trying to predict, also called a `r index('target')`.
...
`r mlr3` includes a few predefined machine learning tasks in the `r ref("mlr_tasks", aside = TRUE, index = TRUE)` `Dictionary`.
...
All object names in `mlr3` are standardized according to the convention: `mlr_<types>_<key>`.
...
To simplify the process of loading objects, you only need to know the object key and the sugar function for construction.

For example: `mlr_tasks_mtcars` / `tsk("mtcars")`;`mlr_learners_regr.rpart` / `lrn("regr.rpart")`; `mlr_measures_regr.rmse` / `msr("regr.rmse")`.
:::
...
Objects of class `r ref("Learner", index = TRUE, aside = TRUE)` provide a unified interface to many popular machine learning algorithms in R.
The `r ref("mlr_learners", index = TRUE, aside = TRUE)` dictionary contains all the learners available in `mlr3`, we will discuss the available learners in @sec-lrns-add, for now we will just use a decision tree regression learner as an example to discuss the `Learner` interface.
...
In the next chapter we will look at resampling and benchmark experiment which will go into more detail about unbiased estimation but for now we will just discuss the simplest method of splitting data using the `r ref("partition()", aside = TRUE, index = TRUE)` function.
...
The final step of the basic ML workflow (@fig-ml-abstraction) is to evaluate the quality of predictions to see if our trained model is any 'good'.
...
But first we will cover the final element that makes ML models powerful predictive tools, which is their hyperparameters that give users control over the fitting and predicting process and ultimately result in more powerful models.
...
All measures implemented in `mlr3` are defined primarily by three properties: 1) the function that defines the measure; 2) whether a lower or higher value is consider 'good'; and 3) the range of possible values the measure can take.
As well as these defining elements, other metadata are important to consider when selecting and using a `Measure`, including if the measure has any special properties (e.g., requires training data), the type of predictions the measure can evaluate, and whether the measure has any hyperparameters.
...
The measure has no special properties (`Properties: -`), it evaluates `response` type predictions for regression models (`Predict type: response`), and it has no hyperparameters (`Parameters: list()`).
...
Note that all task types have default measures that are used if the argument to `$score()` is omitted, for regression this is the mean squared error (`regr.mse`).
...
We will look at `selected_features` in the next section as it includes hyperparameters, which are worth separately discussing first.

### Measure hyperparameters

As with model hyperparameters, measure hyperparameters control how measures are calculated.
Hyperparameters are rare in measures, in fact we do not implement any regression measures with hyperparameters.
Therefore we will use `selected_features` as an example of a measure with hyperparameters.
As with `Learner`s we can view it's hyperparameters with `r index("$param_set")`:
...
This measure calculates how many features were selected as important by the learner.
...
Note that we passed the task and learner as the `selected_features` measure requires these to pull out the selected features from the learner:

```{r}
measure_sf$properties
```
...
In this experiment we loaded the regression task `r ref("mlr_tasks_boston_housing")` with `tsk()` and then split this using `partition` with the default 70/30 split.
...
We then used `msrs()` to load multiple measures at once, the mean squared error (`regr.mse`) and the mean absolute error (MAE) (`regr.mae`).
...
We loaded the featureless classification baseline and a classification decision tree, then the accuracy measure, trained our models then made predictions and scored them.
...
In binary classification, as there are only two possible class types, by convention one of these is known as the `r index('positive class', aside = TRUE)` and the other as the `r index('negative class', aside = TRUE)`; it is arbitrary which is which.
...
Whilst the choice of positive and negative class is arbitrary, it is essential to ensuring results from models are interpreted as expected.

```{r}
splits = partition(task_sonar)
lrn_rpart = lrn("classif.rpart", predict_type = "prob")
task_sonar$positive = "M"
lrn_rpart$train(task_sonar, splits$train)$predict(task_sonar, splits$test)
...
library(mlr3viz)
autoplot(task_penguins, type = "duo")
```
...
However, a key difference is that the possible prediction types in classification are either `"response"` -- predicting an observation's class (a penguins Species in our example) -- or `"prob"` -- predicting the probability of an observation being in each class.
In classification, the latter is more informative as it provides more information about the prediction:
...
Classification measures, which are of class `r ref('MeasureClassif', aside = TRUE, index = TRUE)` are again almost identical to those for regression,

If we want to assess the quality of our probability predictions, we will need to use a measure that evaluates these prediction types, i.e., where the measure predict type is also "prob".
...
Now we can evaluate the quality of our probability predictions, and we can also simultaneously evaluate the quality of our response predictions.
...
Unlike the accuracy measure, the brier score (`classif.mbrier`) and logloss (`classif.logloss`), are hard to interpret by themselves and therefore primarily used to compare multiple learners.

### `PredictionClassif` and Thresholding

`r ref("PredictionClassif", index = TRUE, aside = TRUE)` objects are nearly identical to the regression case.
However, we have added one important method, `r index("$confusion", aside = TRUE, code = TRUE)` that returns the confusion matrix from predictions:

```{r basics-049}
predictions$confusion
```

This is a popular way to show the quality of classification (response) predictions.
...
Confusion matrices are useful for identifying if a model is particularly good at (mis)classifying observations in a particular class.
...
Classification problems with targets that can only be one of two classes, are called `r index('binary classification', aside = TRUE)` tasks.
In binary classification, one of the classes is known as the `r index('positive class', aside = TRUE)`, and the other is the `r index('negative class', aside = TRUE)`.
During construction, the positive class can be specified with the `positive` argument, if this is omitted then the default is the first level of the target feature:

```{r basics-050}
# create a dataset with factor target
data = data.frame(x = runif(5), y = factor(c(0, 1, 0, 0, 1)))
# specifying the positive class:
as_task_classif(data, target = "y", positive = "1")$positive

# default is first class, which here is "0"
classif_task = as_task_classif(data, target = "y")
classif_task$positive

# changing after construction
classif_task$positive = "1"
classif_task$positive
```

Other than this small difference, binary classification tasks are identical to multiclass classification.

### Thresholding

We saw in `r index('multiclass classification')` that the `response` was calculated as the class that has the highest predicted probability.
In binary classification terms, this is simply saying that the positive class will be predicted as the `response` if the predicted probability of being in the class is greater than or equal to 50%.
This 50% value is known as the `r index('threshold', aside = TRUE)` and it can be useful to change this threshold if classes are unbalanced, i.e., where one class is more prevalent than another.

We will first look at thresholding in the simpler binary classification setting and then with multiclass classification.
For both examples we will demonstrate the effect of class imbalance and thresholding using a featureless classifier (`classif.featureless`) using the weighted sampling method, which means the model will simply predict the probability of each class as the probability observed in the training set, e.g. if a model is predicting the colour of an apple and is trained on 100 apples where 10 are green and 90 are red, then going forward the model will predict always predict $P(red) = 0.9, P(green) = 0.1$.

#### Binary classification thresholding {.unnumbered .unlisted}

In the "german_credit" dataset, the credit risk is 'good' for far many more observations and classifiers will therefore commonly overpredict this class.
...
table(task_credit$truth())
splits = partition(task_credit)
learner = lrn("classif.rpart", predict_type = "prob")
learner$train(task_credit)
predictions = learner$predict(task_credit)
predictions$score(msrs(c("classif.acc", "classif.mbrier")))
```

On the surface our classifier may seem to perform well as it has 70% accuracy.
However, on closer inspection we can see that this is simply because it always predicts every customer will have 'good' credit, which will be true 70% of the time due to imbalance in the data.

```{r}
autoplot(predictions)
```

`r index('Thresholding', aside = TRUE)` allows us to change the prediction threshold after predictions are made without having to retrain the model.
This is performed by calling the `r index('$set_threshold()', aside = TRUE, code = TRUE)` method on a `r ref("PredictionClassif")` object.
So now see what happens if we change our threshold to 70%, i.e., where the positive class is only prediction if $P(positive > 70\%)$:
...
predictions$score(msrs(c("classif.acc", "classif.mbrier")))
autoplot(predictions) +
  ggplot2::ggtitle(sprintf("Accuracy = %.0f%%", (1 - predictions$score())*100))
```

Overall our model has performed worse as the accuracy has dropped 20%.
But, this is still more useful as we can now start to make accurate predictions for those with 'bad' credit.

In @sec-pipelines we will see how to automatically choose and set thresholds and in @sec-cost-sens we will look at `r index('cost-sensitive classification')` where each class has a different associated cost.

#### Multiclass classification thresholding {.unnumbered .unlisted}

In multiclass classification, thresholding works slightly differently. First, the probability for a data point for each class is divided by each class threshold resulting in `n` ratios for `n` classes, then the highest ratio is selected (ties are random by default).

To make this clearer we can look at an example.
Say we are predicting if a new observation will be of class A, B, C, or D and we have predicted $P(A = 0.2), P(B = 0.4), P(C = 0.1), P(D = 0.3)$.
For now we will assume that the threshold for all classes is identical, therefore the class thresholds are all $1/4 = 0.25$.

```{r}
probs = c(0.2, 0.4, 0.1, 0.3)
thresholds = c(A = 0.25, B = 0.25, C = 0.25, D = 0.25)
probs/thresholds
```

We will therefore predict our observation is of class B as this is the highest ratio.
However, we could change our thresholds so we want D to be most likely to be predicted, A to be least likely, and B and C to be equal.
...
In `r mlr3`, the same principle is followed with `$set_threshold` by passing a named list to update thresholds from the default, this is demonstrated below with `r ref('mlr_tasks_zoo')`.
...
In the example below we set the new thresholds to be the proportions of each class in the training set, doing so effectively sets the thresholds by the inverse probability of occurring, so classes that are more common have lower thresholds and vice versa.
...
Now the thresholds have been adjusted, there is a much better correspondence between true and predicted labels.
...
We first introduced basic machine learning methodology and then showed how this is implemented in `r mlr3`
...
| --------------------------- | --------------------- | -------------------------------------------- |
| `r ref("Task")` | `r ref("tsk()")`/`r ref("tsks()")` | `$filter()`/`$data()` |
