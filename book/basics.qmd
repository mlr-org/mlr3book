...
The goal is to build a model that can capture the relationship between feature and target variables, in order to make accurate predictions of the target from new, unseen data.
...
Many ML algorithms make use of the concept of `r define("empirical risk minimization")` during training (@sec-training) to produce a fitted model, which means they optimize a loss function (the difference between true and predicted values) on the training data over all candidate models.
...
Note that empirical risk is only an estimate of the true generalization error, or in simpler terms, it is our best guess as to the future error of the predictions from a trained model on new data.

In `r mlr3`, the model constructed by a learner during training is directly saved in the `r ref("Learner")` object and can be used to produce predictions (@sec-predicting) on the test data.
In general, a model is onlly useful if it generalizes beyond limited training data. Therefore, separate test data is used to evaluate models in an unbiased way and assess to what extent they have learned the relationships that underlie the data, i.e., tests how well the model makes predictions on new data as an estimate of future performance.
This forms the basis of model `r index("evaluation")` (@sec-eval), where a model's performance is evaluated using a performance measure, which is usually a loss function.
