...
`r index('Supervised learning')` is a subfield of ML in which datasets consist of labelled observations, which means that each data point consists of `r index('features')`, which are variables to make predictions from, and a `r index('target')`, which is the quantity that we are trying to predict.
...
During training, all machine learning algorithms are 'fitted'/'trained'\index{model training}\index{model fitting|see{model training}} by optimizing a loss-function that quantifies the mismatch between ground truth target values in the training data and the predictions of the model.
...
Below we retrieve `mtcars`, which uses the `r ref("datasets::mtcars")` dataset:
...
Printing a task provides a summary and in this case, we can see the task has `r task_mtcars$nrow` observations and `r task_mtcars$ncol` columns (32 x 3), of which `mpg` is the target, there are no special properties (`Properties: -`), and there are `r length(task_mtcars$feature_names)` features stored in double-precision floating point format.
...
Subsetting by features (columns) is possible with `$select()` with the desired feature names passed as a character vector and subsetting by observations (rows) is performed with `$filter()` by passing the row IDs as a numeric vector \index{Task!\$select()}\index{Task!\$filter()} [`$select()`/`$filter()`]{.aside}:

```{r}
task_mtcars_small = tsk("mtcars") # initialize with the full task
task_mtcars_small$select("cyl") # keep only one feature
task_mtcars_small$filter(2:3) # keep only these rows
...
# the wrong way
task_mtcars = tsk("mtcars")
task_mtcars_wrong = task_mtcars
task_mtcars_wrong$filter(1:2)
# original data affected
task_mtcars$data()

# the right way
task_mtcars = tsk("mtcars")
task_mtcars_right = task_mtcars$clone()
task_mtcars_right$filter(1:2)
# original data unaffected
task_mtcars$data()
```

To add extra rows and columns to a task, you can use `$rbind()` and `$cbind()` respectively \index{Task!\$cbind()}\index{Task!\$rbind()} [`$rbind()`/<br>`$cbind()`]{.aside}:
...
* `r index('Training', "model training", aside = TRUE)`: A training `Task` is passed to the learner's `$train()`  function which trains and stores a `r index('model')`, i.e., the learned relationship of the features to the target.
* `r index('Predicting', "model predicting", aside = TRUE)`: New data, often a different partition of the original dataset, is passed to the `$predict()` method of the trained learner to predict the target values.
...
In the simplest use-case, models are trained by passing a task to a learner with the `r index("$train()", "Learner!\\$train()", aside = TRUE, code = TRUE)` method:
...
lrn_rpart$train(task_mtcars)
```

After training, the fitted model is stored in the `r index("$model", "Learner!\\$model", aside = TRUE, code = TRUE)` field for future inspection and prediction:
...
```{r basics-025-1}
lrn_rpart$train(task_mtcars, row_ids = splits$train)
```
...
Predicting from trained models is as simple as passing your data to the `r index("$predict()", "Learner!\\$predict()", aside = TRUE, code = TRUE)` method of the trained `Learner`.
...
```{r basics-030}
predictions = lrn_rpart$predict(task_mtcars, row_ids = splits$test)
```
...
library(mlr3viz)
predictions = lrn_rpart$predict(task_mtcars, splits$test)
autoplot(predictions)
```

In the examples above we made predictions by passing a task to `$predict()`.
However, if you would rather pass a `data.frame` type object directly, then you can use `r index("$predict_newdata()", "Learner!\\$predict_newdata()", code = TRUE)`:
...
predictions = lrn_rpart$predict_newdata(mtcars_new)
predictions
```
...
To predict this, the `r index('$predict_type', "Learner!\\$predict_type", code = TRUE)` field of a `r ref("LearnerRegr")` must be changed from "response" (the default) to "se" before training.
...
To access the hyperparameters in the decision tree, we use `r index("$param_set", "Learner!\\$param\\_set", aside = TRUE, code = TRUE)`:
...
In the case of `regr.rpart`, we changed the `xval` default to `0` because `xval` controls internal cross-validations and if a user accidentally leaves this at `10` model training can take a long time.
...
To set multiple values at once we recommend either setting these in construction or using `r index("$set_values()", "Learner!\\$set\\_values()", aside = TRUE, code = TRUE)`, which updates the given hyperparameters (argument names) with the respective values.
...
```{r}
#set xval and cp
rpart_params = lrn("regr.rpart", xval = 0, cp = 1)
...
One such example is a `r index('support vector machine')` classifier (`lrn("classif.svm")`).
The field `$deps`\index{ParamSet!\$deps} returns a `data.table`, which lists the hyperparameter dependencies in the `Learner`.
Below we see that the `cost` parameter is dependent on the `type` parameter.

```{r}
lrn("classif.svm")$param_set
```

The `cond` field tells us what the condition is, which will either mean that `id` is dependent on `on` equalling a single value (`r ref("paradox::CondEqual")`) or any value in the listed set `r ref("paradox::CondAnyOf")`.
...
This tells us that the parameter `cost` should only be set if the `type` parameter is set to `"C-classification"`, and `coef0` should only be set if `kernel` is one of `"polynomial"` or `"sigmoid"`.
...
# errors as type is not C-classification
wrap_error(lrn("classif.svm", type = "eps-classification", cost = 0.5))
# works because type is C-classification
lrn("classif.svm", type = "C-classification", cost = 0.5)
```
...
For regression, we have implemented the baseline `regr.featureless`, which always predicts new values to be the average (mean by default) of the target in the training data:
...
lrn_rpart$train(task_mtcars, splits$train)
predictions = lrn_rpart$predict(task_mtcars, splits$test)
```

### Measures

Analogously to `Task`s and `Learner`s, the available measures in `mlr3` are stored in a dictionary called `r ref("mlr_measures", aside = TRUE)` and can be accessed with `r index("msr()", aside = TRUE, code = TRUE)`:
...
To calculate model performance, we simply call the `r index("$score()", "Prediction!\\$score()", aside = TRUE, code = TRUE)` method of a `r ref("Prediction", index = TRUE)` object and pass as a single argument the measure (or measures passed as a list) that we want to compute:
...
predictions$score(measures, learner = lrn_rpart)
```
...
predictions$score(measure_sf, task = task_mtcars, learner = lrn_rpart)
```
...
Finally, we made predictions from our trained models and scored these; note how we used 'method chaining', which is an R6 technique to combine multiple methods (called with `$()`) by calling one immediately after the other.
...
There are two types of classification task supported in `r mlr3`: binary classification\index{classification!binary}, in which the outcome can be one of two categories, and binamulticlassry classification\index{classification!multiclass}, where the outcome can be one of three or more categories.
...
In contrast, `penguins` is a multiclass problem as there are more than two species of penguins, it has the "multiclass" property:
...
In `mlr3`, the only difference between these tasks, is that binary classification tasks have an extra field called `r index('$positive', "TaskClassif!\\$positive", aside = TRUE, code = TRUE)`, which defines the 'positive' class.
...
```{r basics-043, warning = FALSE, message = FALSE, fig.height = 11}
...
lrn_rpart$train(task_penguins, splits$train)
predictions = lrn_rpart$predict(task_penguins, splits$test)
predictions
```
...
The key difference in usage is that you will need to ensure your selected measure evaluates the prediction type of interest.
...
```{r}
as.data.table(msr())[
    task_type == "classif" & predict_type == "prob" & task_properties != "twoclass"]
```
...
The accuracy\index{measures!accuracy} measure evaluates the `"response"` predictions whereas the brier score\index{measures!brier score} (`"classif.mbrier"`) (squared difference between predicted probabilities and the truth) and logloss (`"classif.logloss"`)\index{measures!logloss} (negative logarithm of the predicted probability for the true class) are evaluating the probability predictions.

If no measure is passed to `$score()`, the default is the classification error\index{measures!classification error} (`"classif.ce"`), which is the number of misclassifications divided by the number of predictions, i.e., 1 - `"classif.acc"`.
...
Firstly, the added field `$confusion`, and secondly the added method `r index('$set_threshold()', "PredictionClassif!\\$set\\_threshold()", code = TRUE)`.
...
For binary and multiclass classification, the confusion matrix is stored in the `r index('$confusion', "PredictionClassif!\\$confusion", code = TRUE, aside = TRUE)` field of the `r ref("PredictionClassif")` object:
...
```{r}
#| fig-cap: "Class labels predicted by the learner (right) and ground truth (left). There is good agreement between the learner and the truth."
...
Taking `task_sonar` as an example with ``r task_sonar$positive`` as the positive class:
...
```{r}
#| fig-cap: "Class labels predicted by the learner (right) and ground truth (left). The learner completely ignores the 'bad' class."
...
```{r}
#| fig-cap: "Class labels predicted by the learner (right) and ground truth (left). The learner predictions are closer to the truth."
...
In @sec-cost-sens we will look at `r index('cost-sensitive classification')` where each class has a different associated cost.
...
Column roles are updated using `$set_col_roles()`\index{Task!\$set\_col\_roles())}.
...
# add column where the weight is 2 when class == "malignant", and 1 otherwise
