...
Analogously to `Task`s and `Learner`s, the available measures in `mlr3` are stored in a dictionary called `r ref("mlr_measures")`, which can be converted to a `data.table` to view all available measures; we have a sugar function `r define("msr()", code = TRUE)` to simplify retrieving a measure for you and again you can use the `$help()` method to find documentation for any measure.

```{r}
as.data.table(mlr_measures)
```

All measures implemented in `mlr3` are defined primarily but three elements: 1) the function that defines the measure; 2) whether a lower or higher value is consider 'good'; and 3) the range of possible values the measure can take.
...
To actually calculate model performance, we simply call the `r define("$score()", code = TRUE)` method of a `r ref("Prediction")` object and pass as a single argument the measure (or measures passed as a list) that we want to compute.
Less abstractly:
...
For example, below we compute performance for mean squared error (`regr.mse`)and mean absolute error (`regr.mae`) (note we use `r define("msrs()")` to load multiple measures at once):
