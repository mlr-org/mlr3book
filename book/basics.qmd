...
Subsetting by features (columns) is possible with `$select()` with the desired feature names passed as a character vector and subsetting by observations (rows) is performed with `$filter()` by passing the row IDs as a numeric vector \index{Task!\$select()}\index{Task!\$filter()}:
...
To add extra rows and columns to a task, you can use `$rbind()` and `$cbind()` respectively \index{Task!\$cbind()}\index{Task!\$rbind()}:
