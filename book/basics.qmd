...
This whistlestop tour of ML provides the basic knowledge required to use software in `mlr3` and is summarized in @fig-ml-abstraction-basics.
...
In the next chapter we will look at resampling and benchmark experiments, which will go into more detail about performance estimation, for now we will just discuss the simplest method of splitting data using the `r ref("partition()", aside = TRUE, index = TRUE)` function.
...
Baselines are useful in model comparison (@sec-performance), as fallback learners (@sec-encapsulation-fallback, @sec-fallback), to be 'composed' into more complex models (@sec-surv-comp), and can be used by sophisticated models during training and/or prediction, for example random forests use baselines to make predictions in the terminal nodes.
...
Note that all task types have default measures that are used if the argument to `$score()` is omitted, for regression this is the mean squared error (`regr.mse`), which is the squared difference between true values and predictions, averaged over all predictions.

It is possible to calculate multiple measures at the same time by passing multiple measures to `$score()`.
For example, below we compute performance for mean squared error (`regr.mse`) and mean absolute error (`regr.mae`) (absolute difference between true values and predictions, averaged over all predictions) -- note we use `r index("msrs()", aside = TRUE, code = TRUE)` to load multiple measures at once.
...
We can see that this measure contains `r index('control parameters', aside = TRUE)`, which are parameters that control how the measure is computed.
...
We loaded the featureless classification baseline and a classification decision tree, then the accuracy measure (sum of correct predictions divided by total number of predictions), trained our models then made predictions and scored them.
...
Here the accuracy measure 'acts on' the `"response"` predictions whereas the brier score (`classif.mbrier`) (squared difference between predicted probabilities and the truth) and logloss (`classif.logloss`) (negative logarithm of the predicted probability for the true class) are evaluating the probability predictions, thus evaluating two different aspects of the model performance.
...
Target refers to the column(s) we want to predict and features are the predictors for the target.
