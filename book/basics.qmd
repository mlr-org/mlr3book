...
This section will start with teaching you the basics of how to set (hyper-)parameters, i.e how to change the `$values` slot of a `r ref("ParamSet")`. 
After that, we will briefly cover the conceptual difference between **initial parameter values** and **default values**.
Information on defining search spaces for optimization can be found in @sec-learner-search-space.
Guidance of how to define a new parameter space when e.g. implementing a custom learner, can be found in section TODO.
...
For more on that, we refer to @sec-parameters-defaults.
...
The most common user-interaction with a `r ref("ParamSet")` is to change its `$values`.
...
In the next section we will show why the last method should usually be preferred over the second method.
