...
In supervised learning, datasets consist of data points (rows in classic panel data) that are labelled, which means each each data point includes feature variables (e.g., columns in panel data) and a label (a variable that we are trying to predict, or 'target').
For example, we could consider the problem of trying to predict the miles per gallon (label/target) a car can drive based on features such as its horsepower and the number of gears (a problem familiar to users of the `mpg` dataset).
In this book, you will often see us refer to machine learning problems as [tasks](#sec-tasks), which is because such problems, like the example above, can be thought of as tasks we are trying to solve for a given dataset.

Supervised learning can roughly be divided into two categories: regression (predicting numeric target values) and classification (predicting categorical target values/labels).
The goal is to build a model that captures the relationships between feature and target variables, in order to make accurate predictions of the target for new, unseen data.
Formally, a model is a function that maps a feature vector to a prediction, which is built by applying a learning algorithm to training data -- known as the `r define("training step")`.
Examples of such learners include decision trees, support vector machines, neural networks, and many more.
Many learning algorithms make use of the concept of `r define("empirical risk minimization")` during training (@sec-training) to produce a fitted model, which means they optimize a loss function (loosely speaking the difference between true and predicted values) on the training data over all candidate models.
In general, a loss function quantifies the mismatch between target values as predicted by a model and ground truth values as found in the data.

A model is only useful if it generalizes beyond limited training data.
Therefore, separate test data is used to evaluate models in an unbiased way and to assess to what extent they have learned the relationships that underlie the data, i.e., tests how well the model makes predictions (@sec-predicting) on new data as an estimate of future performance.
This forms the basis of model `r index("evaluation")` (@sec-eval), where a model's performance is evaluated using a performance measure.
This value based on the performance measure is often called the estimate of the generalization error -- given new data that we have not seen before, how well do we estimate the learned model to perform?
...
For example, we can train a gradient boosted trees model using logloss as a loss function but use the classification error as a performance measure during evaluation.
...
Classification tasks with more than two predicted classes are very common and you may wish to change the thresholds for these tasks. 
The default threshold for each class is 0.5.
Thresholds work slightly differently with multiple classes:

* The probability for a data point is divided by each class threshold resulting in n ratios (n classes).
* The highest ratio is selected (ties are random by default).

Lowering the threshold for a class will mean that it is more likely to be predicted and raising it will have the opposite effect.
The `r ref("datasets::zoo", text = "zoo")` dataset illustrates this concept nicely.  
When trained normally some classes are not ever predicted (see below).
...
The classes `mollusc` and `reptile` are predicted more than it appears in the truth data.
We can try to correct for this by lowering the threshold for `amphibian` and `insect`; and/or raising it for `mollusc` and `reptile`.
`$set_threshold()` can be used with a named numeric vector of thresholds like so:

```{r basics-thresholding-4}
# change threshold
# task$class_names = c("mammal", "bird", "reptile", "fish", "amphibian", "insect", "mollusc.et.al")
new_thresh = c(0.5, 0.5, 0.5, 0.5, 0.4, 0.4, 0.5)
names(new_thresh) = task$class_names

# new predictions
#| fig-cap: Stacked barplot of true (truth) and estimated (response) class labels.
autoplot(preds$set_threshold(new_thresh))
```

You can see that altering the thresholds can lead to better performance. Read more about how to automatically tuning thresholds in @sec-pipelines.
