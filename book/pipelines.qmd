...
`r mlr_pkg("mlr3pipelines")` [@mlr3pipelines] is a dataflow programming toolkit.
...
Individual POs offer hyperparameters because they contain `$param_set` slots that can be read and written from `$param_set$values` (via the `r mlr_pkg("paradox")` package).
...
The package `r mlr_pkg("mlr3filters")` contains many different `"mlr3filters::Filter")`s that can be used to select features for subsequent learners.
...
This vignette is an in-depth introduction to `r ref_pkg("mlr3pipelines")`, the dataflow programming toolkit for machine learning in `R` using `r mlr_pkg("mlr3")`.
...
The benefit of using `r mlr_pkg("mlr3")`, for example, is that one can create a `r ref("Learner")`, a `r ref("Task")`, a `r ref("Resampling")` etc. and use them for typical machine learning operations.
...
`r ref_pkg("mlr3pipelines")` takes modularity one step further than `r mlr_pkg("mlr3")`: it makes it possible to build individual steps within a `r ref("Learner")` out of building blocks called `r ref("PipeOp")`s.
...
The true power of `r ref_pkg("mlr3pipelines")` derives from the fact that it can be integrated seamlessly with `r mlr_pkg("mlr3")`.
Two components are mainly responsible for this:

* `r ref("PipeOpLearner")`, a `r ref("PipeOp")` that encapsulates a `r mlr_pkg("mlr3")` `r ref("Learner")` and creates a `r ref("PredictionData")` object in its `$predict()` phase
* `r ref("GraphLearner")`, a `r mlr_pkg("mlr3")` `r ref("Learner")` that can be used in place of any other `r mlr_pkg("mlr3")` `r ref("Learner")`, but which does prediction using a `r ref("Graph")` given to it
...
The `r ref("PipeOpLearner")` is constructed using a `r mlr_pkg("mlr3")` `r ref("Learner")` and will use it to create `r ref("PredictionData")` in the `$predict()` phase.
...
Although a `r ref("Graph")` has `$train()` and `$predict()` functions, it can not be used directly in places where `r mlr_pkg("mlr3")` `Learners` can be used like resampling or benchmarks.
...
`r ref_pkg("mlr3pipelines")` relies on the `r mlr_pkg("paradox")` package to provide parameters that can modify each `r ref("PipeOp")`'s behavior.
`r mlr_pkg("paradox")` parameters provide information about the parameters that can be changed, as well as their types and ranges.
They provide a unified interface for benchmarks and parameter optimization ("tuning").
For a deep dive into `r mlr_pkg("paradox")`, see [the tuning chapter](#searchspace) or the [in-depth `paradox` chapter](#paradox).
