...
Machine learning (ML) toolkits often try to abstract away the processes involved in ML algorithms.
...
The dataflow programming concept implemented by `r mlr3pipelines` is very intuitive.
Seeing a few code snippets will already give you a clear idea of how to use it.
...
The first line tells us the ID of the PipeOp ("`pca`") and that it is not trained yet, meaning it lacks a `$state`.
...
This is implemented through `list`s: Both the `$train()` input and output of a PipeOp are always `list`s; the number of elements that these lists need to have depends on the operation.
...
Building a Graph from a sequence of PipeOps is most convenient and can be achieved using the **`r ref("concat_graphs", "%>>%")`** ("double-arrow") operator.
When given two PipeOps, it creates a Graph that first executes the left-hand PipeOp, followed by the right-hand one.
...
They can be accessed through the `$param_set` field and provide information about the hyperparameters that can be changed.
...
The hyperparameters of a `r ref("GraphLearner")` can be changed directly (recommended), but they can also be accessed indirectly by reading (and modifying) the underlying Graph's, PipeOp's, or learner's hyperparameters.
...
So for example, if a `r ref("LearnerClassifRpart", "lrn(\"classif.rpart\")")` is encapsulated in a Graph, its `maxdepth` hyperparameter becomes `classif.rpart.maxdepth`.
:::
