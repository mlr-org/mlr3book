...
To use a Graph as a learner within `r mlr3`, it is necessary to wrap it in a `r ref("GraphLearner")` object, since there are various differences between the classes.
Most impportantly, the return values of the `$train()` and `$predict()` methods differ between Graphs and learners.
Convert a Graph to a `r ref("GraphLearner")` by using `r ref("as_learner", "as_learner()")`.
...
result$data(
  cols = c("diabetes", "insulin", "missing_insulin", "triceps", "missing_triceps")
)
```
...
4. Consider the `r ref("PipeOpSelect", "po(\"select\")")` in @sec-pipelines-stack that is used to only keep the columns ending in "`M`".
  ...
  (Note: The `r ref("LearnerLogReg", "lrn(\"classif.log_reg\")")` learner used in @sec-pipelines-stack is not able to handle more than two classes, so you would need to use a different learner, such as `r ref("LearnerClassifMultinom", "lrn(\"classif.multinom\")")`, to built the entire stack.)
