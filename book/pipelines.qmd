...
The following code compares it to a single `"classif.rpart"`-`Learner` on the "Sonar" dataset.
...
We note, however, that the bagged decision tree is still outperformed by the random forest (`lrn("classif.ranger")`).
...
grid = benchmark_grid(tsks("sonar"), list(l_bag, l_rpart, lrn("classif.ranger")), list(rsmp_sonar))
bmr = benchmark(grid)
bmr$aggregate()
```

We can, however, use `mlr3pipelines` and `lrn("classif.rpart")` to come very close to an actual random forest!
The main difference is that the random forest also performs "feature bagging", where onlhy a random subset of available features is used for each tree.
We can implement using the `po("select")` operator and a custom `Selector`.
We also use the `ppl("bagging")` method mentioned above.
...
subsample_selector = function(task) {
  sample(task$feature_names, sqrt(length(task$feature_names)))
}

bagging_quasi_rf = ppl("bagging",
  graph = po("select", selector = subsample_selector) %>>% lrn("classif.rpart", minsplit = 1),
  iterations = 300,
...
grid = benchmark_grid(tsks("sonar"), list(l_quasi_rf, lrn("classif.ranger")), list(rsmp_sonar))
bmr = benchmark(grid)
bmr$aggregate(msrs(c("classif.ce", "time_both")))
```

The result shows that we managed to build a learner that behaves and performs very closely to `r ref("ranger")`.
The `time_both` measure also tells us that our implementation is slower by orders of magnitude, since `r ref("ranger")` is written in C++.
However, it also took us much less time construct our custom learner, compared to what the authors of `r ref("rpart")` likely needed.
...
We first create a multinomial classification learner, which we call the "level 0" learner, which is used to create a feature of predictions.

```{r 05-pipelines-non-sequential-015, eval = TRUE}
lrn_0 = po("learner_cv", lrn("classif.multinom", predict_type = "prob"), resampling.folds = 10, id = "multinom_cv")
```
...
stack = combined %>>% po("learner", lrn("classif.rpart"))
```
...
After training this pipeline, we can show that the second `"classif.rpart"` learner used the output of the first `"classif.multinom"` learner as input by inspecting its `$model`.
```{r 05-pipelines-non-sequential-019, eval = TRUE}
set.seed(0)
l_stack = as_learner(stack)
l_stack$id = "rpart_stacking"
l_stack$train(tsk("iris"))
l_stack$graph_model$pipeops$classif.rpart$learner_model$model
```

We can see that the performance of `lrn("classif.rpart")` was enhanced by the use of stacking.
More advanced models may give even better performance, but the advantage of this setup is that both components of the stacing learner -- the multinomial classification learner and the decision tree -- are easier to interpret than most other models.

```{r 05-pipelines-non-sequential-020}
grid = benchmark_grid(tsks("iris"), list(l_stack, lrn("classif.rpart")), rsmps("repeated_cv"))
bmr = benchmark(grid)
bmr$aggregate()
```
