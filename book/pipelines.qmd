...
options(mlr3.exec_chunk_size = 5)
...
graph = pca %>>% lrn("classif.rpart")  # equivalent
...
grid = benchmark_grid(tsks("sonar"),
  list(l_bag, l_rpart, lrn("classif.ranger")), list(rsmp_sonar)
)
bmr = benchmark(grid)
bmr$aggregate()
```
...
bagging_quasi_rf = ppl("bagging",
  graph = po("select", selector = subsample_selector) %>>%
...
lrn_0 = po("learner_cv",
  learner = lrn("classif.multinom", predict_type = "prob", trace = FALSE),
  resampling.folds = 10, id = "multinom_cv"
)
```
...
```{r 05-pipelines-non-sequential-019, eval = TRUE, message = FALSE, warning = FALSE, include = FALSE}
...
```
```{r 05-pipelines-non-sequential-019-x, eval = FALSE}
set.seed(0)
l_stack = as_learner(stack)
l_stack$id = "rpart_stacking"
l_stack$train(tsk("iris"))
...
```
```{r 05-pipelines-non-sequential-019-xx, eval = TRUE, echo = FALSE}
l_stack$graph_model$pipeops$classif.rpart$learner_model$model
```
...
```{r 05-pipelines-non-sequential-019-1-background, include = FALSE, warning = FALSE, message = FALSE}
# weird bugfix
grid = benchmark_grid(tsks("iris"),
  list(l_stack, lrn("classif.rpart")), rsmps("repeated_cv")
)
bmr = benchmark(grid)
```
```{r 05-pipelines-non-sequential-019-1}
grid = benchmark_grid(tsks("iris"),
  list(l_stack, lrn("classif.rpart")), rsmps("repeated_cv")
)
bmr = benchmark(grid)
bmr$aggregate()
```
```{r 05-pipelines-non-sequential-019-1-foreground, echo = FALSE}
bmr$aggregate()
```
