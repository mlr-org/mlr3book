...
2. Applying a PCA
3. Applying a Kernel Principal Component Analysis, as implemented by `r ref("kernlab::kpca")`, by using `r ref("PipeOpKernelPCA", "po(\"kernelpca\")")`
...
graph = po("branch", c("nop", "pca", "kernelpca")) %>>%
  gunion(list(
    po("nop"),
    po("pca", rank. = rank_opt),
    po("kernelpca")
  )) %>>% po("unbranch", c("nop", "pca", "kernelpca"))
```
...
We now extend this example so that both the preprocessing (PCA, Kernel-PCA, or no preprocessing), as well as the model to use (KNN or decision tree) can be tuned.
...
search_space = ps(
  branch.selection = p_fct(c("nop", "pca", "kernelpca")),
...
#| fig-cap: "Observed performance values when optimizing both preprocessing (between \"pca\", \"kernelpca\", and \"nop\", which is no preprocessing) and Learner (between KNN and the decision tree \"classif.rpart\") at the same time.
...
#| fig-alt: "Plot showing performance values of pca, kernelpca, or nop, followed by KNN or decision tree."
#| echo: false
...
#|        In this example, the PCA PipeOp is active, the alternatives (Kernel-PCA, or no operation through PipeOpNop) are inactive.
...
#| "
#| fig-alt:
#|   - "PipeOpBranch, followed by, alternatively, PipeOpNop, PCA, and Kernel-PCA, which are all followed by PipeOpUnbranch and a Learner.
...
* **`r ref("mlr_graphs_branch", "ppl(\"branch\", graphs)")`**: Alternative path branching, as described in @sec-pipelines-branch.
  ...
  The choice between PCA, Kernel-PCA, and no-op that is shown in @sec-pipelines-branch can, for example, be produced by calling:
  ```{r, eval = FALSE}
  ppl("branch", graphs = pos(c("pca", "kernelpca", "nop")))
  ```
