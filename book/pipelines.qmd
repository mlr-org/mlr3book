...
:::{.callout-note}
This chapter is currently being re-written, in [this PR](https://github.com/mlr-org/mlr3book/pull/385). You can continue reading here if you want to learn about `mlr3pipelines`, but you currently don't need to bother checking this chapter for typos etc.
:::
...
`r ref_pkg("mlr3pipelines")` [@mlr3pipelines] is a dataflow programming toolkit.
...
The scope of `r ref_pkg("mlr3pipelines")` is still growing.
Currently supported features are:

* Data manipulation and preprocessing operations, e.g. PCA, feature filtering, imputation
* Task subsampling for speed and outcome class imbalance handling
* `r ref_pkg("mlr3")` Learner operations for prediction and stacking
...
Using methods from `r ref_pkg("mlr3tuning")`, it is even possible to simultaneously optimize parameters of multiple processing units.

A predecessor to this package is the `r ref_pkg("mlrCPO")` package, which works with `r ref_pkg("mlr")` 2.x.
...
* the `r ref_pkg("caret")` package and the related `r ref_pkg("recipes")`  project
* the `r ref_pkg("dplyr")` package

An example for a Pipeline that can be constructed using `r ref_pkg("mlr3pipelines")` is depicted below:
...
The building blocks of `r ref_pkg("mlr3pipelines")` are **PipeOp**-objects (PO).
...
The main purpose of a `r ref("Graph")` is to build combined preprocessing and model fitting pipelines that can be used as `r ref_pkg("mlr3")` `r ref("Learner")`.
...
Individual POs offer hyperparameters because they contain `$param_set` slots that can be read and written from `$param_set$values` (via the `r ref_pkg("paradox")` package).
The parameters get passed down to the `r ref("Graph")`, and finally to the `r ref("GraphLearner")` .
This makes it not only possible to easily change the behavior of a `r ref("Graph")`  / `r ref("GraphLearner")` and try different settings manually, but also to perform tuning using the `r ref_pkg("mlr3tuning")` package.
...
If you are unfamiliar with tuning in `r ref_pkg("mlr3")`, we recommend to take a look at the section about [tuning](#tuning) first.
...
In order to showcase the power of `r ref_pkg("mlr3pipelines")`, we will show a more complicated stacking example.

In this case, we train a `r ref_pkg("glmnet")` and 2 different `r ref_pkg("rpart")` models (some transform its inputs using `r ref("PipeOpPCA")`) on our task in the "level 0" and concatenate them with the original features (via `r ref("gunion")`).
The result is then passed on to "level 1", where we copy the concatenated features 3 times and put this task into an `r ref_pkg("rpart")` and a `r ref_pkg("glmnet")` model.
...
The package `r ref_pkg("mlr3filters")` contains many different `"mlr3filters::Filter")`s that can be used to select features for subsequent learners.
...
This vignette is an in-depth introduction to `r ref_pkg("mlr3pipelines")`, the dataflow programming toolkit for machine learning in `R` using `r ref_pkg("mlr3")`.
It will go through basic concepts and then give a few examples that both show the simplicity as well as the power and versatility of using `r ref_pkg("mlr3pipelines")`.
...
The benefit of using `r ref_pkg("mlr3")`, for example, is that one can create a `r ref("Learner")`, a `r ref("Task")`, a `r ref("Resampling")` etc. and use them for typical machine learning operations.
...
`r ref_pkg("mlr3pipelines")` takes modularity one step further than `r ref_pkg("mlr3")`: it makes it possible to build individual steps within a `r ref("Learner")` out of building blocks called `r ref("PipeOp")`s.

### `r ref("PipeOp")`: Pipeline Operators

The most basic unit of functionality within `r ref_pkg("mlr3pipelines")` is the `r ref("PipeOp")`, short for "pipeline operator", which represents a trans-formative operation on input (for example a training dataset) leading to output.
...
Each `r ref("PipeOp")` is an instance of an "`R6`" class, many of which are provided by the `r ref_pkg("mlr3pipelines")` package itself.
...
The true power of `r ref_pkg("mlr3pipelines")` derives from the fact that it can be integrated seamlessly with `r ref_pkg("mlr3")`.
Two components are mainly responsible for this:

* `r ref("PipeOpLearner")`, a `r ref("PipeOp")` that encapsulates a `r ref_pkg("mlr3")` `r ref("Learner")` and creates a `r ref("PredictionData")` object in its `$predict()` phase
* `r ref("GraphLearner")`, a `r ref_pkg("mlr3")` `r ref("Learner")` that can be used in place of any other `r ref_pkg("mlr3")` `r ref("Learner")`, but which does prediction using a `r ref("Graph")` given to it
...
The `r ref("PipeOpLearner")` is constructed using a `r ref_pkg("mlr3")` `r ref("Learner")` and will use it to create `r ref("PredictionData")` in the `$predict()` phase.
...
Although a `r ref("Graph")` has `$train()` and `$predict()` functions, it can not be used directly in places where `r ref_pkg("mlr3")` `Learners` can be used like resampling or benchmarks.
...
`r ref_pkg("mlr3pipelines")` relies on the `r ref_pkg("paradox")` package to provide parameters that can modify each `r ref("PipeOp")`'s behavior.
`r ref_pkg("paradox")` parameters provide information about the parameters that can be changed, as well as their types and ranges.
They provide a unified interface for benchmarks and parameter optimization ("tuning").
For a deep dive into `r ref_pkg("paradox")`, see [the tuning chapter](#searchspace) or the [in-depth `paradox` chapter](#paradox).
