...
In order to assess this so-called generalization performance, we must first decide on a suitable [performance measure](#measures).
...
We therefore need to adopt a strategy to partition all available data into training and test data in order to mimic the presence of new, unseen data.
...
<!-- Consequently, we usually evaluate the inducer as next best option, i.e., the learning algorithm that induced the final model.  -->
<!-- As a substitute, we evaluate the quality of intermediate models resulting from training the learning algorithm on a similarly large data set as it was the case for the final model. -->
...
This allows us to evaluate as next best option the learning algorithm that induced the final model, rather than the final model itself. 
...
```{r performance-026}
bmr_spam = bmr$filter(task_id = "spam")
