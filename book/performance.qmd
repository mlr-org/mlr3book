...
However, we typically train the final model we want to deploy on all available data, which leaves no data to evaluate its generalization performance.
...
We therefore need to adopt a data partitioning strategy to mimic the presence of new, unseen data.
...
Benchmarking is used to compare the performance of different learning algorithms applied on various tasks using potentially different resampling strategies.
...
* [construct a benchmarking design](#bm-design) to define the benchmark experiments to be performed,
* [run the benchmark experiments](#bm-exec) and aggregate their results, and
* [convert benchmarking objects](#bm-resamp) to other types of objects that can be used for different purposes.
