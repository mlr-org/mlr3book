...
Now that we are familiar with the basics of how to construct tasks and learners and how to fit models them, we would like to know more about how to assess fitted models.
...
In supervised learning contexts, the core idea in performance evaluation is often that we would like to assess how good a fitted model will work on new data points.
...
* [Bischl et al., 2012](https://direct.mit.edu/evco/article-abstract/20/2/249/925/Resampling-Methods-for-Meta-Model-Validation-with?redirectedFrom=fulltext) provide an overview of resampling methods.
...
For `r mlr_pkg("mlr3")` prediction objects, the ROC curve can easily be constructed with `r mlr_pkg("mlr3viz")` which relies on the `r cran_pkg("precrec")` to calculate and plot ROC curves:
