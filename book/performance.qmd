...
Hence, performance estimation is a fundamental concept to understand how well a model generalizes to new data, which is essential in supervised machine learning in the context of model selection, model comparison, and hyperparameter tuning (see @sec-optimization for more details).
...
As time moves on, new data is gathered that was never seen during the model training process. 
The deployed model makes predictions based on this new data and should generalize well to new data.
...
In @sec-basics, we have applied the holdout method by manually partitioning the data contained in a `r ref("Task")` object into a single training set (to train the model) and a single test set (to estimate the generalization performance).
