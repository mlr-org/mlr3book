...
abstract: |
  ...
  Furthermore, we also introduce how to evaluate binary classifiers by means of ROC analysis.
editor_options:
...
The training set is used to create a temporary model as an intermediate result, whose sole purpose is to estimate the performance using the test set.
...
<!-- FIXME: ^ citations from HPO paper re: holdout, not sure if we'd rather present them differently. -->
...
Below, we again summarize the entire performance estimation process of the holdout method, from partitioning the task and choosing a performance measure to calculating the score to estimate the generalization error.
...
After training, we can use the `$predict` method of the `r ref("Learner")` object to make predictions based on the test set and store the resulting `r ref("Prediction")` object which will be later used to calculate the performance measure:
...
<!-- TODO: Some practical hints how to choose a perf. measure here? E.g., mse when outliers should be avoided and large absolute errors should be penalized more, mae when errors should be penalized proportional to the magnitude of the error -->
...
For a compact and summarized overview, we print the first 5 performance measures from the dictionary in a `data.table`:

```{r performance-004}
msr_tbl = as.data.table(mlr_measures)
msr_tbl[1:5, .(key, label, task_type, predict_type, task_properties)]
```
...
For multiclass classification tasks, generalizations of the AUC measure are also available, but need to be selected explicitly (e.g., `"classif.mauc_au1p"`).
...
The generalization performance is finally estimated by averaging the performance scores over multiple resampling iterations (see @fig-ml-abstraction for an illustration).
...
By repeating the data splitting process often enough, each data point will eventually have been part of both the train- and the test set.
<!-- TODO: Rephrase ^ but not sure how -->
...
This effectively means that the risk of the performance estimate being strongly affected by an unlucky split that does not reflect the original data distribution well, which is a known issue of the holdout method.
...
While this sounds like a good strategy in general as it reduces variance, it should be noted that it has drawbacks in the binary classification setting, aside from being computationally expensive due to the need to fit $N$ models.
...
Both procedures are generally recommended to err on the side of more rather than fewer repetitions (e.g. $\geq 200$), and while increasing this value will lead to longer computation times, the benefits of more performance estimates usually outweigh that.
Sampling ratios of 4/5 or 9/10 are common, resulting in training set proportions analogous to 5-fold or 10-fold cross-validation.
...
For example, to construct a `r ref("Resampling")` object for holdout, we use the value shown in the `key` column from the `r ref("mlr_resamplings")` dictionary and pass it to the `r ref("rsmp()")` a convenience function.
...
When a `r ref("Resampling")` object is constructed, it is not yet a concrete set of train-test-splits, but rather represents onoly a certain way of splitting up a task into such a set.
...
The fact that a `Resampling` is instantiated is indicated by the `$is_instantiated` field.
...
::: {.callout-tip}
In @sec-benchmarking, you will learn about the `r ref ("benchmark()")` function, which automatically instantiates `r ref("Resampling")` objects on all tasks. It ensures a fair comparison by making use of the exact same training and test sets for learning and evaluating the fitted intermediate models.
:::

### Perform Resampling {#sec-resampling-exec}

The `r ref("resample()")` function is used to run the resampling strategy defined by a `Resampling` object on a task for a given learner.
It repeatedly fits a model on training sets and stores the predictions made on the corresponding test sets.
Calling `r ref("resample()")` on a task, learner, and resampling object returns a `r ref("ResampleResult")` which contains all information needed to estimate the generalization performance.
We can `print` the `r ref("ResampleResult")` object to see its contents:
...
These measures are motivated by the need to evaluate classifiers by more than accuracy alone, as we have previously done.
Consider applications like spam classification (is a message spam, yes or no?) or even diagnostic tests used in medicine --- there we place much greater emphasis on true or false positives rather than just the average proportion of correct classifications.
For spam classification, we might accept a small number of false negatives, i.e., messages that are spam but not classified as such, as long as we can be sure that important messages are not falsely classified as spam.
However, if we received millions of messages per day, we might have a stronger preference towards a low number of false-negatives as otherwise our inbox would still see too much spam.
More careful considerations need to be made for diagnostic tests, where the a positive test for a medical condition can lead to expensive or invasive treatments, which we would very much prefer to avoid if not truly necessary.
Likewise, if a medical procedure is required, we do not want to falsely assume otherwise based on a test result.
There is always a trade-off between different aspects of classification performance, and a simple measure such as the accuracy can not accurately reflect them.
This is further complication in situations where the target is not equally distributed rather than a roughly even 50/50 split.
In these imbalanced case we can not rely on a score of, say, 85% accuracy without taking into account the relative proportions of true and false positives and negatives, respectively.

Different fields have therefore developed various measures derived from the confusion matrix statistics above, as different applications tend to have different notions of what qualities a "good" classification model should exhibit.
...
Some common performance measures that are based on the confusion matrix and measure the ability of a classifier to separate the two classes (i.e., discrimination performance) include:
...
See also @fig-confusion for their definition based on TP, FP, TN and FN.
