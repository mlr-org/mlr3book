...
Hence, performance estimation is a fundamental concept to understand how well a model generalizes to new data, which is essential in supervised machine learning in the context of model comparison (see @sec-benchmarking), model selection, and hyperparameter tuning (see @sec-optimization for more details).
...
In essence, the process of holding back part of the data for evaluation purposes simulates the application of the deployed model in practice: 
As time moves on, new data is gathered that was never seen during the model training process, and the model is deployed in practice to make predictions based on this new data.
...
A common simple strategy for performance estimation is the `r index("holdout")` method, which randomly partitions the data into a single training and test set using a pre-defined splitting ratio.
...
The next obvious step is to select a suitable performance measure, which can be done as explained in @sec-eval using the `r ref("mlr_measures")` dictionary.
Passing the dictionary to the `as.data.table` function provides an overview of implemented measures with additional information from which we can select a suitable performance measure, which we print here in two parts for compactness:
...
Depending on our task at hand, we will look for a measure that fits our `task_type` (`"classif"` for `penguins`) and `task_properties`.
The latter is important since measures like AUC `"classif.auc"` are only defined for binary classification tasks (see also @sec-roc-space for more details on AUC), which is indicated by `"twoclass"` in the `task_properties` column --- multiclass-generalizations of the AUC measure are available, but need to be selected explicitly.
...
In the definition of the learner above, we have specified `predict_type = "prob"`, which is required for measures that are not defined on class labels, such as the aforementioned AUC.

::: {.callout-tip}
More information about a performance measure, including its mathematical definition, can be obtained using the `$help()` method of a `r ref("Measure")` object, which opens the help page of the corresponding measure, e.g., `msr("classif.acc")$help()` provides all information about the classification accuracy.
:::

The code example below shows how to apply holdout (specified using `rsmp("holdout")`) on the previously specified `r ref("mlr_tasks_penguins")` task to estimate classification accuracy (using `msr("classif.acc")`) of the previously defined decision tree learner from the `r ref_pkg("rpart")` package:
...
<!-- Moved from intro as per BB, not sure if good here -->
An overview of some common performance measures implemented in `r mlr3`, including a short description and a basic mathematical definition, can be found by following the link provided in the overview table under *Measures overview* in @sec-appendix-overview-tables.
...
All implemented resampling strategies can be queried by looking at the `r ref("mlr_resamplings")` dictionary (also listed in Appendix @sec-appendix-overview-tables).
