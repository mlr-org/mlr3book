...
After having trained a model, a natural follow-up step is to evaluate its performance.
In general, due to the potential of overfitting on training data, every model should be evaluated on unseen test data to ensure unbiased performance estimation.
After all, we are interested in how well the model generalizes to data not seen during training.
A natural approach would be to simply split your data into a train and test set, fit the model on the train set and evaluate its performance on the test set.
This so-called holdout approach (holding out the test set during training), however, has several drawbacks.
If the training set is large and therefore the test set is small, then the estimator of the performance has a high variance.
If the training set is small and therefore the test set is large, then only a subset of all available data was used to fit the model and therefore the estimated performance is biased pessimistically.
To overcome the problems of the holdout approach, various other resampling methods exist, e.g., leave-one-out cross-validation, k-fold cross-validation or repeated k-fold cross-validation.
These resampling methods use the data more efficiently by employing repeated performance estimation and averaging over multiple splits resulting in lower variance of the estimator [@Simon2007].
For example, k-fold cross-validation partitions the data into k folds and estimates performance of the model k times by using all data for training except for the current fold which is used for evaluation.
The estimated performance of the model is then given by averaging the k estimates.
An overview of resampling techniques can be found in @bischl2012resampling.
More details on performance estimation and resampling in general is given Chapter 5 of @james2021introduction.
