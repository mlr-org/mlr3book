...
The code example below shows how to apply holdout (specified using `rsmp("holdout")`) on the previously specified `r ref("mlr_tasks_penguins", text = "penguins")` task to estimate classification accuracy (using `msr("classif.acc")`) of the previously defined decision tree learner from the `r ref_pkg("rpart")` package:
...
For example, in the code example above, macro-averaging and micro-averaging yield the same classification accuracy because the `r ref("mlr_tasks_penguins", text = "penguins")` task (consisting of 344 observations) is split into 4 equally-sized test sets (consisting of 86 observations each) due to the 4-fold cross-validation. 
...
The following code performs leave-one-object-out cross-validation using the feature `year` of the `r ref("mlr_tasks_penguins", text = "penguins")` task to determine the grouping.
...
In the code example below, we first retrieve the `r ref("mlr_tasks_german_credit", text = "german_credit")` task which is a binary classification task and construct a classification tree learner that predicts probabilities using the `predict_type = "prob"` option.
...
For example, below we set the threshold to `0.99` and `0.01` for the `r ref("mlr_tasks_german_credit", text = "german_credit")` task to illustrate the two special cases explained above where zero positives and where zero negatives are predicted and inspect the resulting confusion matrix-based measures (note that some measures can not be computed due to division by 0 and therefore produce `NaN` values):
