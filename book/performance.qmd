...
To assess this so-called generalization performance, we first need to decide on a suitable [performance measure](#measures).
This measure computes a *score* indicating, e.g., how well the model predictions match the ground truth.
However, the measure may also reflect other qualities such as the time for training a model.

<!-- motivate the need for splitting the data -->
Once we have decided on a performance measure, we have to pursue a strategy to reliably estimate the generalization performance of the deployed model.
Using the same data to train and test a model would be a bad strategy as this would lead to an overly optimistic performance estimate.
...
Existing strategies therefore partition all available data into training and test data in different ways and use the test data to mimic the presence of unseen and estimate the generalization performance.
