...
For example, a strategy like $k$-fold cross-validation would be preferable over a single train-test split as it randomly partitions the data into $k$ subsets, called folds.
In total, $k$ models are then trained on $k-1$ of the folds, with the remaining fold being used as test data exactly once in each of the $k$ iterations.
