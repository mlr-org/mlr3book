...
This performance measure usually provides a *score* indicating, e.g., how well the model predictions match the ground truth values and may also reflect other qualities such as its runtime.
...
<!-- TODO: add overview / guideline for several resampling strategies or refer to literature? -->
...
Passing the dictionary to the `as.data.table` function provides a more structured output including the parameters we can specify for each resampling strategy (e.g., the train-test splitting ratio or the number of repetitions):
...
For example, to construct a `r ref("Resampling")` object for holdout, we use the value of `key` column from the `r ref("mlr_resamplings")` dictionary and pass it to the convenience function `r `ref("rsmp()")`:
...
In the example below, we create a factor `folds` with 4 levels (each repeated 86 times to account for the 344 observations in the `"penguin"` task) and pass it to the  `f` argument.
