...
The next obvious step is to select a suitable performance measure, which can be done as explained in Section @sec-eval using the `r ref("mlr_measures")` dictionary.
...
::: {.callout-tip}
In Section @sec-benchmarking, you will learn about the `ref ("benchmark()")` function, which automatically instantiates `r ref("Resampling")` objects on all tasks to ensure a fair comparison by making use of the exact same training and test sets for learning and evaluating the fitted intermediate models.
:::
...
By the column names alone, we see that the general contents of a `r ref("BenchmarkResult")` and `r ref("ResampleResult")` which we specified in Section @resampling-inspect is very similar, with the additional unique identification column `"uhash"` in the former being the only difference.
...
There are a multitude of different performance measures that can be derived from a confusion matrix.
Unfortunately, many of them have different names for historical reasons, originating from different fields.
For a good overview of some common confusion matrix-based measures, see the comprehensive table on `r link("https://en.wikipedia.org/wiki/Confusion_matrix#Table_of_confusion", "Wikipedia")` which also provides many common aliases for each measure.

### Confusion Matrix-based Measures

Some common performance measures that are based on the confusion matrix include (see also @fig-confusion for the definition based on TP, FP, TN and FN):
...
Next, we use the `r ref("partition()")` helper function to randomly split the rows of the task into two disjoint set, which acts as a convenience shortcut function to the `"holdout"` resampling strategy.
