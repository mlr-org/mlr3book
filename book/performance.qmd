...
For a compact and summarized overview, we print the first 5 performance measures from the dictionary in two `data.table`s:
...
Asymptotically, in bootstrapping $1 - e^{-1} \approx 63.2\%$ of the data points will be contained in the training set, referred to as "in-bag" samples, conversely to the remaining 36.8% "out of bag" samples that are used as test set.
Both procedures are generally recommended to run with a rather large number of iterations (e.g. 100 - 1000), and while increasing this value will lead to longer computation times, the benefits of more performance estimates usually outweigh that. 
Sampling ratios of 4/5 or 9/10 are common, resulting in training set sizes analogous to 5-fold or 10-fold cross-validation.
