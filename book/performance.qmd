...
`resample()` repeatedly fits a model on the training sets, makes predictions on the corresponding test sets and stores them in a `r ref("ResampleResult", aside = TRUE)` object, which contains all information needed to estimate the generalization performance.
...
In the above example where we used the `penguins` data, there is a difference in the classification error between micro and macro methods because the dataset has 344 rows, which is not divisible by three (the number of folds), hence the test sets are not of an equal size.
:::
...
A custom holdout resampling strategy can be constructed using `rsmp("custom")`, where the row indices of the observations used for training and testing must be defined manually when instantiated on a task.
...
```{r performance-023}
custom_holdout = rsmp("custom")

# holdout strategy with one iteration
custom_holdout$instantiate(task,
  train = list(c(1:50, 54:333)),
  test = list(51:53)
)
resample(task, learner, custom_holdout)$prediction()

# Repeated resampling strategy with multiple iterations
custom_repeated = c(1:5, 153:158, 277:280)
custom_repeated$instantiate(task,
  train = list(train_sets, train_sets + 5),
  test = list(train_sets + 15, train_sets + 25)
)
resample(task, learner, custom_repeated)$prediction()
```
...
custom_cv$instantiate(task_small, f = folds)
...
If we don't do this, we might overestimate the model's generalization capability to unseen individuals.
...
You can view the observations that fall into each stratum using the `$strata` field of a `Task` object, which can be particularly useful when we are interested in multiple strata:
...
Since this process is stochastic, it is necessary to set a seed **prior to** calling `benchmark_grid()` in order to ensure reproducibility of the data splits.
:::
...
The contents of a `r ref("BenchmarkResult")` and `r ref("ResampleResult")` (@sec-resampling-inspect) are almost identical and the stored `r ref("ResampleResult")`s can be extracted via the `$resample_result(i)` method, where `i` is the index of the performed resample experiment.
...
In addition, there is also the `r ref('as_benchmark_result()', aside = TRUE)` function, which can be used to convert objects from `ResampleResult` to `BenchmarkResult`, and then optionally combined, which is useful when conducting experiments across multiple machines.
...
#| fig-cap: "Boxplots of accuracy scores for each learner across resampling iterations and the three tasks. The Random Forest (`classif.ranger`) consistently outperforms the other learners."
...
When a binary classifier predicts probabilities instead of discrete classes, we could set a threshold to cut-off the probabilities to change how we assign observations to the positive/negative class (see @sec-classif-prediction).
