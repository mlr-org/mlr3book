...
abstract: |
  Estimating the generalization performance of a machine learning algorithm on a specific dataset requires data not used during training.
  Resampling refers to the process of repeatedly splitting available data into training and test sets to enable unbiased performance estiamtion.
  This chapter introduces common resampling strategies and illustrates their use with the `mlr3` ecosystem using common machine learning algorithms.
  Benchmarking builds upon resampling, encompassing the fair comparison of multiple machine learning   algorithms on given datasets.
...
To address this problem, existing strategies for estimating generalization performance withhold a subset of the available data. 
...
Existing resampling strategies differ in how the available data is split, a comprehensive overview can be found in @japkowicz2011evaluating.
