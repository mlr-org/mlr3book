...
<!-- In supervised machine learning, we use a learning algorithm (`r ref("Learner")`) to train a model on all available data and deploy the model in practice in the hope that it will provide accurate predictions for future data. -->
<!-- When assessing the generalization performance of such a model, we face a difficult situation because we do not have access to more data to assess its future performance. -->
<!-- The simplest thing we can do is estimate the generalization performance by partitioning all available data into training and test data. -->
<!-- Then, apply the same learning algorithm only on the training data to fit an 'auxiliary model' and assess its performance on the remaining test data.  -->
<!-- The only purpose of this 'auxiliary model' is to estimate how the model deployed in practice (trained on all available data) might perform on new, unseen data. -->
<!-- Assessing how accurate predictions are is done by calculating a *score* based on a [Performance Measure](#measures) that compares the actual target values to the predicted target values. -->

In supervised machine learning, we usually train a learning algorithm (`r ref("Learner")`) on all available data to obtain a model which we want to deploy in practice.
We are often interested in the generalization performance of such a deployed model on new, unseen data.
Therefore, it is out of question to assess the performance of the model on the same data it was trained on.
For this purpose, we first need to select an appropriate [performance measure](#measures) that compares the actual target values to the predicted target values.
Furthermore, since we do not have access to new, unseen data with which we can   calculate the performance measure, we need to find a strategy to at least mimic the presence of new, unseen data.
The simplest thing we can do here is to first partition all available data into training and test data.
We then apply the same learning algorithm only on the training data to produce an 'auxiliary model', and instead assess its performance on the remaining test data. 
The only purpose of this 'auxiliary model' is to estimate how the model deployed in practice (trained on all available data) might perform on new, unseen data.

However, the 'auxiliary model' and the deployed model might be different. 
While the latter is trained on all available data, the former is trained only on a training set which is a smaller subset of all available data.
Especially if the training data is too small, this can lead to a pessimistically biased performance estimate, since it is more difficult for the learning algorithm to learn complex relationships with less data.
Hence, a trade-off must be made in the size of the train-test splits. 
On the one hand, we want the size of the training data to be close as possible to the available data so that the learning algorithm can learn a 'auxiliary model' that is representative to the deployed model. 
On the other hand, we also want many test data points to obtain a better estimate for the generalization performance.
Note that the 'auxiliary model' might perform poorly on the test data just by chance.
Therefore, we might want to repeat the partitioning process and average the performance estimates to reduce the variance of our estimation.
This gives rise to [resampling strategies](#resampling) that (repeatedly) partition the available data in different ways to obtain reliable performance estimates as they reuse the available data (see @fig-ml-abstraction for an illustration).

<!-- In the previous chapter, we have learned how to train a `r ref("Learner")` on a task and make predictions using the resulting model. -->
<!-- To assess the generalization performance of a model on new, unseen data, we usually preserve some data (test data) not used for training on which we make predictions and compute a [Performance Measure](#measures). -->
<!-- In the previous chapter, we have seen how to manually partition the data contained in a task into a single training set (to train the model) and a single test set (to estimate the generalization performance). -->
<!-- This simple partitioning approach is also known as *holdout*. -->
<!-- manually define a training set (i.e., a randomly selected subset of rows of the data) and estimate the generalization performance of the resulting model on the remaining test set.  -->
...
[Resampling](#resampling) is a protocol that defines how to partition the available data into training and test data.
We cover how to
...
[Benchmarking](#benchmarking) is used to compare the performance of different learning algorithms applied on different tasks using different resampling strategies.
The purpose is to ideally rank the learning algorithms regarding their predictive performance, runtime, or any other performance measure of interest and to identify the best learning algorithms for a certain task or across different tasks.
We cover how to

* construct a [benchmarking design](#bm-design) using `benchmark_grid` to define the experiments to be performed,
...
In this section, we will use `Resampling` objects that define the resampling strategy we want to employ to estimate the generalization performance.
Calling the function `resample` on a `Task`, `Learner` and a `Resampling` object then will return a `ResampleResult` object which contains the performance results of applying the resampling strategy on a `Task` and `Learner`.
...
It is considered best practice to repeatedly split the entire data in different ways to ensure that a "lucky" or "unlucky" split does not bias the generalization performance estimate, which is why a strategy like 10-fold cross-validation would be preferrable over a single train-test split if the size of the data allows it.
