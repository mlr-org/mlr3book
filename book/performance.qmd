...
Once we have decided on a performance measure, the next step is to adopt a strategy that describes how to use the available data to obtain a reliable performance estimate.
Unfortunately, using the same data to train and test a model can lead to an overly optimistic performance estimate.
For example, an overfitted model may perfectly fit the data it was trained on but does not generalize well to new data. 
Assessing its performance using the same data it was trained on would misleadingly suggest the overfitted model performed well.
...
Hence, it is common practice to test a model on independent data that has not been used for training a model.
...
<!-- Ideally, we want the size of the training set to be of similar size to all available data so that the intermediate model represents the final model well. -->
...
<!-- If the training data is too small, the performance estimated using the intermediate model may be pessimistically biased as the intermediate model is not representative and usually learns less complex relationships when the amount of training data is small. -->
<!-- If the training data is too small, the performance estimate may be pessimistically biased as the intermediate model is not representative and usually learns less complex relationships when the amount of training data is small. -->
<!-- If the training data is much smaller, the performance estimate may be pessimistically biased because the intermediate model can usually learn less complex relationships than the final model due to the smaller amount of data used for training.  -->
...
By default, it uses the model predictions and the corresponding ground truth values involved in the test set of the corresponding resampling iteration to calculate the performance measure.
...
Using `average = "micro"` allows to combine the predictions of each resampling iteration into a single `r ref("Prediction")` object and to compute a micro averaged performance estimate: 
...
Internal learner-specific error or warning messages may occur at each of these two steps.
...
A `r ref("ResampleResult")` object internally also stores the `r ref("Prediction")` objects produced by each intermediate model from each resampling iteration as they are needed to calculate a performance measure.
A list of `r ref("Prediction")` objects can be extracted by the `$predictions()` method:
...
This can be useful if one wants to analyze the predictions of individual intermediate models or to manually compute a macro averaged performance estimate.
...
The combined prediction object can, e.g., be used to manually compute a micro averaged performance estimate:
...
<!-- By setting the `store_models` argument of the `r ref("resample()")` function to `TRUE`,  -->
We can configure the `r ref("resample()")` function to keep the fitted intermediate models by setting the `store_models` argument to `TRUE`.
