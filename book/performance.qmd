...
In supervised machine learning, a model can only be deployed in practice if it generalizes well to new, unseen data, that is if it has good `r index("generalization performance", aside = TRUE)`.
...
This `r index("test set")` is then used to mimic the presence of unseen data and to estimate the generalization performance.
