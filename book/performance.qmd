...
For `r ref_pkg("mlr3")` prediction objects, the ROC curve can easily be constructed with `r ref_pkg("mlr3viz")` which relies on the `r ref_pkg("precrec")` to calculate and plot ROC curves:
...
There are many different strategies for partitioning a data set into training and test; in `r ref_pkg("mlr3")` we call these strategies "resampling".
`r ref_pkg("mlr3")` includes the following predefined [resampling](#resampling) strategies:
...
`r ref_pkg("mlr3")` resampling strategies and their parameters can be queried by looking at the `data.table` output of the `r ref("mlr_resamplings")` dictionary; this also lists the parameters that can be changed to affect the behavior of each strategy:
...
:::{.callout-tip}
Additional resampling methods for special use cases are available via extension packages, such as `r ref_pkg("mlr3spatiotempcv")` for spatial data.
:::
...
Besides the `"group"` column role, dedicated spatiotemporal resampling methods are available in `r ref_pkg("mlr3spatiotemporal")`.
...
`r ref_pkg("mlr3viz")` provides a `r ref("ggplot2::autoplot()", text = "autoplot()")` method for resampling results.
...
The `r ref_pkg("mlr3")` package offers the `r ref("benchmark()")` convenience function that takes care of most of the work of repeatedly training and evaluating models under the same conditions.

### Design Creation {#bm-design}

Benchmark experiments in `r ref_pkg("mlr3")` are specified through a design.
...
Similar to [tasks](#autoplot-task), [predictions](#autoplot-prediction), or [resample results](#autoplot-resampleresult), `r ref_pkg("mlr3viz")` also provides a `r ref("ggplot2::autoplot()", text = "autoplot()")` method for benchmark results.
...
The package `r ref_pkg("mlr3benchmark")` provides some infrastructure for applying statistical significance tests on `r ref("BenchmarkResult")`.
