...
To assess the `r index("generalization performance")` of a model, we must first decide on a [`r index("performance measure")`](#measures) that is appropriate for our given task and evaluation goal.
...
Once we have decided on a performance measure, we next need a strategy for how to use the available data to reliably estimate the generalization performance of a deployed model.
...
Assessing its performance using the same data a model was trained on would misleadingly suggest a well-performing model.
...
To address this problem, existing strategies for estimating the generalization performance usually withhold a subset of the available data for evaluation purposes.
...
<!-- partition all available data into training and test set in different ways, and use the test set to mimic the presence of unseen data. -->

<!-- describe performance estimation procedure and desiderata -->
The most straightforward strategy is the `r index("holdout")` method, which randomly partitions the data into a single training and test set.
It uses a pre-defined value for the ratio of observations to be included in the training set.
The training set is then used to create an intermediate model whose sole purpose is to be used for performance estimation on the remaining test set.
<!-- We then train an intermediate model only on the training set and use the test set to assess its performance. -->
<!-- The intermediate model is usually discarded as its sole purpose is to be used for performance estimation. -->
This performance estimate is then used as a proxy for the performance of the final model and the intermediate model is usually discarded.
...
<!-- explain resampling -->
To address this issue, [`r index("resampling")` strategies](#resampling) repeatedly split all available data into multiple training and test sets and average the performance across multiple resampling iterations (see @fig-ml-abstraction for an illustration).
A higher number of resampling iterations can reduce the variance and result in a more reliable performance estimate (see TODO for illustration).
...
<!-- Resampling strategies are usually preferable to a single train-test split as produced by the holdout method because they use all available data to assess the performance and thereby avoid the performance estimate from being biased by a particular split. -->
...
`r index("Benchmarking")` in supervised machine learning refers to the comparison different learners on a single task or on multiple tasks. 
...
When comparing learners on a multiple tasks, the main aim is often to gain insights in how different learners perform when they are trained on different data situations.
...
<!-- For example, for a novel learning algorithm to be widely accepted, it is required to conduct benchmark studies that illustrate its outperformance or behavior in different data situations compared to other already existing learners, including its advantages and disadvantages. -->
...
<!-- Selecting a resampling strategy will typically be motivated by the context defined by the task(s) and learner(s) to be evaluated and the goals of the performance assessment. -->
...
<!-- should we add a fig for an overview of other resampling strategies and some remarks on practical guidelines (e.g. take the one from i2ml lecture)?   -->
...
Specifically, we show how to estimate the generalization performance of a learner on a given task via holdout using `r ref("resample()")` and how to use`r ref("benchmark()")` to compare two learners on a given task.
<!-- we will learn how to estimate the generalization performance of a `r ref("Learner")` using resampling strategies implemented in the `r mlr3` package.  -->
...
We define the corresponding `r ref("Task")` and `r ref("Learner")` object used throughout this chapter as follows:
...
For illustration, we show an example code that compares the classification accuracy of the decision tree against a featureless learner:
```{r}
...
Existing resampling strategies differ in how they partition the available data into training and test set, a comprehensive overview can be found in @japkowicz2011evaluating.
For example, the $k$-fold cross-validation method randomly partitions the data into $k$ subsets, called folds (see @fig-cv-illustration).
Then $k$ models are trained on training data consisting of $k-1$ of the folds, with the remaining fold being used as test data exactly once in each of the $k$ iterations.
The $k$ performance estimates resulting from each fold are then averaged to obtain a more reliable performance estimate.
Several variations of cross-validation exist, including repeated $k$-fold cross-validation where the entire process illustrated in @fig-cv-illustration is repeated multiple times and leave-one-out cross-validation where the test set in each fold consists of exactly one observation. 
Other well-kown resampling strategies include subsampling and boostrapping.
...
Bootstrapping creates training sets by randomly drawing observations from all available with replacement.
...
<!-- Selecting a resampling strategy will typically depend on the context presented by the task(s) and learner(s) to be evaluated and the goals of the performance assessment. -->
...
Instantiation is especially useful is when the aim is to fairly compare multiple [learners](#learners).
...
The `$score()` method uses `r ref("Measure")` objects to calculate the performance measure in each resampling iteration. 
...
By default, the model predictions and the corresponding ground truth values involved in the test set of the corresponding resampling iteration are used.
...
If we do not explicitly pass a `r ref("Measure")` object to the `$score()` method, the classification error (`classif.ce`) for classification tasks and the mean squared error (`regr.mse`) for regression tasks are used as default.
...
Similarly, we can pass `r ref("Measure")` objects to the `$aggregate()` method to calculate the aggregated performance across all resampling iterations:
...
The type of aggregation is usually determined by the `r ref("Measure")` object (see also the fields `average` and `aggregator` the in help page of `r ref("Measure")`).
...
However, it is sometimes useful to inspect the individual performance values of each resampling iteration (as returned by the `$score()` method) to see if one (or more) of the iterations lead to very different performance results.
...
- By default, these models are discarded after the prediction step to reduce memory consumption of the resulting `r ref("ResampleResult")` object and because we usually only need the stored predictions to calculate the performance measure. However, we can configure the `r ref("resample()")` function to keep the fitted models (e.g. if we want to use or inspect the intermediate models later) by setting the `store_models` argument of the `r ref("resample()")` function to `TRUE`. Retrieve and inspect the model trained in a specific resampling iteration (if we have set the argument `store_models = TRUE`) via `$learners[[i]]$model` where `i` refers to the `i`-th resampling iteration:
...
Since this table contains R6 columns within list-columns, we unfortunately can not infer too much about `task` column, but the `r ref("ids")` utility function can be used for quick inspection or subsetting:
...
Please note that averaging accuracy scores across multiple tasks is not always appropriate, however.
We show this here for illustration purposes, but you may alternatively perform ranking.
...
TODO: We have removed most ROC-related content from this chapter, so this should probably also go in wherever the "ROC part" ends up going?
...
:Core S3 'sugar' functions for model optimization in mlr3 with the underlying R6 class that are constructed when these functions are called (if applicable) and a summary of the purpose of the functions. {#tbl-api-performance}
...
TODO: Instead of the 3rd example we might find a way to manually reproduce a given OpenML study or something similar, rather than a very constructed example?
