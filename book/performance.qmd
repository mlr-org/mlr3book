...
Hence, performance estimation is a fundamental concept to understand how well a model generalizes to new data and plays an important role in the context of model comparison (which we will cover in @sec-benchmarking), model selection, and hyperparameter tuning (the topic of @sec-optimization).
...
We then create a `r ref("Learner")` object (here a decision tree from the `r ref_pkg("rpart")` package) and train the intermediate model using only the training set.
<!-- which can be accessed by the `$model` field. -->
...
There are also other performance measures that are not based on point-wise losses as they are not defined on observation level but on a set of observations.
Examples include the AUC measure.
...
Specifically, repeating the data splitting process allows using all available data points to assess the performance, as each data point can be ensured to be part of the train- or test set in at least one resampling iteration.
Analogously, a higher number of resampling iterations can reduce the variance and result in a more reliable performance estimate, which allows us to be more confident in our performance estimates.
...
LOO-CV specifically has desirable properties for regression tasks and is an attractive choice for situations where few samples are available.
However, repeated $k$-fold cross-validation or subsampling are preferable if one wants to avoid selecting for too complex models.
...
Asymptotically, $1 - e^{-1} \approx 63.2\%$ of the data points will be contained in the training set, referred to as "in-bag" samples, conversely to the remaining 36.8% "out of bag" samples.
...
Holdout only estimates the generalization performance using a single test set, so our performance estimate might vary greatly depending on random chance.
...
or analogously bootstrapping and subsampling with 100 repetitions and a sampling ratio of 9/10 could be defined:

```{r}
rsmp("bootstrap", repeats = 100, ratio = 0.9)
rsmp("subsampling", repeats = 100, ratio = 0.9)
```
...
The aggregated score returned by `$aggregate()` refers to the generalization performance of our selected learner on the given task estimated by the resampling strategy defined in the `r ref("Resampling")` object.
...
In `r mlr3`, we can assign a special role to a feature contained in the data by configuring the corresponding `$col_roles` field of a `r ref("Task")`, which we introduced in @sec-col-roles.
