...
## ROC Analysis and Thresholds {#sec-roc}

So far we have focused on methods applicable to general classification and regression tasks, but there is of course the special case of binary classification with its specialized performance measures and analysis methods.
...
`r index("ROC Analysis")`, which stands for receiver operating characteristics, applies specifically to this case and allows a better picture of the trade-offs when choosing between the two classes.

We saw in Chapter @sec-basics that one can retrieve the confusion matrix of a `r ref("Prediction")` by accessing the `$confusion` field.
In the following code chunk, we first retrieve the `"sonar"` classification task and construct a classification tree learner.
Next, we use the `r ref("partition()")` helper function to randomly split the rows of the Sonar task into two disjoint set, which acts as a convenience shortcut to the `"holdout"` resampling strategy.
...
The x-axis showing the FPR is labelled "1 - Specificity" by convention, whereas the y-axis shows "Sensitivty" for the TPR.
...
- `r link("mlr3gallery: Resampling - Stratified, Blocked and Predefined", "https://mlr-org.com/gallery/basic/2020-03-30-stratification-blocking/")`
