...
An overview of some common performance measures implemented in `r mlr3`, including a short description and a basic mathematical definition, can be found by following the link provided in the overview table under *Measures overview* in Appendix @appendix-overview-tables.
...
Without specifying a `"stratum"` column role, the `species` column may have quite different class distributions across the corresponding test sets of a 3-fold cross-validation strategy:
...
There are a multitude of different measures that can be derived from a confusion matrix, and unfortunately many of them have different names in different fields for historical reasons.
For a good overview, `r link("https://en.wikipedia.org/wiki/Confusion_matrix", "this Wikipedia article features a comprehensive table")`, including many common aliases for each measure.

When it comes to classification performance, it is generally difficult to achieve a high TPR and low FPR simultaneously.
