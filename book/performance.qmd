...
The `$score()` method summarizes these individual loss values by counting the fraction of observations where the point-wise loss is 1 (i.e., the fraction of observations where `response` and `truth` have the same value).
...
A variety of resampling strategies exist, each with their respective advantages and disadvantages, which in turn may vary depending on the number of available samples, the task complexity, and the type of model.
...
Several variations of cross-validation exist, including repeated $k$-fold cross-validation where the entire process illustrated in @fig-cv-illustration is repeated multiple times, and leave-one-out cross-validation (LOO-CV) where the test set in each fold consists of exactly one observation.
LOO-CV specifically has desirable properties for regression tasks and is an attractive choice for situation where few samples are available.
However, repeated cross-validation or subsampling are preferable if one wants to avoid selecting for too complex models.
...
Therefore bootstrapping always results in training sets of the same size as the original data, but at cost of repeating some observations.
Asymptotically, $1 - e^{-1} \approx 63.2\%$ of the data points will be contained in the training set, referred to as "in-bag" samples, conversely to the "out of bag" samples.
Both procedures are generally recommended to run with a large number of iterations (e.g. 100 - 1000), and while increasing this value will lead to longer computation times, the benefits of more performance estimates usually outweigh that. 
Sampling ratios of 4/5 or 9/10 are common, resulting in training set sizes analogous to 5- or 10-fold cross-validation.
Note that terminology regarding resampling strategies is not consistent across the literature: Subsampling for example is also referred to as "repeated holdout", or "Monte Carlo cross-validation", which is why it is usually to verify formal definitions of resampling techniques applied in literature.
...
If the available data is fairly small (N ≤ 500), holdout with a 2/3 ratio is often used, yet the resulting performance estimates will exhibit high variance. 
Alternatively, repeated cross-validation with a large number of repetitions can be used to keep the variance of the performance estimates low.
...
Properties and pitfalls of different resampling techniques have been widely studied and discussed in the literature, see e.g., @bengio2003no, @molinaro2005prediction, @kim2009estimating, and @bischl2012resampling.
...
To generate the train-test splits for a given task, we need to instantiate a resampling strategy by calling the `$instantiate()` method of the previously constructed `r ref("Resampling")` object on the `r ref("mlr_tasks_penguins")` task.
...
Note that the type of aggregation is usually determined by the `r ref("Measure")` object, see also the fields `$average` and `$aggregator` the in help page of `r ref("Measure")` for more details.
