...
In supervised machine learning, we want a deployed model to generalize well to new, unseen data.
To assess its so-called generalization performance, we need to choose a suitable [performance measure](#measures).
This measure usually computes a *score* indicating, e.g., how well the model predictions match the ground truth.
...
Using the same data to train and test a model can lead to an overly optimistic performance estimate.
For example, an overfitted model may perfectly fit the data it was trained on but does not generalize well to new data. 
Assessing its performance only using the training data would misleadingly suggest a very good model.
To avoid such an optimistically biased performance estimate, we need to test the model on independent data not used for training.
However, we typically train the deployed final model on all available data, which leaves no data to evaluate its generalization performance.
For this reason, we usually adopt a strategy that partitions all available data into training and test data using the test data to mimic the presence of new, unseen data.
...
<!-- explain resampling -->
[Resampling strategies](#resampling) repeatedly split all available data into multiple training and test sets and average the performance across multiple resampling iterations (see @fig-ml-abstraction for an illustration).
...
For example, the $k$-fold cross-validation method randomly partitions the data into $k$ subsets, called folds.
...
In the previous chapter, we have already manually applied the holdout method to partition the data contained in a task into a single training set (to train the model) and a single test set (to estimate the generalization performance).
