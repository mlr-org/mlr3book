...
For example, the column `params` shows the parameters of each resampling strategy (e.g., the train-test splitting `ratio` or the number of `repeats`) and the column `iters` is corresponds to the number of performed resampling iterations, i.e. parameters `folds` for cross-validation or `repeats` for bootstrapping, etc.
Holdout for example shows `iters = 1`, whereas repeated application of the holdout strategy would be identical to choosing the subsampling strategy.
