...
Estimating this `r index("generalization performance")` is crucial for many aspects of machine learning application and research --- whether we want to fairly compare a novel algorithm with established methods or find the best algorithm among many options for a particular task through tuning (which will be discussed in depth in Chapter @sec-optimization) --- we always rely on this performance estimate.
...
<!-- explain resampling -->
To address this issue, [`r index("resampling")` strategies](#sec-resampling) repeatedly split all available data into multiple training and test sets and average the performance over multiple resampling iterations (see @fig-ml-abstraction for an illustration).
A higher number of resampling iterations can reduce the variance and result in a more reliable performance estimate. <!-- Removed reference to exercise since we discussed that this aspect is too important to leave to an exercise, maybe this should be a gallery post or even link to I2ML? -->
Resampling allows using more data points for testing while keeping the training sets as large as possible.
They usually use all available data to assess the performance and thereby avoid the performance estimate from being skewed by a particular split which is a known issue of the holdout method.
...
The `$score()` method uses `r ref("Measure")` objects to calculate the performance measure of each resampling iteration separately, meaning that `$score()` always results in one value per resampling iteration.
...
To understand these better, we first take a glimpse at what is actually contained in the object by converting it to a `data.table` and showing its columns in two steps:
...
```
We see the `r ref("ResampleResult")` contains the the task, learner and resampling strategy that we passed to `resample()` earlier, stored in list-columns.  
...
If we need to investigate a individual resampling iteration, we can do so by accessing the respective elements associated with it.
...
In this context, the leave-one-out cross-validation strategy can be coarsened to the "leave one subject out" cross-validation strategy, where it is not one measurement that is left out, but all measurements associated with one particular individual.
