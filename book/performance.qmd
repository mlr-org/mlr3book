...
The code example below shows how to apply holdout (specified using `rsmp("holdout")`) on the `r ref("mlr_tasks_penguins")` task to estimate classification accuracy of a decision tree from the `r ref_pkg("rpart")` package:
...
The following code uses 3-fold cross-validation and the feature `year` of the `r ref("mlr_tasks_penguins")` task to determine the grouping.
...
Combining multiple `r ref("BenchmarkResult")` objects into a larger result object can be useful if related benchmarks where computed on different machines.
