...
To address this issue, existing strategies for estimating the generalization performance usually withhold a subset of the available data for evaluation purposes.
...
A custom resampling strategy can be constructed using `rsmp("custom")`, where the row indices of the obserations used for training and testing must be defined manually when instantiated in a task.
...
For example, without specifying a `"stratum"` column role, the `species` column will have quite different class distributions across the corresponding test sets of a 3-fold cross-validation strategy:
...
Please note that averaging accuracy scores across multiple tasks is not always appropriate for comparison purposes.
A more common alternative to compare the overall algorithm performace across multiple tasks is to first compute the ranks of each learner on each task separately and then compute the average ranks.
However, for illustration purposes, we show how to average the performance of each individual learner across all tasks:
...
Currently, Friedman tests and pairwise Friedman-Nemenyi tests [@demsar2006] are supported to analyze benchmark experiments with at least two tasks and at least two learners.
