...
That is, we want the model deployed in practice to generalize well to new, unseen data.
To assess this so-called generalization performance, we must first decide on a suitable [performance measure](#measures).
A performance measure usually provides a *score* indicating, e.g., how well the model predictions match the ground truth.
...
Assessing its performance only based on the training data would misleadingly suggest a very good model.
Hence, to avoid an optimistically biased performance estimate, we need to test the model on an independent dataset not used for training.
However, we typically train the final model deployed in practice on all available data, which leaves no data to evaluate its generalization performance.
We, therefore, need to adopt a strategy that partitions all available data into training and test data and thereby mimics the presence of new, unseen data.
<!-- This allows us to evaluate the learning algorithm that induced the final model, rather than the final model itself, by using a similar amount of data to train intermediate models which are only used for performance assessment. -->
<!-- We will therefore evaluate the next best option, which is the learning algorithm that induced the final model by assessing the performance of an intermediate model produced when the learning algorithm used a similar amount of data as for the final model.  -->
<!-- Consequently, we usually evaluate the inducer as the next best option, i.e., the learning algorithm that induced the final model.  -->
<!-- As a substitute, we evaluate the quality of intermediate models resulting from training the learning algorithm on a similarly large data set as was the case for the final model. -->
<!-- Therefore, as a next best option, we will instead evaluate the learning algorithm which induced the final model. -->
<!-- For this purpose, we usually evaluate the performance of one (or more) intermediate models that result from applying the learning algorithm to a similar amount of data as the learning algorithm used to train the final model. -->

<!-- describe performance estimation procedure and desiderata -->

The most straightforward strategy is to partition all available data into a single training and test dataset which is known as holdout.
Then train an intermediate model only on the training data, assess its performance on the test data, and use this performance estimate as a proxy for the performance of the final model.
...
This is because learning algorithms often struggle to learn complex relationships in the data when given a small amount of training data.
On the other hand, we also want as much test data as possible to reliably estimate the generalization performance with a sufficiently high amount of data.
As a trade-off, [resampling strategies](#resampling) typically repeat the data partitioning process in different ways (see @fig-ml-abstraction for an illustration).
...
<!-- explain resampling -->
Note that resampling strategies evaluate the performance of the learning algorithm that induced the final model as a proxy for the performance of the final model. 
However, this is the best we can do if we only have access to a limited amount of data.
Resampling typically provides more reliable performance estimates by ensuring that the majority of all available data points are used at least once for training and once testing across all resampling iterations.
Repeatedly splitting the entire data in different ways and averaging the performance across multiple splits can reduce the variance of the performance estimate and avoids that the estimate is strongly biased by any particular split.
...
TODO: should we add a fig for an overview of other resampling strategies and some remarks on practical guidelines?
...
Specifically, the function will internally use the learner to train a model for each training set determined by the resampling strategy and stores the model predictions of each test set.
...
Benchmarking is used to compare the performance of different learning algorithms applied on one or various tasks using potentially different resampling strategies.
The purpose is to ideally rank the learning algorithms regarding a performance measure of interest and to identify the best learning algorithms for a certain task or across various tasks.
...
We design an examplary benchmark experiment and train a classification tree from the `r ref_pkg("rpart")` package, a random forest from the `r ref_pkg("ranger")` package and a featureless learner (which serves as baseline) on four different binary classification tasks.
...
The constructed benchmark design is a `data.table` that contains the task, learner, resampling combinations in each row that should be performed.
It is also possible to, e.g., exclude a specific task-learner combination by manually removing a certain row from the design table.
...
Unsurprisingly, the featureless learner which by default predicts the majority class and often serves as baseline in benchmarking has lowest performance overall.
...
### Extracting, Converting and Merging

<!-- {{< include _optional.qmd >}} -->

A `r ref("BenchmarkResult")` object is a collection of multiple `r ref("ResampleResult")` objects, which can be extracted via the $resample_result(i)$ method, where $i$ is the index of the performed benchmark experiment.
