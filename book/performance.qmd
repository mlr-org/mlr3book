...
<!-- Repeating the data splitting process often enough increases the number of data points that are part of both the training and test set, allowing more efficient use of all available data for performance estimation. -->
...
Although increasing this value will lead to longer computation times, the benefit of performing more repetitions to obtain a more reliable performance estimate usually outweighs the higher computation times.
...
To actually generate the train-test splits for a given task, we need to instantiate a resampling strategy by calling the `$instantiate()` method of the previously constructed `r ref("Resampling")` object on the `r ref("mlr_tasks_penguins")` task.
...
For example, the code below shows the first list element and shows that we get the same result using the `$predictions()` method:
