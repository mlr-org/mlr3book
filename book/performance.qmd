...
The training set is used to create a provisional model as an intermediate result (hereafter abbreviated as intermediate model), whose sole purpose is to estimate the performance using the test set.
...
Below, we again summarize the entire performance estimation process of the holdout method, from partitioning the task and choosing a performance measure to calculating the score to estimate the generalization performance.
...
A `r ref("Measure")` object can also provide other information such as the time it took to train the model or the time to make predictions (e.g., using the measures with the `key`s `time_train` or `time_predict`).
