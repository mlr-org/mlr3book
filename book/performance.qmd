...
<!-- To avoid optimistically biased performance estimates, we should estimate the generalization performance of the final model using independent data not used to train the prediction model.  -->
...
As we will use the final model in practice to make predictions, we want it to generalize well to new, unseen data.
To assess this so-called generalization performance, we must first choose a suitable [performance measure](#measures). 
The performance measure usually calculates a *score* indicating, e.g., how well the predicted target values match the actual target values. 
Depending on the performance measure we employ, the score may also reflect other qualities of a learning algorithm such as its runtime.
To avoid optimistically biased performance estimates, we need an independent dataset not used for training to test our model on it.
However, the final model deployed in practice is usually trained on all available data. 
We, therefore, have no further data available that we could use to estimate the generalization performance of our model. 
To solve this problem, we need to devise a strategy that mimics the presence of new, unseen data.

The most straightforward strategy is to partition all available data into training and test data.
We then apply the same learning algorithm used to train the final model, but this time only on the training data.
The resulting model, which we refer to as a 'auxiliary model', serves only to estimate how the final model will perform on new, unseen data.
The remaining test data is then used to estimate the performance of the auxiliary model, which is then considered representative of the generalization performance of the final model.

However, the final model and the 'auxiliary model' are not necessarily similar because they were trained on data of different sizes. 
If the training data is too small, the performance estimate may be pessimistically biased because the learning algorithm sees fewer data, making it difficult to learn complex relationships. 
For the auxiliary model to represent the final model well, the training data should preferably be similar in size compared to all available data.
On the other hand, we also would like to use as many test data points as possible to reliably estimate the generalization performance. 
As the auxiliary model might perform poorly on the test data by chance, we usually repeat the partitioning process and average the resulting performance estimates to reduce the variance of our estimate. 
This gives rise to [resampling strategies](#resampling) that (repeatedly) partition the available data in different ways to obtain reliable performance estimates (see @fig-ml-abstraction for an illustration).
...
Additionally, we introduce practical convenience functions to conduct benchmark experiments that can be used to compare the generalization performance of multiple learners across various tasks.
...
In this section, we will use `r ref("Resampling")` objects that encode the resampling strategy we want to employ to estimate the generalization performance.
...
See also the section about [nested resampling](#nested-resampling) in the chapter on [model optimization](#optimization) when the learning algorithm involves tuning of hyperparameters.
...
It is considered best practice to repeatedly split the entire data in different ways to ensure that a "lucky" or "unlucky" split does not bias the generalization performance estimate.
