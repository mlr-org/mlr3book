...
Thus, we must first decide on a suitable [performance measure](#measures) to assess this so-called generalization performance.
This measure usually provides a *score* indicating, e.g., how well the model predictions match the ground truth values and may also reflect other qualities such as its runtime.
...
Hence, to avoid such a biased performance estimate, we should test the model on an independent dataset not used for training.
...
To solve this problem and obtain a reliable performance estimate, we follow a strategy that mimics the presence of new, unseen data.
...
If the training data is too small, the performance estimate may be pessimistically biased because a learning algorithm usually learns complex relationships in the data worse with fewer data. 
...
For example, a strategy like 10-fold cross-validation would be preferable over a single train-test split if the size of the data allows it.
...
In `r mlr3`, the resampling strategy we want to employ is determined by `r ref("Resampling")` objects which are constructed via the function `r ref("rsmp()")`.
Calling the function `r ref("resample()")` on a `r ref("Task")`, `r ref("Learner")` and a `r ref("Resampling")` object will return a `r ref("ResampleResult")` object which provides the performance results.
...
The following sections provide guidance with concrete examples on how to select a resampling strategy and how to use it.
...
It contains several common resampling strategies, including holdout, cross-validation, and bootstrapping.
Passing the dictionary to the `as.data.table` function provides a more structured output including the parameters of each resampling strategy (e.g., the number of repetitions):
...
Here, we will perform holdout resampling, which was already done manually in the [previous chapter](#train-predict).
To construct a `r ref("Resampling")` object for holdout, we can inspect the `r ref("mlr_resamplings")` dictionary and pass the corresponding key to the convenience function `r `ref("rsmp()")`. 
For example:
...
However, we can specify the `ratio` parameters for holdout during construction or update the parameters afterwards.
For example, we construct a `r ref("Resampling")` object for holdout with a 80:20 split (L1) then update to 50:50 (L2):
...
To obtain a more reliable performance estimate, we may use other resampling strategies.
For example, cross-validation randomly partitions the data into k subsets, called folds.
In total, k models are trained on k-1 of the folds, with the remaining fold being used as test data exactly once in each of the k iterations. <!-- TODO: add illustration? -->
The k performance estimates from each fold are then averaged to obtain a reliable performance estimate.
...
We can define 10-fold cross-validation via

```{r}
resampling = rsmp("cv", folds = 10)
```

The choice of the resampling strategy will always depend on the specific task at hand and the goals of the performance assessment.

By default, the `$is_instantiated` field of a `r ref("Resampling")` object which has been constructed as described above is set to `FALSE`.
This means that the resampling strategy is not yet applied to any task, i.e., the indices of the train-test splits are not contained in the `r ref("Resampling")` object.
...
In this section, we show how to instantiate a resampling strategy by applying it to a task.

To actually perform the splitting and obtain indices for the training and the test splits, we call the method `instantiate()` on our `r ref("Task")`.
