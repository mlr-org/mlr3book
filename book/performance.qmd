...
Consider spam classification as application where a model classifies a mail as spam (positive class) or no spam (negative class).
...
Here, we might accept a small number of false negatives, i.e., mails that are spam but not classified as such, as long as we can be sure that important mails are not falsely classified as spam.
...
These examples illustrate that there is a trade-off between different aspects of classification performance and a simple measure such as the classification accuracy can not accurately reflect all of them.
Especially in highly imbalanced cases where the target is not equally distributed, the classification accuracy can be a bad performance measure as it does not take into account the relative proportions of true and false positives or negatives.
For example, if the occurrence of a disease in the population is 1%, a model that always predicts "no disease" for each patient would have an high classification accuracy of 99% but will never be able to identify diseased patients.

Different fields have therefore derived various measures from the confusion matrix, as different applications tend to have different notions of what qualities a "good" classification model should exhibit.
