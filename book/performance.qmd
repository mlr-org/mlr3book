...
<!-- To solve this problem and obtain a reliable performance estimate,  -->

<!-- describe performance estimation procedure and desiderata -->

The easiest strategy is to partition all available data into a single training and test dataset.
...
This is because if the training data is too small, the performance estimate may be pessimistically biased since a learning algorithm usually learns complex relationships in the data worse with fewer data. 
...
As a trade-off, we usually repeat the data partitioning process and average the results to reduce the variance of our performance estimate. 

This allows us to evaluate the next best option the learning algorithm that induced the final model, rather than the final model itself. 
This is because we are evaluating the performance of one (or more) intermediate models that result from applying the learning algorithm to a similar amount of data as the learning algorithm that produced the final model has been trained on.
...
We can also plot ROC (receiver operating characteristics) curves for binary tasks.
For this purpose, we filter the `r ref("BenchmarkResult")` to only contain a single `r ref("Task")` and then use `type = "roc"` to create a ROC curve:
...
The package `r mlr3benchmark` provides some infrastructure for applying statistical significance tests on `r ref("BenchmarkResult")` objects.
...
TODO: Remove next section on extracting resampleresults?
...
```{r performance-028}
tab = bmr$aggregate(measures = msr("classif.acc"))
...
bmr = c(bmr1, bmr2)
bmr$aggregate(measures = msr("classif.acc"))
```
