...
Regardless of the algorithm, model, or type of task we are working with, we generally always apply the same procedure outline in @fig-ml-abstraction.
...
We cover how to select [resampling strategies](#resampling-settings) and obtain [resampling results](#resampling-exec).
...
```{r performance-003, echo=FALSE, fig.align="center"}
knitr::include_graphics("images/ml_abstraction.svg")
```
...
Besides the `"group"` column role, dedicated spatiotemporal resampling methods are available in `r mlr_pkg("mlr3spatiotemporal")`.
...
In the old `r mlr_pkg("mlr")` package, grouping was called "blocking".
:::
...
We create two measures to calculate the area under the curve (AUC) for the training and the test set, where we specify an `id` to identify the respective scores:
...
Next, we use the `r ref("partition()")` helper function to randomly split the rows of the Sonar task into two disjoint sets: a training set and a test set.
