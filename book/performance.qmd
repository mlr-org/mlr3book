...
To recap by example let's split the `tsk("penguins")` task with a 2/3 holdout, which is the default in `mlr3`:
...
For example, the `tsk("spam")` classification tasks is concerned with classifying if mail is spam (positive class) or not (negative class).
