...
For example, below we set the threshold to `0.99` and `0.01` for the `r ref("mlr_tasks_german_credit")` task to illustrate the two edge cases resulting in zero predicted positives and zero predicted negatives:
...
To understand the usefulness of this curve, first consider the simple case of deterministic a classifier that makes a single prediction that classifies an observations as either positive or negative.
...
For example, if each positive instance will be randomly classified with 25% as to the positive class, we would obtain a TPR of 0.25.
...
A natural performance measure that can be derived from the ROC curve is the `r index('area under the curve (AUC)', aside = TRUE)`.
...
We can also plot the `r index('precision-recall (PR) curve', aside = TRUE)` which visualizes the PPV vs. TPR.
