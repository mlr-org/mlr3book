...
For illustration, we fist define the corresponding `r ref("Task")` and `r ref("Learner")` objects which will be used throughout this chapter:
...
We can use the helper function `r ref("partition()")` to randomly create two disjoint sets containing the row indices used to partition the data into training and test set.
We then train the previously defined learner on the training set and make predictions on the test set which will be stored in a `r ref("Prediction")` object:
...
The `r ref("Prediction")` object contains both predictions (column `response`) and ground truth (column `truth`.
In performance estimation, we are usually interested in calculating a numeric score that quantifies the differences between predicted and ground truth values. 
In `r mlr3`, this score is usually determined by `r ref("Measure")` objects which define how the differences between predicted and ground truth values are quantified.
The next obvious step is to select a suitable performance measure from the many possibilities available, which can be done using the `r ref("mlr_measures")` dictionary.
...
Depending on our task at hand, we will look for a measure that fits our `task_type` (`"classif"` for the `penguins` task we are using here) and `task_properties`.
...
In the definition of the learner above, we could specify `predict_type = "prob"` to support measures during scoring that are not defined on class labels, such as the aforementioned AUC.
...
The code example below shows how to estimate the classification accuracy (using `msr("classif.acc")` as performance measure)  the `$score()` method of a `r ref("Prediction")` object  of the previously defined decision tree learner on the specified `r ref("mlr_tasks_penguins")` task:
