# Tuning with Hyperband {#hyperband .unnumbered}

```{r 04-optimization-hyperband-001, include = F}
library(mlr3)
library(mlr3book)
```

Besides the more traditional tuning methods, the ecosystem around `r mlr3book::mlr_pkg("mlr3")` offers another procedure for hyperparameter optimization called Hyperband implemented in the `r mlr3book::mlr_pkg("mlr3hyperband")` package.
...
```{r 04-optimization-hyperband-002, eval = TRUE, echo = FALSE, fig.show='hold', out.width = '99%', fig.align = 'center', fig.cap = "Visulization of how the training processes may look like. The left plot corresponds to the non-selective trainer, while the right one to the selective trainer.", warning=FALSE}
...
```{r 04-optimization-hyperband-003, eval = TRUE, echo = FALSE}
eta = 2
R = 8
result = data.frame()
...
```{r 04-optimization-hyperband-004}
library("mlr3verse")
...
Thanks to the broad ecosystem of the `r mlr3book::mlr_pkg("mlr3verse")` a learner does not require a natural budget parameter.
A typical case of this would be decision trees.
By using subsampling as preprocessing with `r mlr3book::mlr_pkg("mlr3pipelines")`, we can work around a lacking budget parameter.

```{r 04-optimization-hyperband-005}
set.seed(123)
...
```{r 04-optimization-hyperband-006}
...
```{r 04-optimization-hyperband-007}
...
```{r 04-optimization-hyperband-008}
...
```{r 04-optimization-hyperband-009}
instance$result
...
```{r 04-optimization-hyperband-010}
search_space = ps(
  nrounds = p_int(lower = 1, upper = 16, tags = "budget"),
...
```{r 04-optimization-hyperband-011}
...
Furthermore, we extended the original algorithm, to make it also possible to use `r mlr3book::mlr_pkg("mlr3hyperband")` for multi-objective optimization.
To do this, simply specify more measures in the `r ref("TuningInstanceMultiCrit")` and run the rest as usual.

```{r 04-optimization-hyperband-012}
...
```{r 04-optimization-hyperband-013}
instance$result
