...
```{r 08-special-density-001, include = F}
library(mlr3)
library(mlr3book)
```

```{r 08-special-density-002, include = F}
library(mlr3)
library(mlr3proba)
...
The conditional case, where a distribution is predicted conditional on covariates, is known as ‘probabilistic supervised regression’, and will be implemented in `r mlr3book::mlr_pkg("mlr3proba")` in the near-future.
...
The package `r mlr3book::mlr_pkg("mlr3proba")` extends `r mlr3book::mlr_pkg("mlr3")` with the following objects for density estimation:

* `"mlr3proba::TaskDens", text = "TaskDens")` to define density tasks
* `"mlr3proba::LearnerDens", text = "LearnerDens")` as base class for density estimators
* `"mlr3proba::PredictionDens", text = "PredictionDens")` as specialized class for `"Prediction")` objects
* `"mlr3proba::MeasureDens", text = "MeasureDens")` as specialized class for performance measures

In this example we demonstrate the basic functionality of the package on the `"datasets::faithful", text = "faithful")` data from the `r mlr3book::cran_pkg("datasets")` package.
This task ships as pre-defined `"TaskDens")` with `r mlr3book::mlr_pkg("mlr3proba")`.

```{r 08-special-density-003}
library("mlr3")
library("mlr3proba")
...
Hence, `TaskDens` is an unsupervised task which inherits directly from `r ref("Task")` unlike `TaskClassif` and `TaskRegr`.
...
In training, a `r mlr3book::cran_pkg("distr6")` object is created,
...
Finally, we conduct a small benchmark study on the `"mlr_tasks_precip", text = "precip")` task using some of the integrated survival learners:

```{r 08-special-density-005}
