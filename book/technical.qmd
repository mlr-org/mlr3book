...
This chapter provides an overview of technical details of the `r ref_pkg("mlr3")` framework.
...
Settings threads for implicit parallelization also works for filters from the `r ref_pkg("mlr3filters")` package as well as lists of objects, even if some objects do not support threading at all:
```{r technical-004}
library("mlr3filters")
...
We talk about explicit parallelization if `r ref_pkg("mlr3")` starts the parallelization itself.
The abstraction implemented in `r ref_pkg("future")` is used to support a broad range of parallel backends.
There are two use cases where `r ref_pkg("mlr3")` calls the `future` package: `r ref("resample()")` and `r ref("benchmark()")`.
...
Extension packages like `r ref_pkg("mlr3tuning")` internally call `r ref("benchmark()")` during tuning and thus work in parallel, too.


:::{.callout-tip}
When computational problems are so easy to parallelize, they are often referred to as "embarrassingly parallel".

Whenever you loop over elements with a map-like function (e.g., `r ref("lapply()")`, `r ref("sapply()")`, `r ref("mapply()")`, `r ref("vapply()")` or a function from `r ref_pkg("purrr")`), you are facing an embarrassingly parallel problem.
Such problems are straight-forward to parallelize, e.g., with the `r ref_pkg("furrr")` package providing map-like functions executed in parallel via the `r ref_pkg("future")` framework.
...
In `r ref_pkg("mlr3")`, this is no longer required (except for nested resampling, briefly described in the following section).
...
:::{.callout-caution}
During tuning with `r ref_pkg("mlr3tuning")`, you can often adjust the **batch size** of the `r ref("Tuner")`, i.e., control how many hyperparameter configurations are evaluated in parallel.
...
To demonstrate how to properly deal with misbehaving learners, `r ref_pkg("mlr3")` ships with the learner `r ref("mlr_learners_classif.debug", "classif.debug")`:
...
By default, `r ref_pkg("mlr3")` does not catch conditions such as warnings or errors raised by third-party code like learners:
...
`r ref_pkg("callr")` spawns a new R process to execute the respective step and thus even guards the current R session against segfaults.
...
So whenever the debug learner fails (which is every single time with the given parametrization) and encapsulation is enabled, `r ref_pkg("mlr3")` falls back to the predictions of the featureless learner internally:
...
The processed table is now used to create a `r ref("mlr3db::DataBackendDplyr")` from `r ref_pkg("mlr3db")`:
...
We create the following `r ref_pkg("mlr3")` objects:
...
It is heavily utilized in other [mlr-org](https://github.com/mlr-org) packages such as `r ref_pkg("mlr3")`, `r ref_pkg("mlr3pipelines")`, and `r ref_pkg("mlr3tuning")`.
...
When trying to set parameter values, e.g. for `r ref_pkg("mlr3")` `r ref("Learner")`s, it is the `$values` slot of its `$param_set` that needs to be used.
...
### Changing `r ref_pkg("mlr3")` logging levels

To change the setting for `r ref_pkg("mlr3")` for the current session, you need to retrieve the logger (which is a `r ref_pkg("R6")` object) from `r ref_pkg("lgr")`, and then change the threshold of the like this:
...
Also note that the optimization packages such as `r ref_pkg("mlr3tuning")`  `r ref_pkg("mlr3fselect")` use the logger of their base package `r ref_pkg("bbotk")`.
To disable the output from `r ref_pkg("mlr3")`, but keep the output from `r ref_pkg("mlr3tuning")`, reduce the verbosity for the logger `r ref_pkg("mlr3")`
...
`r ref_pkg("mlr3")` uses the [future](#parallelization) package and [encapsulation](#encapsulation) to make evaluations fast, stable, and reproducible.
