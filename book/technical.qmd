...
abstract:
    ...
    These are particularly important when computing on large, unmanageable datasets.
...
Note that the multicore backend is used here with `r ref("parallel::mclapply()")` which is not supported on Windows systems.
...
# unchunked approach
results = parallel::mclapply(seq_along(x), f, mc.cores = cores)
unlist(results, use.names = FALSE)

# chunked approach
chunks = split(seq_along(x), rep(1:cores, each = length(x) %/% cores))
results = parallel::mclapply(chunks, function(chunk) lapply(chunk, f), mc.cores = cores)
unlist(results, use.names = FALSE)
```
...
However, in practice, you cannot expect the runtime to fall linearly as the number of cores increases (`r link("https://en.wikipedia.org/wiki/Amdahl's_law", "Amdahl's law")`).
...
But unlike benchmarking, the loop iterations are not necessarily independent of each other and thus the nested loops cannot be flattened.
Instead, the user has to choose which of the loops to parallelize.
Let's consider the following example:
You want to tune the `minsplit` argument of a classification tree using the `r ref("AutoTuner")` of `r mlr3tuning` (simplified version taken from @sec-model-performance):
...
Usually, reproducibility is a major concern during parallelization because special `r link("https://en.wikipedia.org/wiki/Pseudorandom_number_generator", "this blog post")` (PRNGs) are required.
...
```
If this has been a regular learner, we could now start debugging with `r ref("traceback()")` (or create a `r link("https://stackoverflow.com/help/minimal-reproducible-example", "Minimal Reproducible Example (MRE)")` to file a bug report upstream).
