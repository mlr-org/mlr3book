...
* Optimizer proposes `batch.size` point proposals $theta$
* For each theta, a learner is created
* All learners are executed (possibly in parallel) via the benchmark() function and with the given (inner) resampling strategy

!!! batch.size * iterations should be a multiple of the number of available CPUs

!!! Terminator interrupts the processess only after complete batches, not inbetween. E.g., terminator PerformanceReached only interrupts after all thetas have been evaluated and one theta results in a performance better than the required threshold.
