...
* We call the parallelization platform together with its implementation for R the `r define("parallelization backend")`.
  As many parallelization backends have a different API, we are using the `r ref_pkg("future")` package as an abstraction layer for many parallelization backends.
...
An important step in parallel programming involves the identification of sections of the program flow which are time-consuming (otherwise parallelization would not make sense) and also can run in parallel to a different section.
...
2. A resampling consists of independent repetitions of train-test-splits.
3. A benchmark consists of multiple independent resamplings.
4. Tuning is repeated benchmarking (although embedded in a sequential procedure which determines the hyperparameter configurations to try next).
    Additionally, some tuners propose multiple configurations to evaluate independently in each step, which provides a second level for parallelization.
...
If the sequential execution is comparably fast, enabling parallelization often just introduces additional complexity and can even slow down the execution.
