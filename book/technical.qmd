...
While we have already learned how to set parameters in @sec-training and to define tuning spaces in @sec-tuning-instance, this chapter will provide a more in-depth understanding of the involved concepts.
...
<!-- FIXME: the footnote does not help, this is just confusing -->
...
* `r ref("TuneToken")` which allows for a convenient way to configure tuning spaces from existing `r ref("ParamSet")`s.

<!-- FIXME: maybe (re)move this? why do we need intermediate objects? can't we introduce tokens later, as "sugar"? -->
...
An empty parameter set -- albeit not yet very useful -- can be created by calling `ps()` without any arguments.
<!-- FIXME: what is a domain? do we need to explain this (here?)? -->
...
<!-- FIXME: we don't need an interesting example, we want to learn how to construct a param set -->
<!-- can we stick to one param set here in general? first retrieve it from a learner/pipeop, then manually re-construct it? -->
...
<!-- FIXME: start with the easy ones, then explain uty -->
...
1. `tags` which allow organizing parameters.^[An important example are the parameters of a learner, which are tagged with `"train"` and `"predict"` to annotate whether they influence the training phase, the prediction phase, or both.]
...
param_set
```
<!-- FIXME: should we start with a less complex example (w/o dependencies)? -->
...
<!-- FIXME: try to stick to the same names: univariate constraints == boundaries -->
...
Constraints of the form `==` are internally converted to objects of class `r ref("CondEqual")`, while `%in%` is converted to `r ref("CondAnyOf")`.
...
When using either of these functions, the resulting parameter set is checked for feasibility.
...
The reasoning is quite simple: `xval` controls the number of cross-validations to perform internally in order to find a good value for the complexity parameter required for pruning.
...
Initial values are also important for setting of parameters as you can (accidentally or intentionally) overwrite them.
...
The transformation that would tune `class.weights` for the `r ref("mlr_tasks_spam", text = "span")` dataset could be:
...
<!-- FIXME: reference mlr3tuningspaces somewhere -->
