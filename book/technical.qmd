...
Within the `r ref_pkg("mlr3verse")`it is used to define admissible configurations of objects such as the hyperparameter space of a learner or to specify the search space of an optimization algorithm, for example for hyperparameter optimization of a machine learning model.
...
* `r ref("ParamSet")` being a container object that combines multiple `r ref("Param", text = "parameters")` into a set.^[This is an oversimplification, as the `ParamSet` itself can also hold information beyond what is stored in the individual parameters.]
* `r ref("ParamSetCollection")` which allows to combine multiple `r ref("ParamSet", text = "parameter sets")` into a new `r ref("ParamSet")`.

In addition to those concepts, there are two other important classes that are used as intermediate objects to provide a more user friendly interface.
They are the

* `r ref("Domain")`, which is an intermediate object that is converted to a `r ref("Param")` object.
* The `r ref("TuneToken")` which enables a user friendly way to configure tuning spaces.

The first part of this chapter focuses on the usecase of `paradox` to define a configuration space of an object and the second part focuses on defining a tuning space.

For a more practical guide on how to define a parameter space when implementing a learner, we refer to the TODO section learner extending

### Defining and using Parameter Spaces

Within the `r ref_pkg("mlr3verse")`, parameters are only encountered in a pack, i.e. together with possibly other parameters in a `ParamSet`.
...
`r ref("ps")` takes named `r ref("Domain")` arguments that are turned into parameters, i.e. objects of class `r ref("Param")`.
Here, we create an artificual parameter set with parameters `method` and `iters`, where method can take the values `"m1"` (default) and `"m2"`, while `iters` must be a positive integer that can only be set when `method` is equal to `"m1"`.
...
There are five domain constructors that produce a parameter when passed as arguments to `r ref("ps")`:
...
The most important arguments for these domain constructors alllow to specify
...
1. the `default` value that specifies the behaviour if no value is set
1. parameter transformations (e.g. `trafo` and `logscale`)
1. `tags` which allow to organize parameters.
...
However, the dependencies and stored as meta information of the param set in the slot `$deps`.

```{r}
param_set$deps
```

Once a parameter set is defined, we can set its state, i.e. the slot `$values` that is being used when the configured object is called.
If we set this slot, the parameters are checked for compliance with the defined limits.
...
When working with mlr3 objects, it is important to be aware of the difference between *initial parameter values* and *defaults*.

As a practical example, we use the `classif.rpart` learner with a focus on its parameters `cp` and `xval`

```{r}
learner = lrn("regr.rpart")
as.data.table(learner$param_set)[id %in% c("cp", "xval"), c("id", "default")]
```

The parameter `cp` has a default of `0.01` and the parameter `xval` has a value of `10`.
...
In this case, the `rpart()` function uses the value `0.01` for `cp` if no value is provided and `10` for `xval`.
For this specific learner, we initialize the parameter values to the list shown above, which means that when using this learner without changing any parameters, it will not run with the default parameters.
Note that such changes are always documented in the learner's documentation.

If we want to go with the default behaviour of the `rpart` algorithm, we would have to do do the following
...
```{r}
#| echo: false
#| label: fig-technical-paradox-deps
#| fig-cap: Generated Design for Grid Search
library(ggplot2)
dat = rbindlist(generate_design_grid(search_space, 3)$transpose(), fill = TRUE)
dat$degree = as.character(dat$degree)
dat$degree[is.na(dat$degree)] = "NA"
theme_set(theme_bw())
ggplot(data = dat, aes(x = cost, y = kernel)) +
    geom_point() +
    geom_text(aes(cost, kernel, label = degree), .nudge_x = .5, .nudge_y = .5)
```
