...
* `r index("Main", aside = TRUE)` is the R session or process that orchestrates the computational work, called `r index("jobs", aside = TRUE)`.
...
So nowe we will turn to extending the `r ref("Measure")` class to implement new metrics.
