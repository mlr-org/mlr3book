...
We don't want to delve too deep into such details here, but want to introduce some terminology which helps us to discuss parallelization on a more abstract level:
...
An important step in parallel programming involves the identification of sections of the program flow which are time-consuming (`r define("bottlenecks")`) and also can run in parallel to a different section.
...
1. The learning algorithm (or other computationally intensive parts of a machine learning pipeline (@sec-pipelines)) *may* contain independent sections which can run in parallel, e.g.
   ...
   This parallelization level is covered in @sec-parallel-learner.
...
4. Tuning is repeated benchmarking, embedded in a sequential procedure which determines the hyperparameter configurations to try next.
...
The most atomic part of `r mlr3` which can be parallelized is a `r ref("Learner")`.
Here, `r mlr3` just provides a unified interface to control the execution, the parallel programming is implemented by the respective package author of the (external) code `r mlr3` calls.
...
The R interpreter calls the external code and waits for the results to be returned - without noticing that the computations were executed in parallel.
...
To enable the parallelization for this learner, we provide the helper function `r ref("set_threads()")`:
```{r technical-002}
# use 4 CPUs
...
:::{.callout-tip}
When computational problems are as easy to parallelize as resampling, they are often referred to as "embarrassingly parallel".
...
We use the `r ref("future::multisession")` parallel backend that should work on all operating systems.
...
```{r, eval = FALSE}
n = length(resamplings)
for (i in 1:n) {
    resampling = resamplings[[i]]
    m = resampling$iterations
    for (j in 1:m) {
...
All experiments are rolled out on the same level, i.e., `benchmark()` iterates over the elements of the Cartesian product of all resampling settings (combination of task, learner, and resampling strategy) and their respective resampling iterations.
...
Additionally, you can compute on up to $i \times j$ instead of only up to $\max(i, j)$ workers.
...
One general recommendation here is switching from the default `r link("Mersenne Twister", "https://en.wikipedia.org/wiki/Mersenne_Twister")` to Pierre L'Ecuyer's RngStreams (see `r ref("Random")` and `r ref("parallel::RNGstreams")`).
