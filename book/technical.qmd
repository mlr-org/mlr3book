...
For illustration (and internal testing) of error handling, `r mlr3` ships with the learners `lrn("classif.debug")` and `lrn("regr.debug")`.
...
`r mlr3` includes two baseline learners: `lrn("classif.featureless")`, which always predicts the majority class, and `lrn("regr.featureless")`, which always predicts the mean response (extension packages include task-specific baselines).
