...
The `r ref_pkg("paradox")` package (developed by the mlr3 team) implements a language for systematically representing parameter spaces that is used throughout the `r ref_pkg("mlr3verse")`.
One of the most important use cases is to define the feasible space of parameter settings for objects like `r ref("Learner", text = "learners")` or `r ref("PipeOp", text = "pipeline operators")`.
This provides the following advantages:

1. Available parameter settings, their data type and boundaries can easily be listed without crawling through the respective manual pages.
1. Hyperparameter settings can be checked for feasibility to immediately throw an error to inform the user about the invalid setting.
1. One or more parameters can be marked as search space for optimization algorithms.

While we have already learned how to set parameters in @sec-training and to define tuning spaces in @sec-tuning-instance, this chapter will provide a more in-depth coverage of the involved concepts.

The two most important classes of the `r ref_pkg("paradox")` package are the

* `r ref("Param")` representing a single parameter and the
* `r ref("ParamSet")`, which is a container object that combines multiple `r ref("Param")`s.
...
To illustrate how to work with them, we will be using the parameter set of `r ref("LearnerClassifSVM", text = "Support Vector Machine (SVM)")` from the `r ref_pkg("mlr3learners")` package throughout most of this section.
Akin to function arguments, the parameters allow to configure the behaviour of the `r ref("Learner")` during training and prediction.
Unlike function arguments, these parameters are explicitly annotated with the type of values they expect.

For simplicity, we consider only a subset of the parameters of the `r ref("LearnerClassifSVM")`. 
We can obtain this subset by calling the method `$subset()` with the ids of the parameters we want to keep.
```{r}
library("mlr3learners")
param_set = lrn("classif.svm")$param_set$subset(
  c("cost", "kernel", "degree", "shrinking", "type"))
param_set
```

The printed output above informs us what values are expected: 

* `cost` must be a "double" greater than 0 and the default is 1.
* `degree` is required to be an integer greater than 1. 
* `kernel` is a "factor", with 4 available choices that are listed in the documentation: `"linear"`, `"polynomial"`, `"radial"`, and `"sigmoid"`. The default is `"radial"`
* `shrinking` is a logical flag, the default being `TRUE`.
* `type` is also a "factor", where the 2 options are `"C-classification"` and `"nu-classification"`, the former being the default.

In addition, there are the columns *parents* and *value*.
The *parent* column indicates on which other parameters a `r ref("Param")` depends. 
For example, the parameter `cost` can only be set if `type` is set to `"C-classification"`, because the parameter does otherwise not apply.
Similarly, the (polynomial) `degree` can only be specified if the `kernel` is `"polynomial"`.

The *value* column indicates to which value the parameters are set.
This parameter set has none of its values set after initialization.
Note that some other learners such as `r ref("LearnerClassifRpart")` have values initialized that differ from their defaults.
For more on that, we refer to @sec-paradox-defaults.

We can inspect the individual parameters by accessing the field `$params`, which returns a named list of `r ref("Param")`s.
As an example, we show the parameter `kernel`. 
The printed information is similar to what we have already seen above, however it now also includes the available levels.

```{r}
names(param_set$params)
param_set$params$kernel
```


#### Defining a Parameter Set {#sec-paradox-define}

We can define a parameter set like seen above, by calling the `r ref("ps")` function.
...
The purpose of the `r ref("Domain")` object is merely to allow for a more convenient user interface.
In order to show how a parameter set can be created, we will reconstruct the parameter subset of the `r ref("LearnerClassifSVM",)` we saw earlier.
...
1. the admissible set of values for the parameter; for example `lower` and `upper` for `r ref("p_int")` and `r ref("p_dbl")` or `levels` for `r ref("p_fct")`. 
The `r ref("p_uty")` even allows to specify a `custom_check` function.
...
1. the `default` value that allows to record the behaviour if no value is set.
1. parameter transformations via a `trafo`. These will be covered in @sec-paradox-tune that deals with tuning spaces.
1. `tags` allow to organize parameters. An important example are the `"train"` and `"predict"` tags that indicate in which phase a parameter is used. These will be omitted here and are covered in @sec-extending-learners that explains how to write custom learners.

We can re-create the parameter set from earlier as follows.
```{r}
param_set_reconstr = ps(
  cost = p_dbl(lower = 0, default = 1, 
    depends = (type == "C-classification")),
  kernel = p_fct(levels = c("linear", "polynomial", "radial", "sigmoid"), 
    default = "radial"),
  degree = p_int(lower = 1L, default = 3L, 
    depends = (kernel == "polynomial")),
  shrinking = p_lgl(default = TRUE),
  type = p_fct(c("C-classification", "nu-classification"), 
...
In the section we will cover how to set its `$values`.

#### Setting Parameter Values {#sec-paradox-configure}

Once a parameter set is defined, we can set its state, i.e., the field `$values`.
The values in this slot are used when the configured object is called, e.g. when a learner is trained.
There are three ways to change the values of an existing parameter set that are summarized in the table below.
...
: Different ways of setting parameter values for an existing parameter set {#tbl-paradox-configure}


When using either of these functions, the resulting parameter set is checked for feasibility within the specified boundaries.
After setting the values, they can be accessed via the slot `$values`.

```{r}
#| error: true
# Shrinking must be logical
param_set_reconstr$set_values(shrinking = "FALSE")
param_set_reconstr$set_values(shrinking = FALSE)
# Parameter cost depends on type being "C-classification"
param_set_reconstr$set_values(cost = 2)
param_set_reconstr$set_values(type = "C-classification", cost = 2)

param_set_reconstr$values
```

#### Initial Values vs. Defaults {#sec-paradox-defaults}

We have mentioned above that parameter sets can have **initial values** (that are not set by the user), as well as **default values**.
An example for such a learner is the `r ref("LearnerClassifRpart")`.
Its parameter `xval` has a default of of 10.
However, its value is initialized to 0.

```{r}
learner_rpart = lrn("classif.rpart")
learner_rpart$param_set$default$xval
learner_rpart$param_set$values$xval
```

The default value of a parameter indicates what value is used when no value is set.
In this case, the `r ref("rpart::rpart")` function that is called under the hood when training `r ref("LearnerClassifRpart")` uses the value 10 for `xval`.

Because the value for parameter `xval` has been initially set to 10, the `r ref("LearnerClassifRpart")` will not run with the default values (yes this is slightly confusing!).
The reasoning for this change is quite simple: `xval` controls the number of cross-validations to perform internally in order to find a good value for the complexity parameter required for pruning.
As `r mlr3` does not prune the tree, the internal cross-validation is just wasting computation time and therefore turned off by setting `xval` to 0 initially during construction of the learner.
In other words, the default value *documents* the default which is used in absence of value while an initial value refers to the default value the mlr3 team favors.
We set initial values very sporadically, i.e., only when it really makes sense from a technical point of view.
All initial parameter values are documented in the description of the respective `r ref("Learner")`.

Being aware of initial values is also important when setting of parameters, as you can (accidentally or intentionally) overwrite them.
Consider the difference between the following parameter sets.
```{r}
learner_rpart$param_set$set_values(cp = 0.3)
learner_rpart$param_set$values

learner_rpart2 = lrn("regr.rpart")
learner_rpart2$param_set$values = list(cp = 0.3)
learner_rpart2$param_set$values
```

When one decides to replace all parameter `$values` with a new list, the initial values are unset.
This is not the case, when using method 1 and 2 from @tbl-paradox-configure.

#### Combining Parameter Sets

In this section, we will learn how to combine multiple parameter sets.
In the `r mlr3verse` this is commonly encountered when working with `r ref("Graph")`s from the `r ref_pkg("mlr3pipelines")` package.

Coming back to our support vector machine from earlier, we might have a dataset that contains missing values.
Because the SVM cannot handle them, some preprocessing is required, for which we can use the `r ref("PipeOpImputeHist", text = "PipeOp for histogram imputation")`.

```{r}
library("mlr3pipelines")
imputehist = po("imputehist")
classif_svm = po("learner", lrn("classif.svm"))
graph = imputehist %>>% classif_svm
```

The resulting `graph` contains a `r ref("ParamSetCollection")` from the two parameter sets of the `r ref("PipeOp")`s.
Note that a `r ref("ParamSetCollection")` inherits from the class `r ref("ParamSet")`, so it can be treated just the same.

In order to avoid name clashes for the ids, the newly created parameters have their ids prefixed by their `$set_id`.

```{r}
graph$param_set$ids()[1:2]
imputehist$param_set$set_id
classif_svm$param_set$set_id
```

To create such a `r ref("ParamSetCollection")` manuall, the `r ref("psc")` function can be used.

In the next section, we will explain how to configure the search space of optimization algorithms using the `r ref_pkg("paradox")` package.
...
As a guiding example, we will configure the tuning space for the `r ref("LearnerClassifSVM", text = "SVM")` that was already used earlier.
If we want to tune an SVM, we have to configure the search space that is explored by the tuner.
...
One way to configure a tuning space is to define a `r ref("ParamSet")` analogously to @sec-paradox-configure.
...
:::{.callout-tip}
The `r ref_pkg("mlr3tuningspaces")` extension contains predefined search spaces that obviate the need to define them manually.
:::
...
#| fig-cap: Design points from a grid search when tuning an SVM. The resolution is 5.
library(ggplot2)
...
#| fig-cap: Design grid for tuning a SVM. The resolution is 5, the cost parameter is logaithmically transformed when points with a `kernel` equal to `"polynomial"` are shifted to the right by a value of 2.
...
The transformation that would tune `class.weights` for the `r ref("mlr_tasks_spam", text = "spam")` dataset could be:
...
Like in @sec-paradox-define it is possible to specify dependencies between parameters.
...
A `r ref("TuneToken")` can also be constructed with a `r ref("Domain")` object, i.e. something constructed with a `p_<type>` call from @tbl-paradox-configure.
This way it is possible to tune continuous parameters with discrete values, or to give transformations or dependencies.
...
:::{.callout-note}
When introducing the dependency, we need to use the `degree` value from *before* the implicit transformation, which is the name or `as.character()` of the respective value, here `"val2"`!
:::
...
```{r optimization-042}
learner$param_set$set_values(cost = NULL, shrinking = NULL)
...
This `r ref("ParamSet")` must, however, have a `.extra_trafo` that returns a list with a single element, because it corresponds to a single hyperparameter that is being tuned.
