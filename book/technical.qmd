...
abstract:
    ...
    The chapter covers parallelization, error handling, logging, and how to operate on out-of-memory data.
    Additionally, the extension of the Measure class is demonstrated.
    Note that not all of this chapter is suitable for beginners and most of it can be skipped, but we recommend that you have at least a superficial knowledge of the topics so that you can skip to the appropriate section if you encounter a problem.
---
...
* Option `mlr3.exec_chunk_size` can be used to control how many jobs are mapped to a single `future` and defaults to 1.
  The value of this option is passed to `r ref("future.apply::future_mapply()")` while `future.scheduling` is set to `TRUE`.

In general, however, the defaults should be in order, adjusting these options is worthwhile only in rare cases.
...
3. the points proposed by the random search in a single batch.
...
* @fig-parallel-outer as an example for parallelizing the outer 5-fold cross-validation.
  ...
  Note that just setting up the parallelization with a simple `future::plan("multisession")` has the same effect - the most outer loop is parallelized while all subsequent loops are executed sequentially.
...
%%| fig-cap: CPU utilization for 4 CPUs while parallelizing the outer 5-fold cross-validation with a sequential 2-fold cross-validation inside.
gantt
...
%%| fig-cap: CPU utilization for 4 CPUs while parallelizing the inner 2-fold cross-validation with a sequential 5-fold cross-validation outside.
gantt
    title CPU Utilization
    dateFormat  s
    axisFormat %S
...
1. `r ref("DataBackendDplyr")` which interfaces the R package `r ref_pkg("dbplyr")`, extending `r ref_pkg("dplyr")` to work on many popular `r index("SQL")` databases like `r link("https://mariadb.org/", "MariaDB")`, `r link("https://www.postgresql.org/", "PostgresSQL")` or `r link("https://www.sqlite.org", "SQLite")`.
...
If you had a real `r index("database management system")` (DBMS), this would be the first step now:

```{r technical-026, echo = FALSE}
...
`r index("DuckDB")` databases (using `r ref("DataBackendDuckDB")`) provide a modern alternative to SQLite, tailored to the needs of machine learning.
...
While this is useful while working with many tasks simultaneously in order to keep the memory requirements reasonable, the more frequent use case for DuckDB are nowadays [`r index("Parquet")` files](https://en.wikipedia.org/wiki/Apache_Parquet).
...
The function $\mathbb{I(C)}$ denotes the `r link("https://en.wikipedia.org/wiki/Indicator_function", "indicator function")` with $\mathbb{I(C)} = 1$ if condition $C$ is true and $\mathbb{I(C)} = 0$ otherwise.
In code the measure may be written as:
...
I.e., we covered parallelization, error handling, logging, and working with data bases - all these topics might not be needed if you work on rather small and clean datasets.
Additionally, we demonstrated how to extend mlr3 with additional measures.

The sections presented in this chapter deal with problems that many users will encounter with some regularity.
Therefore, at least superficial knowledge is necessary to be able to identify the problem specifically and to be able to return to the correct section or, for more background information, the following resources.
