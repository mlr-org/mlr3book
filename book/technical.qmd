...
If there is a good estimate for the runtime, `batchtools::binpack()` (create an arbitrary number of chunks with a specified maximum combined runtime) and `batchtools::lpt()` (pack a specified number of chunks with arbitrary but homogeneous runtime) can prove useful - both are documented together with the `r ref("batchtools::chunk()")` helper.
...
For larger benchmarks with a cumulative runtime of weeks, months or even years, see @sec-large-benchmarking which covers parallelization on high-performance computing clusters.
