...
jobs = split(x, chunks)
system.time({
    parLapply(cl, jobs, function(chunk, .fun) sapply(chunk, .fun) , .fun = f, chunk.size = 1)
})
```
...
A very simple way to avoid this is given by just converting the `r ref("DataBackendDataTable")` to a .
...
* Assuming that the learner would actually calculate something and not just sleep: Would both CPUs be busy?
