...
`r mlr_pkg("paradox")` is the spiritual successor to the `r ref_pkg("ParamHelpers")` package and was written from scratch using the `r ref_pkg("R6")` class system.
...
To change the setting for `r mlr_pkg("mlr3")` for the current session, you need to retrieve the logger (which is a `r ref_pkg("R6")` object) from `r ref_pkg("lgr")`, and then change the threshold of the like this:
