...
As we have already demonstrated how to operate on a SQLite data base, and DuckDB is not different in that regard.
...
Accessing the data internally triggers a query and data is fetched to be stored in a in-memory `data.frame`, but only the required subsets.
