...
Settings threads for implicit parallelization also works for filters from the `r mlr3filters` package as well as lists of objects, even if some objects do not support threading at all:
```{r technical-004}
library("mlr3filters")
...
We talk about explicit parallelization if `r mlr3` starts the parallelization itself.
The abstraction implemented in `r ref_pkg("future")` is used to support a broad range of parallel backends.
There are two use cases where `r mlr3` calls the `future` package: `r ref("resample()")` and `r ref("benchmark()")`.
...
To demonstrate how to properly deal with misbehaving learners, `r mlr3` ships with the learner `r ref("mlr_learners_classif.debug", "classif.debug")`:
