...
This chapter provides an overview of technical details of the `r mlr3` framework.
This includes the following topics:

* Parallelization with the `r ref_pkg("future")` framework,
...
During threading, the implementation instructs some sequential parts of the code to be executed independently from the other parts in the same process.

For example, while fitting a decision tree, each split that divides the data into two disjoint partitions requires a search for the best cutpoint on all $p$ features.
So instead of iterating over all features sequentially, the search can be broken down into $p$ threads, each searching for the best cutpoint on a single feature.
...
I.e., for our example of the decision tree, (1) the $p$ best cutpoints per feature are collected and then (2) aggregated to the single best cutpoint across all features by just iterating over the $p$ results sequentially.


:::{.callout-warning}
It does not make practical sense to actually execute in parallel every operation that can be parallelized.
Starting and terminating workers (i.e., threads) as well as possible communication between workers comes at a price in the form of additionally required runtime and is called (parallelization) overhead.
...
The function which determines the number of CPUs is implemented in `r ref("parallelly::availableCores()")` and comes with reasonable heuristics for many setups.
...
Whenever you loop over elements with a map-like function (e.g., `r ref("lapply()")`, `r ref("sapply()")`, `r ref("mapply()")`, `r ref("vapply()")` or a function from package `r cran_pkg("purrr")`), you are facing an embarrassingly parallel problem.
Such problems are straightforward to parallelize with R, e.g., with the `r cran_pkg("furrr")` package providing map-like functions executed in parallel via the `r cran_pkg("future")` framework.
...
resample(
  task = tsk("penguins"),
  learner = at,
  resampling = rsmp("boot", folds = 5) # outer CV
)
```
...
Lets say that we have a single CPU with four cores available.
...
The later yields better runtimes if the runtimes are heterogeneous, especially if the parallelization overhead is neglectable in comparison with the runtime for the computation.
...
However, `r cran_pkg("future")` allows you to first chunk jobs together which combines multiple tasks into blocks which run sequentially on a worker, avoiding the intermediate synchronization steps.
:::

The resulting CPU utilization is visualized in Figure XX/Mermaid.
To optimize the overall runtime, the red boxes which indicate idling cores have to be minimized.
Apparently there is room for optimization, as three cores have nothing to work on while waiting for the computation of the fifth resampling iteration.
Note that in this simple example, the scheduling strategy has no influence since the runtimes are identical.

Alternatively, you can also opt to parallelize the inner CV with 2 folds, which yields a CPU utilization as displayed in Figure YY/Mermaid.
The outer loop runs sequentially and starts the parallelization five times on two cores.
As a result, two of the four available cores are constantly underutilized.


We can choose different parallelization backends for the inner and outer resampling loop, respectively.
We just have to pass a list of `r ref_pkg("future")` backends:
...
It can be achieved with `r ref_pkg("future")` by forcing a fixed number of workers for each loop:
...
To demonstrate how to properly deal with misbehaving learners, `r mlr3"` ships with the learner `r ref("mlr_learners_classif.debug", "classif.debug")` and `r ref("mlr_learners_regr.debug", "regr.debug")`.
Here, we will concentrate on the former debug learner for classification:
...
By default, `r mlr3` does not catch conditions such as warnings or errors raised by third-party code like learners:
...
One way to encapsulate the execution is provided by the package `r ref_pkg("evaluate")` (see the documentation of the `r ref("encapsulate()")` helper function for more details):
...
`r ref_pkg("callr")` spawns a new R process to execute the respective step and thus even guards the current R session against segfaults tearing down the complete R session.
...
Unfortunately, catching errors is only half the battle.
...
To handle the missing predictions gracefully during `r ref("resample()")` or `r ref("benchmark()")`, fallback learners are introduced next.
...
3. Impute a value that corresponds to a (weak) baseline.
   ...
   Note that a reasonable baseline value depends on the data in the training split.
...
The default backend uses `r ref_pkg("data.table")` via the `r ref("DataBackendDataTable")` as an very fast and efficient in-memory data base.

For bigger data, or when working with many tasks simultaneously in the same R session, it can be advantageous to interface an out-of-memory data to reduce the memory requirements.
There are multiple options to archive this:

1. `r ref("DataBackendDplyr")` which interfaces the R package `r ref_pkg("dbplyr")`, extending `r ref_pkg("dplyr")` to work on many popular data bases like [MariaDB](https://mariadb.org/), [PostgreSQL](https://www.postgresql.org/) or [SQLite](https://www.sqlite.org).
2. `r ref("DataBackendDuckDB")` for the impressive [DuckDB](https://duckdb.org/) data base connected via `r ref_pkg("duckdb")`: a fast, zero configuration alternative to SQLite.
3. `r ref("DataBackendDuckDB")`, again, but to work directly on [parquet files](https://parquet.apache.org/).


### DataBackendDplyr

To demonstrate the `r ref("DataBackendDplyr")` we use the NYC flights data set from the `r ref_pkg("nycflights13")` package and move it into a SQLite data base.
...
With the SQLite database in `path`, we now re-establish a connection and switch to `r ref_pkg("dplyr")`/`r ref_pkg("dbplyr")` for some essential preprocessing.
...
The processed table is now used to create a `r ref("mlr3db::DataBackendDplyr")` from `r mlr3db`:
...
We create the following `r mlr3` objects:
