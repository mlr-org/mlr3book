...
The `r ref_pkg("paradox")` package implements a language for systematically representing and handling parameter spaces.
One important usecase within the `r ref_pkg("mlr3verse")` is to define the space of valid parameter settings for a learner (and other objects).
...
The three central classes of the `r ref_pkg("paradox")` package are the:

* `r ref("Param")` representing a single parameter
* `r ref("ParamSet")` being a container object that combines multiple `r ref("Param")`s into a set.^[This is an oversimplification, as the `ParamSet` itself can also hold information beyond what is stored in the individual parameters.]
* `r ref("ParamSetCollection")` which allows to combine multiple `r ref("ParamSet")`s into a new `r ref("ParamSet")`.

Beyond that there are two other important classes that are used as intermediate objects to provide a more user friendly interface.
They are the:

* `r ref("Domain")`, which is an intermediate object that is converted to a `r ref("Param")` object.
* The `r ref("TuneToken")` which allows for an alternative way to configure tuning spaces.

The *first* part of this section covers on the definition of valid parameter settings for objects, while the *second* part focuses on the definition of tuning spaces.

### Defining and Configuring a ParamSet


#### Defining a ParamSet

Within the `r ref_pkg("mlr3verse")`, `r ref("ParamSet")`s are ubiquitous.
A very simple example is the repeated cross-validation, which has only two parameters `folds` and `repeats`.

```{r}
param_set = rsmp("repeated_cv")$param_set
param_set
```

We can inspect the contained `r ref("Param")` objects by accessing the field `$params`, which returns a named list of objects of type `r ref("Param")`.

```{r}
param_set$params
```

From the printed outputs, we can learn that both parameters must be integers, `folds` being at least $2$ and `repeats` at least $1$.
...
Such an empty parameter set -- albeit not yet very useful -- can be constructed using just the `ps()` call:

```{r}
ps()
```

`r ref("ps")` takes named `r ref("Domain")` arguments that are converted into `r ref("Param")`s and creates a `r ref("ParamSet")` from them.

As a next step, we can create an artifical parameter set with parameters `method` and `iters`, where the former can take the values `"m1"` (default) and `"m2"`, while the latter must be a positive integer that can only be set when `method` is equal to `"m1"`.

```{r}
param_set = ps(
  method = p_fct(default = "m1", levels = c("m1", "m2")),
  iters  = p_int(lower = 1, depends = method == "m1")
)
param_set
```

There are five domain constructors that produce a parameter when passed as arguments to `r ref("ps")` that can be used to represent different types of parameters.
The most flexible one is `r ref("p_uty()")` which always has to be used when none of the others are fitting.
...
1. the admissible set of values for the parameter: `lower` and `upper` for `p_int()` and `p_dbl()`, `levels` for `p_fct()`, `custom_check` for `p_uty()` and `special_vals` for all of the above.
...
1. parameter transformations (e.g. `trafo`)
1. `tags` which allow to organize parameters.^[An important example are the parameters of a learner, which tagged with `"train"` and `"predict"` to annotate whether they influence the training or the prediction phase.]

The univariate information stored in the `r ref("Domain")` objects are converted into `r ref("Param")`s, which can be accesses through the slot `$params`.

```{r}
param_set$params
```

However, we also specified higher-dimensional constraints via the parameter `dependencies`.
This information -- because it affects two parameters simultaneously -- is recorded in the slot `$deps` instead.

```{r}
param_set$deps
```

The rows in the `data.table` can be read as follows: "Whether the parameter `<id>` can can be set, depends on the parameter `<on>` satisfying the condition `<cond>`.
$hen constructing a `r ref("ParamSet")` from `r ref("Domain")` objects in the `ps()` call, constraints of the form `==` are converted to objects of class `r ref("CondEqual")`, while `%in%` is converted to `r ref("CondAnyOf")`.

#### Setting Parameter Values

Once a parameter set is defined, we can set its state, i.e. the slot `$values` that is being used when the configured object is executed.
When setting new values, the parameters are checked for compliance with the defined limits.

```{r}
#| error: true
param_set$values$method = "m3"
param_set$values$method = "m2"
```

```{r}
#| error: true
param_set$values$iters = 10
param_set$values = list(method = "m1", iters = 10)
```
...
To showcast their difference, we use a classification partition tree and focus on its parameters `cp` and `xval`

```{r}
param_subset = lrn("regr.rpart")$param_set$subset(c("cp", "xval"))
param_subset
```

The parameter `cp` has a default of `0.01` and `xval` of `10`.
However, the parameter `xval` is initialized to `0`.

This might be slightly confusing initially, but the explanation of this is as follows: The default value indicates what parameter is used when no value of the parameter set is set.
In this case, the `r ref("rpart::rpart")` function that is called by `r ref("LearnerClassifRpart")` uses the value `0.01` for `cp` if no value is provided and `10` for `xval`.

For this specific learner, we initialize the value of `xval` to `0`, which means that when using this learner without changing any parameters, it will not run with the default parameters.
Note that such changes are always documented in the learner's documentation.

If we want to go with the default behaviour o `r ref("rpart::rpart")`, we would have to set the parameter `xval` to either `10` or `NULL`.

This also has practical consequences, i.e. note the difference between the two parameter sets below.

```{r}
rpart1 = lrn("regr.rpart", cp = 0.3)
rpart1$param_set$values
rpart1$param_set$values
...
:::{.callout-tip}
In general it is a good idea to directly set parameter values when constructing a learner in the `lrn(...)` call.
This avoids accidentally un-setting initial values.
An alternative way to achieve this is the `$set_values()` method of the `ParamSet` that was already covered in (TODO).
:::


#### Combining Parameter Sets

Finally, we want to mention how it is possible to merge different `ParamSet`s.
In the `mlr3verse` this is encountered when working with `r ref("Graph")` objects from `r ref_pkg("mlr3pipelines")`.
...
The resulting `graph` creates a `r ref("ParamSetCollection")` from the two parameter sets of the pipeops and this paramset is created by the following call:
...
It is to be noted that the parameter ids have a new suffix, which was inferred from their `$set_id`.
...
While it is possible that a parameter sets has no set id, it always has to be ensured that all parameters have unique ids.


### Defining a Tuning Spaces {#sec-tuning-space}

#### Creating a Tuning Space from Scratch

This section will put an emphasis of those features of `r ref("paradox")` that are relevant for defining the search space of an optimization algorithm.
Aspects that are relevant for tuning, which were not relevant in the previous section are the property of `$is_bounded`, the `r ref("TuneToken")` and parameter transformations aka the `$trafo`.

As a guiding example, we will configure the tuning space for a `r ref("LearnerRegrSVM", text = "svm")` from the `r ref_pkg("mlr3learners")` package.
For a background on hyperparameter optimization within the `r ref_pkg("mlr3verse")` we refer to @sec-model-tuning.

If we want to tune a svm, we have to configure the search space that we want to explore.
One important aspect here is that when tuning in mlr3, the search space has to be bounded:
...
ps(cost = p_uty())$is_bounded
```

One way to configure a tuning space is to define a `r ref("ParamSet")` just like we did before.
...
Optimizers such as `r ref("TunerGridSearch")` are able to generate proposed points (aka a `r ref("Design")`) from such a `ParamSet`.
Generating a grid with resolution 5, will result in the following search space to be explored by the tuner. ^[Note that for factorial parameters, always all factors are used, independent of the resultion.]

```{r}
#| echo: false
#| label: fig-technical-paradox-vanilla
#| fig-cap: Vanilla Design Grid for SVM Tuning. Resolution 5.
library(ggplot2)
...
ggplot(dat1, aes(cost, kernel)) +
    geom_point() +
    labs(title = "Grid Based Tuning Space for SVM")
```
...
```{r optimization-031}
search_space = ps(
  cost = p_dbl(log(0.1), log(1), trafo = function(x) exp(x)),
...
ggplot(data = dat2, aes(cost, kernel)) +
    geom_point() +
    labs(title = "Grid Based Tuning Space for SVM with log transformation")
```

:::{.callout-tip}
Because the logscale tranformation is so common, the domain constructors `p_int()` and `p_dbl()` have a flag `logscale` that can be enabled to use a logarithmic parametrization.
:::
...
#| fig-cap: Design Grid for SVM Tuning with Logscale Transformation for Parameter `kernel`. Resolution 5.
search_space = ps(
  cost = p_dbl(log(0.1), log(1), trafo = function(x) exp(x)),
  kernel = p_fct(c("polynomial", "radial")),
...
Although we have already covered *parameter dependencies* in the previous section, we will quickly cover their role when defining search spaces.
