...
* working with out-of-memory data, e.g., data stored in data bases (@sec-backends),
...
In general, there are many possibilities to parallelize, depending on the hardware to run the computations: If you only have a single machine but multiple cores, [threads](https://en.wikipedia.org/wiki/Thread_(computing)) or [forks](https://en.wikipedia.org/wiki/Fork_(system_call)) are ways to utilize all cores.
...
In this case, the number of workers must be manually tweaked so that the system does not get overburdend:
...
```
If this would be a regular learner, we could now start debugging with `r ref("traceback()")` (or create a [MRE](https://stackoverflow.com/help/minimal-reproducible-example) to file a bug report upstream).

However, as ML algorithms raising errors is not uncommon and we often just need to move on, we need a mechanism to

  1. capture all signaled conditions such as messages, warnings and errors so that we can analyze them post-hoc (@sec-encapsulation), and
  1. a statistically sound way to proceed while being able to aggregate over partial results (@sec-fallback).

These two mechanisms are explained in the following subsections.

### Encapsulation {#sec-encapsulation}

Encapsulation ensures that signaled conditions (such as messages, warnings and errors) are intercepted: all conditions are logged into the learner, and errors do not interrupt the program flow.
Each `r ref("Learner")` has a field `encapsulate` to control how the train or predict steps are wrapped.
The easiest way to encapsulate the execution is provided by the package `r ref_pkg("evaluate")` (see the documentation of the `r ref("encapsulate()")` helper function for more details):
...
With either of these encapsulation methods, we can now catch errors and post-hoc analyse the messages, warnings and error messages.
...
For bigger data, or when working with many tasks simultaneously in the same R session, it can be required to interface out-of-memory data to reduce the memory requirements.
...
3. `r ref("DataBackendDuckDB")`, again, but to work directly on [parquet files](https://parquet.apache.org/) stored on the local file system.
...
As we have already demonstrated how to operate on a SQLite data base, we just briefly demonstrate how to convert a task to a DuckDB and then focus on parquet files.

To convert a `data.frame` to DuckDB, we have the helper function `r ref("as_duckdb_backend()")` and provide a path.
Here, we create the flights dataset in a temporary folder:
```{r}
library("mlr3db")
data("flights", package = "nycflights13")
backend = as_duckdb_backend(flights[complete.cases(flights),], path = tempfile())
```

Next, we convert it to a regression task with the target column `"arr_delay"`:
```{r}
task = as_task_regr(backend, target = "arr_delay")
```
We can work with the task normally.
When the data is requested, it is queried in the background from the database on the file system and the required parts are loaded into memory, e.g., for learners to train and predict on:
```{r}
learner = lrn("regr.rpart")

# remove POSIXct column for the rpart learner
task$set_col_roles("time_hour", remove_from = "feature")

# train on the first 1000 rows
learner$train(task, row_ids = 1:1000)

learner$predict(task, row_ids = 1001:2000)$score()
```

It is also possible to directly operate on [parquet files](https://en.wikipedia.org/wiki/Apache_Parquet), without converting them.
To demonstrate this, we first query the location of an example parquet file shipped with `r ref_pkg("mlr3db")`:

```{r}
path = system.file(file.path("extdata", "spam.parquet"), package = "mlr3db")
```
We can then again create a `r ref("DataBackendDuckDB")` based on this file and create a classification task without loading the dataset into memory:
```{r}
