...
Second, they are used to define search spaces for the optimization of hyperparameters.
...
### Defining and Configuring Objects {#sec-paradox-define-configure}


Within the `r ref_pkg("mlr3verse")`, `r ref("ParamSet")`s are ubiquitous.
A relatively simple example are the parameters of the `r ref("PipeOpClassBalancing")`, which contains four parameters `ratio`, `reference`, `adjust`, and `shuffle`.
Akin to function arguments, they allow to configure the behaviour of the `r ref("PipeOp")` during training. 
Unlike function arguments, these parameters are explicitly annotated with the type of values that they accept.

The printed output below shows that the `ratio` must be a double(-like) value greater than `0`, `reference` and `adjust` categoricals from a predefined set, and `shuffle` a logical flag.
None of the parameters has a default, but they have their `$values` slot initialized, which can be seen in the rightmost column.^[More on the difference between default values and initial values can be found in @sec-paradox-configure .]
...
We can inspect the individual parameters by accessing the field `$params`, which returns a named list of objects of type `r ref("Param")`.
...
#### Defining a ParamSet {#sec-technical-paradox-define}

We can define parameter set by calling the `r ref("ps")` function.
...
: Domain Constructors {#tbl-paradox-define}
...
1. `tags` which allow to organize parameters.^[An important example are the parameters of a learner, which are tagged with `"train"` and `"predict"` to annotate whether they influence the training or the prediction phase.]

For each of the domain objects, a `r ref("Param")` is created which can be accesses through the slot `$params`.
...
This information -- because it affects two parameters simultaneously -- is recorded in the field `$deps`.
```{r}
param_set$deps
```
...
When constructing a `r ref("ParamSet")` from `r ref("Domain")` objects in the `ps()` call, constraints of the form `==` are converted to objects of class `r ref("CondEqual")`, while `%in%` is converted to `r ref("CondAnyOf")`.


#### Setting Parameter Values {#sec-paradox-configure}

Once a parameter set is defined, we can set its state, i.e. the field `$values`.
The values in this slot are used when object that is configured in the parameter set is called.

There are four different ways to change the values of an existing parameter set:
...
To show their difference we inspect the parameter set of the classification and partition tree, with a focus on the  `cp` and `xval` parameters.
...
However, the parameter `xval` is initialized to 0, which means that the slot `$values` has the following value

```{r}
learner$param_set$values
```

Here the default value indicates what value is used when nothing is specified.
...
For this specific learner, the value of `xval` is initialized to 0, which means that when using this learner without changing anything, it won't run with the default values (yes this is slightly confusing!).
...
While the methods in table 
As a final note, it has to be mentioned, that not all values that are set during construction of objects via their sugar function (`lrn()`, `po()`, ...) are parameters. 
These sugar functions also allow to set fields and construction arguments, such as the predict type or the id of an object.
...
In this section, we will learn how it is possible to combine multiple parameter sets.
In the `r mlr3verse` this is encountered when working with `r ref("Graph")` objects from the `r ref_pkg("mlr3pipelines")` package.
...
The resulting `graph` creates a `r ref("ParamSetCollection")` from the two parameter sets of the pipeops.
...
In order to avoid id conflicts, the newly created parameters a new prefix, which was inferred from their `$set_id`.
...
A special instance of this problem is the optimization of hyperparameters of ML algorithms through e.g. the `r mlr3tuning` package.

Aspects that are relevant for tuning, which were not relevant in the previous section are the property `$is_bounded`, the `r ref("TuneToken")`, and parameter transformations aka the `$trafo`.
...
For that the search space has to be bounded:
...
Optimizers such as `r ref("TunerGridSearch")` generate proposed points (aka a `r ref("Design")`) from a search space that is then systematically explored.
Generating a grid with resolution 5, will result in the following search space. ^[Note that for factorial parameters all levels are used, independent of the resolution.]
...
#| fig-cap: Design Grid for SVM Tuning. Resolution 5 with logarithmic transformation for the cost parameter.
...
:::{.callout-tip}
Because the logscale tranformation is so common, the domain constructors `p_int()` and `p_dbl()` have a flag `logscale` that can be set to use a logarithmic parametrization.
:::
...
We believe that the parameter `cost` should be set to a higher value when the `kernel` is `"polynomial."` and therefore shift it to the right by a value of 4.
...
#| fig-cap: Design Grid for SVM Tuning with Logscale Transformation for Parameter `cost`. Resolution is 5.
