...
Not all algorithms can be parallelized, but if they are, parallelization allows significant savings in computing time.
...
Most of these algorithms are parallelized via threading, e.g., the random forest implementation in `r ref("mlr_learners_classif.ranger", text = "ranger")` or the boosting implemented in `r ref("mlr_learners_classif.xgboost", text = "xgboost")`.
...
The loop is executed via `r ref_pkg("future")` parallelization framework, using the parallel backend configured by the user via the `r ref("future::plan()")` function.
...
The multicore backend (`plan("multicore")`) comes more overhead than threading, but considerably less overhead in comparison with the multisession backend.
