...
We refer to the respective documentation of the learner's implementation, e.g., `r link("https://xgboost.readthedocs.io/en/stable/gpu/index.html")` for `r ref("mlr_learners_classif.xgboost")`.
:::
...
Many machine learning algorithms can parallelize their model fit using threading, e.g., the random forest implementation in `r ref("mlr_learners_classif.ranger")` or the boosting implemented in `r ref("mlr_learners_classif.xgboost")`.
...
In this section, we will use the `r ref("mlr_tasks_spam")` task and a simple classification tree (`r ref("mlr_learners_classif.rpart")`) to showcase the explicit parallelization.
...
For illustration (and internal testing) of error handling, `r mlr3` ships with the learners `r ref("mlr_learners_classif.debug")` and `r ref("mlr_learners_regr.debug")`.
...
In mlr3, `r ref("Task")`s store their data in an abstract data object, the `r ref("DataBackend")`.
...
* A `r ref("TaskRegr")`, based on the previously created `r ref("mlr3db::DataBackendDplyr")`.
* A regression learner (`r ref("mlr_learners_regr.rpart")`).
...
* Measures "`r ref("mlr_measures_regr.mse")`", "`r ref("mlr_measures_time_train")`" and "`r ref("mlr_measures_time_predict")`"
...
A list of all slots can be found in the help page of `r ref("Param")`.
...
For example we can consider the `minsplit` parameter of `r ref("mlr_learners_classif.rpart")`.
...
For `r ref("Learner")` or `r ref("PipeOp")` objects, typically "unbounded" `r ref("ParamSet")`s are used.
Here, however, we will mainly focus on creating "bounded" `r ref("ParamSet")`s that can be used for tuning.
See @sec-paradox for more details on using `r ref("ParamSet")`s to  define parameter ranges for use-cases besides tuning.
...
Having to define a tuning `r ref("ParamSet")` for a `r ref("Learner")` that already has parameter set information may seem unnecessarily tedious, and there is indeed a way to create tuning `r ref("ParamSet")`s from a `r ref("Learner")`'s `r ref("ParamSet")`, making use of as much information as already available.
