...
Currently, controlling the number of threads is possible for some learners and filters from the `r mlr_pkg("mlr3filters")` package:
...
Here, we talk about explicit parallelization if `r mlr_pkg("mlr3")` starts and controls the parallelization itself.
...
All kinds of experiments are rolled out to be on the same level.
...
You want to tune the `minsplit` argument of a classification tree using the `r ref("AutoTuner")` of `r mlr_pkg("mlr3tuning")` (simplified version taken from the [nested resampling section](#sec-model-performance)):
...
To demonstrate how to properly deal with misbehaving learners, `r mlr_pkg("mlr3")` ships with the learner `r ref("mlr_learners_classif.debug", "classif.debug")` and `r ref("mlr_learners_regr.debug", "regr.debug")`.
...
`r cran_pkg("callr")` spawns a new R process to execute the respective step and thus even guards the current R session against segfaults tearing down the complete R session.
...
3. `r ref("DataBackendDuckDB")`, again, but to work directly on [parquet files](https://parquet.apache.org/) stored on the file system.
