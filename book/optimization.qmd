...
We start by tuning our SVM on a training dataset and then estimating the generalization performance on unseen data.

```{r optimization-032}
split = partition(task_sonar)
train_task = tsk("sonar")$filter(split$train)
test_task = tsk("sonar")$filter(split$test)
...
rr = resample(task_sonar, at, rsmp3)
rr$aggregate()
```
