...
Until now, we have focused on working examples but to be demonstrate encapsulation we will now consider 'broken' examples, e.g., where learners do not converge, run out of memory, or terminate with an error.
...
After the previous section has covered how to make the tuning process more robust to failures, we will now show how to exercise more fine-grained control over the sampling distribution.
In `r ref_pkg("mlr3")`, the standard way to modify the sampling distribution -- besides changing the tuner from e.g. grid search to random search -- is to apply a transformation.
To show how this works, we use the SVM's `cost` and `gamma` parameters that we have tuned in the example above.
These are usually not tuned on a linear, but logarithmic scale.
...
Hyperparameters can be set to the `r ref("TuneToken")` in the same way that they are set to specific values.
