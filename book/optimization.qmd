...
An example is given by the `mtry` parameter of a Random Forest that determines the number of variables to
possibly split at in each node when constructing a tree.
As hyperparameters often strongly influence the performance of a model, their values should be selected carefully.

The goal of Hyperparameter optimization (HPO) is to find the optimal configuration of hyperparameters of an ML
...
Beyond the definition of the search space, neither a closed-form mathematical representation nor analytic gradient information is available.
...
Many sophisticated HPO methods have been developed over the last decades which can be used to tackle the HPO problem efficiently and also ensure reproducibility.
