...
In @sec-search-space-scratch we will look at how to implement more complex, customt transformations to any hyperparameter or combination of hyperparameters.
...
To best estimate the generalization performance we make use of the `"moons"` `r ref("TaskGenerator", index = TRUE, aside = TRUE)`.
...
You can select one configuration and pass it to a learner for training using `$result_learner_param_vals`, so if we want to select the first configuration we would run:
...
To unpick what these terms mean and to motivate multi-fidelity tuning, say that we think a random forest with 1000 trees will be a very good fit to our training data however we are concerned this model will take too long to run.
Therefore, we want to predict the performance of this model using a less complex model that is quicker to train by setting a smaller number of trees.
In this example, the hyperparameter controlling the number of trees is a `r index('fidelity parameter', aside = TRUE)`, as it controls the tradeoff between model performance and complexity.
The different configurations of this parameter are known as `r index('fidelity levels', aside = TRUE)`.
We refer to the model with 1000 trees as the model at `r index('full-fidelity', aside = TRUE)` and we want to approximate this model's performance using model's at different fidelity levels.
...
Other common model's that have fidelity parameters include neural networks (number of epochs) and gradient boosting machines (iterations and data subsampling).
...
@tbl-hyperband-eg demonstrates how this schedule may look for a resulting hyperparameter configuration in each stage and bracket where the best configurations from each stage are promoted to the next assuming we are tuning the `max_depth` parameter of `classif.xgboost` between $1,...,20$.
...
|     | $s = 3$ | $s = 2$ | $s = 1$ | $s = 0$ |
|---- | ---------------- | ---------------- | ---------------- | ---------------- |
| $i = 0$   | $\{2, 6, 7, 10, 13, 15, 18, 19\}$ | $\{9, 11, 14, 15, 18, 20\}$ |  $\{2, 4, 16, 19\}$ | $\{11, 12, 14, 15\}$ |
| $i = 1$   | $\{7, 13, 18, 19\}$ | $\{9, 14, 15\}$ | $\{16, 19\}$ |  |
| $i = 2$   | $\{13, 18\}$ | ${14}$ | |  |
| $i = 3$   | $\{13\}$ |  |  |  |
| $HPC^*_s$   | $\{13\}$ | $\{14\}$ | $\{16\}$ | $\{14\}$ |
| $HPC^*$   | `max_depth = 14` |

: Hyperparmeter configurations in each stage and bracket from schedule in @tbl-hyperband. $HPC^*_s$ is the optimal hyperparameter configuration in bracket $s$ and $HPC^*$ is the optimal hyperparameter configuration across all brackets. {#tbl-hyperband-eg}
...
<!-- FIXME: REDO BELOW ONCE MOVED AFTER RESTRUCTURE -->
...
We will first look at how to handle any errors that may occur during tuning, and then how to manage tuning experiments that may use up a lot of computer memory.
