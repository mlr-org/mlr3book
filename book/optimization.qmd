...
Therefore we must define a `r index("tuning budget", aside = TRUE)` which specifies when to terminate the HPO process.
...
The `resolution` and `batch_size` parameters are termed `r index("control parameters", aside = TRUE)`, and other tuners will have other parameters that can be set, as with learners these are accessible with `$param_seta`.
...
<!-- FIXME: AUTOTUNER TUNES MODEL ON GIVEN DATA THEN FITS MODEL ON ALL DATA -->
...
<!-- FIXME: RENAME TO META-OVERFITTING OR OVERFITTING AND TUNING -->
...
Using `$aggregate()` is therefore the only appropriate way to return an unbiased estimate of model performance.
 ...
 <!-- FIXME: SHOW WHTHAT AGGREGATE IS CLOSE TO SCORE ON TEST -->
...
<!-- FIXME: Generators nice because we have unlimite -->
...
So far we have considered optimizing a model with respect to one metric, but multi-metric, or `r index("multi-objective", aside = TRUE)` optimization, is also possible.
