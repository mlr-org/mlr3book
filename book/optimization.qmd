...
These methods are particularly useful for identifying if the optimized hyperparameters are (meta)overfitting the data, as we will demonstrate now.

### Overfitting in tuning {#sec-resample-overfitting}

{{< include _optional.qmd >}}

In this short section we will briefly demonstrate the right and wrong way to report model performance when running nested resampling experiments.
By example we will tune the complexity parameter of a regression tree with 4-fold cross-validation, grid search, and use mean absolute error to optimize hyperparameters.
We optimize the model on the `kc_housing` task, which has 21,613 rows and can therefore provide a good approximation to the model's generalization performance.
First we remove the date column which cannot be handled by our learner, and then split the task into a training dataset with 100 rows and a test set with all remaining observations.

```{r}
at_RF = auto_tuner(
  tuner = grid_search,
  learner = lrn("classif.rpart", cp = 0, minsplit = to_tune(1, 100)),
  resampling = rsmp("cv", folds = 5),
  measure = msr_ce,
)

task = tgen("moons", sigma = 20)
train_task = task$generate(1e4)
test_task = task$generate(1e7)
# task = tsk("kc_housing")
# task = task$select(setdiff(task$feature_names, "date"))
# train_data = sample(task$nrow, 50)
# test_data = setdiff(seq(task$nrow), train_data)
# train_task = task$clone()$filter(train_data)
# test_task = task$clone()$filter(test_data)
```

In this experiment we want to demonstrate that only `$aggregate()` provides a reliable estimate of the model's generalization performance.
So in the first instance we will compute the generalization performance in the usual way by training our learner (in this case an `AutoTuner`) on a training dataset and estimating the performance on a separate, test dataset, we then run our resampling experiment by passing the `AutoTuner` to `resample`, below we use 4-fold CV for the inner resampling and then calculate the results using `$aggregate()`.

```{r}
pred = at_RF$train(train_task)$predict(test_task)
rr_overfit = resample(train_task, at_RF, rsmp("holdout"), store_models = TRUE)

c(generalization = pred$score(),
  inner_resamp = extract_inner_tuning_results(rr_overfit)[, .(classif.ce)],
  outer_resamp = rr_overfit$aggregate(msr_ce))
```

The first table shows that in most iterations the optimal configurations in the inner resampling have a lower MAE than the tuned model in the outer resampling, this demonstrates that the optimized hyperparameters are overfitting the data as the optimal hyperparameters perform best on their training data.
Hence by taking the mean over the scores (i.e., by using `$aggregate()`) we obtain a performance estimate that better reflects the hyperparameter performance on external test data.
<!-- FIXME: NEED TO FIX OR DELETE ABOVE -->
...
<!-- FIXME: PROBABLY DELETE THIS EXAMPLE -->
