...
Hyperparameter tuning is supported via the `r ref_pkg("mlr3tuning")` extension package.
The `r ref_pkg("mlr3verse")` loads all functions of the extension package.

```{r optimization-002}
library("mlr3verse")
```

At the heart of `r ref_pkg("mlr3tuning")` are the R6 classes
...
We use the `r ref_pkg("rpart")` classification tree and choose a subset of the hyperparameters we want to tune.
...
The `r ref_pkg("mlr3tuningspaces")` extension package provides predefined tuning spaces from scientific articles.
...
`r ref_pkg("mlr3tuning")` allows specifying complex termination criteria by selecting one of the available `r ref("Terminator", text = "Terminators")`.
...
There are multiple `r ref("Tuner", "Tuners")` in `r ref_pkg("mlr3tuning")`, which implement different HPO algorithms.
...
Not all available algorithms are implemented in `r ref_pkg("mlr3tuning")`.
...
The `r ref_pkg("mlr3viz")` package provides visualizations for tuning results.
...
## FIXME #463 autoplot(instance, type = "surface")
```
...
```{r optimization-017}
## FIXME #464 - learner$param_set$values = instance$result_learner_param_vals
```

We can train the learner on the full dataset.

```{r optimization-018}
## FIXME #465 - learner$train(tsk("penguins"))
```
...
We can automate the tuning process in `r ref_pkg("mlr3")` so that learners are tuned transparently, without the need to extract information on the best hyperparameter settings at the end.
