...
%%| fig-cap: Illustration of biased tuning. That's not how you should tune. An algorithm is tuned by training on the `Train` dataset and then the optimal configuration is selected by evaluation on the `Test` data. The model's performance is then evaluated with the optimal configuration on the same data.
%%| alt-text: Flowchart representing biased tuning. That's not how you should tune. The diagram shows input 'Search Space' passed into 'Algorithm' which then has a black arrow to 'Train',then a black arrow to 'Test', then a black arrow to 'Optimal configuration'. This has a red arrow back to 'Algorithm' then to 'Train' then to 'Test' then to 'Performance'. The diagram clearly shows the reuse of the same training and testing data.
...
flowchart LR
    ...
    linkStyle 0,2,4,5 stroke:#000
    linkStyle 1,3,6,7 stroke:#1F988BFF
    classDef node fill:#39568CFF, color:#fff, stroke:#fff
```
