...
Note that we make use of a `r ref("TaskGenerator")` in this example, which are used to create simulated data for experiments that are typically 'easy' to solve, we will not discuss them in detail in this book but refer the reader to the `r ref("mlr_task_generators")` dictionary to see implemented generators, and the help page for each of these to see how the data is simulated.
