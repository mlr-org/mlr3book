...
To best illustrate overfitting we will run the experiment by tuning a few parameters from `classif.xgboost` and make use of the `"moons"` `r ref("TaskGenerator", index = TRUE, aside = FALSE)`.
...
We begin by loading our learner, task generator, and generating 1,000 training data points and 1,000,000 testing data points.
...
task = tgen("moons")
train_task = task$generate(1000)
test_task = task$generate(1000000)
```

Now we will tune the learner with respect to the classification error and use holdout resampling and random search with 500 evaluations.

```{r}
tnr_rdm = tnr("random_search")
holdout = rsmp("holdout")
trm_500 = trm("evals", n_evals = 500)
...
Finally we will compute the in-sample performance, which is the performance of the optimal hyperparameter from `tune`, the out-of-sample performance, which is the aggregate of scores over the outer resampling method (below we use 5-fold CV), and then estimate the generalization error by training the tuned learner on the full training data again and predicting on the test data.
...
Comparing these results we can see that the insample performance overestimates the model performance, which indicates some overfitting to the optimal hyperparameters.
On the other hand, the out-of-sample performance from nested resampling provides a much better estimate of the generalization performance, despite the nested resampling only accessing 1,000 data points and not all 1,000,000.

```{r}
c(generalization = as.numeric(generalization),
