...
Examples include setting the number of trees in a random forest, penalty variables in support vector machines, or the learning rate in a neural network.
...
Besides simple variants such as a random search or grid search, most HPO methods are iterative and propose different configurations (usually over time more and more guided towards a potential optimum) until some termination criterion is met, at which point the optimal configuration is then returned (@fig-optimization-loop).
...
Moreover, both are 'naive' algorithms in the sense that they propose new configurations while ignoring performance from previous evaluations and are not guided towards potential optima during the optimization process.

In contrast, more sophisticated algorithms such as Iterative Racing, CMA-ES or Bayesian optimization - also called model-based optimization\index{MBO} - learn from previously evaluated configurations to find good configurations more quickly.
...
Advanced optimization algorithms within the mlr3 ecosystem are included in extension packages, for example the package `r mlr3mbo` implements Bayesian optimization algorithms (see @sec-bayesian-optimization), and `r mlr3hyperband` implements algorithms of the `r index("hyperband")` [@li2017] family (see @sec-hyperband).
...
```{r optimization-019}
#| label: fig-surface
#| fig-cap: Model performance with different configurations for `cost` and `gamma`. Bright yellow regions represent the model performing worse and dark blue performing better. We can see that high `cost` values and low `gamma` values achieve the best performance. Note that we should not directly infer the performance of new unseen values from the heatmap since it is only an interpolation based on a surrogate model. However, we can see the general interaction between the hyperparameters.
...
The number of boosting iterations increases the time to train a model and improves the performance until the model is eventually overfitting to the training data.
...
In the following, fidelity and budget are often used interchangeably.
...
In this practical example, we will optimize hyperparameters of XGBoost on the `r ref("mlr_tasks_spam", "Spam")` data set.
We begin by constructing the `r ref("mlr_learners_classif.xgboost", "classif.xgboost")` learner.
...
Note that we use the `"none"` terminator because Hyperband terminates itself after all brackets have been evaluated.
...
We then load the `r ref("TunerHyperband", "hyperband")` tuner and specify `eta = 2`.
...
This does not necessarily have to be a configuration evaluated with the highest budget since XGBoost can in general overfit the data when using too many boosting iterations.
...
In this example, we will optimize the hyperparameters of a support vector machine on the `r ref("mlr_tasks_sonar", "Sonar")` data set.
...
The `r ref_pkg("mlr3pipelines")` package contains a `r ref("PipeOp")` for subsampling data.
...
We combine the `r ref("PipeOp")` with the learner and create a `r ref("GraphLearner")`.
...
The graph learner first subsamples the training data and then fits a support vector machine on the subsampled data.
The parameter set of the graph learner is a combination of the parameter sets of the subsample `r ref("PipeOp")` and learner.
...
Note that we have to prefix the hyperparameters with the id of the `r ref("PipeOp", "PipeOps")`, because this reflects the way how they are represented in the parameter set of the graph learner.
...
For the other hyperparameters, we take the search space for support vector machines from @kuehn_2018.
This search space usually work well for a wide range of data sets.
...
Again, we use the `"none"` terminator because Hyperband controls the termination itself.
...
We create the `r ref("TunerHyperband", "hyperband")` tuner and set `eta = 3`.
...
The classification error was estimated to be 33% on 11% of the data set and decreased to 15% on the full data set (see dark blue line in @fig-hyperband).

```{r optimization-081}
#| echo: false
#| label: fig-hyperband
#| fig-cap: "Optimization path of the 8 configurations evaluated on the full data set."
library(ggplot2)
...
We also have to set `store_models = TRUE` because this required by the selected features measure.
