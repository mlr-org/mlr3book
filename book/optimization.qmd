...
The `r mlr_pkg("mlr3tuningspaces")` extension package provides predefined tuning spaces from scientific articles.
...
`r mlr_pkg("mlr3tuning")` allows specifying complex termination criteria by selecting one of the available `r ref("Terminator", text = "Terminators")`.
...
There are multiple `r ref("Tuner", "Tuners")` in `r mlr_pkg("mlr3tuning")`, which implement different HPO algorithms.
...
Not all available algorithms are implemented in `r mlr_pkg("mlr3tuning")`.
...
The `r mlr_pkg("mlr3viz")` package provides visualizations for tuning results.
...
We can automate the tuning process in `r mlr_pkg("mlr3")` so that learners are tuned transparently, without the need to extract information on the best hyperparameter settings at the end.
