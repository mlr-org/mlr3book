...
For our SVM example, we will use a grid search with a resolution of 5 for runtime reasons here (in practice a larger resolution would be preferred).
...
The `$result$learner_param_vals` field lists the optimal hyperparameters from tuning, as well as the values of any other hyperparameters that were manually set, this is useful for onward model use (@sec-analyzing-result).
...
In @sec-tune-trafo we will look at how to implement more complex, custom transformations to any hyperparameter or combination of hyperparameters.
...
Finally, to visualize the results, you can use the `autoplot` method implemented in `r mlr3viz` (@fig-surface).
...
Once we found good parameters for our learner through tuning, we can use the found configuration to train a model on the whole data.
...
task_gen = tgen("moons")
train_task = task_gen$generate(100)
test_task = task_gen$generate(1000000)
```
...
Note that in many cases, even nested resampling will overestimate the generalization error of the final learner, because the models fit during tuning use only a subset of the data, whereas the final model uses all available observations.
...
```{r}
lrn("classif.svm", kernel = to_tune())$param_set$search_space()
```
...
As a simple example, let us look at how to create a search space to tune `cost`, `gamma` and `shrinking` again:
...
Simple transformations such as this can even be added directly to a learner by passing a `Domain` object to `to_tune`:
