...
There is an extra [section](#searchspace) on building more complex tuning spaces.
...
`r mlr3book::mlr_pkg("mlr3tuning")` allows to specify complex termination criteria by selecting one of the available `r ref("Terminator", text = "Terminators")`.
...
```{r}
as.data.table(instance$archive, measures = msr("classif.acc"))
```
...
The `r ref("BenchmarkResult")` of the tuning instance contains all `r ref("ResampleResult")`s.
...
```{r optimization-018, echo = FALSE, out.width="98%"}
