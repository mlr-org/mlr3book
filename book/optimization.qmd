...
Whether you use `r ref("ti")` or `r ref("tune")` the output is the same and the `r index("archive")` lists all evaluated hyperparameter configurations:
...
We begin by constructing the `lrn("classif.xgboost")` learner
...
Note that we use the `trm("none")` terminator because Hyperband terminates itself after all brackets have been evaluated.
...
We then construct the tuner `tnr("hyperband")` and specify `eta = 2`.
...
Again, we use the `trm("none")` because Hyperband controls the termination itself.
...
We create the tuner `tnr("hyperband")` and set `eta = 3`.
