...
Note that we set `cost` and `gamma` values to be log-transformed and so `x_domain` contains the hyperparameter values after the transformation i.e. `exp(5.76)` and `exp(-5.76)`:
...
It is possible to simply overwrite a predefined tuning space in constructin, for example here we change the range of the `nrounds` hyperparameter in XGBoost:
