...
Now we will tune the learner with respect to the classification error and use holdout resampling and random search with 500 evaluations, we compute the in-sample performance as the performance for the optimal hyperparameter configuration.
...
The out-of-sample performance from nested resampling provides a much better estimate of the generalization performance, despite the nested resampling only accessing 1,000 data points and not all 1,000,000.
