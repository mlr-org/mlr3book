...
We apply a support vector machine with a radial basis function from the `r mlr3book::cran_pkg("e1071")` package and use it as a classification machine by setting `type` to `"C-classification"`.
...
The algorithms of the hyperband family are included in `r hyperband`.
...
All configurations will be examined by the tuner (in random order) until either all configurations are evaluated or the terminator signals that the budget is exhausted, i.e. here the tuner will stop after evaluating 20 of the 25 total configurations.
...
To get statistically unbiased performance estimates for a given task, [nested resampling](#sec-nested-resampling) is required.
:::
...
As another example, we may be interested in good performance but also low model complexity as simpler models are easier to interpret.
...
The goal of multi-objective HPO is to approximate the true, unkown Pareto front.
...
```{r optimization-021, results="hide"}
tuner = tnr("random_search", batch_size = 20)
tuner$optimize(instance)
```

```{r optimization-021-out}
...
## Nested Resampling for Unbiased Performance Estimation {#sec-nested-resampling}

Based on an HPO run, a HPC can be selected that results in a model that performs best with respect to a performance metric.
However, if the same data is used for the HPC selection step and the evaluation of the resulting model itself, the actual performance estimate of the model might be severely biased [@Simon2007].
...
To ensure unbiased performance estimation another resampling step is needed, i.e., the performance is estimated in an outer resampling loop.
This results in the concept of nested resampling - using an inner resampling for the HPO run and an outer resampling for final performance estimation.
More details on nested resampling can also be found in [@hpo_practical].
