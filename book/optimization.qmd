...
The goal of `r index("hyperparameter optimization", "HPO: Hyperparameter Optimization", aside = TRUE)` (@sec-model-tuning) or model `r index("tuning")` is to find the optimal configuration of hyperparameters of an ML algorithm for a given task.
...
Therefore a core part of configuring tuning is to specify when to terminate the algorithm, this is also called the `r index("tuning budget", aside = TRUE)`.
...
A `r index("tuning instance", aside = TRUE)` can be constructed manually (@sec-tuning-instance) with the `r ref("ti()")` function, or automated (@sec-simplified-tuning) with the `r ref("tune()")` function.
...
While some algorithms are provided by external packages, others are implemented by the mlr3 team.
...
Similar to learners, which have hyperparameters, one can configure tuners through what we call `r index("control parameters", aside = TRUE)`.
...
The SVM's `cost` and `gamma` parameters that we have tuned in the example above, are usually tuned on a ` r index("logarithmic scale", aside = TRUE)` (as opposed to linear).
...
Since tuning is an automated process, there is no opportunity for manual intervention, we therefore, mitigate errors by making use of `r index("encapsulation", aside = TRUE)`.
...
All configurations that are not Pareto-dominated by any other configuration are called Pareto efficient and the set of all these configurations is the `r index("Pareto set", aside = TRUE)`, whereas the corresponding metric values are referred to as the `r index("Pareto front", aside = TRUE)` (@fig-pareto).
