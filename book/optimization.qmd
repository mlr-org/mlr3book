...
Hyperparameters, in contrast, are configured by the user and determine how the model will fit its parameters.
...
The goal of [hyperparameter optimization](#sec-model-tuning) (HPO) or model tuning is to find the optimal configuration of hyperparameters of an ML algorithm for a given task.
Human trial-and-error to select a hyperparameter configuration (HPC), i.e., the hyperparameter values of an ML algorithm, is time-consuming, often biased and error-prone, and computationally irreproducible.
The mathematical formalization of HPO is essentially black-box optimization:
The ML algorithm configured by an HPC produces a model whose performance with respect to a given measure is evaluated via a resampling method.
Beyond the definition of the [search space](#sec-tuning-instance), i.e., the domain of the optimization problem, neither a closed-form mathematical representation nor analytic gradient information is available for model agnostic HPO.

Many sophisticated [HPO methods](#sec-tuner) (or 'tuners') have been developed over the last decades which can be used to tackle HPO efficiently.
Most HPO methods are iterative, i.e., new candidate HPCs are proposed and evaluated in a sequential manner (see @fig-optimization-loop) and after some termination criterion is met (e.g., exceeding a given run time budget), the best-performing HPC is returned.
...
The section [Advanced Search Spaces](#sec-advanced-search-spaces) describes the construction of more complex search spaces.

The bounds are usually set based on experience.
The section [Tuning Spaces](#sec-tuning-spaces) introduces the `r mlr3tuningspaces` extension package which is a collection of search spaces from scientific articles.
...
The most commonly used terminators are those that stop the tuning after a certain time  (`"run_time"`) or the number of evaluations (`"evals"`). Choosing a runtime is often based on practical considerations and intuition.
Using a time limit can be important on clusters so that the tuning is finished before the account budget is exhausted. The `"perf_reached"` terminator stops the tuning when a certain performance level is reached. This can be helpful if a certain performance is seen as sufficient for the practical use of the model. However, when the level is set to optimistically, the tuning might never terminate. The `"stagnation"` terminator stops when no progress is made in a certain amount of iterations. However, this could result in the optimization being terminated too early when the search space is too complex.
...
* Random Search `"random_search"` - Samples HPCs from a uniform distribution randomly [@bergstra2012].
* Grid Search `"grid_search"` - Discretizes the range of each HPC and exhaustively evaluates each combination.
* Iterative Racing `"irace"` - Races down a random set of HPCs and uses the surviving ones to initialize a new set HPCs which focus on a promising region of the search space [@lopez2016].
...
Not all available algorithms are implemented in `r mlr3tuning`.
...
The `batch_size` controls how many configurations are evaluated at the same time (see chapter [Parallelization](#parallelization)).
...
You can do this with `r ref("TuningInstanceMultiCrit")`, see [Tuning with Multiple Performance Measures](#sec-multi-metrics-tuning).
...
as.data.table(instance$archive, measures = msr("classif.acc"))[, list(cost, gamma, classif.ce, classif.acc)]
```
...
However, it often happens that learners with certain HPCs do not converge, run out of memory, or terminate with an error.
...
See the section on [Fallback Learner](#sec-fallback-learner) for more detailed information.
...
The [gallery](https://mlr-org.com/gallery.html#category:tuning) features a collection of case studies and demos about optimization.
  ...
  - Learn more advanced methods with the [Practical Tuning Series](https://mlr-org.com/gallery.html#category:practical_tuning_series).
  ...
  - Make us of proven [search space](https://mlr-org.com/gallery/2021-07-06-introduction-to-mlr3tuningspaces/).
...
As different metrics are typically in competition no single HPC exists that optimizes all metrics.
Focus is therefore given to the concept of Pareto optimality.
One HPC is said to Pareto-dominate another one if the resulting model is equal or better in all metrics and strictly better in at least one metric.
All HPCs that are not Pareto-dominated are referred to as the Pareto set and their corresponding metrics (the image of the Pareto set) are referred to as the Pareto front.
...
After triggering the tuning, we can inspect the estimated Pareto set and visualize the estimated Pareto front:

```{r optimization-034, results="hide"}
#| output: false
...
```{r optimization-035}
instance$archive$best()[, c("cp", "minsplit", "maxdepth", "classif.ce", "selected_features")]
```
...
As an example, consider an HPO run, on which basis we can select an HPC that results in a model that performs best with respect to a performance metric.
If the same data is used for the HPC selection step and the evaluation of the resulting model itself, the actual performance estimate of the model might be severely biased [@Simon2007].
...
The previous [sections](#sec-tuning-instance) examined the optimization of a simple classification tree on the `r ref("mlr_tasks_sonar", text = "Sonar")` data set.
...
```{r optimization-044}
extract_inner_tuning_archives(rr)[, list(iteration, cost, gamma, classif.ce)]
```
...
### Functions and Classes {#sec-nested-resampling-r6}
...
### Exercise {#sec-nested-resampling-exercise}
