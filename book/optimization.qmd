...
#| echo: false
#| layout-ncol: 2
#| fig-cap: Histogram of uniformly sampled values `sim` between $log(1e-5)$ and $log(1e5)$.
#| fig-subcap:
#|   - "`sim`"
#|   - "`exp(sim)`"
library(ggplot2)
...
ggplot(data, aes(x = x)) + geom_histogram(bins = 15) + theme_minimal()

data = data.frame(x = exp(sim))
ggplot(data, aes(x = x)) + geom_histogram(bins = 15) + theme_minimal()
```
...
Now we know what paramters to tune over, we can look at *how* to tune.
