...
For example for a search space $\{1, 2, 3, 4, 5, 6, 7, 8, 9\}$ a grid search with resolution 5 would pick five values evenly apart in this search space, i.e., $\{1, 3, 5, 7, 9\}$.
...
#| fig-cap: An illustration of nested resampling. The green blocks represent 3-fold cross-validation for the outer resampling for model evaluation and the blue and gray blocks represent 4-fold cross-validation for the inner resampling for HPO.
...
1. HPO -- Tune the hyperparameters on the outer training set using the inner data splits (blue and gray blocks).
