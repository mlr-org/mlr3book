...
We will now show that nested resampling works using an example with a feature-independent learner and simulated data.
The learner `lrn("classif.featureless")` randomly predicts a class label with equal probability.
This means the true generalization error of the model is 50%.
We add the hyperparameter `x` which has no effect.

```{r}
learner = lrn("classif.featureless", method = "sample")
learner$param_set$add(paradox::ParamDbl$new("x", lower = 0, upper = 1))
learner$param_set$set_values(x = to_tune())
```

The `tgen("2dnormals")` task generator creates a binary classification problem with equal class proportions.

```{r}
generator = tgen("2dnormals")
task = generator$generate(n = 200L)
```

We will now start the tuning using a random search with 100 evaluations.
...
The measured classification error of the best configuration is much lower than 50%.
This means the measured performance while tuning overestimates the true performance of the model.
Although `x` has no effect on the performance, the best classification error is decreasing over time.

```{r optimization-032}
#| label: fig-nested-resampling-example-1
#| fig-cap: The classification error of the best configuration over time.
#| fig-alt: Line plot with iteration on the x-axis and classification error on the y-axis.
#| echo: false
library(ggplot2)
library(viridisLite)

data = as.data.table(instance$archive)
data[, tuning_error := cummin(classif.ce)]

ggplot(data, aes(x = seq(100), y = tuning_error)) +
  geom_line(
    colour = viridis(3, end = 0.8)[2],
    linewidth = 1) +
  xlab("Number of evaluations") +
  ylab("Best classification error") +
  theme_minimal()
```

We now run nested resampling to estimate the unbiased performance of the model.
...
rr = resample(task, at, rsmp("cv", folds = 10))
rr$aggregate()
```

We see that the performance estimated by nested resampling is much closer to the true generalization error of 50%.
