...
The sections @sec-search-space-token and @sec-search-space-scratch give a more detailed insight into the creation of tuning spaces.
...
::: {.callout-note}
The measured performance is different from the previous section because not the same train-test splits were used.
...
We will now show that nested resampling works using an example with simulated data.
The task generator `tgen("moons")` creates two interleaving half circles ("moons") as a binary classification problem.

```{r}
generator = tgen("moons")
task = generator$generate(n = 500L)
```

We tune the hyperparameters of an XGBoost model using the random search tuner.
...
instance = tune(
  tuner = tnr("random_search", batch_size = 50L),
  task = task,
  learner = learner,
  resampling = rsmp("cv", folds = 3),
  measures = msr("classif.ce"),
...
The best configuration has a classification error of 1.2%.

```{r}
#| eval: false

learner = lrn("classif.xgboost")
learner$param_set$set_values(.values = instance$result_learner_param_vals)

learner$train(task)
pred = learner$predict(generator$generate(n = 1000000L))
pred$score()
...
We train a model with the best configuration on the entire data set.
Predicting on 1 million observations yields a classification error of 1.9%.
We see that the performance estimated while tuning overestimates the performance of the final model.
...
When adding an outer resampling, the performance estimate is 1.8%.
This is much closer to the performance of the model trained on the entire data set.
...
We will now look at another advanced topic, which is using transformations to exercise more fine-grained control over the sampling distribution.
To show how this works, we use the SVM's `cost` and `gamma` parameters that we have tuned in the example above, these are usually tuned on a logarithmic (as opposed to linear) scale.
...
In this section, we will provide a more detailed explanation of the underling mechanism and its capabilities.

If we want to tune a parameter, we can set its value to a `r ref("TuneToken")` instead of a fixed value.
...
Above we explicitly set the range to tune over, however we could also tune over the complete range of the parameter bounds, for example the `shrinking` parameter in an  SVM:
...
:::{.callout-note}
When introducing the dependency, we need to use the `cost` value from *before* the implicit transformation. Concretely, we write the dependency as `cost == "val2"` and not `cost == 0.7`.
:::
...
This can be done using an `.extra_trafo`, which is similar to the `logscale` transformation from earlier, but is applied to all parameters.
It can be set to a function with arguments `x` and `param_set` -- the parameter values and the `r ref("ParamSet")` itself -- and must return the transformed parameters.
You will learn more about it in the next section and @sec-paradox.
...
::{.callout-tip}
The `.extra_trafo` from `par` parameter set only has access to the parameters `spam` and `nonspam` and not the other parameters from the learner's search space.
:::

### Creating Search Spaces from Scratch {#sec-search-space-scratch}

We have already learned in the previous section how to create a search space with a learner and the `r ref("to_tune()")` function.
But is it also possible to create the search space from scratch.
...
One way to configure a tuning space is to define a `r ref("ParamSet")` analogously to @sec-fundamentals.
...
Instead of using the `logscale` option, we can also manually set a transformation (`trafo`).
...
Such a transformation is only possible via a `.extra_trafo`, because it accesses information from multiple parameters at once.
...
We will now look at commonly used transformations and show how they can be expressed in `r ref_pkg("paradox")`.
...
Keep in mind that factorial tuning parameters are always `character` values:
