...
For example, determining if a linear regression model should be fit with an intercept or not can be thought of as configuring a hyperparameter.
Other common hyperparameter examples include, setting the number of trees in a random forest, penalty variables in SVMs, or the learning rate in a neural network.
...
So in this chapter we'll demonstrate how to make this into more of a science.
...
To ensure unbiased performance estimation of the tuned ML algorithm another resampling step is needed, i.e., the performance is estimated in an outer resampling loop.
