...
Hyperparameter tuning is supported via the `r mlr3tuning` extension package.
The `r mlr3verse` loads all functions of the extension package.

```{r optimization-002}
library("mlr3verse")
```

At the heart of `r mlr3tuning` are the R6 classes
...
The `r mlr3tuningspaces` extension package provides predefined tuning spaces from scientific articles.
...
The package `r mlr3mbo` makes Bayesian optimization (`"mbo"`) (also called Model-Based Optimization) available.
...
The package `r mlr3hyperband` implements algorithms of the hyperband family.
