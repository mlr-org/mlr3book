...
For example, we could benchmark three `r index("support vector machine")` (SVM) with three difference `cost` values.
...
Throughout this section, we will look at optimizing an SVM classifier from `r ref_pkg("e1071")` on the `sonar` data set as a running example.
...
In this example we will tune the numeric regularization and influence hyperparameters, `cost` and `gamma`.
See, the help page for `r ref("e1071::svm()")` for full details on the model implementation including hyperparameters.
...
::: {.callout-note}
In rare cases parameters sets may include hyperparameters that cannot be tuned.
These will usually be 'meta' parameters that provide information about *how* the model is being fit but do not control the training process itself, for example the `verbose` hyperparameter in `classif.ranger` controls how much information is displayed to the user during training.
:::
...
Now we have decided which hyperparameters to tune, we must now specify when we will be happy that they have been tuned 'enough', i.e., how we define the optimal hyperparameter configuration.

### Terminator {#sec-terminator}

Theoretically, a tuner could search an entire search space exhaustively, however practically this is rarely possible and mathematically this is impossible for continuous hyperparameters.
Therefore we must define a `r define("tuning budget")` which specifies when to terminate the HPO process.
`r mlr3tuning` includes many methods to specify when to terminate an algorithm (@tbl-terms), which are implemented in `r ref("Terminator")`\index{Terminator}[Terminator]{.aside} classes.
...
A `r define("tuning instance")` can be constructed manually with the `r ref("ti()")` function, or automated (@sec-simplified-tuning) with the `r ref("tune()")` function.
...
There are multiple `r ref("Tuner")`\index{Tuner}[Tuner]{.aside} classes in `r mlr3tuning`, which implement different HPO (or more generally speaking `r index('black-box optimization')`) algorithms (@tbl-tuners).
...
Bayesian optimization (@sec-bayesian-optimization) [e.g., @Snoek2012] describes a family of iterative optimization algorithms that use a so-called surrogate model to represent the unknown function that is to be optimized -- in HPO the mapping from a hyperparameter configuration to the estimated generalization performance.
...
Iterative Racing [@lopez2016] starts by 'racing' down an initial population of randomly sampled configurations and then uses the surviving configurations of the race to stochastically initialize the population of the subsequent race to focus on promising regions of the search space, and so on.
...
These can be useful if evaluations are rather cheap as they require more function evaluations and are therefore less sample efficient than other sophisticated methods (e.g., MBO) but are more commonly used for general black-box optimization.
...
For our SVM example, we will use a grid search with a resolution of 5 for efficiency reasons (in practice a larger resolution would be preferred).
...
The `resolution` and `batch_size` parameters are termed `r define("control parameters")`, and other tuners will have other parameters that can be set, as with learners these are accessible with `$param_seta`.

```{r optimization-009}
tuner$param_set
```

Whilst changing the parameters from their defaults can improve performance of the tuner, it is likely the default settings will fit most needs.
...
The first columns will be named after the tuned hyperparameters and show the optimal values from the searched tuning spaces.
The `$x_domain` field contains the optimal values of the tuned hyperparameters, whereas `$learner_param_vals` *also* includes the values of set hyperparameters, which is useful for further model use (@sec-analyzing-result).

```{r}
instance$result$learner_param_vals
```

Before we look at how to make use of these results, we will first consider logarithmic transformations, which are very common in HPO to improve the efficiency of tuning.
...
The correct method is to test the model on more unseen data, which can be efficiently performed with nested resampling, we will discuss this in @sec-nested-resampling.
:::
...
We will return to transformations in @sec-search-space-scratch.
...
Manually inspecting the archive to determine other important features such as time of evaluation, model runtime, and any errors or warnings that occurred during tuning.
...
2. `r index("Inner resampling")` -- Within the outer training data instantiate 4-fold cross-validation to create different inner testing and training data sets.
3. HPO -- Tune the hyperparameters on the outer training set (large, light blue blocks) using the inner data splits.
4. Training -- Fit the learner on the outer training data set using the optimal hyperparameter configuration obtained from the inner resampling (small blocks).
...
In practice, you may often see closer to three folds used in inner resampling or even holdout resampling, but if you have the resources then we recommend parallelization (@sec-parallelization).
:::
...
Therefore, **nested resampling should not be used to construct a final model** [@Simon2007].
We will show two examples in @sec-resample-overfitting of why using nested resampling for hyperparameter selection can cause mistakes.
...
Note that we set `store_models = TRUE` so that the `r ref("AutoTuner")` models are stored, which enable investigation of the inner tuning instances.
...
These methods are particularly useful for identifying if the optimized hyperparameters are overfitting the data, as we will demonstrate now.

### Overfitting and unbiased estimation {#sec-resample-overfitting}

{{< include _optional.qmd >}}

We will first look at an example that shows overfitting of parameters and then an example of how different performance estimates can give different results.

#### Model parameter overfitting {.unnumbered .unlisted}

As an example, we will resample our `AutoTuner` on a filtered clone of the `sonar` dataset so there are less rows (40) than features (61):

```{r}
tsk_overfit = tsk("sonar")$filter(sample(208, 40))
rr_overfit = resample(tsk_overfit, at, rsmp("cv", folds = 5), store_models = TRUE)
extract_inner_tuning_results(rr_overfit)[,
  .(iteration, cost, gamma, classif.ce)]

rr_overfit$score()[, .(iteration, classif.ce)]

rr_overfit$aggregate()
```

Notice how the results show in most iterations that the optimal configurations in the inner resampling have a lower classification error than the tuned model in the outer resampling.
This demonstrates that the optimized hyperparameters are overfitting the data and the model artifically looks to be better performing than it is.
Using `$aggregate()` is therefore the only appropriate way to return an unbiased estimate of model performance.

#### Reporting of performance estimates {.unnumbered .unlisted}

In this example we will demonstrate that nested resampling better estimates the generalization performance of a tuned model than the internal resampling results.
We start by tuning a decision tree on a training dataset and then estimating the generalization performance by making predictions on new, unseen data.
<!-- FIXME: Below is first mention of generators, either need comprehensive explanation here or something in a different section we can reference. -->

```{r optimization-032}
lrn_rpart = lrn("classif.rpart", cp = to_tune(0, 1))
generator = tgen("moons")
train_task = generator$generate(100)
test_task = generator$generate(1e6)
...
# fit optimized model
tuned_learner = lrn("classif.rpart")
tuned_learner$param_set$values = instance$result_learner_param_vals
tuned_learner$train(train_task)
pred = tuned_learner$predict(test_task)

pred$score()
```

Now we can compare the true generalization error to the performance of the optimal hyperparameter configuration:

```{r}
instance$result_y
```

And to the estimate from nested resampling:
...
rr = resample(train_task, at, rsmp3)
rr$aggregate()
```

We can see that the nested resampling is a much closer estimate of the true generalization error, and in fact in the case the internal performance measure was actually worse than the true generalization error.
...
In this section we will first look at how to tune different scalar parameter classes with `r ref("to_tune")`, and then how to define your own search space with `r ref("ParamSet")` to create more advanced search spaces that may include vector tuning, transformations, and parameter dependencies.
Finally, we will consider how to derive parameter sets from other learners and establish search spaces in the literature.

### Scalar parameter tuning

The `to_tune` function can be used to parameters of any class, whether they are scalar or vectors.
...
Finally, we can only use `to_tune` to treat numeric parameters as factors if we only want to tune over a small subset of possible values, for example if we wanted to find the optimal number of trees in a random forest we might only consider three scenarios: 100, 200, or 400 trees:

```{r}
lrn("classif.ranger", num.trees = to_tune(c(100, 200, 400)))$param_set$search_space()
```

Before we look at vector parameter tuning, we must first learn how to create parameter sets from scratch.

:::{.callout-warning}
Treating an integer as a factor for tuning results in “unordered” hyperparameters.
Therefore algorithms that make use of ordering information, e.g., evolutionary strategies, will perform worse when ordering is ignored.
...
```{r}
ti(task_sonar, lrn("classif.svm", type = "C-classification"), rsmp3, msr_ce, trm("none"), search_space = search_space)
```
...
### Transformations and vector tuning {#sec-tune-trafo}

{{< include _optional.qmd >}}

In @sec-logarithmic-transformations we saw how to quickly apply log transformations with `to_tune`.
As you now know, `to_tune` is just a wrapper that creates `r ref("ParamSet")` objects, so let's look at what is actually taking place when we set `logscale = TRUE`:
...
In practice, the underlying implementation would simply ignore unused parameters, so if we passed `degree` to `r ref("e1071::svm()")` when `kernel` is not polynomial, then the `degree` parameter would simply be ignored.
However, this would be incredibly wasteful during tuning if we were trying to tune `degree` when the kernel was not polynomial, hence setting the dependency tells the tuning process to tune `degree` iff `kernel` is `"polynomial"` and to ignore it otherwise.
...
In another example, consider the single-objective problem of tuning a deep neural network to enable the best possible performance.
The best performing model is likely to be the hardest to interpret with many layers, which would not be appropriate when interpretability is very important, for example in healthcare.
...
All configurations that are not Pareto-dominated by any other configuration are called `r index('Pareto-efficient')` and the set of all these configurations is the `r define("Pareto set")`.
The metric values corresponding to these Pareto set are referred to as the `r define("Pareto front")`.

The goal of multi-objective HPO is to approximate the true (but known) Pareto front.
...
As multi-objective tuning requires manual intervention to select a configuration, it is not possible to use `auto_tuner`.
...
This section is devoted to advanced tuning techniques, which are not necessary for a basic understanding of tuning but will make tuning more efficient and robust.

### Error Handling {#sec-encapsulation-fallback}

Error handling is discussed in detail in @sec-error-handling, however we will briefly discuss it here in the context of tuning as encapsulation and fallback learners are essential in complex tuning experiments.

Until now, we have focused on examples that do not error, but to demonstrate the purpose of encapsulation and fallback learners, we must consider examples that *will* error, for example those where learners do not converge, run out of memory, or terminate with an error.
To demonstrate error handling then we will filter the `sonar` task to only one class.

```{r}
task = tsk("sonar")$filter(seq(10))
```

#### Encapsulation {.unnumbered .unlisted}
...
In `r mlr3`, encapsulation is controlled by passing arguments to the `encapsulate` field in any learner, as in the example below, note that encapsulation is set individually for training and predicting, which can be useful if, for example, we are happy to ignore errors in training but want to manually debug any errors in prediction.
...
The possible options to `encapsulate` are `evaluate` and `callr` named after the packages of the same name.
...
If encapsulation is used during training then a fallback learner must also be selected because the archive needs to include performance values for each configuration.

#### Fallback learners {.unnumbered .unlisted}

Fallback learners are essentially learners that are fitted if the learner of interest fails.
A common approach is to use a featureless learner, `classif/regr.featureless`.
See @sec-fallback for more detailed information.

Below we set `classif.featureless`, which always predicts the most frequent label, by passing this learner to the `$fallback` field.

```{r optimization-037}
learner$fallback = lrn("classif.featureless")
```

We can now run our experiment and see errors and warnings that occurred during tuning in the archive.
...
We can see the tuning ran without breaking because encapsulate captured the errors and logged them before the fallback learners were used for fitting and predicting.
