...
We begin by loading the `lrn("classif.xgboost")` learner.
...
We use the `trm("none")` because Hyperband terminates itself when all brackets are evaluated.
...
We load the `tnr("hyperband")` and set `eta = 2`.
...
In this example, we will optimize the hyperparameters of the support vector machine on the `sonar` data set.
...
The `r mlr3pipelines` package features a `r ref("PipeOp")` for subsampling.
...
We use tune tokens to mark which hyperparameters should be tuned.
We have to prefix the hyperparameters with the id of the pipeline operators.
...
SVMs often crash or never finish the training with certain hyperparameter configurations.
...
We use the `trm("none")` because Hyperband controls the termination itself.
...
We load the `tnr("hyperband")` and set `eta = 3`.
...
The classification error was estimated to be 30% on 33% of the data set and decreased to 14% on the full data set (see birght purple line in @fig-hyperband).
