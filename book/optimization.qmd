...
author:
  ...
  - name: Lennard Schneider
    ...
    affiliations:
      - name: Ludwig-Maximilians-Universität München
abstract:
  We present an introduction to hyperparameter optimization in the mlr3 ecosystem.
  We start by defining the search space for a support vector machine.
  Next, we choose a termintor to stop the optimization.
  Then we create a tuning instance to describe the tuning problem.
  We choose an optimization algorithm and run the optimization.
  After that, we show how to analyse the results.
  We also show how to perform multi-objective optimization with multiple measures.
  Finally, we show how to use nested resampling to estimate the unbiased performance of optimized models.
---
...
#| fig-cap: Representation of the hyperparameter optimization loop in mlr3tuning. Blue - Hyperparameter optimization loop. Purple - Objects of the tuning instance supplied by the user. Blue-Green - Internally created objects of the tuning instance. Green - Optimiation Algorithm.
#| echo: false
...
We do this by constructing a learner and using `r ref("to_tune()")` to set the lower and upper limits for parameters we want to tune.
...
Note the use of `logscale = TRUE`, which means the optimization algorithm searches in $[log(1e-5), log(1e5)]$ but transforms the selected configuration with `exp()` before passing to the learner.
...
The summaries show how the algorithm searches within a narrow range but exponentiating then results in the majority of points being reltively small but a few being very large.
...
The columns here show the tested configurations, the measure we optimise,
the completed configuration time stamp, and the total train and predict times.
If we only specify a single-objective criteria then the instance will return the configuration that optimises this measure however we can manually inspect the archive to determine other important features.
...
Note we set `store_models = TRUE` because so that the `r ref("AutoTuner")` models to investigate the inner tuning.
In this example we utilised the same resampling strategy (K-fold cross-validation) but the mlr3 infrastructure is not limited to this, you can freely combine different inner and outer resampling strategies as you choose.
You can also mix-and-match parallelisation methods for controlling the process (@sec-nested-resampling-parallelization).
...
The most important functions and classes we learnt about are in @tbl-api-optimization alongside their R6 classes.
If you are interested in learning more about the underlying R6 classes to gain a finer control on these methods, then take a look at the online API.
