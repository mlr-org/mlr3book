...
The (**M**achine **L**earning in **R**) `r mlr3` [@mlr3] package and ecosystem provide a generic, object-oriented, and extensible framework for [classification](#tasks), [regression](#tasks), [survival analysis](#survival), and other machine learning tasks for the R language [@R].
...
*   **Tabular data**.
Embrace `r ref_pkg("data.table")` for fast and convenient computations on tabular data (but we support other types of data as well).
...
`r mlr3` uses the following packages that are not developed by core members of the `r mlr3` team:
...
::: callout-tip
A complete list with links to the repositories for the respective packages can be found on our `r link("https://github.com/mlr-org/mlr3/wiki/Extension-Packages", "package overview page")`.
:::
