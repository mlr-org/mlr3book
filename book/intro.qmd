...
If you want to contribute to our universe, ask any questions, read documentation, or just chat to the team, head to `r link("https://github.com/mlr-org/mlr3")` which has several useful links in the README.

The `r mlr3` [@mlr3] package and the wider `mlr3` ecosystem provide a generic, `r index("object-oriented")`, and extensible framework for `r index("regression")` (@sec-tasks), `r index("classification")` (@sec-classif), and other machine learning `r index("tasks")` (@sec-special) for the R language [@R].
...
This book describes the resulting features of the `r mlr3verse` and discusses best practices for ML, technical implementation details, and in-depth considerations for model optimization.
...
Whilst we hope this book is accessible for a wide range of readers and levels of ML expertise, we do assume that readers have taken at least an introductory ML course or have the equivalent expertise and some basic experience with R.
...
This will give you a flavour of the building blocks of the `mlr3` universe and the basic tools you will need to tackle most machine learning problems.
...
The performance of ML models can be improved by tuning hyperparameters but also by carefully selecting features from the training dataset. @sec-feature-selection introduces manual and automated feature selection with filters and wrappers implemented in `r mlr3filters` and `r mlr3fselect`.
...
In @sec-pipelines we will show you how to build your own pipeline out of discrete configurable operations and how to treat complex pipelines as if they were any other machine learning model.
...
In the final part of the book we will look at advanced implementation as well as theory that can be used to fine-tune experiments, analyze results in details, and apply domain-specific methods.
...
We have included a box at the top of all 'optional' sections that briefly summarizes the section.
These sections are particularly complex with respect to either technical or methodological detail and could be skipped on a first read.

Each chapter includes tutorials, API references, and explanations of methodologies.
At the end of each part of the book we have included exercises for you to test yourself on what you have learnt, you can find the solutions to these exercises at `r link("https://mlr3book.mlr-org.com/solutions.html")`.
...
If you have experience with any (class) object-oriented programming before then R6 should feel familiar.
We focus on the parts of R6 that you need to know to use `mlr3`.

`r index('Objects', "R6!Objects", aside = TRUE)` are created by constructing an instance of a `r ref("R6::R6Class")` variable using the `$new()` initialization method.
...
Some `R6` objects may have mutable states that are encapsulated in their `r index('fields', "R6!Fields", aside = TRUE)`, which can be accessed through the dollar, `$`, operator.
...
In addition to fields, `r index('methods', "R6!Methods", aside = TRUE)` allow users to inspect the object's state, retrieve information, or perform an action that changes the internal state of the object.
...
To copy an object, use the `$clone(deep = TRUE)` method, so to copy `foo`: `foo2 = foo$clone(deep = TRUE)`[`$clone()`]{.aside}\index{R6!\$clone()}.
...
We heavily use helper functions throughout this book and provide the equivalent "full form" for complete detail at the end of each chapter.
The helper functions are designed to cover the majority of use-cases for most users; knowledge about the full `R6` backend is most commonly required if you want to build custom objects or extensions.
...
So in 2018 we set about working working on a reimplementation, which resulted in the first release of `r mlr3` on CRAN in July 2019.
