...
The `r mlr3` [@mlr3] package and ecosystem provide a generic, object-oriented, and extensible framework for [classification](#tasks), [regression](#tasks), [survival analysis](#survival), and other machine learning tasks for the R language [@R].
...
:::{.callout-note}
`r mlr3` provides a unified interface to existing [learners](#learners) in R. With few exceptions, we do not implement any learners ourselves.
:::

## Target Audience

We expect that users of `r mlr3` have at least basic knowledge of machine learning and R.
The later chapters of this book describe advanced functionality that requires more advanced knowledge of both.
`r mlr3` is suitable for complex projects that use advanced functionality as well as one-liners to quickly prototype specific tasks.
...
* **Command-line before GUI**.
  ...
  Our core packages do not provide a graphical user interfaces (GUIs) whose dependencies would make an installation on a computing server unnecessarily tedious; visualizations of data and results are provided in extra packages like `r mlr3viz`. `r ref_pkg("mlr3shiny")` provides an interface for some basic machine learning tasks using the `r ref_pkg("shiny")` package.
...
The `r mlr3` package itself provides the base functionality that the rest of ecosystem rely on and some fundamental building blocks for machine learning.
The following packages extend `r mlr3` with capabilities for preprocessing, pipelining, visualizations, additional learners, additional task types, and more:
