...
This unified interface provides functionality to extend and combine existing machine learning (ML) algorithms, intelligently select and tune the most appropriate technique for a given machine learning task, and perform large-scale comparisons that enable meta-learning.
...
While `r ref_pkg("tidymodels")` in particular makes it very easy to perform simple machine learning tasks, `r mlr3` is more geared towards advanced machine learning.
...
The `r ref_pkg("mlr")`\index{mlr} package [@mlr] was first released to `r link("https://cran.r-project.org", "CRAN")` in 2013, with the core design and architecture dating back much further.
...
The `r mlr3` *package* provides the base functionality that the rest of the ecosystem (`mlr3verse`) depends on for building more advanced ML tools.
...
A complete list of extension packages with links to their respective repositories for the respective packages can be found on the `r link("https://mlr-org.com/ecosystem.html", "package overview page")`.
...
For an in-depth introduction, we recommend the excellent data.table introduction vignette (@datatable).
...
Note that in general not all fields in an `R6` object will be mutable, but to be consistent, in the `mlr3` universe they are.
<!-- FIXME: Maybe just rewrite above in terms of active bindings that are getters and setters, and that code is actually being run. This means that not *everything* is accessible -->
...
To copy an object, use the `$clone(deep = TRUE)` method, so to copy `foo`: `foo2 = foo$clone(deep = TRUE)`.
...
For example, you can see an overview of available learners (that are in loaded packages) with `as.data.table(mlr_learners)`.
