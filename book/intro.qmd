...
The `r ref_pkg("mlr3")` [@mlr3] package and ecosystem provide a generic, object-oriented, and extensible framework for [classification](#tasks), [regression](#tasks), [survival analysis](#survival), and other machine learning tasks for the R language [@R].
...
:::{.callout-note}
`r ref_pkg("mlr3")` provides a unified interface to existing [learners](#learners) in R. With few exceptions, we do not implement any learners ourselves.
:::

## Target Audience

We expect that users of `r ref_pkg("mlr3")` have at least basic knowledge of machine learning and R.
The later chapters of this book describe advanced functionality that requires more advanced knowledge of both.
`r ref_pkg("mlr3")` is suitable for complex projects that use advanced functionality as well as one-liners to quickly prototype specific tasks.

`r ref_pkg("mlr3")` provides a domain-specific language for machine learning in R.
...
Instead, we decided to start working on a reimplementation in 2018, which resulted in the first release of `r ref_pkg("mlr3")` on CRAN in July 2019.
The new design and the integration of further and newly-developed R packages (especially `r ref_pkg("R6")`, `r ref_pkg("future")`, and `r ref_pkg("data.table")`) makes `r ref_pkg("mlr3")` much easier to use, maintain, and in many regards more efficient compared to its predecessor `r ref_pkg("mlr")`.

## Design Principles

We follow these general design principles in the `r ref_pkg("mlr3")` package and ecosystem.

* **Command-line before GUI**.
  Most packages of the `r ref_pkg("mlr3")` ecosystem focus on processing and transforming data, applying machine learning algorithms, and computing results.
  Our core packages do not provide a graphical user interfaces (GUIs) whose dependencies would make an installation on a computing server unnecessarily tedious; visualizations of data and results are provided in extra packages like `r ref_pkg("mlr3viz")`. `r ref_pkg("mlr3shiny")` provides an interface for some basic machine learning tasks using the `r ref_pkg("shiny")` package.
...
* **Be light on dependencies**.
  ...
  We require far fewer packages in `r ref_pkg("mlr3")` to make installation and maintenance easier.

## Package Ecosystem

`r ref_pkg("mlr3")` makes use of the following packages not developed by core members of the `r ref_pkg("mlr3")` team:
...
The `r ref_pkg("mlr3")` package itself provides the base functionality that the rest of ecosystem rely on and some fundamental building blocks for machine learning.
The following packages extend `r ref_pkg("mlr3")` with capabilities for preprocessing, pipelining, visualizations, additional learners, additional task types, and more:
...
To view the mlr3verse image for an overview of the`r ref_pkg("mlr3")`package ecosystem, follow this link: https://raw.githubusercontent.com/mlr-org/mlr3/master/man/figures/mlr3verse.svg.
```
...
We focus on the parts of R6 that you need to know to use `r ref_pkg("mlr3")` here.
...
Objects are created by calling the constructor of an `"R6::R6Class()"` object, specifically the initialization method `$new()`.
  ...
  Most objects in `r ref_pkg("mlr3")` are created through special functions (e.g. `lrn("regr.rpart")`) that encapsulate this and are also referred to as *sugar functions*.
...
Many functions of `r ref_pkg("mlr3")` return `data.table`s which can conveniently be subsetted or combined with other outputs.
