...
The (**M**achine **L**earning in **R**) `r mlr3` [@mlr3] package and ecosystem provide a generic, object-oriented, and extensible framework for [classification](#tasks), [regression](#tasks), [survival analysis](#survival), and other machine learning tasks for the R language [@R] (task types discussed in detail in [@tasks-types]).
...
`r mlr3` is comparable to caret, tidymodels, scikit-learn, and MLJ. `r mlr3` can provided more flexibility than caret and titdymodels while still offering powerful machine learning tools. See more in the [mlr3 cheatsheets](https://cheatsheets.mlr-org.com/).
...
We assume that users of `r mlr3` have the equivalent knowledge of an introductory machine learning course and some experience in R.
...
`r mlr3` is suitable for complex projects that utilize the high degree of control as well as the highly abstracted "syntactic sugar" to mock-up specific tasks.

`r mlr3` provides a domain-specific language for machine learning in R.
...
`r ref_pkg("mlr")` [@mlr] was first released to [CRAN](https://cran.r-project.org) in 2013, with the core design and architecture dating back much further.
...
With hindsight, we saw that some design and architecture choices in `r ref_pkg("mlr")` made it difficult to support new features, in particular with respect to pipelines.
...
It would have been nearly impossible to integrate all of these changes into the original design of `r ref_pkg("mlr")`.
...
The new design and the integration of further and newly-developed R packages (especially `r ref_pkg("R6")`, `r ref_pkg("future")`, and `r ref_pkg("data.table")`) makes `r mlr3` much easier to use, maintain, and in many regards more efficient compared to its predecessor `r ref_pkg("mlr")`.

## Design Principles :star:

We follow these general design principles in the `r mlr3` package and ecosystem.

-   **Command-line before GUI**. Most packages of the `r mlr3` ecosystem focus on processing and transforming data, applying machine learning algorithms, and computing results. Our core packages do not provide a graphical user interfaces (GUIs) whose dependencies would make an installation on a computing server unnecessarily tedious; visualizations of data and results are provided in extra packages like `r mlr3viz`. `r ref_pkg("mlr3shiny")` provides an interface for some basic machine learning tasks using the `r ref_pkg("shiny")` package.
...
-   **Be light on dependencies**. One of the main maintenance burdens for `r ref_pkg("mlr")` was to keep up with changing learner interfaces and behavior of the many packages it depended on. We require far fewer packages in `r mlr3` to make installation and maintenance easier.
-   **mlr3viz**. The visualization functionality for `r mlr3` is partitioned to its own package rather than being integrated into each package within the `r mlr3` ecosystem.
Some visualization tasks require additional dependencies like `r ggplot2`.
By creating a dedicated package for visualization tasks, `r mlr3` developers have ensured that non-visualization tasks can be run anywhere that lack these extra dependencies.

## Package Ecosystem

`r mlr3` makes use of the following packages not developed by core members of the `r mlr3` team:
...
-   `r ref_pkg("lgr")`: Logging facility.
-   `r ref_pkg("mlbench")`: A collection of machine learning data sets.
-   `r ref_pkg("evaluate")`: For [capturing output, warnings, and exceptions](#error-handling).
-   `r ref_pkg("future")` / `r ref_pkg("future.apply")`: For [parallelization](#parallelization).

These are core packages within the R ecosystem.

The `r mlr3` package itself provides the base functionality that the rest of ecosystem (`mlr3verse`) rely on and some fundamental building blocks for machine learning.
@fig-mlrverse shows the packages within `mlr3verse` that extend `r mlr3` with capabilities for preprocessing, pipelining, visualizations, additional learners, additional task types, and more.
...
![mlr3verse](mlr3verse.svg){@fig-mlrverse}

```{asis 01-intro-001a, include=knitr::is_latex_output(), results='asis'}
To view the mlr3verse image for an overview of the`r mlr3`package ecosystem, follow this link: https://raw.githubusercontent.com/mlr-org/mlr3/master/man/figures/mlr3verse.svg.
```
...
In the following subsections, we briefly introduce both packages since at least a basic understanding is required to work efficiently with `r mlr3`.
...
We focus on the parts of R6 that you need to know to use `r mlr3` here.
...
Most objects in `r mlr3` are created through special functions (e.g. `lrn("regr.rpart")`) that encapsulate this and are also referred to as *sugar functions*.
...
::: callout-tip
For more details on R6, have a look at the excellent [R6 vignettes](https://r6.r-lib.org/), especially the [introduction](https://r6.r-lib.org/articles/Introduction.html). For comprehensive R6 information, we refer to the [R6 chapter from Advanced R](https://adv-r.hadley.nz/r6.html).
:::
...
Many functions of `r mlr3` return `data.table`s which can conveniently be subsetted or combined with other outputs.
