...
Our core packages do not provide visualizations because their dependencies would make installation unnecessarily complex, especially on headless servers. (many headless servers do not have the dependencies `r mlr3` requires)
...
## How R6 & data.table applies to `r mlr3`
...
Values in dictionaries are often accessed through sugar functions (i.e. `lrn()`) that automatically use the applicable dictionary (i.e. `mlr3learners`) without the user having to specify it; only the key to be retrieved needs to be specified.
...
We heavily use helper functions throughout this book and give the equivalent "full form" for full detail.
In most cases, the helper functions will achieve what you want to do, and you only have to consider using the full R6 code if you program custom objects or extensions.
