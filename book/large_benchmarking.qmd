...
The `stdout` of the individual jobs is saved in the batchtools registry and can be accessed through functions like `r ref("getLog()")` or `r ref("grepLogs()")`.
...
Therefore, we should focus on the logs saved in the learners, as warnings and errors were suppressed from `stdout`.
...
For most use cases it is recommended to simply use `r ref("reduceResultsBatchmark()")`.
