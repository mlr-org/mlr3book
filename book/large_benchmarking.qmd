...
#| eval: true
set.seed(1)
options(mlr3oml.cache = file.path(here::here(), "book", "openml", "cache"))
...
The code below compares a logistic regression with a random forest using a simple holdout resampling and three classification tasks that come with `r ref_pkg("mlr3")`.
...
In order to be able to draw meaningful conclusions from benchmark experiments, a good choice of the datasets and tasks is essential.
...
In this section we will cover the main features of OpenML and how to use them via the interface package `r ref_pkg("mlr3oml")`.
...
*   `r define("OpenML Dataset")`: A (usually tabular) dataset with additional metadata.
    ...
    As most OpenML data sets also have a designated target column, they can often directly be converted to a `r ref("mlr3::Task")`.
...
We will now start with covering the three object types listed above.
Afterwards, we will show how one can query OpenML to find objects with specific properties.
...
<!-- FIXME: Use mlr3::benchmark_grid_pa once available -->
...
### Filtering {#sec-openml-listing}

In the last sections, we have learned how to work with the OpenML task, dataset and collection.
Another important question is how to find relevant objects on OpenML.
...
:::{.callout-tip}
These filtering operations can also be conducted through the OpenML website.
:::
...
While access to HPC clusters is widespread, the effort required to operate these systems is still considerable.
...
We now add all 10 resampling iterations as experiments, for both the logistic regression and the ranger learner.
We use a robustified `ranger` and logistic regression which we have already defined earlier.
...
TODO:
* `r ref("esimateRuntimes")`
* `r ref("testJob")` with external = TRUE
...
#### Job Monitorning and Error Handling {@sec-batchools-supervission}

In an ideal world, we submit the jobs, wait for them to finish, and then collect the results.
However, in practice things will often go wrong and it is crucial to monitor jobs to ensure they are running correctly.
In this section, we will cover a few important functions of `r ref_pkg("batchtools")` that make this process easier.

* `r ref("getJobStatus")`
* `r ref("getErrorMessages")`
* `r ref("findExpired")`
* `r ref("findErrors")`
* `r ref("esti")`
* `r ref("grepLogs")`
...
There might be cases however, where an algorithm simply fails on a dataset.
For such cases it can be useful to register a fallback learner.

We have already covered fallback learners
The objective of using fallback learners is to generate scoring results even when a r ref("Learner") is unable to fit a model, fails to provide predictions for all observations, or predicts missing values.
...
TODO: How to access individiual result and reassemble them using `c()`.
