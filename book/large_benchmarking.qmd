...
Once the registry has been created, we need to populate it with problems and algorithms to form the jobs, this is most easily carried out with `r ref_pkg("mlr3batchmark")`, although finer control with `r ref_pkg("batchtools")` is possible and will be explored in @sec-custom-experiment-definition.
...
#| label: fig-lsb-cd
#| fig-cap: "Critical difference diagram comparing the random forest, logistic regression, and featureless baseline. The critical difference of 1.35 in the title refers to the different in mean rank required to conclude that one learner performs statistically different to another."
#| fig-align: "center"
...
autoplot(bma, type = "cd", ratio = 1/8)
```
