...
Such studies evaluate a variety of algorithms on a wide range of datasets, with the aim of identifying which method perform best under the given circumstances.
These empirical investigations are essential for understanding the capabilities and limitations of different algorithms and for developing new and improved approaches.
To carry them out effectively, researchers require access to a large collection of datasets, spanning a wide range of domains and problem types.
...
Finally, once the experiments are complete, statistical analysis is used to answer the original scientific question that motivated the benchmark experiment in the first place.
...
The performance is evaluated using measures $M_k$ with $k = 1, \ldots, m$.
...
@tbl-ml-benchmark illustrates an experimental design and should seem familiar, as it is the result of calling the `$aggregate()` method of a `r ref("BenchmarkResult")`.
...
@sec-performance already covered how to conduct such benchmark experiments and we recommend going back to @sec-performance if anything is unclear.
...
In the subsequent sections, we will turn this example into a more realistic experiment.
...
This includes for example a license.
...
OpenML tasks are built on top of OpenML datasets and additionally specify the variable, features, and the train-test splits.
...
From the printed output below we can see that the dataset from the task is indeed the adult data with ID 1590.
It also tells us the task type, the target variable^[This may differ from the default target used in the previous section.] and the estimation procedure.
...
Besides working with objects with known IDs, another important question is how to find data.
Because objects on OpenML have strict metadata, they can be filtered.
This is possible through either the website or the REST API, which we show here.
...
We can use the `r ref("list_oml_tasks()")` function to filter the CC-18 further.
...
The computation is conceptually straight-forward to parallelize, because we are facing an embarrassingly parallel problem (see @sec-parallelization).
...
If we were using problem parameters, we could, e.g., modify the problem instance depending on their values.
...
The algorithm function receives a list containing the task and resampling as the problem `instance`, the `learner_id` as the algorithm parameter, and the `job` object, through which wecan access the experiment replication in the `$repl` field.
...
Note that these are rather internal data structures and you should know what you are doing when you are working with them.
For this reason, the `r ref_pkg("mlr3batchmark")` package that is covered in @sec-mlr3batchmark takes care of such things for you.
:::

As we have now defined the problems and the algorithm, we can define concrete experiments using`r ref("batchtools::addExperiments()")`, which among other things takes in the
...
1. `design_table`, which will give access to the `r ref("Task")` and `r ref("Resampling")` object for a given `learner_id` and `task_id`.
1. `job_table`, which maps `job.id`s to `learner_id` and `task_id`
1. `result_table`, which maps the `job.id`s to the trained `learner` and `prediction` object.
...
Next, we subset the job table and `r ref("batchtools::unwrap()")` it to unpack the list column `"algo.pars"`.
...
Then, we define the result table from the experiment `results`, which are lists containing the trained learner and prediction object.
...
Not only does the package simplify the process, it is also a more efficient solution than shown in the previous sections, where we have sacrificed some efficiency for simplicity.
...
This does something similar as the steps that did by hand before, i.e. register problems, algorithms, and experiments.
