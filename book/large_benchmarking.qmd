...
This can be represented like in @tbl-ml-benchmark below, where we summarize the task-resampling combination as a Problem.
...
```{r large_benchmarking-015_experiments-015}
...
:::{.callout-info}
The terms `Job` and `Experiment` are very similar in `r ref_pkg("batchtools")` and for our purposes in this chapter they are interchangeable.
:::
...
```{r large_benchmarking-036_experiments-036}
...
```{r large_benchmarking-044_experiments-044}
```
...
1. In this exercise, we are investigating the effect of the number of folds on the performance estimate and we will execute the experiment using `r ref_pkg("batchtools")`.
   ...
   d. Add an experiment using the iris task, the `classif.rpart` learner and the accuracy measure.
