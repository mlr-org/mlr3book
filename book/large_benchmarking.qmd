...
`r authors("Large-Scale Benchmark Experiments")`
...
The central concept of `r ref_pkg("batchtools")` is the experiment or `r define("job")`:
...
We will chunk the IDs in such a way that 5 iterations of one resample experiment are run sequentially in one computational job.
...
chunks = data.table(
  job.id = job_table$job.id, chunk = rep(1:36, each = 5)
)
chunks
```
...
Note that the object returned by `loadResult()` is optimized for efficiency and it is therefore recommended to use the latter of the two functions.
...
It is therefore advised to register a fallback learner to deal with such situations in a statistically sound fashion.
...
| `r ref("as_benchmark_aggr()")`      | `r ref("BenchmarkAggr")`   | Convert to a benchmark aggregate                                    |
| `r ref("autoplot()")`             |-                            | Visualize an object                                                  |
