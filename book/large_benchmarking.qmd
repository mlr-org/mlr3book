...
abstract:
  (ABSTRACT START) In the field of machine learning, benchmark experiments are used to evaluate and compare the performance of algorithms.
  ...
  Once the experiments are complete, statistical analysis is required to draw conclusions from the results using the [`mlr3benchmark`](https://github.com/mlr-org/mlr3benchmark) package for post-hoc analyses. (ABSTRACT END)
params:
    rebuild_cache: false
---
...
In the subsequent sections, we will show the steps require to turn this analysis on only three tasks into a more meaningful experiment using the tools mentioned earlier.
...
Our goal in this section is to run the benchmark design shown in @sec-openml-collection on an HPC cluster.
...
While it might seem natural to define one such triplet as a job, each resample experiment can be split up even further, namely into its iterations $E^1_{i, j}, ..., E_{i, j}^{n_j}$, where $n_j$ is the number of iterations of resampling $R_j$, making the computation more fine granular and giving you the opportunity to utilize more CPUs in parallel (c.f., @#sec-parallelization).
...
An exemplary approach is covered in the following Section-FIXME.
...
#| output: false
library(data.table)
learner_ids = list("classif.ranger", "classif.logreg", "classif.featureless")
addExperiments(
...
#| cache: false
featureless_ids = findExperiments(algo.pars = (learner_id == "classif.featureless"))
...
Before we can use these methods, we have to convert the benchmark result to a `r ref("mlr3benchmark::BenchmarkAggr")` using `as_benchmark_aggr()`.
