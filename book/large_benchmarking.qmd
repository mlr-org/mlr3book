...
@tbl-ml-benchmark illustrates such an experiment design and should seem familiar, as it is the result of calling the `$aggregate()` method of a `r ref("BenchmarkResult")`.
...
In the following sections, we will now turn this example into a more realistic benchmark experiment.
...
::: {.callout-tip}
These filtering operations are also available on the OpenML website.
:::
...
A solution to this problem is to use aan existing curated task collection, which we will cover next.
...
Coming back to our original goal of comparing the `r ref_pkg("ranger")` random forest implementation with the logistic regression, we will now define our experiment design with the help of the CC-18.
...
Note that the problem parameters $\xi_P$ can e.g. useful in simulation studies but we won't use them for our purposes.
...
There is not a single right way to do this and we here show a solution in which we use the notion of an experiment replication to represent the resampling iterations.
...
Note that such a job does not have to coincide with the notion of a computational job defined earlier.
...
* `fun`, which takes in the problem `data`, the parameters $\xi_P$ (here unused) and the `job` definition, c.f. `r ref("batchtools::makeJob()")` for an explanation of the `job` object.
...
We define one algorithm `"run_learner"` and make the learner itself an algorithm parameter.
The algorithm function receives a list containing the task and resampling as the problem `instance`, the `learner` as the algorithm parameter and the `job` object, which contains the experiment replication in the `$repl` field.
...
* `repls`, the experiment replications. We use this this to represent the resample iterations.
...
addExperiments(
  ...
  algo.designs = list(
    run_learner = data.table(learner = list(lrn_logreg, lrn_ranger))),
  repls = 10
)
```
...
1. (optionally) grouping multiple jobs into one computational job (chunking, see @sec-parallelization).
...
The following code snippets illustrate their usage, by first finding all jobs of the logistic regression method and then testing one of them.
...
The variable `rr_no` enumerates the resample experiments.

```{r}
job_table$learner_id = sapply(job_table$algo.pars, function(p) p$learner$id)

tbl = job_table[,
  .(job.id, repl, rr_no = rep(.GRP, times = .N)),
  by = list(problem, learner_id)]

tbl[c(1:3, 11:13), ]
```

For each resample experiment, we now load all the predictions and learners.
In order to create the `r ref("ResampleResult")` object, we also need to create the task and resampling.
One way to achieve this is by creating a batchtools job object, by passing the respective `job.id` to `r ref("batchtools::makeJob()")`.
This object stores all the information needed to run the given job in the first place.
It therefore also contains the resampling and task by first accessing the static problem part via `$problem$data`, as well as the replication via `$repl`.

  <!-- # FIXME: as_resample_result() when merged in mlr3 -->
```{r}
resample_results = lapply(unique(tbl$rr_no), function(i) {
  tbl_subset = tbl[rr_no == i, .(job.id, repl)]
  ids = tbl_subset$job.id
  learners = lapply(results[ids], function(x) x$learner)
  predictions = lapply(results[ids], function(x) list(test = x$prediction))
...
bma$friedman_test()
```
These results indicate difference in performance between the random forest and the logistic regression, although they are not conclusive.
