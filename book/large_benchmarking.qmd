...
While `r ref_pkg("mlr3batchmark")` might be enough, there can be situations in which more fine-grained control over the experiment definition is beneficial or even required.
...
This differs from the `r ref_pkg("mlr3batchmark")`, where one resampling iteration corresponded to one `r ref_pkg("batchtools")`
...
By leaving `prob.designs` unspecified, experiments for all existing problems are created.
...
While we took a different route than in @sec-mlr3batchmark to define the experiments, we arrived at the same result.
