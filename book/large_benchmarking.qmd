...
unlink("experiment_custom", recursive = TRUE)
...
# IMPORTANT: 
# For both experiments, we always create the registry in a "normal" way up until the experiments are run
# After that, we load the experiment registry (which is under a different path howver)
# For this reason the experiment registry must not be printer after the experiments are done
# attemps to solve this did not work (for me at least) because of cache dependencies etc.
...
In the introduction of this chapter, we have defined a benchmark experiment as evaluating a number of triples $E_{i, j} = (L_i, C_j, M)$ that are defined by a learner $L_i$, a resampled task $C_j = (T_j, R_j)$ (a resampling instanted on a task) and measures $M$.
While it might seem natural to define one such triplet as a job, each resampled task $C_j$ can be split up even further, namely into its resampling iterations $C^1_{i, j}, \ldots, C^{n_j}_{i, j}$, where $n_j$ is the number of iterations of resampling $R_j$.
...
Moreover, because the evaluation of the measures $M$ is (usually) computationally negligible, it common to only parallelize the execution of the resample experiments and evaluate the measures $M$ afterwards.
For these reasons we will understand one pair $(L_i, C^l_{i, j})$ as a single batchtools experiment.^[Note that such a job does not have to coincide with the notion of a computational job defined earlier, more on that in @sec-batchtools-submission.]
...
#| include: false
#| cache: false
if (params$rebuild_cache || !length(list.files(file.path("experiment_finished", "results")))) {
  submitJobs(ids = chunks, reg = reg)
  waitForJobs(reg = reg)
  if (file.exists("experiment_finished")) unlink("experiment_finished", recursive = TRUE)
  fs::dir_copy("experiment", "experiment_finished")
}
reg = loadRegistry("experiment_finished")
# DONT PRINT THE REGISTRY AFTER THAT. FIX IN THE FUTURE BUT THE CACHING IS QUITE TRICKY
```
...
In our case, all experiments finished and none expired or crashed.
Nonetheless, we should inspect warning messages and log outputs.
The `stdout` of the individual jobs can be accessed via the batchtools registry through functions like `r ref("getLog()")` or `r ref("grepLogs()")`.
However, encapsulation (c.f. @sec-encapsulation) was automatically set to `r ref_pkg("evaluate")` when we specified the fallback learner for the logistic regression and the random forest.
Therefore, we should focus on the logs saved in the learners.
To do so, we first use `r ref("mlr3batchmark::reduceResultsBatchmark()")` to assemble the individual job results into a `r ref("BenchmarkResult")`.
The resulting object is the same (except for seeding) that we would have obtained by calling `r ref("benchmark()")` on the `large_design` from earlier.
...
The printed output shows that the logistic regression threw warnings for three tasks.
However, no error was encountered and therefore no fallback learner triggered.
As an example, we access the warning for the first resample result, which was caused by 0-1 probabilities we already encountered earlier when testing the job.

```{r large_benchmarking-045}
bmr$resample_result(1)$learners[[1]]$warning
```

We can also load individual experiment results.
Below, we first search for specific job IDs using `r ref("findExperiments()")` and then load the result using `r ref("loadResult()")`.

```{r large_benchmarking-042}
ids = findExperiments(
  algo.pars = learner_id == "logreg", prob.pars = task_id == "kr-vs-kp", 
  reg = reg
)
ids$job.id
result_1 = loadResult(ids$job.id[1], reg = reg)
names(result_1)
```

This is the same object as returned by `r ref("testJob()")` earlier and an internal data structure.
For most use cases it is recommended to utilize `r ref("reduceResultsBatchmark()")`.
...
if (params$rebuild_cache || !length(list.files(file.path("experiment_custom_finished", "results")))) {
  submitJobs(ids = chunks, reg = reg)
  waitForJobs(reg = reg)
  if (file.exists("experiment_custom_finished")) unlink("experiment_custom_finished", recursive = TRUE)
  fs::dir_copy("experiment_custom", "experiment_custom_finished")
}
reg = loadRegistry("experiment_custom_finished")
```
