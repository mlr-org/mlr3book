...
summarizeExperiments(by = c("task_id", "learner_id"), reg = reg)
```
...
In the following, we extend the design with the debug learner which randomly errors with a probability of 50%.
By just calling `r ref("batchmark()")` with the new design again, the new experiments will be added.
...
We can check the new state and submit the jobs which have not been submitted yet (i.e., the new jobs):
```{r}
#| cache: false
...
After these jobs terminated, we can get a summary of all jobs which failed:

```{r}
error_ids = findErrors(reg = reg)
summarizeExperiments(error_ids, by = c("task_id", "learner_id"), reg = reg)
```

Unsurprisingly, all failed jobs have one thing in common: the debug learner.

Finally, it is time to collect the results.
...
It is also possible to retrieve single results with `r ref("loadResult()")`, but the returned values would need to reconstructed to mlr3 objects manually as only a subset of the objects is returned for efficiency.
Here, we only collect results which were not produced by the debug learner.

```{r}
ids = findExperiments(algo.pars = learner_id != "classif.debug", reg = reg)
