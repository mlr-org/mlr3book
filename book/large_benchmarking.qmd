...
Such an ID can either be found by searching for objects on the OpenML website or through the functions provided by `r ref_pkg("mlr3oml")`.
...
This can be done using `r ref("list_oml_tasks()")` and works analogously to the previous example.
...
The `r ref_pkg("mlr3batchmark")` package assists in translating the machine learning problem defined with syntax and objects from `r ref_pkg("mlr3")` to the more general "apply some algorithm A on some problem P" approach in `r ref_pkg("batchtools")`, which is briefly outlined in @sec-custom-experiments-batchtools.
...
The custom definition of problems and algorithm which yields a different granularity is demonstrated in @sec-custom-experiments-batchtools.
...
Among other things, we are informed that the "Interactive" cluster function (see `r ref("batchtools::makeClusterFunctionsInteractive()")`) is used - this is the abstraction for the scheduling system, and "interactive" here means to not use a real scheduler but instead to use the interactive R session for computation of jobs.
...
In general, algorithms in `batchtools` may return arbitrary objects - the returned object is simply stored on the file system and can be processed with a custom function while collecting the results.
...
We can do this, by iterating over all resample results using `r ref("batchtools::reduceResults()")` together with the combine function `c()`.
The combine function merges multiple objects of type `ResampleResult` or `BenchmarkResult` to a single `BenchmarkResult`.
