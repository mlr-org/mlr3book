...
abstract:
  ...
  The first part of this chapter covers the basics of OpenML and how it can be used via the [mlr3oml](https://github.com/mllg/batchtools) interface package.
...
Such an experiment usually tries to answer which learner performs best on the given tasks when evaluated with performance measure $M$.
The whole benchmark experiment consists of many individual resample experiments, each defined by a triple $E_{i, j} = (L_i, T_j, R_j)$.
Running the benchmark experiment consists of running each resample experiment and then evaluating the results using the measure $M$.
...
For the second issue, we will illustrate how the R package `r ref_pkg("batchtools")` [@batchtools] and its mlr3 integration `r ref_pkg("mlr3batchmark")` can greatly simplify the effort required to interact with a scheduling system on a HPC cluster.
...
However, if the experiment is large, parallelization via `r ref_pkg("future")` (see @sec-parallelization) might not be enough and usage of a HPC cluster is required.
...
Multiple users -- in this case Ann and Bob --  can SSH into the head node and add their computational workloads of the form "Execute Computation X using Resources Y for Z amount of time" to the queue.
...
The R package `r ref_pkg("batchtools")` also has a different registry object that can be constructed with `r ref("batchtools::makeRegistry()")` whih we will not cover in this book.
:::
