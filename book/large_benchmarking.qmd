...
Finally in @sec-benchmark-analysis we will demonstrate how to make use of `r ref_pkg("mlr3benchmark")` to analyze the results.

Throughout this chapter we will use the running example of benchmarking a random forest against logistic regression (this problem has been extensively studied, e.g. @couronne2018random).
...
Note that we "robustify" both learners to work on a wide range of tasks using the respective preprocessing pipeline, c.f. @sec-preprocessing.
...
Finding datasets from OpenML is possible via the website or via the functions provided by `r ref_pkg("mlr3oml")`.
...
For example, let us consider the 'adult' dataset with ID 1590.
...
Data can then be converted into backends (see @sec-backends) with the `r ref("as_data_backend()")` function and then converted into tasks:
...
To extract a list of tasks and a list of corresponding resampling strategies for the experiment, you can use `r ref("as_tasks()")` and `r ref("as_resamplings()")`, respectively:
...
To define the design table, we use `r ref("benchmark_grid()")` and set `paired` to `TRUE`, which is used in situations where each resampling is instantiated on a corresponding task (therefore the list of `tasks` and list of `resamplings` below must have the same length) and each learner should be evaluated on every resampled task.
...
For this section we will define a single batchtools experiment as one resampling iteration of one learner on one task, in @sec-custom-experiments-batchtools we will look at different ways of defining an experiment.
...
Among other things, we are informed that the "Interactive" cluster function (see `r ref("batchtools::makeClusterFunctionsInteractive()")`) is used - this is the abstraction for the scheduling system, and "interactive" here means to not use a real scheduler but instead to use the interactive R session for computation.
...
Doing this step with `r ref_pkg("batchtools")` is also possible and gives you more flexibility and is demonstrated in @sec-custom-experiments-batchtools.
...
Once jobs have been submitted, an aggregated overview can be queried with `r ref("batchtools::getStatus()", aside = TRUE)` and the results (or errors) can be investigated in a second step.
...
getStatus(reg = reg)
```
The functions `r ref("batchtools::getJobStatus()")` and `r ref("batchtools::getJobTable()")` provide a more detailed and unaggregated view of the computational state.
To specifically query the ids of jobs in the respective categories, see `r ref("batchtools::findJobs()")` and, e.g., `r ref("batchtools::findNotSubmitted()")` or `r ref("batchtools::findDone()")`.
...
### Custom Experiments with batchtools {#sec-custom-experiments-batchtools}
...
This method first performs a global to see if any learner is statistically better than another.
...
This is in line with the large benchmark study conducted by @couronne2018random, where the random forest outperformed the logistic regression in 69% of 243 world datasets.
However, a contrary picture may emerge for datasets from a particular domain or if the datasets are selected differently - these are merely empirical observations.
