...
Benchmark experiments involve evaluating a variety of algorithms on a wide range of datasets, with the aim of identifying which method perform best under the given circumstances.
...
To carry out benchmark experiments effectively, researchers require access to a large collection of datasets, spanning a wide range of domains and problem types.
...
This analysis allows researchers to answer the original scientific question that motivated the benchmark experiment in the first place.
The answers to these questions provide valuable information for both theoretical and practical applications of machine learning.
We will show how to conduct such statistical analysis using the `r ref_pkg("mlr3benchmark")` package.

A common benchmarking scenario in machine learning is to compare a set of Learners $L_1, \ldots L_n$ by evaluating their performance on a set of tasks $T_1, \ldots, T_k$ which each have an associated resampling $R_1, \ldots, R_k$.
The performance is evaluated using measures $M_k, k = 1, \ldots, m$.
Such experiments can, e.g., answer which learner performs best on the given tasks w.r.t. the given measures.
...
The execution of a single resample experiment can again be subdivided into computationally independent resampling iterations $E^l_{i, j}$.
For a more in-depth coverage of resampling and its purpose, we refer to @sec-resampling.
@tbl-ml-benchmark illustrates such an experimental design and should seem familiar, as it is the result of calling the `$aggregate()` method of a `r ref("BenchmarkResult")`.
Note that the evaluated performance measures $m_{i, j}$ are vectors if there is more than one performance measure $M_k$.
...
@sec-performance already covered how to create and run such an experiment.
We will now briefly walk through the involved steps, but recommend going back to @sec-benchmarking if anything is unclear.
...
The following illustrative example compares the two methods using a simple holdout resampling and three classification tasks that come with `r ref_pkg("mlr3")`.
...
In this simple example, the random forest outperforms the logistic regression on two out of three datasets.
In the following sections, we will turn this example into a more realistic experiment.
...
*   `r define("OpenML **Task**")`: A machine learning task, i.e. a concrete problem specification on an OpenML dataset.
    This includes splits into training and test sets, thereby differing from the mlr3 task definition.
...
Such IDs can either be found by searching for objects on the OpenML website, or through the REST API.
...
This dataset contains information about `r odata$nrow` adults -- such as their age or education -- and the goal is usually to predict the *class* variable, which indicats whether a person has an income above 50K dollars per year.
...
This includes for example a license of the dataset.
...
The mlr3 class that comes closest to the OpenML dataset is the `r ref("mlr3::DataBackend")` (see @sec-backends) and it is possible to convert `r ref("OMLData")` objects by calling `r ref("as_data_backend()")`.
...
If we wanted to use this dataset as a task in our comparison of the random forest with the logistic regression, we could create an mlr3 classification task using `r ref("as_task_classif()")`.
...
For the first of these we can use `r ref("as_task()")`.
...
Besides working with objects with known IDs, another important question is how to find objects that satisfy specific properties.
Because objects on OpenML have strict metadata, they can be filtered.
The `r ref("list_oml_data()")` function allows to search for datasets.
...
::: {.callout-tip}
One can also search for datasets and tasks on the OpenML website.
On one hand, the same queries that we just demonstrated can be performed.
On the other hand, searching for specific datasets, such as accessing the renowned "adult" dataset through OpenML, is possible through targeted text searches on the site.
:::

We could now start looking at the returned IDs in more detail to verify whether they are suitable for our purposes.
This process is tedious, as some datasets have quirks to look out for.
...
One example of such a benchmark suite is the `r link("https://www.openml.org/search?type=study&study_type=task&id=99", "OpenML CC-18")` which contains curated classification tasks [@bischl2021openml].
...
Coming back to our original goal of comparing the `r ref_pkg("ranger")` random forest implementation with the logistic regression, we will now define our experimental design with the help of the CC-18.
...
As learners in our benchmark design, we use the robustified logistic regression and random forest which we have defined in the introduction of this chapter.
Additionally, we will include the featureless learner as a baseline.
We also add a featureless learner as a simple baseline.

In the code below, we define the tasks, learners, and resamplings.
...
We have now defined all the objects we will use in the benchmark experiment and can create the design using `r ref("benchmark_grid()")`.
Because the tasks and resamplings are paired, i.e. the resamplings are instantiated on the tasks, we set `paired` to `TRUE`.
We also add a featureless learner as a simple baseline.
...
Such experiments are conceptually straight-forward to parallelize, because we are facing an embarrassingly parallel problem (see @sec-parallelization).
...
If we want to execute this experimental design using `r ref_pkg("batchtools")`, we need to understand how the batchtools package conceptualizes benchmark experiments.
...
Each experiment $E$ is independent of all other experiments and constitutes the basic level of computation batchtools can parallelize.

There is not a single right way to use batchtools and we here show one solution in which we use the notion of an experiment replication to represent the resampling iterations.
...
We will therefore understand one resampling iteration $E^l_{i, j}$ as a job (aka. experiment). ^[Note that such a job does not have to coincide with the notion of a computational job defined earlier, more on that later.]
...
It is a more generic registry that can be used in situations where the computation is not defined by algorithms or specific problems.
:::
...
Furthermore, we set the registry's `seed` to 1 and the `packages` to `r ref_pkg("mlr3verse")`, which will load the package in all our experiments.
...
* `fun`, which takes in the problem `data`, the parameters $\xi_P$ and the `job` definition, c.f. `r ref("batchtools::makeJob()")` for an explanation of the `job` object. The last two are unused here.

We register all task-resampling combination of the `large_design` using the task ID as the name^[The mlr3 task ID is not the same as the OpenML task ID.].
The problem `fun` takes in the static problem `data` and simply returns it as the problem instance.
This is also the default behaviour but we here include it for illustrative purposes.
If we were using problem parameters, we could modify the problem depending on their values.
Recall that the `tasks` and `resamplings` were originally used to define the `large_design`.
...
* `fun`, which takes in the problem instance, the parameters $\xi_A$, and the `job` definition.
  It defines the computational steps of an experiment and its return value is the experiment result.

The algorithm function receives a list containing the task and resampling as the problem `instance`, the `learner_id` as the algorithm parameter and the `job` object, which contains the experiment replication in the `$repl` field.
The algorithm executes the given resampling iteration and returns the trained learner and prediction object.
...
Note that these are rather internal data structures and for expert use only.
The `r ref_pkg("mlr3batchmark")` package (c.f. @sec-mlr3batchmark) takes care of these issues.
:::

As we have now defined the problems and the algorithm, we can define concrete experiments.
We can do this using `r ref("batchtools::addExperiments()")`, which among other things takes in the
...
* `repls`, the experiment replications. We use this this to represent the resample iterations. NB: Notice that this means that our replications are not iid, as the training sets of the different folds are overlapping.
...
By printing the registry we confirm that the algorithm, problem and experiments (jobs) were added succesfully.
...
```
We can use `r ref("batchtools::getJobTable()")` to get more detailed information about existing jobs.
...
We are now almost ready to send computational jobs to the scheduler.
...
It returns the job IDs of all experiments that match the given criteria.
The following code snippets illustrate their usage, by first finding all jobs of the featureless method and then testing one of them.
...
However, if something goes wrong, `r ref_pkg("batchtools")` comes with a bunch of useful debugging utilities, some of which are covered in @sec-batchtools-monitoring.
Once we are confident that the jobs are defined correctly, we can proceed with the job submission, which requires:
...
* `ids`, which are either a vector of job ids to submit, or a data frame with columns `job.id` and `chunk`, which allows to group multiple jobs into one computational job. 
 ...
 (chunking, see @sec-parallelization).
, and
...
To create these objects, we need information from three different sources:

1. The experiment design (`large_design`), which contains the actual learner, task and resampling objects
1. The `job_table`, which creates a mapping from `job.ids` to the batchtools problem and algorithm, i.e. the learner and task.
1. The `result_table`, which contains the `job.ids` and the experiment result, i.e. the trained learners and the predictions.

In the code below, we create a `data.table` containing the results and the job ids.

```{r}
library(mlr3misc)
result_table = data.table(
  job.id = job_table$job.id,
  learner = map(results, "learner"),
...
To structure the objects necessary, we will join the `large_design` with a subset of the `job_table` from earlier on the `learner_id` and `task_id`.
Then we will join the resulting table with the `result_table` using the `job.id`.
This will allow us to clearly map the job ids, to the corresponding 

We first add the `leaner_id` and `task_id` columns to the `large_design`

```{r}
design_table = data.table(
  learner_id = map_chr(large_design$learner, "id"),
  task_id = map_chr(large_design$task, "id"),
  task = large_design$task,
  resampling = large_design$resampling
)
```

Furthermore, we create a subset of the job table with the relevant information.
Because the `lerner_id` is nested in a `list()`, we use batchtools's `r ref("unwrap")` function.

```{r}
job_table2 = unwrap(job_table[, .(job.id, algo.pars, task_id = problem, repl)])
```
...
Using this table, we can now define the resample results.
As each learner-task combination defines one resampling, we loop all possible pairs and create the resample result.

```{r}
learner_ids = unique(table$learner_id)
task_ids = unique(table$task_id)
# TODO: Crossproduct
resample_results = pmap(list(lid = learner_ids, tid = task_ids), 
  function(lid, tid) {
...
The previous section showed that running mlr3 benchmark experiments with batchtools requires some advanced understanding of mlr3 and batchtools.
To make this process 2
All of what we have shown in the previous section can be considerably simplified with the use of the `r ref_pkg("mlr3batchmark")` packages, which conn
...
The whole process, of adding the algorithm, problems, and experiments can be achieved by simply replacing the common `r ref("mlr3::benchmark()")` call with `r ref("mlr3batchmark::batchmark()")`.
...
When printing the registry, we see that one algorithm, six problems, and 160 experiments are registered.
...
Once they are done, we can either load the individual results using `r ref("batchtools::loadResult()")`, or the complete `r ref("BenchmarkResult")` by calling `r ref("mlr3batchmark::reduceResultsBatchmark()")`.
...
Before we can use these methods, we have to convert the benchmark result to a `r ref("BenchmarkAggr")` using `r ref("mlr3benchmark::as_benchmark_aggr()")`.
...
bma$friedman_test()
```
These results indicate a difference between the performance of the algorithms.
The ranks that were used to conduct the friedman test can be accessed via `$rank_data()`.
We can now apply the `$friedman_posthoc()` test to see which learners differ.
...
We can also visualize the results using a critical difference plot, by specifying `type` to be `"cd"`.
...
We see that the random forest outperforms the logistic regression an all but one task.
This result is in line with the results from a large benchmark study conducted by @couronne2018random that studied the same question.
