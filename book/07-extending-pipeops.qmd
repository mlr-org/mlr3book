# Adding new PipeOps {#extending-pipeops .unnumbered}

```{r 07-extending-pipeops-001, include = F}
library(mlr3)
library(mlr3book)
```

This section showcases how the `r mlr3book::mlr_pkg("mlr3pipelines")` package can be extended to include custom `r ref("PipeOp")`s.
To run the following examples, we will need a `r ref("Task")`; we are using the well-known "Iris" task:

```{r 07-extending-pipeops-002}
library("mlr3")
task = tsk("iris")
task$data()
```

`r mlr3book::mlr_pkg("mlr3pipelines")` is fundamentally built around [`R6`](https://r6.r-lib.org/).
...
```{r 07-extending-pipeops-003, echo=FALSE}
...
```{r 07-extending-pipeops-004, eval = FALSE, tidy = FALSE}
...
```{r 07-extending-pipeops-005, eval = FALSE}
initialize = function(id = "copy.two") {
...
```{r 07-extending-pipeops-006, eval = FALSE}
.train = function(inputs) {
  self$state = list()
  c(inputs, inputs)
}
```
```{r 07-extending-pipeops-007, eval = FALSE}
.predict = function(inputs) {
  c(inputs, inputs)
}
```
...
```{r 07-extending-pipeops-008, tidy = FALSE}
PipeOpCopyTwo = R6::R6Class("PipeOpCopyTwo",
  inherit = mlr3pipelines::PipeOp,
  public = list(
    initialize = function(id = "copy.two") {
...
Because `r mlr3book::mlr_pkg("mlr3")` `"Task", text = "Tasks")` only contain a view to the underlying data, it is not necessary to modify data to remove rows with missing values.
...
Furthermore, `r mlr3book::mlr_pkg("mlr3")` expects a `r ref("Learner")` to always return just as many predictions as it was given input rows, so a `r ref("PipeOp")` that removes `r ref("Task")` rows during training can not be used inside a `r ref("GraphLearner")`.
...
```{r 07-extending-pipeops-010, tidy = FALSE}
...
Compare this `PipeOpScaleAlways` operator to the one defined inside the `r mlr3book::mlr_pkg("mlr3pipelines")` package, `r ref("PipeOpScale")`.

```{r 07-extending-pipeops-013, tidy = FALSE}
...
_(Note for the observant: If you check `PipeOpScale.R` from the `r mlr3book::mlr_pkg("mlr3pipelines")` package, you will notice that is uses "`get("type")`" and "`get("id")`" instead of "`type`" and "`id`", because the static code checker on CRAN would otherwise complain about references to undefined variables. This is a "problem" with `data.table` and not exclusive to `r mlr3book::mlr_pkg("mlr3pipelines")`.)_
...
```{r 07-extending-pipeops-015, tidy = FALSE}
...
```{r 07-extending-pipeops-016}
irishead = task$clone()$filter(1:5)
irishead$data()
```

```{r 07-extending-pipeops-017}
...
```{r 07-extending-pipeops-018}
gr$pipeops$drop.const$state
```

```{r 07-extending-pipeops-019}
...
```{r 07-extending-pipeops-020, tidy = FALSE}
...
```{r 07-extending-pipeops-022}
result_posa$data()
```

```{r 07-extending-pipeops-023}
result_posa_simple$data()
```

## Hyperparameters {#ext-pipe-hyperpars}

`r mlr3book::mlr_pkg("mlr3pipelines")` uses the [`r mlr3book::mlr_pkg("paradox")`](https://paradox.mlr-org.com) package to define parameter spaces for `r ref("PipeOp")`s.
...
The `r ref("Graph")`s, when encapsulated in `r ref("GraphLearner")`s, can even be tuned using the tuning functionality in `r mlr3book::mlr_pkg("mlr3tuning")`.
...
The `param_set` must be a `r ref("ParamSet")` from the `r mlr3book::mlr_pkg("paradox")` package; see [the tuning chapter](#searchspace) or the [in-depth `r mlr3book::mlr_pkg("paradox")` chapter](#paradox) for more information on how to define parameter spaces.
...
This can easily be done when using the `r mlr3book::mlr_pkg("mlr3misc")` library's `r ref("mlr3misc::invoke()")` function, which has functionality similar to `"do.call()"`.
...
```{r 07-extending-pipeops-025}
pss = po("scale")
print(pss$param_set)
```

```{r 07-extending-pipeops-026}
pss$param_set$values$center = FALSE
print(pss$param_set$values)
```

```{r 07-extending-pipeops-027, error = TRUE}
