...
xgboost_preproc <- GraphLearner$new(
    factor_pipeline %>>%
...
ranger_preproc <- GraphLearner$new(
    impute_pipeline %>>%
        lrn("regr.ranger"),
    id = "regr.ranger_imp"
)
...
1) Constant features are removed
2) Character features are cast to categorical features
3) Date/time features are encoded as numeric features
4) Ordinal features are encoded as categorical features
5) Numeric features are imputed by sampling from the empirical distribution and missingness indicators are added
6) Categorical features are encoded with a new level
...
9) Categorical features levels are collapsed (starting from the rarest factors in the training data) until there are no more than 1000 levels
10) Categorical features are encoded by one-hot encoding
...
Unfortunately, we don't have any information about the quality or number of kitchen appliances, which can have a significant impact on the sale price.
...
```{r preproc-016, echo = TRUE, eval = FALSE, message=FALSE, warning=FALSE}
...
operators <- c("min", "max", "mean")

mutators <- lapply(operators, function(op) {
    as.formula(paste("~", op, "(", paste0("att", c(1:720), collapse = ", "), ")"))
})

names(mutators) <- operators

pom <- po("mutate")

pom$param_set$values$mutation <- mutators
```
