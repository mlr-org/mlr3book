...
@sec-pipelines provided a technical introduction to `r mlr3pipelines`, this chapter will now demonstrate how to use those pipelines to tackle common problems when `r index('preprocessing')` data for machine learning, including factor encoding, imputation of missing values, feature and target transformations, and feature extraction.
...
`r index('Data cleaning', aside = TRUE)`\index{exploratory data analysis|see{Data cleaning}} is an important part of preprocessing that involves the removal of errors, noise, and redundancy in the data; we only consider data cleaning very briefly as it can be simply performed without any `mlr3` functionality.
...
Before we get started with preprocessing, let us load the data and plot the target variable.

```{r preprocessing-001, echo = TRUE, eval = TRUE, message=FALSE}
#| label: fig-sale
#| fig-cap: Distribution of house sales prices (in USD). There is a skewed distribution of prices with a few outliers of very expensive properties.
#| fig-alt: Boxplot of house prices up to $600,000, the majority of prices are between roughly $100,000-$200,000.
library(data.table)
library(ggplot2)
...
autoplot(as_task_regr(ames, target = "Sale_Price"))
```
...
A common problem in real-world data is missing data or '`r index('missingness')`'.
...
It is very important that you keep track of missing data as it is common for missing data to be informative in itself.
...
To extract features we will write our own `PipeOp` that inherits from `PipeOpTaskPreprocSimple` (@sec-pipelines).
...
In this chapter we built on everything learnt in @sec-pipelines to look at concrete usage of pipelines for data preprocessing.
...
: `PipeOp`s and `Graph`s discussed in this chapter. {#tbl-prepro-api, tbl-colwidths="[40,60]"}
