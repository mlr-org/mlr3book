...
ranger_preproc = GraphLearner$new(impute_pipeline %>>% lrn("regr.ranger"), id = "regr.ranger")
```
...
lm_preproc = GraphLearner$new(robustify %>>% lrn("regr.lm"), id = "regr.lm")

design = benchmark_grid(ames_task, learners = list(ranger_preproc, lm_preproc), cv10)
...
For distance based methods such as K-nearest neighbor models, normalization is required as otherwise features with larger scale would have an higher impact on the neighborhood estimation.
Luckily, most distance based models internally scale the data so most of the time we do not need to manually do this in preprocessing.
...
log_lm_preproc = GraphLearner$new(log_lm_preproc, id = "regr.log_lm")

design = benchmark_grid(ames_task, learners = log_lm_preproc, cv10)
bmr_new = benchmark(design)
bmr$combine(bmr_new)
plot(bmr, measure = measure)
bmr$aggregate(measure = measure)[, .(learner_id, regr.mae)]
