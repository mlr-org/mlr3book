...
Although many learners are already included in the mlr3verse, there might be a situation in which your algorithm of choice is not implemented.
Here, we show how to create a custom mlr3learner step-by-step using `r ref("mlr3extralearners::create_learner")`.
If you think the learner will be useful to others as well, you can consider creating a pull request in the [mlr3extralearners](https://github.com/mlr-org/mlr3extralearners) repository.
If you intend to add a learner to mlr3extralearners, **it is strongly recommended** that you **first** open a [learner request issue](https://github.com/mlr-org/mlr3extralearners/issues/new?assignees=&labels=new+learner&template=learner-request-template.md&title=%5BLRNRQ%5D+Add+%3Calgorithm%3E+from+package+%3Cpackage%3E) to discuss the learner you want to implement.
This allows us to discuss the purpose and necessity of the learner before you start to put the real work in!
Note that some of the steps shown here are only relevant if the learners is being added to
mlr3extralearners.
...
2. [Install](#setup) `r mlr3book::mlr_pkg("mlr3extralearners")` and if you want to create a PR, also [fork and clone](#setup) it.
3. [Run](#create-learner) `r ref("mlr3extralearners::create_learner()")`.
4. Add the learner's [`param_set`](#param-set).
5. Manually add [`.train`](#learner-train) and [`.predict`](#learner-predict) private methods to the learner.
6. If the learner has the property

    *  `"importance"` add the method `importance`
    * `"oob_error"` add the method `oob_error`
    * `"loglik"` add the method `loglik`
    * `"selected_features"` add the method `selected_features`
    *  `"hotstart_{forward, backward}"` add the private method `.hotstart`

For more information see the section [other methods](#other-methods).

7. Fill out the missing parts in the learner's description. To add references you first need to
create an entry in bibentries.R
8. Check [unit tests](#learner-test) and [parameter tests](#learner-test) pass (these are automatically created).
...
**Do not copy/paste the code shown in this section. Use `r ref("create_learner()")` to start.**

## Setting-up mlr3extralearners {#setup}

In order to use the `r ref("mlr3extralearners::create_learner")` function you must have mlr3extralearners installed.
To ensure that you have the latest version, run `devtools::install_github("mlr-org/mlr3extralearners")`
...
Otherwise, all files are being created in the folder.
If the files already exist, they are not being modified.
...
1. `predict_types = c("response", "prob"),` - Sets the possible prediction types as response (deterministic) and prob (probabilistic). See [meta information](#learner-meta-information).
1. `properties = c("importance", "missings", "multiclass", "selected_features", "twoclass", "weights")` - Sets the properties that are handled by the learner.
By including `"importance"` a public method called `importance` will be created that must be manually filled.
See [meta information](#learner-meta-information).
...
The first script to complete after running `r ref("create_learner()")` is the file with the form `learner_package_type_key.R`, in our case this will actually be `learner_rpart_classif_rpart.R`.
...
1. Add the learner's parameters to the [parameter set](#param-set)
1. Optionally [change default values](#param-set) for parameters in `<param_vals>`
...
1. As we included "importance" in `properties` we have to add a function to the public method `importance()` which returns a named numeric vectors with the importance scores (values) for the different features (names).
...
* `predict_types`: Set of predict types the learner is able to handle.
  ...
  * `r ref("LearnerDens")`
    ...
    * `distr` - The probability distribution.
      Implemented in `r mlr3book::cran_pkg("distr6")`.
  * `r ref("LearnerClust")`
    * `partition` - The partition
    * `prob` - The probabilities for the various partitions
...
* `properties`: Set of properties of the learner. See `r ref("mlr_reflections", text = "mlr_reflections$learner_properties")` for the full list of feature types supported by `r mlr3book::mlr_pkg("mlr3")`. Possible properties include:
  ...
  * `"selected_features"`: The learner supports extracting the features which were used.
  ...
  * `"oob_error"`: The learner supports extracting the out of bag error.
    If this property is set, you must also implement a public method `oob_error()` to retrieve the out of bag error from the model.
  * `"loglik"`: The learner supports extracting the log-likelihood of the learner.
    If this property is set, you must also implement a public method `loglik()` to retrieve the log-likelihood from the trained model.
  * `"hotstart_forward"`: The learner allows to continue training the model (e.g. by adding more trees to a random forest).
    If this property is set, you must also implement a private method `.hotstart()`.
...
The `param_set` is the set of hyperparameters used in model training and predicting, this is given as a `r ref("paradox::ParamSet")`.
...
There is one special tags `"threads"` that should be used to tag the parameter that allows to determine the number of threads of the learner's internal parallelization and can be set using `r ref("set_threads")`
...
You can see we have done this for `"classif.rpart"` where the default for `xval` is changed to `0`. Note that the default in the `r ref("ParamSet")` is recorded as our changed default (0), and not the original (10).
...
To get the hyperparameters, we call `self$param_set$get_values()` and query all parameters that are using during `"train"`. Then, the dataset is extracted from the `r ref("Task")`.
...
As mentioned above, some methods only have to be implemented, when the learner has a certain property.
The blueprint for these methods is only included in the template, if the property is set when calling `create_learner()`.
The comments in the templates will include references to other learners that have this property and can be used as a reference.

The following methods are possible:

* `importance` - Must return a named numeric vector, where the names are the features and the values the corresponding importance score.
* `selected_features` - Must return a character vector containing the features that were selected during training.
* `loglik` - Must return the log-likelihood of the model.
* `oob_error` - Must return the out of bag error of the model.
* `.hotstart` - Depending on whether the learner has the property `"hotstart_forward"` or `"hotstart_backward"`, this should undo some of the training in the model or continue training the already trained model with respect to a specific parameter, for example the number of trees or gradient steps.
  All other parameters are assumed to be constant.
  See `r ref("HotstartStack")` for more information.
...
- If the control parameters need to be passed via a separate argument, one can e.g. use
  `formalArgs(glmnet::glmnet.control)` to get the names of the control parameters and then extract them from the `pars`.
...
Therefore we have written a "Parameter Check" that runs regularly for every learner. This "Parameter Check" compares the parameters of the `r mlr3book::mlr_pkg("mlr3")` ParamSet against all arguments available in the upstream function that is called during `$train()` and `$predict()`. Again the file is automatically created and opened by `r ref("create_learner()")`, this will be named like `test_paramtest_package_type_key.R`, so in our example `test_paramtest_rpart_classif_rpart.R`.
...
```r
#' @section Custom`r mlr3book::mlr_pkg("mlr3")`defaults:
#' - `<parameter>`:
#'   - Actual default: <value>
#'   - Adjusted default: <value>
#'   - Reason for change: <text>
