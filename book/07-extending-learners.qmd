...
3. [Run](#create-learner) `"mlr3extralearners::create_learner()")`.
...
* Open a new R session, set your working directory as your newly cloned repository, run `"devtools::load_all")`, and then [run](#create-learner) `r ref("mlr3extralearners::create_learner()")`, leaving `pkg = "."`.
* In your newly cloned repository, open the R project, which will automatically set your working directory, run `"devtools::load_all")`, and then [run](#create-learner) `r ref("mlr3extralearners::create_learner()")`, leaving `pkg = "."`.
...
The first script to complete after running `r ref("create_learner()")` is the file with the form `learner_package_type_key.R`, in our case this will actually be `learner_rpart_classif_rpart.key`. **This name must not be changed** as triggering automated tests rely on a strict naming scheme.
