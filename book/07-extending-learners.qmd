...
2. [Fork, clone and load](#setup) `r mlr3book::mlr_pkg("mlr3extralearners")`.
...
In order to use the `r ref("mlr3extralearners::create_learner")` function you must have a local copy of the `r mlr3book::mlr_pkg("mlr3extralearners")` repository and must specify the correct path to the package. To do so, follow these steps:
...
* `r ref("devtools::load_all()")` - Loads all functions from `r mlr3book::mlr_pkg("mlr3extralearners")` locally, including hidden helper functions.
...
1. `pkg = "."` - Set the package root to the current directory (assumes `r mlr3book::mlr_pkg("mlr3extralearners")` already set as the working directory)
...
* `predict_types`: Set of predict types the learner is able to handle.
  These differ depending on the type of the learner. See `r ref("mlr_reflections", text = "mlr_reflections$learner_predict_types")` for the full list of feature types supported by `r mlr3book::mlr_pkg("mlr3")`.
...
Within `r mlr3book::mlr_pkg("mlr3")` packages we suggest to stick to the lengthly definition for consistency, however the `<param_set>` can be written shorter, using the `r ref("paradox::ps")` shortcut:
