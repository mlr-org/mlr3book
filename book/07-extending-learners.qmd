...
1. Check [unit tests](#learner-test) and [parameter tests](#learner-test) pass (these are automatically created).
...
1. `properties = c("importance", "missings", "multiclass", "selected_features", "twoclass", "weights")` - Sets the properties the learner supports.
...
The `newdata` line ensures the ordering is the same by calling the saved order set in `$.train`, don't delete either of these lines!
:::
...
For more information see `r ref("HotstartStack")`. A learner that supports hotstarting are the Xgboost learners.
