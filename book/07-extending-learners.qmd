...
Note that some of the steps shown are only applicable in case you plan on creating a pull request to the mlr-org.
...
6. If the learner has the property
   ...
   More information on those methods can be found [here](#other-methods).

7. Fill out the missing parts in the learner's description.
...
1. `predict_types = c("response", "prob"),` - Sets the possible prediction types as response (pointwise prediction) and prob (probabilities). See [meta information](#learner-meta-information).
...
:::{.callout-warning}
You cannot rely on the column order of the data returned by `task$data()` as the order of columns may be different from the order of the columns during `$.train`. The `newdata` line ensures the ordering is the same by calling the saved order set in `$.train`, don't delete either of these lines!
:::
## Other methods {#other-methods}

As mentioned above, some methods only have to be implemented when the learner the property.
The blueprint for these methods is only included in the generate learner template, if the property is set when calling `create_learner()`.
...
* `importance` - Must return a named numeric vector, where the names are the features and the values the corresponding sorted importance score.
