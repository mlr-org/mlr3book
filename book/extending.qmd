...
1. As we included "importance" in `properties`, we have to add the public method `importance()` which returns a named numeric vectors with the sorted importance scores (values) for the different features (names).
