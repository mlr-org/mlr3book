...
1. [Install](#setup) `r mlr_pkg("mlr3extralearners")` and if you want to create a PR, also [fork and clone](#setup) it.
...
* `predict_types`: Set of predict types the learner supports.
  These differ depending on the type of the learner. See `r ref("mlr_reflections", text = "mlr_reflections$learner_predict_types")` for the full list of predict types supported by `r mlr_pkg("mlr3")`.
  ...
  * `r ref("LearnerDens")`
    ...
    * `distr` - Predicts a distribution as implemented in `r cran_pkg("distr6")`.
  * `r ref("LearnerClust")`
...
* `properties`: Set of properties of the learner. See `r ref("mlr_reflections", text = "mlr_reflections$learner_properties")` for the full list of learner properties supported by `r mlr_pkg("mlr3")`.
...
Within `r mlr_pkg("mlr3")` packages we suggest to stick to the shorthand notation above for consistency, however the `param_set` can be written with the underlying R6 classes as shown here
...
This "Parameter Check" compares the parameters of the `r mlr_pkg("mlr3")` ParamSet against all arguments available in the upstream function that is called during `$train()` and `$predict()`.
...
This will likely be required for all learners as common arguments like `x`, `target` or `data` are handled by the `r mlr_pkg("mlr3")` interface and are therefore not included within the ParamSet.
...
A good starting point is writing down the loss function independently of `r mlr_pkg("mlr3")` (we also did this in the `r mlr_pkg("mlr3measures")` package).
...
Before you start with the implementation make yourself familiar with the main R6-Objects in `r mlr_pkg("bbotk")` (Black-Box Optimization Toolkit).
...
b) If the new custom tuner is only usable for hyperparameter tuning, for example because it needs to access the task, learner or resampling objects it should be directly implemented in `r mlr_pkg("mlr3tuning")` as a `r ref("Tuner")`.
