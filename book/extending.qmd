...
1. Picking the right `r mlr3` class to inherit from.
...
We start with showing the code and will then explain what it does.
...
You can see we have done this for `regr.rpart2` where the default for `xval` is changed to `0`.
...
Because the `regr.rpart2` learner we are implementing has only one available predict type (`"response"`) we do not have to account for it.
...
test_that("autotest", {
  ...
  result = run_autotest(LearnerRegrRpart2$new())
