...
# Implementing New Measures and Learners {#sec-extending}
...
This chapter assumes good knowledge of `R6`, see @sec-r6 for a brief introduction and references to further resources.
...
Many learners are already included in the `r mlr3` ecosystem but there are still many more that have not been and so in this section we will look at how to add a new `r ref("Learner")` to `r mlr3`.
Please read `r link("", "extending vignette (TODO)")` if you want to add your new learner to `r mlr3extralearners`.

:::{.callout-tip}
The `r ref("mlr3extralearner::create_learner")` function can be used to generate files containing templates for the learner and test files required to create and test a `Learner`.
...
As before, we will first demonstrate what the final class looks like and then we will explain it line by line (and method by method).
As a working example we will implement a stripped down version of `regr.rpart`.

```{r}
#| cache: false
LearnerRegrRpartSimple = R6::R6Class("LearnerRegrRpartSimple",
  inherit = LearnerRegr,
  public = list(
    initialize = function() {
      param_set = ps(
  ...
  private = list(
    ...
    .predict = function(task) {
      ...
      newdata = task$data(cols = intersect(names(self$state$data_prototype), task$feature_names))
...
However, there are some aspects of parameters that we have not or only briefly covered so far.
These are:
...
Parameters with certain tags, can be retrieved by calling the `$get_values()` method of a `r ref("ParamSet")` with the `tag` argument.
We will make use of this later when implementing the train and predict methods.
...
* The tag `"threads"` should be used (if applicable) to tag the parameter that determines the number of threads used for the learner's internal parallelization.
  Once a learner is constructed, this parameter can be set using `r ref("mlr3::set_threads()")`.
...
In such cases, a practical compromise is to not set any default value when calling `r ref("paradox::p_int()")`.
...
In `r ref_pkg("paradox")` only integers, numerics, factors, and logicals have decicated parameter types.
For all other parameters, the `r ref("ParamUty")` class has to be used, which is constructable via the `r ref("paradox::p_uty()")` function.
...
The train function takes a `r ref("Task")` as input and must return a model, i.e. some `R` object that allows the learner to make predictions.
...
Next, we transition from `data` to a `r ref("Task")` again and construct a list with the return type requested by the user.
This is stored in the `$predict_type` slot of a learner class.
Note that the `r ref("Task")` is automatically passed to the prediction object, so all you need to do is return the predictions!
Make sure the list names are identical to the task predict types.
Because the `regr.rpart_simple` learner we are implementing has only one available predict type (`"response"`) we do not have to account for it.
...
If it is one of the standardized extractors, some custom code might have to be written for the return value to comply with the mlr3 standards (see `r ref("Learner")`).
The `$importance()` method for example must return the importance scores as a sorted numeric vector, the names being the features.
...
LearnerRegrRpartSimple$set("public", "importance", importance)
```
...
LearnerRegrRpartSimple$set("public", "selected_features", selected_features)
```

### Testing the learner {#sec-learner-test}

Once your learner is created, you should write tests to verify its correctness.
Besides manual tests, the `r mlr3` package also provides automatic tests to check that a learner satisfies some basic sanity checks and that it implements all the available parameters of the upstream function: (TODO: Add this when merged)
...
See section *run_autotest* of (TODO: mlr_test_helpers man page) for an explanation.
Because these functions are not in the mlr3 namespace, we have to source them from the `./testthat` subfolder of the installed `r mlr3` pacakge.
...
:::{.callout-tip}
When implementing survival learners, you must also source these help files from the `r mlr3proba` package.
:::
...
test_that("autotest", {
  ...
  result = run_autotest(LearnerRegrRpartSimple$new())
...
See section *run_paramtest* of (TODO: mlr_test_helpers man page) for an explanation.
