...
![A typical machine learning workflow](images/ml_abstraction.svg)
...
:::{.callout-note}
In many cases, the sketched workflow is not sufficient to deal with real-world data, which may require normalization, imputation of missing values, or feature selection.
...
Before we get into the details of how to use `r mlr_pkg("mlr3")` for machine learning, we give a brief introduction to R6 and `data.table` since you need at least basic understanding to work efficiently with our packages.