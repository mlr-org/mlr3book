...
```{r 02-basics-001, echo = FALSE, message = FALSE}
library("data.table")
library("mlr3")
library("mlr3book")
...
```{r 02-basics-002, echo=FALSE, fig.align="center"}
...
```{r 02-basics-003.table-001.table-002}
library("data.table")
...
```{r 02-basics-004.table-002.table-003}
df[df$y == "a", ]
dt[y == "a", ]
```
...
```{r 02-basics-005.table-003.table-004}
dt[, mean(x), by = "y"]
```
...
```{r 02-basics-008, message=FALSE}
library("mlr3viz")
autoplot(task_mtcars, type = "pairs")
```

:::{.callout-tip}
Instead of loading multiple extension packages individually, it is often more convenient to load the `r mlr_pkg("mlr3verse")` package instead.
`mlr3verse` imports the Namespace of most `r mlr_pkg("mlr3")` packages and re-exports functions which are used for common machine learning and data science tasks.
:::
...
As getting a task from a dictionary is a very common problem, `r mlr_pkg("mlr3")` provides the shortcut function `r ref("tsk()")`.
Here, we retrieve the `r ref("mlr_tasks_penguins", text = "palmer penguins classification task")`, which is provided by the imported package `r cran_pkg("palmerpenguins")`:
...
To get more information about a particular task, it is easiest to use the `help()` method that all `r mlr_pkg("mlr3")`-objects come with:
```{r 02-basics-012, eval = FALSE}
task_penguins$help()
```
Alternatively, the corresponding man page can be found under `mlr_tasks_[id]`, e.g. `r ref("mlr_tasks_penguins")`:
```{r 02-basics-013, eval = FALSE}
help("mlr_tasks_penguins")
```


:::{.callout-tip}
Thousands more data sets are readily available via [Openml.org](https://openml.org) ([@openml2013]) and `r mlr_pkg("mlr3oml")`.
...
```{r 02-basics-030, warning = FALSE, message = FALSE}
library("mlr3viz")
...
```{r 02-basics-031, warning = FALSE, message = FALSE}
library("mlr3viz")
...
```{r 02-basics-032, echo=FALSE, fig.align="center"}
...
```{r 02-basics-033, R.options=list(max.print = 3)}
...
```{r 02-basics-058, message = FALSE, warning = FALSE}
task = tsk("penguins")
