...
```{r 06-technical-paradox-006, R.options=list(width = 120)}
as.data.table(parA)
```
...
```{r 06-technical-paradox-010, R.options=list(width = 120)}
as.data.table(ps)
```
...
```{r 06-technical-paradox-013, error = TRUE}
ps$values$B = 100
```
...
```{r 06-technical-paradox-023, echo = FALSE, out.width="45%", fig.show = "hold", fig.width = 4, fig.height = 4}
#| layout: [[40, 40]]
par(mar=c(4, 4, 2, 1))
