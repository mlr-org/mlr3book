# Parameters (using `r mlr3book::mlr_pkg("paradox")`) {#paradox}

```{r 06-technical-paradox-001, include = F}
library(mlr3)
library(mlr3book)
```

The `r mlr3book::mlr_pkg("paradox")` package offers a language for the description of *parameter spaces*, as well as tools for useful operations on these parameter spaces.
...
The tools provided by `r mlr3book::mlr_pkg("paradox")` therefore relate to:
...
`r mlr3book::mlr_pkg("paradox")` is, by nature, an auxiliary package that derives its usefulness from other packages that make use of it.
It is heavily utilized in other [mlr-org](https://github.com/mlr-org) packages such as `r mlr3book::mlr_pkg("mlr3")`, `r mlr3book::mlr_pkg("mlr3pipelines")`, and `r mlr3book::mlr_pkg("mlr3tuning")`.

## Reference Based Objects

`r mlr3book::mlr_pkg("paradox")` is the spiritual successor to the `r mlr3book::cran_pkg("ParamHelpers")` package and was written from scratch using the `r mlr3book::cran_pkg("R6")` class system.
The most important consequence of this is that all objects created in `r mlr3book::mlr_pkg("paradox")` are "reference-based", unlike most other objects in R.
...
```{r 06-technical-paradox-003}
ps$add(ParamLgl$new("a"))
```

```{r 06-technical-paradox-004}
...
Consider, for example, trying to configure the `r mlr3book::cran_pkg("rpart")` package's `rpart.control` object.
...
```{r 06-technical-paradox-007, R.options=list(width = 120)}
as.data.table(parA)
```
...
```{r 06-technical-paradox-011, R.options=list(width = 120)}
as.data.table(ps)
```
...
When trying to set parameter values, e.g. for `r mlr3book::mlr_pkg("mlr3")` `r ref("Learner")`s, it is the `$values` slot of its `$param_set` that needs to be used.

```{r 06-technical-paradox-013}
...
```{r 06-technical-paradox-014, error = TRUE}
ps$values$B = 100
```
...
```{r 06-technical-paradox-017}
ps$deps
```

### Vector Parameters

Unlike in the old `r mlr3book::mlr_pkg("ParamHelpers")` package, there are no more vectorial parameters in `r mlr3book::mlr_pkg("paradox")`.
...
```{r 06-technical-paradox-018}
ps2d = ParamDbl$new("x", lower = 0, upper = 1)$rep(2)
print(ps2d)
```

```{r 06-technical-paradox-019}
ps$add(ps2d)
print(ps)
```
...
`r mlr3book::mlr_pkg("paradox")` offers a variety of functions that allow creating evenly-spaced parameter values in a "grid" design as well as random sampling.
...
```{r 06-technical-paradox-021}
design = generate_design_grid(psSmall, 2)
print(design)
```

```{r 06-technical-paradox-022}
generate_design_grid(psSmall, param_resolutions = c(B = 1, C = 2))
```

### Random Sampling

`r mlr3book::mlr_pkg("paradox")` offers different methods for random sampling, which vary in the degree to which they can be configured.
The easiest way to get a uniformly random sample of parameters is `r ref("generate_design_random()")`.
It is also possible to create "[latin hypercube](https://en.wikipedia.org/wiki/Latin_hypercube_sampling)" sampled parameter values using `r ref("generate_design_lhs()")`, which utilizes the `r mlr3book::cran_pkg("lhs")` package.
LHS-sampling creates low-discrepancy sampled values that cover the parameter space more evenly than purely random values.

```{r 06-technical-paradox-023}
pvrand = generate_design_random(ps2d, 500)
pvlhs = generate_design_lhs(ps2d, 500)
```

```{r 06-technical-paradox-024, echo = FALSE, out.width="45%", fig.show = "hold", fig.width = 4, fig.height = 4}
#| layout: [[40, 40]]
par(mar=c(4, 4, 2, 1))
...
`r mlr3book::mlr_pkg("paradox")` uses the `r ref("Sampler")` abstract base class for sampling, which has many different sub-classes that can be parameterized and combined to control the sampling process.
...
Every `r ref("Sampler")` object has a `r ref("sample()")` function, which takes one argument, the number of instances to sample, and returns a [`r ref("Design")`](#parameter-designs) object.
