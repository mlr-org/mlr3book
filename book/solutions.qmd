...
#| echo: false
library(mlr3oml)
collection = readRDS(here::here("book", "openml", "collection_297.rds"))
tbl = readRDS(here::here("book", "openml", "tbl_297.rds"))
```

We access the OpenML collection with ID 297 using the `r ref("mlr3oml::ocl()")` function.

```{r solutions-022}
#| eval: false
library(mlr3oml)
collection = ocl(id = 297)
```

We can then find all tasks with less than 10000 observations using `r ref("mlr3oml::list_oml_tasks()")`.
...
This returns a table with the relevant IDs.

```{r solutions-025}
task_ids = tbl$task_id
task_ids
```


We can create mlr3 tasks using the `"oml"` task and resampling.

```{r solutions-026}
tasks = lapply(task_ids, function(id) tsk("oml", task_id = id))
resamping = rsmp("holdout")
```

We continue with defining the learners and robustifying them.
We also set

```{r solutions-027}
tasks = lapply(task_ids, function(id) tsk("oml", task_id = id))
resamplings = lapply(task_ids, function(id) rsmp("oml", task_id = id))

learner_ranger = as_learner(
  ppl("robustify", learner = lrn("regr.ranger")) %>>% po("learner", lrn("regr.ranger"))
)
learner_ranger$id = "ranger"

learner_rpart = as_learner(
  ppl("robustify", learner = lrn("regr.rpart")) %>>% po("learner", lrn("regr.rpart"))
)
...
Finally, we create the experiment design using `r ref("benchmark_grid")`.
...
Then, we change the cluster function to "Multicore".
...
After they are finished, we can collect the results.
...
As a first step, we load `r ref_pkg("mlr3benchmark")` and create a benchmark aggregate.
...
# 1. Create an experiment design that compares the random forest in `r ref_pkg("ranger")` with the regression tree from `r ref_pkg("rpart")`.

```{r solutions-034}
# TODO Cache the tsk table and the collection.
```
