...
We then evaluate the given initial design on the instance and optimize it with the first BO algorithm using a Gaussian Proccess as surrogate model:
...
ggplot(aes(x = nr_eval, y = y_min, colour = surrogate), data = all_data) +
  ...
  labs(y = "Best Observed Function Value", x = "Number of Function Evaluations", colour = "Surrogate Model") +
  theme_minimal() +
...
Using the surrogate, acquisition function and acquisition function optimizer that are provided, we can procced to optimize the instance via ParEGO:
...
Although the points are correctly evaluated in batches of size `q = 4` (note that the first batch is given by the initial design that is constructed automatically), we do not gain much without explicit parallelization of the function evaluation.
...
We then parallelize the evaluation of the objective function and proceed to optimize the instance again:
