...
2. After training, the underlying `r ref("LearnerClassifLogReg", "lrn(\"classif.log_reg\")")` can be accessed through the `$base_learner()` method.
  A second way to access the learner is to use access the `r ref("PipeOpLearner", "po(\"learner\")")` explicitly.
```{r pipelines-002}
...
3. Set the `$keep_results` flag of the Graph to `TRUE` to keep the results of the individual PipeOps.
  You can then get the input of the `r ref("LearnerClassifLogReg", "lrn(\"classif.log_reg\")")` by accessing the `$.result` field of its predecessor, the `r ref("PipeOpScale", "po(\"scale\")")`.
...
4. To use `r ref("PipeOpSelect", "po(\"select\")")` to *remove*, instead of *keep*, a feature based on a pattern, use `r ref("selector_invert")` together with `r ref("selector_grep")`.
  So to remove the "`R`" class columns in @sec-pipelines-stack, one could use the following `r ref("PipeOpSelect", "po(\"select\")")`:
```{r pipelines-004}
...
6. As the first hint states, it is necessary to use two `r ref("PipeOpImputeLearner", "po(\"imputelearner\")")` objects: one to impute missing values in factor columns using a classification learner, and one to impute missing values in numeric colums using a regression learner.
  We also use `r ref("pipeline_robustify", "ppl(\"robustify\")")` together with the `r ref("ranger::ranger")`-based learners inside `r ref("PipeOpImputeLearner", "po(\"imputelearner\")")` because the data that is passed to the imputation learners still has missing values, which `r ref("ranger::ranger")` cannot handle.
```{r pipelines-006}
