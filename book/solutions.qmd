...
Set the seed, load the `tsk("sonar")` task, then the classification tree with `predict_type = "prob"` (needed for exercise 3), and the required measure.
...
1. Apply the "bootstrap" resampling strategy on the `tsk("mtcars")` task and evaluate the performance of the `lrn("classif.rpart")` decision tree learner.
...
2. Use the `tsk("spam")` task and 5-fold CV to benchmark Random Forest (`classif.ranger`), Logistic Regression (`classif.log_reg`), and XGBoost (`classif.xgboost`) with regards to AUC.
...
3. A colleague claims to have achieved a 93.1% classification accuracy using the `lrn("classif.rpart")` learner on the `tsk("penguins_simple")` task.
...
1. Tune the `mtry`, `sample.fraction`, ` num.trees` hyperparameters of a random forest model (`regr.ranger`) on the `tsk("mtcars")` task.
...
1. Tune and benchmark an XGBoost model against a logistic regression model on the `tsk("spam")` task and determine which has the best Brier score.
...
1. Calculate a correlation filter on the `tsk("mtcars")` task.
...
2. Use the filter from the first exercise to select the five best features in the `tsk("mtcars")` dataset.
...
1. Apply a backward selection to the `tsk("penguins")` task with a classification tree learner `"classif.rpart"` and holdout resampling by the measure classification accuracy. Compare the results with those in @sec-fs-wrapper-example.
...
4. Automate the feature selection as in @sec-autofselect with the `tsk("spam")` task and a logistic regression learner (`lrn("classif.log_reg")`). Hint: Remember to call `library("mlr3learners")` for the logistic regression learner.
...
```
which would have the benefit that it would keep the columns pertaining to all other classes, even if the `tsk("sonar")` task had more target classes.

1. A solution that does not need to specify the target classes at all is to use a custom `r ref("Selector")`, as was shown in @sec-pipelines-bagging:
```{r pipelines-005}
...
3. Improve your model by employing pipelines that use pre- or post-processing methods for fairness.
...
4. Add `"race"` as a second sensitive attribute to your dataset.
