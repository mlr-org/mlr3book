...
```{r 08-special-survival-001, include = F}
library(mlr3)
library(mlr3book)
library(mlr3proba)
...
The package `r mlr3book::mlr_pkg("mlr3proba")` [@mlr3proba] extends `r mlr3book::mlr_pkg("mlr3")` with the following objects for survival analysis:

* `"mlr3proba::TaskSurv", text = "TaskSurv")` to define (censored) survival tasks
...
`survival::Surv` object and has three to four target arguments (dependent on censoring type).
A `TaskSurv` can be constructed with the function `"as_task_surv()"`:

```{r 08-special-survival-002, eval=FALSE}
library("mlr3")
library("mlr3proba")
library("survival")
...
* `distr` - Predicted survival distribution, either discrete or continuous. Implemented in `r mlr3book::cran_pkg("distr6")`.
...
The example below uses the `"mlr_tasks_rats", text = "rats")` task shipped with `r mlr3book::mlr_pkg("mlr3proba")`.

```{r 08-special-survival-003}
task = tsk("rats")
...
Finally we take a look at the `r ref("PipeOp")`s implemented in `r mlr3book::mlr_pkg("mlr3proba")`, which are used for composition of predict types.
...
See `r mlr3book::mlr_pkg("mlr3pipelines")` for full tutorials and details on `r ref("PipeOp")`s.


```{r 08-special-survival-004, eval = FALSE}
...
Finally, we conduct a small benchmark study on the `"mlr_tasks_rats", text = "rats")` task using some of the integrated survival learners:

```{r 08-special-survival-005}
library("mlr3learners")
