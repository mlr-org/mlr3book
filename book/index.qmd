...
We chose the 'penguins' "task" (the `r mlr3` term for a dataset), randomly split the task into 67% training data and 33% testing data (this is the default when using the `partition()` function), trained a tree model on the training data to predict the type of penguin given its features, showed the fitted model, and finally made prediction for the test data, showed these predictions, and computed the accuracy of the model on the test set.

`r mlr3` makes training and predicting easy, but it also allows to perform some very complex operations in just a few lines of code:
