...
Any section that provides complex technical information is marked with this symbol in the margin.
::: {.column-margin}
![complex section](complex.png)
:::
Any section that is optional, such as deep dives into machine learning theory, is marked with this symbol in the margin.
::: {.column-margin}
![optional section](optional.png)
:::

Chapters 1-3 cover the basics of the book. They will take you through loading the most important objects, training models and using them for new predictions, and evaluating predictions. In addition we discuss model comparison and resampling. These chapters are essential to understanding the core infrastrucure in mlr3.
...
Finally, the other important style point to note is the usage of S3 and R6 throughout this book. In the introduction we will discuss R6 in more detail and provide a short overview to our S3 'sugar' functions. For this preamble, all you need to know is that throughout the book we will refer to simple S3 functions that provide the user with less flexibility and then at the end of each chapter we will provide a mini-API that links S3 functions to backend R6 classes.
...
5. We do not include required function parameter names in code chunks, only optional ones. But we always include the full function signature the first time it is introduced. For example:
...
We highly recommend to take a look at our `r ref_pkg("cheatsheets")` while diving deeper into `r mlr3`.
::: -->
