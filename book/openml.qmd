...
abstract:
  ...
  The first part of this chapter teaches the OpenML basics and shows how it can be used via the interface package mlr3oml that conveniently integrates mlr3 and OpenML through its REST API.
  We will then show how the execution of benchmark experiments on HPC clusters can be simplified using the R package [batchtools](https://github.com/mllg/batchtools), which provides a convenience layer for interoperating with different scheduling systems in a standardized way.
---

# OpenML and Benchmarking {#sec-openml-benchmarking}
...
TODO: Reorder this, first talk about the experiment structure and then batchtools. 
Afterwards talk about the problem of how to get the tasks / reproducible resamplings.

`r ref_pkg("mlr3")` is a versatile machine learning framework that caters to the needs of both applied data scientists seeking to solve real-world problems, as well as researchers focused on benchmarking.
...
The main focus in this chapter is the empirical evaluation of machine learning methods.

A very common benchmarking scenario is to compare a set of algorithms $A_1, ..., A_n$ by evaluating their performance on a set of problems $P_1, ..., P_k$, using evaluation measure $M$.
Oftentimes, the cross-product between the algorithms and experiments define the experiment design, and we will restrict ourselves to this scenario for simplicity.
...
This is depicted in the table below, where each row corresponds to one experiment.
...
One experiment then consists of conducting a `r ref("Resampling")` using the tuple $(A_i, T_j, R_j)$ and evaluating the result using measure $M$, which is a `r ref("Measure")`.
The result of such an experiment can be depicted in tabular format as shown below.
...
The driving question to such an experiment is often how a algorithm $A_i$ compares with other algorithms $A_j$ where $i \neq j$.
To answer these questions, statistical methods such as the Friedman test by @friedman1940comparison can be employed, which are available via the `r ref_pkg("mlr3benchmark")` extension.
Note that in many cases, the algorithms $A_i$ or the problems $P_j$ might also be parameterized, which we will revisit later.

What we have not focused on so far in the book, is 

2. how to execute large-scale benchmark experiments on High-performance computing (HPC) clusters.
1. how to easily find relevant tasks $T_i$ and

For the first issue, we will show how the R package `r ref_pkg("batchtools")` and its mlr3 integration `r ref_pkg("mlr3batchmark")` can greatly simplify the efforts required to operate a scheduling system of a HPC cluster.
...
As a brief primer, we show how one could compare a classification tree with a random forest using the pima and iris dataset when using the `r ref_pkg("mlr3batchmark")` package.
We only need to replace the typical `benchmark()`-call with `batchmark()` and afterwards submit the jobs.
...
reduceBenchmarkResult()
```

**OpenML**

In order to be able to derive meaningful conclusions from benchmark experiments, a good choice of the tasks $T_i$ is tantamount. 
...
* [**OML Dataset**](https://www.openml.org/search?type=data\&status=active): A (usually tabular) data with additional meta-data. 
...
<!-- ## Flow and Run {#sec-openml-flow-run} -->
<!---->
