...
<!-- As a brief primer, we show how the previous experiment design could be executed on a HPC cluster using the `r ref_pkg("mlr3batchmark")` package. -->
<!-- We essentially only need to replace the typical `benchmark()`-call with `batchmark()` and afterwards submit the jobs to the scheduler using the function `submitJobs()`. -->
<!-- Once the results are computed we can obtain the benchmark result using `reduceBenchmarkResult()`. -->
<!---->
<!-- ```{r} -->
<!-- #| eval: false -->
...
We ended the last section, with a reasonably large benchmark experiment that will now serve as a starting point
As a starting point, we show how the large design from before, can be executed on a HPC using just a few lines of code.
The first step is to create an experiment registry, using the function `makeExperimentRegistry()` 
...
To understand what these functions do, it is essential to first start covering 

1. Start with the mlr3 batchmark example that we have shown above and explain what is going on.

In the introduction of this section, we have already seen a brief preview of how the `r ref_pkg("batchtools")` connector `r ref_pkg("mlr3batchmark")` allows to execute experiments on HPC cluster.
We will start with revisiting this preview and explaining what actually went on under the hood.
We will continue working with the example where we left of in the previous section, where we wanted to apply the random forest algorithm to all tasks of the AutoML Classification Suite.
...
### The HPC Architecture {#sec-hpc-architecture}
