...
**General Formalization of a Benchmark Experiment**

Even though we have have already covered the concept of benchmarking in @sec-benchmarking, we will now introduce a more general formularization of the problem.
The reason for this is that most of the tools shown here can also be applied in other fields besides machine learning.
...
For example, the algorithms $A_i$ could be black-box optimisers, the problems $P_j$ functions to be minimised and the associated performance metric $M$ the absolute difference to the global optimum.
We are then interested in comparing the performance of algorithms $A_i$ with respect to measure $M$ on the problems $P_j$.

The whole benchmark experiment consists of many individual experiments, each defined by a tuple $(A_i, P_j)$.
Running it consists of running each of these experiments and evaluating the results using the measure $M$.
This is shown in the table below, where each row corresponds to an experiment.
Because all of the experiments are independent, we have an embarrassingly parallel problem that is conceptually straight-forward to parallelize.
...
: Illustration of a generic benchmark experiment. {#tbl-generic-benchmark}
...
One experiment then consists of executing the experiment defined by the triple $(L_i, T_j, R_j)$ using `r ref("resample")` and scoring the result using metric $M$, which is a `r ref("mlr3::Measure")`.

It is important to note, that other categorizations are of course also possible.
...
Usually however, one is interested in the comparison of learners and table @tbl-generic-benchmark can be rewritten as shown below.
...
The table above should seem familiar as it is the result of calling the `$score()` method on a `r ref("BenchmarkResult")`.
...
If the experiment is large, parallelization via `r ref_pkg("future")` might not be enough and usage of an HPC cluster is required.
...
It is built around the concepts introduced earlier, namely the algorithm $A_i$, the problem $P_j$ and the experiment $E_{i, j} = (A_i, P_j)$.
...
The most important difference between such a cluster and a personal computer (PC), is that one has no direct control over the nodes, but has to submit computational jobs through a scheduling system like SLURM (Simple Linux Utility for Resource Management).
...
Multiple users -- in this case Ann and Bob -- can log in to a Head Node and submit jobs into the queue.
The workload manager than determines when these jobs are being executed.
Apart from submitting jobs, other relevant operations are killing and listing jobs.
...
As mentioned above, we are interested in the execution of embarrassingly parallel jobs.
Using the terminology from earlier, we have independent experiments $E_{i, j} = (A_i, P_j)$ (see @tbl-generic-benchmark) that we want to execute, potentially even repeatedly if the experiments are stochastic.
When we operate such a scheduling system, we can e.g. characterize one job as running an experiment $E_{i, j}$ once.

<!-- Why is this hard and why do we need tooling? -->

Operating such a scheduling system directly can be unnecessarily tedious, mostly because of the following two reasons.

1. Code that runs locally cannot be deployed on the cluster and
2. the code running the experiment depends on the scheduling system. 
...
<!-- A quick walkthrough with the earlier introduced OpenML example -->
...
<!-- Explain all the steps and the batchtools concepts in detail -->
...
2.  Add one parametrized algorithm.
...
TODO: Exercises (Compare resampling method and not algorithms)
