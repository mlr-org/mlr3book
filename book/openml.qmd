...
In this chapter, we will cover the most important aspects of OpenML and how to use it via the interface package `r ref_pkg("mlr3oml")`.
This will give you an idea of what you can do with the platform, as well as how to do it.
The entities that we will cover are the Dataset, Task, and Task Collection.

Note that we will only cover datasets, tasks, and task collections in this section.
...
For more information about e.g. the Flow or Run, which represent algorithms and experiment results, we refer to the OpenML website or the documentation of the `r ref_pkg("mlr3oml")` package.
...
The mlr3 counterpart of the task collection would be a simple list of `r ref("Task")`s.
...
We could now compare the performance of the random forest with the results from the AutoML benchmark.
...
We can know inspect the returned `data.table`, where we only show a subset of the columns for readability.
...
The last entity that we will cover in this chapter is the **task collection**, which allows to bundle tasks into benchmark suites. 
Once again, we will use the AutoML benchmark as an example.
