...
1. `x`: Object to convert. Works for `data.frame()`/`data.table()`/`tibble()` abstract data backends implemented in the class `r ref("DataBackendDataTable")`.
...
We have seen that during task creation target feature roles are assigned to columns.
Target reffers to the variable we want to predict and feature reffers to the predictors for the task.
