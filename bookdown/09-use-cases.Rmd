...
We use the `house_sales_prediction` dataset contained in this book in order to provide a use-case for the application of `r mlr_pkg("mlr3")` on real-world data.
...
In `r mlr_pkg("mlr3")`, we do not create `train` and `test` data sets, but instead keep only a vector of train and test indices.
...
Our use case is primarily concerned with representing the complete `r mlr_pkg("mlr3")` workbench.
...
The modeling process in `r mlr_pkg("mlr3")` can be split into five components:
...
## Tuning random forests for credit worthiness assessments {#use-case-german-credit2}

In one of our previous use cases (), we already worked with the `german_credit` dataset.
We used both, logistic regression and random forests to predict credit worthiness.

Now, we are interested in finding the __best__ (or at least a very good) model instead.
For simplicity, we limit ourselves to only random forests.

Finding a good model configuration can be done in `r mlr_pkg("mlr3")` with the `r mlr_pkg("mlr3tuning")` package.
It basically performs a smart hyperparameter tuning.

If you want to get to know the data set in greater detail, please refer to ().

In this previous use case we found out that a random forest did a good job explaining credit worthiness.
However, we did not systematically explore different specifictions of random forests but only benchmarked different models where we a priori fixed the parameters.

### Defining the task

The `german_credit` data set is a ready-to-use task in the `r mlr_pkg("mlr3")` package.

```{r 09-use-cases-3-001}
task = tsk("german_credit")
```

The task is a `r ref("TaskClassif")` where we aim to predict the following target:

```{r 09-use-cases-3-002}
task$target_names
```

The data of the task can be inspected this way:

```{r 09-use-cases-3-003}
task$data()
```

General information on the task can be obtained via:

```{r 09-use-cases-3-004}
print(task)
```

### Learner Definition

As previously outlined, we aim to focus on the random forest from the `r cran_pkg("ranger")` package.

```{r 09-use-cases-3-005}
library(mlr3learners)
learner = LearnerClassifRanger$new()
```

We can access the parameters of the learner via:

```{r 09-use-cases-3-006}
learner$param_set
```



Not all of these paramters ae eventually meaningful to be tuned.
However, many of them directly affect the learner's performance.
In the next section we will tune the learner only with a subset of these.

### Tuning

In `r mlr_pkg("mlr3")` we typically directly distinguish between hyperparameter tuning and feature selection.
We will neglect feature selection in this use case, as the random forest has a kind of "built-in" feature selection.
Hyperparameter tuning is occupied with choosing __good__ value combinations for all or some of the model paramters.
In the case of the random forest, for example, this could be the number of trees or the number of variables to possibly split at in each node.
However, also discrete choices such as the sampling strategy (`replace`) can be tuned.

To perform hyperparameter tuning in `r mlr_pkg("mlr3")` we need to use the `r mlr_pkg("mlr3tuning")` package and need to set up a `r ref("TuningInstance")`.
The instance needs __6__ different inputs:

- A `task`

- A `learner`

- A `resampling` (to determine the goodness of the different hyperparameter combinations)

- A (list of) `measures` (to evaluate the goodness)

- A `param_set` to be tuned

- A `terminator` that determines the budget spent to do the tuning

#### Resampling

A resampling strategy which is typically chose is cross-validation.
In our case we choose 5 folds.

```{r 09-use-cases-3-007}
cv_rsmp = rsmp("cv", folds = 5L)
```

#### Measures  

Usually, the misclassifaction rate serves as a good evaluation criterion.

```{r 09-use-cases-3-008}
measure = msr("classif.ce")
```

#### Parameter set

The paramter set `param_set` contains information on which paramters are to be tuned and what is the paramter space to be searched.

The `r mlr_pkg("paradox")` package supplies us with the appropiate syntax and commands to create a `param_set`.
We limit ourselves to `num.trees`, `mtry` and `replace`.
But feel free to add more meaningful parameters and change to upper / lower bounds of the searched space.

```{r 09-use-cases-3-009}
library(paradox)
tune_ps = ParamSet$new(list(
  ParamInt$new("num.trees", lower = 5L, upper = 1000L),
  ParamInt$new("mtry", lower = 1L, upper = 15L),
  ParamLgl$new("replace", default = TRUE)
))
tune_ps
```
...
The terminator sets a budget for the tuning.
In this example we choose `r ref("TerminatorClockTime")`, to make sure that the use case does not run forever.
`r ref("TerminatorClockTime")` allocates a fixed budget in terms of seconds.
In order to compile the book in a reasonable time (this is not the only use case), we only allow 100 seconds here.
Also feel free to increase this number.

```{r 09-use-cases-3-010}
eval_100_sec = term("clock_time", secs = 100)
```

### Setting up a Tuning instance

```{r 09-use-cases-3-011}
library(mlr3tuning)
...
instance = TuningInstance$new(
  ...
  measures = measure,
  param_set = tune_ps,
  terminator = eval_100_sec
)
print(instance)
```

### Setting up a `Tuner` Class

We set up a `Tuner` referring to a random search (see: `r ref("TunerRandomSearch")`).

```{r 09-use-cases-3-012}
...
We tune the learner by calling the `tune` method of the `R6` class `Tuner`.
The result is typically, `NULL`.
Don't be confused because of this.

```{r 09-use-cases-3-013}
set.seed(20200210)
result = tuner$tune(instance)
print(result)
```

One better accesses the results via the `archive` method.


```{r 09-use-cases-3-014}
instance$archive()
```

Most of the information in the archive is redundant, so we focus on the essential columns.

```{r 09-use-cases-3-015}
tuning_result =
  instance$archive(unnest = "params")[, c("num.trees", "mtry", "replace", "classif.ce")]
tuning_result[order(tuning_result$classif.ce), ]
```

At first glance, a rather medium value for `num.trees` and a large value for `mtry` seems to work best.
However, we want to more systematically evaluate the results.

```{r 09-use-cases-3-016}
ggplot(data = tuning_result, aes(x = num.trees, y = mtry)) +
  geom_point(aes(shape = replace, col = classif.ce)) + 
  scale_color_gradient2(midpoint = mean(c(max(tuning_result$classif.ce), min(tuning_result$classif.ce))),
                        low = "blue", mid = "white", high = "red", space = "Lab")
```

Using the graph one can validate that small values of the numeric hyper parameters tend to be associated with higher misclassifaction.
Only for medium and large values in both, `num.trees` and `mtry` low missclassification rates are observed.
We observe an accumulation of low missclassification rates for models with both, high `num.trees` and `mtry`.
A strcuture induced by the sampling strategy cannot be easily observed.

As an additional evaluation tool we can use linear regresion to predict the missclassification rate using the three features we varied as features.
Doing so, we regard the random search as an experiment and analyse the resulting variance (ANOVA).

```{r 09-use-cases-3-017}
tuning_result$replace = as.factor(ifelse(tuning_result$replace, 1, 0))
tuning_result$mtry = as.numeric(tuning_result$mtry)
tuning_result$num.trees = as.numeric(tuning_result$num.trees)
tuning_task = TaskRegr$new("tuning", tuning_result, target = "classif.ce")
tuning_learner = LearnerRegrLM$new()
tuning_learner$train(tuning_task)
summary(tuning_learner$model)
```

This model is in line with our visual observation:
A higher complexity of the model is associated with significantly lower misclassification error.

We can use the model predictions in order to visually present the results.

```{r 09-use-cases-3-018}
expanded_result = expand.grid(num.trees = seq(1, 1000, by = 3), 
                               mtry = seq(1, 15, by = 0.05), 
                               replace = c(1, 0))

expanded_result$replace = as.factor(expanded_result$replace)
expanded_predictions = tuning_learner$predict_newdata(expanded_result)
expanded_result$response = expanded_predictions$response

ggplot(expanded_result, aes(num.trees, mtry, fill= response)) + 
  geom_tile() 

```

One has to remember that this graphic itself is based on a (rather simplistic) model which can only explain `summary(tuning_learner$model)$adj.r.squared * 100` percent of the variable is the misclassification rate.
Additionally, we have now idea yet when we leave the parameter spaces defined earlier.
However, for the moment, we seem to fancy more complex models while the absolute difference in performance between complex and simple models seems rather low.
