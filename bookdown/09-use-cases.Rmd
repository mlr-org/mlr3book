...
For this purpose we can use a `r ref("mlr3featse::Filter")` implemented in `r mlr_pkg("mlr3filters")`.
...
The data features personal, demographic and bank account related data on individuals.
...
library(mlr3book)
data("german_credit", package = "mlr3book")
```
...
The variable we want to predict is `class` (either **good** or **bad**).
...
DataExplorer::plot_boxplot(german_credit, by = "class", nrow = 2, ncol = 3)
```
...
The modeling in `mlr3` can be split into five components:
...
Tasks are not only different words for problem.
They also reflect some technical properties with respect to `mlr3`.

In our case it is clearly our objective to model or predict the binary `factor` variable `class`.
Thus, we use `TaskClassif`.

With `TaskClassif$new()` we can initialize a classification task. 
We need to specify a name for the task (we choose `"GermanCredit"`), the data we want to use (`credit`) and name of the target (`"class"`):

```{r 09-use-cases-2-006, error=TRUE}
library(mlr3)
task = TaskClassif$new("GermanCredit", german_credit, "class")
```

We see that we need to make the target explicitely to a factor.

```{r 09-use-cases-2-007}

german_credit$class = as.factor(german_credit$class)
task = TaskClassif$new("GermanCredit", german_credit, "class")
...
However, you will immidiatley see that the procedure generalises to any learner very easily.
...
The remaining 20 percent are used later on for evaluation.
...
We use the fast implementation from the `ranger` package (@ranger).
...
Doing so we override the default and let the learner do feature importance determination based on permutation feature importance (e.g. see here: @permutation).
...
After taining the machine learning model can be used for prediction.
...
From these, class predictions are derived.
...
We display the precition results aggregated in a confusion matrix.
...
We should do [Performance Evaluation](#performance-evaluation), [Tuning](#tuning) and [benchmarks](#performance-comparison-and-benchmarks).
...
This is because the wrapped package requires this.
...
Note that the probability interpretation of the predictions is sometimes to be dealt with caution.
For a discussion on this topic, have a look at this article:
...
We use the `resample()` function to do the resampling calculation:
...
We can parametrise our learners in two different manners.
