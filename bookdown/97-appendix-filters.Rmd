...
These learners support filtering of features via feature importance and `r ref("FilterImportance")`:
...
The following learners return a set of used features and work with `r ref("FilterSelectedFeatures")`:

```{r 98-appendix-filters-003, echo = FALSE, message = FALSE}
requireNamespace("mlr3learners")
requireNamespace("mlr3proba")
d = suppressWarnings(data.table::as.data.table(mlr_learners))
d[mlr3misc::map_lgl(properties, is.element, el = "selected_features"), key]
```
