...
For this, several brackets are constructed with different sampled starting configurations and different starting budget in each.
Each bracket has a different amount of stages depending on the starting budget.
Once a stage of a bracket is evaluated, the best `1/eta` configurations are kept, while the rest is discarded.
The remaining configurations are then transfered to the next bracket stage, where training is continued with an increased budget by the factor of `eta`.
