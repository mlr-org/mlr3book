...
```{r 02-tuning-009}
learner$param_set$values = result$tune_result(instance)$values
learner$train(task)
```

The trained model could now be used to make a prediction on external data.
Note that predicting on observations present in the `task` is statistically should be avoided as the model has seen these already during tuning.
