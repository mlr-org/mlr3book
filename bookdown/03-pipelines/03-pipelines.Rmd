...
```{r 03-pipelines-001, echo=FALSE,fig.align='center'}
...
Other packages that provide, to varying degree, some preprocessing functionality or machine learning domain specific language, are the `r cran_pkg("caret")` package and the related `r cran_pkg("recipies")`  project, and the `r cran_pkg("dplyr")` package.

An example for a Pipeline that can be constructed using `r mlr_pkg("mlr3pipelines")` is depicted below:

```{r 03-pipelines-002, echo = FALSE, width = 10, height = 10, eval = TRUE}
