...
In summary, this sub-chapter explains how to account for these considerations and conduct survival analysis using the `r mlr_pkg("mlr3proba")` extension package.
...
See `mlr-org/mlr3ordinal` for the current state.
...
In this example we demonstrate the basic functionality of the package on the `survival::rats", text = "rats` data from the `survival` package.
This task ships as pre-defined `TaskSurv` with `r mlr_pkg("mlr3proba")`.
...
This is work in progress.
See `mlr-org/mlr3ordinal` for the current state of the implementation.
