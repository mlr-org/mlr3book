...
```{r 08-house-prices-003, message = FALSE}
library(lubridate)
...
```{r 08-house-prices-015, width = 10, height = 10}
plot(lrn$model)
text(lrn$model)
```
...
```{r 08-house-prices-016, message = FALSE}
...
```{r 08-house-prices-020, eval = FALSE}
set.seed(444L)
library(mlr3tuning)
library(paradox)
...
```{r 08-house-prices-021, echo = FALSE, results = 'hide', eval = FALSE}
# And resample the AutoTuner
res = resample(tsk_nozip$clone()$filter(train.idx), at, cv3)
```

```{r 08-house-prices-022, eval = FALSE}
res$performance("regr.mse")
