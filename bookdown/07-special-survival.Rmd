...
Finally we take a look at the `PipeOp`s implemented in `r mlr_pkg("mlr3proba")`, which are used for composition
of predict types. For example, a predict linear predictor does not have a lot of meaning by itself, but it can be composed into a survival distribution. See `r mlr_pkg("mlr3pipelines")` for full tutorials and details on `PipeOp`s.
