...
All this information is communicated with objects of the class `"ParamSet")`, which is defined in `r mlr3book::mlr_pkg("paradox")`.
...
For an in-depth description of `r mlr3book::mlr_pkg("paradox")` and its classes, see the [`paradox` chapter](#paradox).
...
See the [in-depth `paradox` chapter](#paradox) for more details on using `"ParamSet", text = "ParamSets")` to  define parameter ranges for use-cases besides tuning.
...
We can use the `r mlr3book::mlr_pkg("paradox")` function  `"generate_design_grid")` to look at the values that would be evaluated by grid search.
...
The `search_space()` picks up dependencies fromt the underlying `"ParamSet")` automatically.
