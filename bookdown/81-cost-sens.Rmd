...
We will work on the data from the `r ref("mlr_tasks_german_credit", text = "German Credit Data")`:
```{r}
data = mlr_tasks$get("german_credit")$data()
...
Here, we compare the logistic regression to a simple featureless learner:
```{r}
bmr = benchmark(expand_grid(task, c("classif.log_reg", "classif.featureless"), "cv"))
print(bmr)
```
...
Such learners calculate a probability $p$ for the positive class.
...
```{r}
# fit model with probability prediction
...
Instead of manually trying different threshold values, we here use `r ref("optimize()")` to find a good value.
```{r}
# we write a simple wrapper function which takes a single threshold and returns the resulting performance
