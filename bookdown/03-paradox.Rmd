...
The tools provided by `paradox` therefore relate to parameter checking---verifying that a set of parameters satisfies the conditions of a parameter space---and parameter sampling---generating parameter values that lie in the parameter space for systematic exploration of program behaviour depending on these parameters.

`paradox` is, by its nature, an auxiliary package that derives its usefulness from other packages that make use of it.
...
`paradox` is the spiritual successor to the `r cran_pkg("ParamHelpers")` package and was written from scratch using the `r cran_pkg("R6")` class system.
...
```
Every parameter must have an **`id`**---its name within the parameter set---, and may also have a default value (**`default`**), a list of values that are accepted even if they do not conform to the type (**`special_vals`**), and **`tags`** that can be used to organize parameters.
...
All values that are given to the constructor are then accessible from the object for inspection using `$`.
Although all these values can be changed for a parameter after construction, this may be a bad idea and should be avoided when possible; instead, a new parameter should usually be constructed.
...
Parameters can also be added to the constructed `ParamSet` using the **`$add()`** function. It is even possible to add whole `ParamSet`s to other `ParamSet`s.
```{r paradox-set}
...
Just as for `Param`s, and much more usefully, it is possible to get the `ParamSet` as a `data.table` using `as.data.table`.
...
The list does not need to be complete, however: It is possible to check only a subset of parameters.
...
The `Condition` object represents the check to be performed on the dependee; currently it can be created using **`CondEqual$new()`** and **`CondAnyOf$new()`**.
...
A `ParamSet` containing a `ParamUty` can be used for parameter checking, but not for [sampling](#parameter-sampling), however.
...
`paradox` uses the **`Sampler`** abstract base class for sampling, which has many different subclasses that can be parameterised and combined to control the sampling process.
...
These are initialised with a single `Param`, and can then be used to sample values.
...
While the different `Sapmler`s allow for a wide specification of parameter distributions, there are cases where the simplest way of getting a desired distribution is to sample parameters from a simple distribution (such as the uniform distribution) and then transform them.
...
The transformation is performed when calling the **`$transpose`** function of the `Design` object returned by a `Sampler` with the **`trafo`** param set to `TRUE` (the default).
...
The `ParamSet`s that can be used for random sampling, however, are restricted in some ways: They must have finite bounds, and they may not contain untyped (`ParamUty`) parameters.
...
Suppose, for example, that a certain method requires a *function* as a parameter, for example a function that summarises its data in a certain way.
...
Imagine now that a different kind of parameterisation of the function is desired: The user wants to give a function that selects a certain quantile, where the quantile is set by a parameter.
