...
1. Adding a new R source file in `R/mlr_learners_[classif/regr]_[your learner name].R` by first deciding on whether you want to implement a classificator or regression learner.
2. Copy the contents of `R/template_[classif/regr].R` into your new learner implementation.
3. Register your learner in `R/zzz.R` under either classification or regression learners.
4. Add a test for your learner: Copy the file `tests/autotest_template.R` into the folder `tests/testthat/` and name it `test_[classif/regr]_[your learner name].R`.
...
- Todo: Add the template files we are talking about.
