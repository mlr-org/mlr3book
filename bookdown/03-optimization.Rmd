...
A potential tuning process is illustrated in the graphic below: 

```{r 03-optimization-001, echo = FALSE}
knitr::include_graphics("images/svg-sources/tuning-rescaled.svg")
```
...
The sub-chapter on model tuning requires the package `mlr3-tuning`, an extension package which supports hyperparameter tuning.
...
```{r 03-optimization-002, echo = FALSE}
knitr::include_graphics("images/pdf-sources/tuning_process.pdf")
```
...
```{r 03-optimization-014, echo = FALSE}
knitr::include_graphics("images/svg-sources/mlr_fs.svg")
```
...
```{r 03-optimization-020, echo = FALSE, out.width="98%"}
knitr::include_graphics("images/png-sources/nested_resampling.png")
```
