...
Hyperparameter tuning is supported via the extension package `mlr3tuning`.
The heart of `mlr3tuning` are the R6 classes:
...
We use the classification tree from `rpart` and choose a subset of the hyperparameters we want to tune.
...
In order to determine the evaluation method,  one has to choose a `Resampling", text = "resampling strategy` and a `Measure", text = "performance measure`.
...
Since we have only numeric parameters, `TunerGridSearch` will create a grid of equally-sized steps between the respective upper and lower bounds.
...
To start the tuning, we simply pass the `TuningInstance` to the `$tune()` method of the initialized `Tuner`.
...
2. For each configuration, a `Learner` is fitted on `Task` using the provided `Resampling`.
   The results are combined with other results from previous iterations to a single `BenchmarkResult`.
...
One can investigate all resamplings which where undertaken, using the `$archive()` method of the `TuningInstance`.
...
Now the optimized hyperparameters can take the previously created `Learner`, set the returned hyperparameters and [train](#train-predict) it on the full dataset.
...
The embedded feature-selection methods of some `Learner`.
...
This is done similar to setting hyperparameters of a `Learner` using `.$param_set$values`:
...
For example, learner `classif.ranger` (in the package `mlr3learners`) comes with multiple integrated methods.
...
In `mlr-org/mlr3`, you can get nested resampling for free without programming any looping by using the `mlr3tuning::AutoTuner` class.
...
We use the example from section ["Automating the Tuning"](#autotuner) and pipe the resulting learner into a `resample()` call.
...
See also section [Resampling](#resampling) for more detailed information about `ResampleResult` objects.
...
<!-- We can also query the tuning result of any learner using the `$tune_path` field of the `AutoTuner` class stored in the `ResampleResult` container `rr`. -->
