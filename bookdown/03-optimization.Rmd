...
Ở đây chúng tôi chọn 2 tham số tên là complexity `cp` và tiêu chí `minsplit`.
...
Chúng tôi tạo một terminator cho phép 10 đánh giá và sử dụng một thuật toán tìm kiếm ngẫu nhiên đơn giản làm thuật toán điều chỉnh.
...
Tuy nhiên, lần này, chúng ta chuyển nó vào `r ref("benchmark()")` để so sánh auto tuner (at) với classification tree mà không có điều chỉnh.
...
Để có được ước tính hiệu suất không thiên vị cho bất kỳ leaners nào, tất cả các phần của quá trình xây dựng mô hình (tiền xử lý và các bước chọn mô hình) phải được resampling, tức là, được lặp lại cho mỗi cặp dữ liệu huấn luyện / kiểm tra.
Đối với các bước mà bản thân yêu cầu resampling như điều chỉnh hyperparameter hoặc feature-selection (thông qua cách tiếp cận wrapper), điều này dẫn đến hai vòng lặp lấy mẫu lồng nhau (nested resampling).
...
Hình ở trên minh họa việc lấy mẫu lồng nhau để điều chỉnh tham số với xác thực chéo 3 lần (3-fold cross-validation) đối với xác thực chéo bên ngoài (outer) và 4 lần đối với vòng lặp bên trong (inner).

Trong vòng lặp lấy mẫu lại bên ngoài, chúng ta có ba cặp dữ liệu huấn luyện / kiểm tra.
Trên mỗi tập huấn luyện bên ngoài này, việc điều chỉnh tham số sẽ được thực hiện, vì vậy trước hết phải thực hiện vòng lặp lấy mẫu bên trong trước.
Bằng cách này, chúng ta nhận được một bộ hyperparameters tương ứng với mỗi bộ dữ liệu huấn luyện bên ngoài (outer training set).
Sau đó, learner được fitted trên mỗi tập dữ liệu huấn luyện bên ngoài bằng cách sử dụng các hyperparameters được chọn tương ứng.
Tiếp theo, chúng ta có thể mô tả hiệu suất của learner dựa trên các bộ dữ liệu kiểm tra bên ngoài (outer test sets).

Trong `r gh_pkg("mlr-org/mlr3")`, bạn có thể lấy mẫu lồng nhau thoải mái mà không cần lập trình bất kỳ vòng lặp nào bằng cách sử dụng lớp `r ref("mlr3tuning::AutoTuner")`.
Điều này được thực hiện như sau:

1. Tạo một wrapped Learner thông qua lớp `r ref("mlr3tuning::AutoTuner")` hoặc `mlr3filters::AutoSelect` (chưa được triển khai).
2. Chỉ định tất cả các cài đặt cần thiết - xem phần ["Automating the Tuning"](#autotuner) để được trợ giúp.
3. Hàm gọi `r ref("resample()")` hoặc `r ref("benchmark()")` với `r ref("Learner")` đã tạo.

Bạn có thể tự do kết hợp các chiến lược inner và outer resampling khác nhau.

Một thiết lập phổ biến là dự đoán và đánh giá hiệu suất trên một bộ dữ liệu kiểm tra bên ngoài cố định. Điều này có thể đạt được bằng cách chuyển chiến lược `r ref("Resampling")` (`rsmp("holdout")`)  làm đối tượng lấy mẫu bên ngoài cho `r ref("resample()")` hoặc `r ref("benchmark()")`.

Chiến lược lấy mẫu lại bên trong có thể là một chiến lược xác thực chéo (`rsmp("cv")`) vì kích thước của các bộ huấn luyện bên ngoài có thể khác nhau.
Theo mặc định, mô tả inner resample được khởi tạo một lần cho mỗi tập huấn luyện bên ngoài.

Lưu ý rằng việc lấy mẫu lồng nhau là tính toán tốn kém.
Vì lý do này, chúng tôi sử dụng các không gian tìm kiếm tương đối nhỏ và số lần lặp lại lấy mẫu lại thấp trong các ví dụ được triển khai bên dưới.
Trong thực tế, bạn thường phải tăng cả hai.
Vì đây là tính toán chuyên sâu, bạn có thể xem thêm trong phần [Parallelization](#parallelization).

### Execution {#nested-resamp-exec}

Để tối ưu hóa hyperparameters hoặc tiến hành feature selection trong lấy mẫu lồng nhau, bạn cần tạo learners bằng cách sử dụng:

* lớp `r ref("AutoTuner")` hoặc
* lớp `mlr3filters::AutoSelect` (chưa được triển khai)

Chúng tôi sử dụng ví dụ từ phần ["Automating the Tuning"](#autotuner) và chuyển kết quả learner vào một `r ref("resample()")`.
...
Bây giờ xây dựng `r ref("resample()")`:
...
Với `r ref("ResampleResult")` đã tạo, bây giờ chúng ta có thể kiểm tra các lần lặp lại lấy mẫu được thực hiện chặt chẽ hơn.
Xem thêm phần [Resampling](#resampling) để biết thêm thông tin chi tiết về các đối tượng `r ref("ResampleResult")`.

Ví dụ: chúng ta có thể truy vấn kết quả hiệu suất tổng hợp:
...
Kiểm tra xem có bất kỳ lỗi gì trong các phần trong khi thực hiện (nếu không có bất kỳ đầu ra, cảnh báo hoặc lỗi được ghi lại, thì đây là một `data.table ()` trống:

```{r 03-optimization-021}
rr$errors
```

Hoặc xem ma trận nhầm lẫn (confusion matrix) của các dự báo của 3 fold:

Lớp `$prediction` đã gộp 3 fold của `$predictions`
