...
If your new tuner can be applied to any kind of optimization problem it should be implemented as a `bbotk::Optimizer`.
Any `bbotk::Optimizer` can be easily transformed to a `mlr3tuning::Tuner`.
Only if your tuner needs to access the task, learner or resampling objects, it is recommended to implement a `mlr3tuning::Tuner`.
However, the procedure is similar for both:

1. Check, that the tuner does not already exist as a `bbotk::Optimizer` or `mlr3tuning::Tuner`.
2. Use one of the existing optimizers as a [template](#tuner-template).
