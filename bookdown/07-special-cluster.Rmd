...
Cluster analysis is a type of unsupervised machine learning where the goal is to find patterns within the data by grouping them into clusters.
Each cluster contains similar observations, where the similarity is based on specified metrics that are task and application dependent. 
...
However, unlike classification problem where each observation is labeled, cluster analysis does not work on labeled data sets. 
...
More specifically:
  ...
  * `truth()` function is missing because observations in cluster analysis are not labeled. 
  * `target` field is empty and will return `character(0)` if accessed anyways. 


### Train and Predict
Cluster learners provide both `train` and `predict` methods. 
Cluster analysis typically consists of building clusters on all of the available data. 
Nevertheless, some learners can assign new observations to existing groups with `predict` method.
However, `predict` does not always make sense (such as in hierarchical clustering).
...
### Visualization
Cluster analysis in mlr3 is integrated with `r mlr_pkg("mlr3viz")` which provides a number of cluster related plots. 

```{r 07-special-cluster 004}
task = tsk("usarrests")
learner = lrn("clust.kmeans")
learner$train(task)
prediction = learner$predict(task)
