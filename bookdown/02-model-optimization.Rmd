...
The heart of `r gh_pkg("mlr-org/mlr3tuning")` are the R6 classes
...
In the following, we will look into optimizing a simple classification tree on the `r ref("mlr_tasks_pima", text = "Pima Indian Diabetes")` data set.
```{r}
task = tsk("pima")
print(task)
```

We use the classification tree from `r cran_pkg("rpart")` and choose a subset of the hyperparameters we want to tune. This is often referred to as the tuning space.
```{r}
learner = lrn("classif.rpart")
learner$param_set
```
Here, we opt to tune two parameters: the complexity `cp` and the termination criterion `minsplit`.
As tuning spaces has to be bound, we have to set lower and upper bounds:
```{r}
library(paradox)
...
tune_ps
```
Next, we need to define how we want to evaluate the performance.
...
Finally, we have to decide the budget available to solve this tuning instance.
...
For this short introduction, we grant a budget of 50 evaluations:
```{r}
evals20 = TerminatorEvals$new(20)
```

We can now put everything together to construct a `r ref("TuningInstance")`:
```{r}
...
The following algorithms are currently implemented in `r gh_pkg("mlr-org/mlr3tuning")`:
...
```{r}
tuner = TunerGridSearch$new(resolution = 5)
```
...
1. The `r ref("Tuner")` proposes at least one hyperparameter configuration (the `r ref("Tuner")` may propose multiple points to improve parallelization, this can be controlled via the setting `batch_size`).
...
3. The `r ref("Terminator")` is queried if the budget is exhausted. If the budget is not exhausted, go to 1.
...
We can investigate all resamplings which where conducted by calling the `$archive()` method of the `r ref("TuningInstance")`.
...
learner$param_set$values = result$values
learner$train(task)
```
The trained model could now be used to predict on independent, external data.
Note that predicting on observations present in the `task` is not statistically sound as these has been used for tuning.
The resulting performance measure would be over-optimistic.
Instead, to get unbiased performances for the current task, nested resampling is required.
In `mlr3` this is done via the `r ref("AutoTuner")`.
...
Analogously to the previous subsection, we create a new classification tree learner which automatically tunes the parameters `cp` and `minsplit` using an inner resampling (holdout).
...
terminator = TerminatorEvals$new(10)
...
We can now use the learner like any other learner, calling its `$train()` and `$predict()` method.
Instead, we pass it to `r ref("benchmark()")` to compare the tuner to a classification tree without tuning.
By doing so, the `r ref("AutoTuner")` will do its resampling for tuning on the training set of the respective split of the outer resampling.
The learner then predicts on the test set of the outer resampling.
...
```
Note that we do not expect to any differences here, for multiple reasons:
...
tuner = TunerGridSearch$new(resolution = 10)
