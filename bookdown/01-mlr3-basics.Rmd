...
* Classes have mutable state which is encapsulated in the member variables.
  Continuing with the previous example, assuming that the constructor sets the public member variable `bar` to `1`, we can access its value through the dollar operator: `foo$bar` or `foo$bar = 2`.
* In addition to member variables, objects expose methods that may allow to inspect the object's state or retrieve information and perform an action that may change the internal state of the object.
  As an example, the `$train` method of a learner changes the internal state by producing and storing a trained model.
* Objects can have public and private member variables and methods.
