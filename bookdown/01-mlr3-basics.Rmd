...
This chapter will teach you the essential building blocks, R6 classes and operations of `mlr3`, so you can create supervised ML task like classification and regression, train and predict models and cross-validate and benchmark them.
...
R6 is one of R's more recent dialects for object-oriented programming. While R6 might take a little while to learn for readers who have never seen OO before, in order to design your own R packages with it or extend mlr3, you can easily pick up here what you need to know in order to simply use R6 with mlr3 from the outside.

* Objects are created from a so-called factory method, with a constructor. So `foo = Foo$new(bar = 1)` creates a new object of class `Foo`, by passing a `1` into its constructor.
* Classes encode their state via so called member variables. Assuming, the constructor of `Foo` would directly store the state of `bar=1` in the created object in a public member variable (that does not happen automatically, some code would need to decide to do that), we could access this state now via `foo$bar` and retrieve its value or change it.
...
* R6 classes are implemented via environments / references. This means that R6 variables do not actually hold the object itself but rather a reference (pointer) to them. So `foo2 = foo` does not copy the object, but only copies the reference, which now points to the object.
...
**Learners** store information about and interface the machine learning algorithms.
