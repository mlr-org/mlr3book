...
This chapter will teach you the essential building blocks, R6 classes and operations of `mlr3`, so you can create supervised ML task like classification and regression, train learners, predict with models and cross-validate and benchmark them.
...
Data is usually split up intro training and testing portion, with a learning algorithm we induce a model on training data, label the test instances with our model, and compare true test label to predicted labels to obtain a scalar numeric score which quantifies the predictive power of our learner given the current data situation.
...
You will later learn how to enhance learners to full pipelines and tune hyperparameters, but for we will stick this simply workflow.

## Quick R6 Intro for Newbies

R6 is one of R's more recent dialects for object-oriented programming.
While R6 might take a little while to learn for readers who have never seen OO before, in order to design your own R packages with it or extend mlr3, you can easily pick up here what you need to know in order to simply use R6 with mlr3 from the outside.

* Objects are created from a so-called factory method, with a constructor.
  So `foo = Foo$new(bar = 1)` creates a new object of class `Foo`, by passing a `1` into its constructor.
* Classes encode their state via so called member variables.
  Assuming, the constructor of `Foo` would directly store the state of `bar=1` in the created object in a public member variable (that does not happen automatically, some code would need to decide to do that), we could access this state now via `foo$bar` and retrieve its value or change it.
* Classes usually expose some methods to a) retrieve information, b) perform some action, c) change the state of the object.
  E.g. `foo$inc()` might increment the value of `bar` in `foo` by `+1`, and hence change the state of the object.
* Classes have public and private parts of their interface, regarding methods and member variables. We as outside users can only access the public parts and do not have to worry about the private.
* R6 classes are implemented via environments / references.
  This means that R6 variables do not actually hold the object itself but rather a reference (pointer) to them. So `foo2 = foo` does not copy the object, but only copies the reference, which now points to the object.
* If you really want to fully copy an object call `foo$clone()`, possibly `foo$clone(deep = TRUE)`.

This should be enough for now, but if you want more details, have a look at the [R6 vignettes](https://r6.r-lib.org/).
...
Classification problems with only two classes within the target variable are called **binary classification**.
For those, you can additionally specify a **positive class** within the `task` object during task creation or afterwards, which is used afterwards.
If you are not setting this manually, `mlr3` sets the positive class automatically to the first class level which appears in the target variable in the dataset.
...
In `mlr3`, the ROC curve can be created by...

```{r 01-mlr3-basics-058}
# How to Plot a ROC Curve
# FIXME: Not there right now!
```
...
```{r 01-mlr3-basics-059, echo = FALSE}
# t = "
