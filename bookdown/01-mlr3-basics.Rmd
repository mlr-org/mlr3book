...
The package ships with a minimal set of classification and regression learners to avoid lots of dependencies; more are implemented in the [mlr3learners](https://mlr3learners.mlr-org.com) package.
The [mlr3learners](https://mlr3learners.mlr-org.com) package also has some documentation on creating custom learners, for example to add support for a new machine learning package.
...
All the information in the previous summary table is also accessible in public fields of the learner object, for example `id`, `feature_types`, `packages`, `properties`, and `predict_types`.
Additionally, `predict_type` returns the configured predict type of the learner.
