...
This can easily be done when using the `r mlr_pkg("mlr3misc")` library's `r ref("mlr3misc::invoke()")` function, which has functionality similar to `r ref("base::do.call()")`.
...
How `PipeOpScale` handles its parameters can be seen in its `$train` method: It gets the relevant parameters from its `$values` slot and uses them in the `r ref("mlr3misc::invoke()")` call.
