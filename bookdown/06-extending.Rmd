...
## Extending Learners {#ext-learner}
...
```{r 05-extending-learner-001, eval = FALSE}
...
Possible properties include:
  ...
  * `"selected features"`: The learner supports extracting the features which where used.
...
```{r 05-extending-learner-002, eval = FALSE}
...
```{r 05-extending-mlr3pipelines-001}
library(mlr3)
...
```{r 05-extending-mlr3pipelines-002, echo=FALSE,fig.align='center'}
...
```{r 05-extending-mlr3pipelines-003, eval = FALSE}
...
```{r 05-extending-mlr3pipelines-004, eval = FALSE}
    initialize = function(id = "copy.two") {
...
```{r 05-extending-mlr3pipelines-005, eval = FALSE}
    train = function(inputs) {
      self$state = list()
      c(inputs, inputs)
    },
...
```{r 05-extending-mlr3pipelines-006}
PipeOpCopyTwo = R6::R6Class("PipeOpCopyTwo",
  inherit = PipeOp,
  public = list(
    initialize = function(id = "copy.two") {
...
```{r 05-extending-mlr3pipelines-007}
...
```{r 05-extending-mlr3pipelines-008}
...
```{r 05-extending-mlr3pipelines-009}
...
```{r 05-extending-mlr3pipelines-010}
gr = Graph$new()
...
```{r 05-extending-mlr3pipelines-011}
...
```{r 05-extending-mlr3pipelines-012}
gr = Graph$new()
...
```{r 05-extending-mlr3pipelines-013}
...
```{r 05-extending-mlr3pipelines-014}
irishead = task$clone()$filter(1:5)
irishead$data()
```

```{r 05-extending-mlr3pipelines-015}
...
```{r 05-extending-mlr3pipelines-016}
gr$pipeops$drop.const$state
```

```{r 05-extending-mlr3pipelines-017}
...
```{r 05-extending-mlr3pipelines-018}
...
```{r 05-extending-mlr3pipelines-019}
...
```{r 05-extending-mlr3pipelines-020}
result_posa$data()
```

```{r 05-extending-mlr3pipelines-021}
...
```{r 05-extending-mlr3pipelines-022}
...
```{r 05-extending-mlr3pipelines-023}
pss = PipeOpScale$new()
print(pss$param_set)
```

```{r 05-extending-mlr3pipelines-024}
pss$param_set$values$center = FALSE
print(pss$param_set$values)
```

```{r 05-extending-mlr3pipelines-025, error = TRUE}
...
```{r 05-extending-mlr3pipelines-026}
...
```{r 05-extending-mlr3pipelines-027}
