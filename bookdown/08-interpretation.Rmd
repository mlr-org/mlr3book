...
`iml` is an R package that interprets the behavior and explains predictions of machine learning models. The functions provided in the `iml` are model-agnostic which gives the flexibility to use any machine learning model.

This chapter provides examples of how to use `iml` with `mlr3`

### Example{setup}

The goal of this example is to figure out the species given the features.


The first example uses the `palmerpenguins` data set which is an alternative to the `iris` data set. `palmerpenguins` contains two data sets: 

  -[palmerpenguins::penguins](https://allisonhorst.github.io/palmerpenguins/reference/penguins.html) which can be seen as a direct alternative to `iris`
...
$ bill_length_mm    <dbl> 39.1, 39.5, 40.3, 36.7, 39.3, 38.9, 39.2, 41.1, 38.6, 34.6, 36.6, 38.7, 42.5, 34.4, 46.0, 37.8, 37.7, 35.9, 38.2, 38.8, 35.3, 40.6,…
$ bill_depth_mm     <dbl> 18.7, 17.4, 18.0, 19.3, 20.6, 17.8, 19.6, 17.6, 21.2, 21.1, 17.8, 19.0, 20.7, 18.4, 21.5, 18.3, 18.7, 19.2, 18.1, 17.2, 18.9, 18.6,…
$ flipper_length_mm <int> 181, 186, 195, 193, 190, 181, 195, 182, 191, 198, 185, 195, 197, 184, 194, 174, 180, 189, 185, 180, 187, 183, 187, 172, 180, 178, 1…
$ body_mass_g       <int> 3750, 3800, 3250, 3450, 3650, 3625, 4675, 3200, 3800, 4400, 3700, 3450, 4500, 3325, 4200, 3400, 3600, 3800, 3950, 3800, 3800, 3550,…
$ sex               <fct> male, female, female, female, male, female, male, female, male, male, female, female, male, female, male, female, male, female, mal…
$ year              <int> 2007, 2007, 2007, 2007, 2007, 2007, 2007, 2007, 2007, 2007, 2007, 2007, 2007, 2007, 2007, 2007, 2007, 2007, 2007, 2007, 2007, 2007,…
```


The goal of this example is to figure out the specific penguin species given the provided features.
...
Next is the core of the offerings from `iml`. In this example three separate interpretation methods will be used: [FeatureEffects](https://github.com/christophM/iml/blob/master/R/FeatureEffects.R), [FeatureImp](https://github.com/christophM/iml/blob/master/R/FeatureImp.R), [Shapley](https://github.com/christophM/iml/blob/master/R/Shapley.R)


  -`FeatureEffects` computes the effects what the given features have on the prediction. Different methods are implemented: [Accumulated Local Effect (ALE) plots](https://christophm.github.io/interpretable-ml-book/ale.html), [Partial Dependence Plots (PDPs)](https://christophm.github.io/interpretable-ml-book/pdp.html) and [Individual Conditional Expectation (ICE) curves](https://christophm.github.io/interpretable-ml-book/ice.html)
...
`effect` defines the interpretation model by wrapping `Predictor`. After this is done the selected features can be plotted. In this exampe all of the features provided by the `palmerpenguins` data set were used. 

The results are:
```
{r feateff, echo=FALSE, fig.cap='Plot of the results from FeatureEffects.FeatureEffects computes and plots feature effects of prediction models..', out.width = '92%', fig.align='center'}
knitr::include_graphics("images/feateff.png")
```


All features outside of `year` provide meaningful interpretable information. It should be clear why `year` doesn't provide anything of significance. `bill_length_mm` shows for example that when the bill length is smaller than roughly 40mm, there is a high chance that the penguin is an Adelie. 
...
```
```
{r shapley, echo=FALSE, fig.cap='Plot of the results from Shapley. \phi gives the increase or decrease in probabilty given the values on the vertical axis.', out.width = '92%', fig.align='center'}
knitr::include_graphics("images/shapley.png")
```

The variable \phi on the horitonal axis provides insight into the probability given the values on the vertical axis. For example, a penguin is less likely to be Gentoo if the bill_depth_=18.7 is and much more likely to be Adelie than Chinstrap.
...
```
```
{r featimp, echo=FALSE, fig.cap='Plot of the results from FeatureImp. FeatureImp visualizes the importance of features given the prediction model.', out.width = '92%', fig.align='center'}
knitr::include_graphics("images/featimp.png")
```
...
It is also interesting to see how well a model has trained a data set. For this section, exactly as was recommended in section [2.4](https://mlr3book.mlr-org.com/train-predict.html) of the mlr3book, 80% of the penguin data set will be used for the training set and 20% for the test set:
...
```
{r featimp, echo=FALSE, fig.cap='FeatImp test data set.', out.width = '92%', fig.align='center'}
knitr::include_graphics("images/featimptest.png")
```


```
{r featimp, echo=FALSE, fig.cap='FeatImp train data set.', out.width = '92%', fig.align='center'}
knitr::include_graphics("images/featimppred.png")
```



```
{r featimp, echo=FALSE, fig.cap='The results from the training set for FeatureEffect.', out.width = '92%', fig.align='center'}
knitr::include_graphics("images/featefftest.png")
```

```
{r featimp, echo=FALSE, fig.cap='FeatEffect train data set.', out.width = '92%', fig.align='center'}
knitr::include_graphics("images/pred_feateffreal.png")
...
The `r cran_pkg("DALEX")` package X-rays any predictive model and helps to explore, explain and visualize its behaviour. The package implements a collection of methods for [Explanatory Model Analysis](https://pbiecek.github.io/ema/). It is based on a unified grammar summarised in Figure \@ref(fig:DALEXema).
...
```{r DALEXema, echo=FALSE, fig.cap='Taxonomy of methods for model exploration presented in this chapter. Left part overview methods for instance level exploration while right part is related to dataset level model exploration.', out.width = '92%', fig.align='center'}
...
```{r dalex-fifa, message=FALSE, warning=FALSE}
library("DALEX")
...
```{r dalex-fifa-dim, message=FALSE, warning=FALSE}
dim(fifa)
```
...
```{r dalex-fifa-clean, message=FALSE, warning=FALSE}
...
```{r dalex-mlr3, message=FALSE, warning=FALSE}
library("mlr3")
library("mlr3learners")
```
...
```{r dalex-ranger-define, message=FALSE, warning=FALSE}
...
```{r dalex-ranger-train, message=FALSE, warning=FALSE}
...
```{r dalex-dalex-intro, message=FALSE, warning=FALSE}
library("DALEX")
library("DALEXtra")
...
```{r dalex-ranger-parts, message=FALSE, warning=FALSE}
...
```{r dalex-ranger-parts-plot-empty, message=FALSE, warning=FALSE, echo=FALSE}
tmp <- ingredients:::plot.feature_importance_explainer(fifa_vi, max_vars = 12, show_boxplots = FALSE)
```

```{r dalex-ranger-parts-plot, message=FALSE, warning=FALSE, fig.width=8, fig.height=4.5}
...
```{r dalex-ranger-profile, message=FALSE, warning=FALSE}
...
```{r dalex-ranger-profile-plot, message=FALSE, warning=FALSE, fig.width=8, fig.height=8}
library("ggplot2")
plot(fifa_pd) +
...
```{r dalex-ranger-predict-parts, message=FALSE, warning=FALSE}
...
```{r dalex-ranger-predict-parts-plot-empty, message=FALSE, warning=FALSE, echo=FALSE}
tmp <- iBreakDown:::plot.break_down(ronaldo_bd_ranger)
```

```{r dalex-ranger-predict-parts-plot, message=FALSE, warning=FALSE, fig.width=8, fig.height=4.5}
...
```{r dalex-ranger-predict-shap, message=FALSE, warning=FALSE, fig.width=8, fig.height=4.5}
...
```{r dalex-ranger-predict-profile, message=FALSE, warning=FALSE, fig.width=8, fig.height=8}
selected_variables <- c("age", "movement_reactions",
                "skill_ball_control", "skill_dribbling")
