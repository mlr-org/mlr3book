...
Summed up, this chapter contains sub-chapters on the following components of supervised machine learning:
...
* specify the [task types](#tasks-types)
* create different [tasks](#tasks-creation)
* use the [task's public fields and methods](#tasks-api)
* assign roles to [rows and collums](#tasks-roles)
* implement [task mutators](#tasks-mutators)
* [retrieve data](#tasks-retrieved) that is stored in tasks
...
Specifically, this sub-chapter shows how to:

* access the set of [classification and regression learners](#learners-predefined) that are predefined in mlr3
* determine a [specific learner](#learners-predefined)
* modify the [current hyperparameter values](#learners-predefined) of a learner

To provide guidance on how to individually customize learners, advanced technical material on learners can be found in the [extending learners](#ext-learner) section.

[**train and predict**](#train-predict)

[Train and predict](#train-predict) illustrates how to use [tasks](#tasks) and [learners](#learners) to train a model and make [predictions](#predicting) on a new data set.
Furthermore, [train and predict](#train-predict) provides information on how to:

* set up [tasks](#train-predict-objects) and [learners](#train-predict-objects) in a proper manner
* set up [train and test splits](#split-data) of the data
* training the [learner](#training) on the 'train' data set
* generate [predictions](#predicting) on the 'test' data set
* assess the [performance](#measure) of the model
...
* instantiate the [splitting](#resamp-inst)
* derive [resampling results](#resamp-exec)
...
[Benchmarking](#benchmarking) is used to assess multiple performances.
This can be the performance of different learners on multiple tasks and/or on different resampling schemes.
...
* convert specific benchmark objects to [resample objects](#bm-resamp)

[**Binary classification**](#binary)

[Binary classification](#binary) refers to classification problems with a target variable that only has two classes.
[Binary classification](#binary) provides information on how to:

* determine and evaluate the [ROC curve and threshold](#binary-roc)
* conduct threshold tuning (WIP)

This chapter starts with a brief introduction on R6 for people that have not worked with R6 before.
...
The section on extending learners](#ext-learner) entails further information on how to:
...
### Basic concept {#train-predict-basic}
...
`mlr3` entails 6 different predefined [resampling](#resampling) strategies: 
...
### Converting specific benchmark objects to resample objects {#bm-resamp}
