...
Tasks encapsulate the data with meta-inforamtion, such as what the prediction target is.
We cover how to
...
Of course, you can do the same for regression tasks (inheriting from `r ref("TaskRegr")`) as documented in `r ref("mlrviz::autoplot.TaskRegr")`:
```{r, warning = FALSE, message = FALSE}
library(mlr3viz)
...
### Plotting Predictions {#prediction-plot}

Analogously to [plotting tasks](#tasks-plot), `r mlr_pkg("mlr3viz")` provides a `r ref("ggplot2::autoplot()", text = "autoplot()")` method.
