...
* How to create [various tasks](#task-creation)
* Which tasks are predefined in `mlr3`
* How to use the task's public fields and methods in a sound manner

[**learner**](#learner)

[Learners](#learner) encapsulate one of R's many machine learning algorithms and allows to train models and make predictions.
Most [learners](#learner) have hyperparameters that affect their operation.
In this sub-chapter, the following features of [learners](#learner) are illustrated:

* Which set of classification and regression learners are entailed in mlr3
* How to access [predefined learners](#learner-predefined)
* How to access specific learners
* How to modify the current hyperparameter values

Advanced technical material on learners can be found in the chapter [extending learners](#ext-learner)

[**train and predict**](#train-predict)

[Train and predict](#train-predict) provides information on how to:

* use tasks and learners in a proper manner to derive reliable predictions
...
`mlr3` entails 6 different predefined resampling strategies namely: 
Cross-validation, Leave-one-out cross validation, Repeated cross-validation, Out-of-bag bootstrap and other variants (b632), Monte-Carlo cross-validation and holdout.
Resampling is used to assess the performance of a learner.
This sub-chapter entails instructions on how to: 

* access and select one of the predefined resampling strategies 
* access additional resampling strategies
...
The chapter on [Benchmarking](#benchmarking) shows how to:
...
Furthermore, the `r mlr_pkg("mlr3")` package provides R6 classes for the essential building blocks of this machine learning workflow:

* A [tasks](#tasks)
* A [learners](#learners)
* A [measure](#measures)
* A [resampling](#resampling)
...
### Predefined Learners {#learner-predefined}
