...
In order to get fast results, we decide to tune only parts of the hyperparameter space that can be adjusted for `r pkg("xgboost")`. Additionally, we only allow for 
$20$ iterations of random search, which is usually to little for real-world applications. Nonetheless, we are able to obtain an improved performance when comparing to the `r pkg("ranger")` model.
...
tsk_ftsel = filter$filter_nfeat(tsk$clone(), nfeat = 14)
```
...
res$performance("regr.mse")
sprintf("RMSE of ranger with zip_status: %s", round(sqrt(res$aggregate()), 2))
```
