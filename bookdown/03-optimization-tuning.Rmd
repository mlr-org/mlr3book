...
Each configuration serves as hyperparameter setting for the previously defined `r ref("Learner")` and triggers a 3-fold cross validation on the task.
...
To start the tuning, we simply pass the `r ref("TuningInstance")` to the `$optimize()` method of the initialized `r ref("Tuner")`.
...
2. For each configuration, the given `r ref("Learner")` is fitted on the `r ref("Task")` using the provided `r ref("Resampling")`.
   All evaluations are stored in the archive of the `r ref("TuningInstance")`.
...
One can investigate all resamplings which were undertaken, as they are stored in the `$archive` slot of the `r ref("TuningInstance")`:
...
learner$param_set$values = instance$result$params #FIXME: When https://github.com/mlr-org/bbotk/pull/53 and https://github.com/mlr-org/mlr3tuning/pull/240 is merged
learner$train(task)
```
