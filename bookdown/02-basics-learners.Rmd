...
You can get a specific learner using its `id`, listed under `key` in the dictionary by just calling the shortcut function `r ref("lrn()")`:
...
It is also possible to set specific hyperparameters or the some fields during construction with `r ref("lrn()")`:

```{r 02-basics-learners-007}
lrn("classif.rpart", id = "rp", cp = 0.001)
```

If you pass hyperparameters here, they are added to the default parameters in a `r ref("mlr3misc::insert_named()", text = "insert_named()")`-fashion.
