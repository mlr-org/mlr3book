...
To obtain an object from the dictionary we can use `r ref("lrn()")` or the generic `mlr_learners$get()` method, e.g. `lrn("classif.rpart")`.
...
Note that the `r ref("lrn()")` function also accepts additional arguments which are then used to update hyperparameters or set fields of the learner in one go:
