...
Note that not all functionality will be covered, and that some packages are still in early lifecycles.
...
The following examples use the [ecuador](https://mlr3spatiotempcv.mlr-org.com/reference/mlr_tasks_ecuador.html) dataset from Jannes Muenchow.
It contains information on the occurrence of landslides (binary) in the Andes of Southern Ecuador.
The landlides were mapped from aerial photos taken in 2000.
The dataset is well suited to serve as an example dataset because it it relatively small and due to the spatial distribution of the observations.
Please refer to @muenchow2012 for a detailed description of the dataset.

To account for the spatial autocorrelation probably present in the landslide data, we will make use one of the most popular spatial partitioning methods, a cluster-based k-means grouping after @brenning2012 (`"spcv_coords"` in `r gh_pkg("mlr-org/mlr3spatiotemporal")`).
...
By contrast, when using random partitioning with spatial data, train and test observations would be located side-by-side across the full study area (a visual example is provided further below).
...
If you want to learn even more about the field of spatial partitioning, STAC and the problems associated with it, the works of [Prof. Hanna Meyer](https://scholar.google.com/citations?user=9YibxW0AAAAJ&hl=en) are very much recommended for further reference. 
