...
```{r 02-basics-binary-00sdawas00, include = F}
library(mlr3)
```
...
```{r 04-pipelines-non-linear-015, eval = TRUE}
lrn = lrn("classif.rpart")
lrn_0 = po("learner_cv", lrn$clone())
lrn_0$id = "rpart_cv"
```

We use `"PipeOpNOP")`  in combination with `"gunion")`, in order to send the unchanged Task to the next level.
There it is combined with the predictions from our decision tree learner.

```{r 04-pipelines-non-linear-016, eval = TRUE}
...
```{r 04-pipelines-non-linear-021, warning=FALSE}
lrn$
