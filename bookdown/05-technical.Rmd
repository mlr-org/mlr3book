...
The section [Database Backends](#backends) describes how to work with database backends that `mlr3` supports.
...
Lastly, we provide a [Transition Guide](#transition) for users of the old `mlr` who want to switch to `mlr3`.
...
Because _lgr_ comes with its own exhaustive vignette, we will just briefly give examples how you can change the most important settings related to logging in `mlr3`.

### Available logging levels

_lgr_ comes with certain numeric thresholds which correspond to verbosity levels of the logging.
For `mlr3` the default is set to 400 which corresponds to level "info".
...
In case you have already worked with `mlr`, you may want to quickstart with `mlr3` by looking up the specific equivalent of an element of `mlr` in the new version `mlr3`.
