...
```{r 04-parameters-006, R.options=list(width = 120)}
as.data.table(parA)
```
...
```{r 04-parameters-010, R.options=list(width = 120)}
as.data.table(ps)
```
...
```{r 04-parameters-013, error = TRUE}
ps$values$B = 100
```
...
```{r 04-parameters-023, echo = FALSE, out.width="45%", fig.show = "hold", fig.width = 4, fig.height = 4}
par(mar=c(4, 4, 2, 1))
...
```{r 04-transition-guide-001, echo = FALSE}
# t = "
