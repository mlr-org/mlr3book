...
2. A [`train()` function](#learner-train) which takes a `r ref("TaskClassif")` as input and returns an arbitrary model.
3. A [`predict()` function](#learner-predict) which operates on the model returned by `train()` and a (differently subsetted) `r ref("TaskClassif")` to calculate predictions for each observation in the task.
...
```
In the final learner, `self` will reference of course the learner itself.
...
The predict function also operates on a `r ref("Task")` as well as on the model returned by the `train()` function.
...
# inputs:
task = mlr_tasks$get("iris")
model = rpart::rpart(task$formula(), data = task$data())

response = predict(model, newdata = data, type = "class")
prob = predict(model, newdata = data, type = "prob")
```
...
```{r, eval = FALSE}
self = list(predict_type = "response")

if (self$predict_type == "response") {
  list(response = predict(model, newdata = task$data(), type = "class"))
} else {
  list(prob = predict(model, newdata = task$data(), type = "prob"))
}
```
...
```
Note that if the learner would need to handle hyperparameters during the predict step, we would preoceed accordingly to the `train()` step and use `self$params("predict")` in combination with `r ref("mlr3misc::invoke()")`.
  ...
  public = list(
    ...
    predict = function(task, model = self$model) {
      if (self$predict_type == "response") {
        ...
        list(prob = predict(model, data = newtask$data(), type = "prob"))
      }
    }
  )
)
