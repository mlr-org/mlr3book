...
For an in-depth description of `r mlr_pkg("paradox")` and its classes, see the [`paradox` chapter](#paradox).
...
(We are using `rbindlist()` here because the result of `$transpose()` is a list that is harder to read. If we didn't use `$transpose()`, on the other hand, the transformations that we investigate here are not applied.) In generate_design_grid(search_space, 3), `search_space` is the `r ref("ParamSet")` argument and 3 is the specified resolution in the parameter space. The resolution for categorical parameters is ignored, these parameters always produce a grid over all their valid levels. For numerical parameters the endpoints of the params are always included in the grid. (So if there were 3 levels for the kernel instead of 2 there would be 9 rows in the resulting table, or if the resolution was 4 in this example there would be 8 rows in the resulting table).
...
`r ref("to_tune")` can also be constructed with a `r ref("Domain")` object, i.e. something constructed with a `p_***` call.
...
```{r 03-optimization-searchspace-017}
