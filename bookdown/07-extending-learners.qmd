...
```{r 02-basics-binary-00wwwww00, include = F}
library(mlr3)
```
...
* Open a new R session, call `library("mlr3extralearners")` (install if you haven't already), and then [run](#create-learner) `"mlr3extralearners::create_learner")` with the `pkg` argument set as the path (the folder location) to the package directory.
* Open a new R session, set your working directory as your newly cloned repository, run `"devtools::load_all")`, and then [run](#create-learner) `"mlr3extralearners::create_learner")`, leaving `pkg = "."`.
* In your newly cloned repository, open the R project, which will automatically set your working directory, run `"devtools::load_all")`, and then [run](#create-learner) `"mlr3extralearners::create_learner")`, leaving `pkg = "."`.
...
5. Fill in the private [`.train`](#learner-train) method, which takes a (filtered) `"Task")` and returns a model.
6. Fill in the private [`.predict`](#learner-predict) method, which operates on the model in `self$model` (stored during `$train()`) and a (differently subsetted) `"Task")` to return a named list of predictions.

### Meta-information {#learner-meta-information}

In the constructor (`initialize()`) the constructor of the super class (e.g. `"LearnerClassif")`) is called with meta information about the learner which should be constructed.
This includes:
...
* `param_set`: A set of hyperparameters and their descriptions provided as a `"paradox::ParamSet")`.
  For each hyperparameter the appropriate class needs to be chosen. When using the `"paradox::ps")` shortcut, a short constructor of the form `p_***` can be used:
  * `"paradox::ParamLgl")` / [`paradox::p_lgl`](https://paradox.mlr-org.com/reference/Domain.html) for scalar logical hyperparameters.
  * `"paradox::ParamInt")` / [`paradox::p_int`](https://paradox.mlr-org.com/reference/Domain.html) for scalar integer hyperparameters.
  * `"paradox::ParamDbl")` / [`paradox::p_dbl`](https://paradox.mlr-org.com/reference/Domain.html) for scalar numeric hyperparameters.
  * `"paradox::ParamFct")` / [`paradox::p_fct`](https://paradox.mlr-org.com/reference/Domain.html) for scalar factor hyperparameters (this includes characters).
  * `"paradox::ParamUty")` / [`paradox::p_uty`](https://paradox.mlr-org.com/reference/Domain.html) for everything else (e.g. vector paramters or list parameters).
* `predict_types`: Set of predict types the learner is able to handle.
  These differ depending on the type of the learner. See `"mlr_reflections", text = "mlr_reflections$learner_predict_types")` for the full list of feature types supported by `mlr3`.
  * `LearnerClassif`
...
* `feature_types`: Set of feature types the learner is able to handle.
  See `"mlr_reflections", text = "mlr_reflections$task_feature_types")` for feature types supported by `mlr3`.
* `properties`: Set of properties of the learner. See `"mlr_reflections", text = "mlr_reflections$learner_properties")` for the full list of feature types supported by `mlr3`. Possible properties include:
