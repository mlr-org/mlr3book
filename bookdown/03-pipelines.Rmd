...
```{r 03-pipelines-001_setup, message = FALSE, warning = FALSE, echo = FALSE, include = FALSE}
...
`r mlr_pkg("mlr3pipelines")` is a dataflow programming toolkit for machine learning in R utilizing the `r mlr_pkg("mlr3pipelines")` package. A more in-deptth and technically oriented vignette can be found in the [mlr3pipeline vignette](https://mlr3pipelines.mlr-org.com/articles/introduction.html).
...
knitr::include_graphics("images/mlr3pipelines.png")
```
...
Other packages that provide, to varying degree, some preprocessing functionality or machine learning domain specific language, are the 
`r cran_pkg("caret")` package and the related `r cran_pkg("recipies")`  project, and the `r cran_pkg("dplyr")` package.
...
The manual way (= hard way) to construct a `r ref("Graph")`  is to create an empty one first, fill it with POs, and then connect edges between the POs.
...
This makes it not only possible to easily change change the behavior of a `r ref("Graph")`  / `r ref("GraphLearner")` and try different settings manually, but also to perform tuning using the `r mlr_pkg("mlr3tuning")` package.
...
If you are unfamiliar with tuning in `r mlr_pkg("mlr3")` yet, we recommend to take a look at the section about [tuning](#tuning) first.
...
```{r 03-pipelines-023}
(graph %>>%
  mlr_pipeops$get("unbranch", c("null", "pca", "scale")))$plot(html = TRUE) %>%
...
```{r 03-pipelines-028, fig.width=7.5}
bagging$plot(html = TRUE) %>% visInteraction(zoomView = FALSE) # disable zoom
```

This pipeline can again be used in conjunction with
`r ref("GraphLearner")`  in order for Bagging to be used like a
[mlr3::Learner].
...
In cases when the data does not fully fit in memory, we can obtain a fraction of the data for each learner from a `r ref("DataBackend")`  and then aggregate predictions over all learners.
...
We additionally `clone()` the learner object to obtain a copy of the learner, and set a custom id for the `r ref("PipeOp")` .
...
Additionally, we use a `r ref("PipeOpNULL")`  in parallel to the "level 0" learner, in order to send the unchanged Task to the next level, where it is then combined with the predictions from our decision tree learner.
...
```{r 03-pipelines-033, fig.width=7.5}
stack = combined %>>% PipeOpLearner$new(lrn$clone())
stack$plot(html = TRUE) %>% visInteraction(zoomView = FALSE) # disable zoom
```
...
In this case, we train a _glmnet_ and 2 different _rpart_ models (some transform its inputs using `r ref("PipeOpPCA")` ) on our task in the "level 0" and concatenate them with the original features (via `r ref("PipeOpNULL")` ).
...
Additionally, we keep a version of the "level 0" output (via `r ref("PipeOpNULL")` ) and pass this on to "level 2".
In "level 2" we simply concatenate all "level 1" outputs and train a final decision tree.

```{r 03-pipelines-035, eval = FALSE}
...
level_2$plot(html = TRUE) %>% visInteraction(zoomView = FALSE) # disable zoom
...
### Imputation: `r ref("PipeOpImpute")`

An often occuring setting is the imputation of missing data. 
Imputation methods range from relatively simple imputation using either *mean*, *median* or histograms to way more involved methods including using machine learing algorithms in order to predict missing values. 

The folling `PipeOp` imputes numeric values from a histogram, adds a new level for factors and additionally adds a column marking whether a value for a given feature was missing or not.

```{r}
poi = PipeOpImpute$new(param_vals =
  list(method_num = "hist", method_fct = "sample", add_dummy = "all"))
```

we can then add a learner to the Pipeop to obtain a learner equipped with automatic imputation of missing values.

```{r}
polrn = PipeOpLearner$new(mlr_learners$get("classif.rpart"))
lrn = GraphLearner$new(graph = poi %>>% polrn)
```

### Feature Engineering: `r ref("PipeOpMutate")`
...
### Training on data subsets: `r ref("PipeOpChunk")`

In cases, where data is too big to fit into the machine's memory, an often-used technique is to split the data into several parts, train on each part of the data and afterwards aggregate the models.
...
Afterwards we can use `r ref("PipeOpMajorityVote")`  to aggregate the predictions from the 4 different models into a new one.
...
```{r 03-pipelines-039, fig.width=7.5, fig.height = 9}
...
### Feature Selection: `r ref("PipeOpFilter")` , `r ref("PipeOpSelect")`

`r mlr_pkg("mlr3featsel")` contains many different `Filters` that can be used to select features for subsequent learners.
This is often required when the data has a large amount of features.

```{r 03-pipelines-041}
PipeOpFilter$new(mlr3featsel::FilterInformationGain$new())
```
