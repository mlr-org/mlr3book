...
In this case, we train a **glmnet** and 2 different *rpart* models (some models transform their inputs using `PipeOpPCA`) on our task in the level $0$ and concatenate them with the original features (via `PipeOpNull`).
...
## Interesting Operators {#pipe-special-ops}
...
an environment, where expressions are evalutated (`env` defaults to `.GlobalEnv`).


### Training on data subsets: `PipeOpChunk`

In cases, where data is too big to fit into the machine's memory, an often-used technique is
...
### Feature Selection: `PipeOpFilter`, `PipeOpSelect`

`mlr_pkg("mlr3featsel")` contains many different `Filters` that can be used to select 
features for subsequent learners. This is often required when the data
has a large amount of features.

```{r}
library(mlr3featsel)
PipeOpFilter$new(FilterInformationGain$new())
```
