...
Nested resampling process:
1. Use a 3-fold cross-validation to get testing and training data sets (outer resampling)
2. Within the training data use a 4-fold cross-validation to get sub-testing and sub-training data sets (inner resampling)
3. Tune the hyperparameters on sub-training data
4. Fit the learner on the training data set using the tuned hyperparameter configuration
5. Evaluate the performance of the learner on the testing data
6. 2-5 is done on each of the three folds (outer resampling)
7. The three performances are aggregated for an unbiased performance estimate.
[k-fold cross validation](https://machinelearningmastery.com/k-fold-cross-validation/) explanation
