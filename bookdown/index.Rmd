...
To run the code examples in this book, you need to install the `r gh_pkg("mlr-org/mlr3book")` package using the `r cran_pkg("remotes")` package:

```{r index-002, eval = FALSE}
remotes::install_github("mlr-org/mlr3book", dependencies = TRUE)
```

After installation, load the `r mlr_pkg("mlr3")` package and you are good to go:
