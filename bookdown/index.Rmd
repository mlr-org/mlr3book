...
To run the code here, install the `r gh_pkg("mlr-org/mlr3book")` package using the `r cran_pkg("remotes")` package:

```{r index-002, eval = FALSE}
remotes::install_github("mlr-org/mlr3book", dependencies = TRUE)
```

You don't need to load `r mlr_pkg("mlr3book")` though, just load the `r mlr_pkg("mlr3")` package and you are good to go:
