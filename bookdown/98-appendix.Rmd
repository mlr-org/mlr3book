...
## Integrated PipeOp's {#list-pipeops}

```{r 98-appendix-005, echo = FALSE}
library(mlr3pipelines)
dt = as.data.table(mlr_pipeops)
na_to_dash = function(x) {x = as.character(x); ifelse(is.na(x), "-", x)}
star_to_any = function(x) {ifelse(x == "*", "Any", x)}
char_to_dash = function(x) {ifelse(x == "character(0)", "-", x)}
dt = dt[, lapply(.SD, na_to_dash)][, lapply(.SD, star_to_any)][, lapply(.SD, char_to_dash)]

colnames(dt) = gsub(".type", "", colnames(dt))

dt %>%
  as.data.frame() %>%
  dplyr::mutate(packages = kableExtra::cell_spec(packages,  italic = TRUE)) %>%
...
While `r cran_pkg("mlr")` wrappers are generally less verbose and require a little fewer code, this heavily inhibits flexibility.
As an example, wrappers can generally not process data in parallel.

#### mlr

```{r 98-appendix-013, eval = FALSE}
library("mlr")
