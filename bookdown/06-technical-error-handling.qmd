...
To demonstrate how to properly deal with misbehaving learners, `r mlr3book::mlr_pkg("mlr3")` ships with the learner `"mlr_learners_classif.debug", "classif.debug")`:
...
One way to encapsulate the execution is provided by the package `r mlr3book::cran_pkg("evaluate")` (see `"encapsulate()"` for more details):
