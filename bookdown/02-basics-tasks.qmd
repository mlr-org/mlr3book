...
* **Survival Task**: The target is the (right-censored) time to an event. More censoring types are currently in development → `"mlr3proba::TaskSurv")` in add-on package `r mlr3book::mlr_pkg("mlr3proba")`.

* **Density Task**: An unsupervised task to estimate the density → `"mlr3proba::TaskDens")` in add-on package `r mlr3book::mlr_pkg("mlr3proba")`.

* **Cluster Task**: An unsupervised task type; there is no target and the aim is to identify similar groups within the feature space → `"mlr3cluster::TaskClust")` in add-on package `r mlr3book::mlr_pkg("mlr3cluster")`.

* **Spatial Task**: Observations in the task have spatio-temporal information (e.g. coordinates) → `"mlr3spatiotempcv::TaskRegrST")` or `"mlr3spatiotempcv::TaskClassifST")` in add-on package `r mlr3book::mlr_pkg("mlr3spatiotempcv")`.

* **Ordinal Regression Task**: The target is ordinal → `TaskOrdinal` in add-on package `r mlr3book::gh_pkg("mlr-org/mlr3ordinal")` (still in development).

### Task Creation {#tasks-creation}

As an example, we will create a regression task using the `"datasets::mtcars", text = "mtcars")` data set from the package `datasets`.
...
Instead, we are calling the converter `"as_task_regr()"` to convert our `data.frame()` stored in the variable `data` to a task and provide the following information:

1. `x`: Object to convert. Works for rectangular data formats such as `data.frame()`, `data.table()`, or `tibble()`.
   Internally, the data is converted and stored in an abstract `"DataBackend")`.
   This allows connecting to out-of-memory storage systems like SQL servers via the extension package `r mlr3book::mlr_pkg("mlr3db")`.
...
We can also plot the task using the `r mlr3book::mlr_pkg("mlr3viz")` package, which gives a graphical summary of its properties:
...
Note that, instead of loading all the extension packages individually, it is often more convenient to load the `r mlr3book::mlr_pkg("mlr3verse")` package instead.
...
`r mlr3book::mlr_pkg("mlr3")` includes a few predefined machine learning tasks.
...
As getting a task from a dictionary is a very common problem, `mlr3` provides the shortcut function `"tsk()"`.
Here, we retrieve the `"mlr_tasks_penguins", text = "palmer penguins classification task")`, which is provided by the package `r mlr3book::cran_pkg("palmerpenguins")`:
...
For example, `r mlr3book::mlr_pkg("mlr3data")` adds some more example and toy tasks for regression and classification, and `r mlr3book::mlr_pkg("mlr3proba")` adds survival and density estimation tasks.
Both packages are loaded automatically when the `r mlr3book::mlr_pkg("mlr3verse")` package is loaded:
...
In `r mlr3book::mlr_pkg("mlr3")`, each row (observation) has a unique identifier, stored as an `integer()`.
...
You can specify the *positive class* within the `"TaskClassif", text = "classification task")` object during task creation.
...
The `r mlr3book::mlr_pkg("mlr3viz")` package provides plotting facilities for many classes implemented in `r mlr3book::mlr_pkg("mlr3")`.
The available plot types depend on the class, but all plots are returned as `r mlr3book::cran_pkg("ggplot2")` objects which can be easily customized.
