...
This characteristic must be defined via column role `"group"` during Task creation (see also the [help page](https://mlr3.mlr-org.com/reference/Resampling.html#grouping-blocking) on this column role).
...
{mlr3} supports this via the `"custom_cv"` resampling method.
This method accepts a factor vector with the same length as `task$nrow()` or a string of an existing variable in the data.
...
The term "blocking" is overloaded in the resampling context.
In the field of spatial resampling (see `r mlr_pkg("mlr3spatiotempcv")`), "blocking" is used as a general concept for grouping observations while on the other hand there are also specific resampling methods which have "blocking" in their name (`"spcv_block") and focus on rectangular partitioning.
```
