...
The following sub-section examines the feature selection on the `r ref("mlr_tasks_sonar", text = "Sonar")` data set used to discriminate between sonar signals bounced off a metal cylinder and those bounced off a roughly cylindrical rock.
...
To start the feature selecton, we still need to select a feature selection algorithm which are defined via the `r ref("FSelector")` class.
...
To start the feature selection, we simply pass the `r ref("FSelectInstanceSingleCrit")` to the `$optimize()` method of the initialized `r ref("FSelector")`. The algortihm proceeds as follows:

1. The `r ref("FSelector")` proposes at least one feature subset  (the `r ref("FSelector")` may propose multiple subsets to improve parallelization, which can be controlled via the setting `batch_size`).
...
5. Store the best feature subset as result in the instance object.
...
The `uhash` column links the resampling iterations to the evaluated feture subsets stored in `instance$archive$data()`. This allows e.g. to score the included `r ref("ResampleResult")`s on a different measure.
...
We create a terminator which allows 10 evaluations, and use a simple random search as feature selection algorithm:
...
This time however, we pass it to `r ref("benchmark()")` to compare the optimized featue subset to the complete feature set.
