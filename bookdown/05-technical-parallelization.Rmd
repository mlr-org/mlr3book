...
Unfortunately, threading often conflicts with explicit parallelization, causing the system to be overutilized in the best case and causing hangs or segfaults in the worst case.
For this reason, we introduced the convention that implicit parallelization is turned off in the defaults, but can be enabled via a special hyperparameter which is tagged with the label `"threads"`.
...
set_threads(learner)
```
This also works for filters from `r mlr_pkg("mlr3filters")` and lists of objects:
```{r}
# retrieve 2 filters
...
# unchanged, no threading supported for the variance filter
filters[[1]]$param_set
...
We talk about explicit parallelization in mlr3 if mlr3 starts the parallelization itself.
