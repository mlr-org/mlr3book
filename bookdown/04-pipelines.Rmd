...
`mlr3pipelines` is a dataflow programming toolkit.
...
The scope of `mlr3pipelines` is still growing.
...
The building blocks of `mlr3pipelines` are **PipeOp**-objects (PO).
...
If you are unfamiliar with tuning in `mlr3`, we recommend to take a look at the section about [tuning](#tuning) first.
...
The package `mlr3filters` contains many different `mlr3filters::Filter`s that can be used to select features for subsequent learners.
