...
* `param_set`: A set of hyperparameters and their descriptions provided as a `r ref("paradox::ParamSet")`.
  ...
  * `r ref("paradox::ParamLgl")` / `r ref("paradox::p_lgl")` for scalar logical hyperparameters.
  * `r ref("paradox::ParamInt")` / `r ref("paradox::p_int")` for scalar integer hyperparameters.
  * `r ref("paradox::ParamDbl")` / `r ref("paradox::p_dbl")` for scalar numeric hyperparameters.
  * `r ref("paradox::ParamFct")` / `r ref("paradox::p_fct")` for scalar factor hyperparameters (this includes characters).
  * `r ref("paradox::ParamUty")` / `r ref("paradox::p_uty")` for everything else (e.g. vector paramters or list parameters).
...
This can be written shorter, using the `r ref("paradox::ps")` shortcut:
```{r 06-extending-learners-003-1, eval = FALSE}
      ps = ps(
