...
10. Open a pull request to \url{https://github.com/mlr-org/mlr3extralearners/pulls} with the new learner template.
...
3. `algorithm = "decision tree"` - Create the title as "Classification Decision Tree Learner", where "Classification" is determined automatically from `type` and "Learner" is add for all learners.
...
7. `caller = "rpart"` - This tells the `.train` function, and the description which function is called to run the algorithm, with `package` this automatically filles `rpart::rpart`.
...
lrns_dict$add("classif.rpart", LearnerClassifRpart)
```
...
6. Fill in the private [`.predict`](#learner-predict) method, which operates on the model in `self$model` (stored during `$train()`) and a (differently subsetted) `r ref("Task")` to return a named list ofpredictions.
...
We could omit the `pars` line as there are no parameters with the `"predict"` tag but we keep it here to be consistent.
...
Note that you cannot rely on the column order of the data returned by `task$data()`: The order of columns may be different from the order of the columns during `$train()`.
...
1. `r ref("devtools::document(roclets = c('rd', 'collate', 'namespace'))")`
1. If you haven't already done this, run: `r ref("remotes::install_github('pat-s/styler@mlr-style")`
1. `r ref("styler::style_pkg(style = styler::mlr_style")`
1. `r ref("usethis::use_tidy_description()")`
