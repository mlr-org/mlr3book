...
We can see that the price for most houses lie between 75.000 and 1.5 million dollars. 
...
Before we start with defining and training learners, we create a `r ref("Resampling")`
...
For now we do not use the **zipcode** column, as `r cran_pkg("xgboost")` can not naturally
...
**NOTE:** In order to get fast results, we decide to tune only parts of the hyperparameter space that can be adjusted for `r cran_pkg("xgboost")`.
Additionally, we only allow for $10$ iterations of random search, which is usually to little for real-world applications.
