...
The variable we want to predict is the `price` column.
...
We can now plot the density of the **price** to get some impression on its distribution.
...
Knowledge about the data, for example that if a house has `yr_renovated == 0` means that the house has not been renovated can thus be incorporated.
...
Note that we use `.$clone()` and `.$select()` to clone the task and select only a subset of the features for the autoplot, as `autoplot` per default uses all features.
...
Decision tree's can not only be used as powerful predictive models, but they can also be used as a powerful tool for exploratory data analysis.
...
The features we split along seem to be **grade**, **sqft_living**, but also some features related to the area (longitude and latitude).
...
baseline, in order to see what a simple model already can achieve.
...
Before we start with defining and training learners, we create a `r ref("Resmapling")`
in order to make sure that we always compare on exactly the same data.
...
We might be able to improve upon the **RMSE**.
We first load the `mlr3learners` package, which contains **ranger**.

```{r}
lrn_ranger = mlr_learners$get("regr.ranger", param_vals = list(ntrees = 50L))
...
Oftentimes tuning **RandomForest** methods does not increase predictive performances drastically. If time permits, it can nonetheless lead to improvements and should thus be tried out. In this case, we resort to tune a different kind of model: **Gradient Boosted Decision Trees** from the package `r pkg("xgboost")` instead.
Those models often outperform **Random Forests** but on the other hand require extensive tuning.
...
For now we do not use the **zipcode** column, as `r pkg("xgboost")` can not naturally
...
ps = ParamSet$new(
  params = list(
    ...
    ParamInt$new(id = "nrounds", lower = 1L, upper = 50)
))
...
We can obtain the resulting params in the respective splits by accessing the 
`r ref("ResampleResult")`.

```{r}
sapply(res$learners, function(x) x$param_set$values)
```

In order to get fast results, we decide to tune only parts of the hyperparameter space that can be adjusted for `r pkg("xgboost")`.
Additionally, we only allow for $20$ iterations of random search, which is usually to little for real-world applications.
Nonetheless, we are able to obtain an improved performance when comparing to the `r pkg("ranger")` model.

In order to further improve our results we have several options:

- Find or Engineer better features
- Obtain additional data (often prohbitive)
- Try more models
- Removing Features to avoid overfitting
- Improve the tuning
  - Increase the tuning budget
  - Enlarge the tuning search space
- Stacking and Ensembleing (See Pipelines!)

Below we will investigate some of those possibilities and investigate whether this improves performance.

### Engineer better Features: Deal with ZIP-Codes
...
For this purpose we can use a `r ref("Filter")` implemented in `r mlr_pkg("mlr3featsel")`.
This can prevent our learner from overfitting, and at the same time makes it easier for humans to interpret models, as fewer variables influence the resulting prediction.
...
The resulting **RMSE** is slightly higher, but at the same time we only use $12$ features.
...
they have learned. The package `r pkg("iml")` has many useful post-hoc interpretation methods, such as Partial Dependency Plots, Ale Plots, ICE Curves and many more.
