...
* `predict_types`: Set of predict types the learner is capable of. These differ depending on the type of learner.
    * `LearnerClassif`
        * `response`: Predicts only a class label for each observation in the test set.
        * `prob`: Also predicts the posterior probability for each class for each observation in the test set.
    * `LearnerRegr`
        * `response`: Predicts only a numeric response for each observation in the test set.
