...
[Classification Task](#cost-sens): The target is a label (stored as `character()`or`factor()`) with only few distinct values.
<br/>→ `r ref("mlr3::TaskClassif")`

[Regression Task](#functional): The target is a numeric quantity (stored as `integer()` or `double()`).
...
Changing the role does not change the underlying data, but only the view on it -- the data is not copied in the code above.
