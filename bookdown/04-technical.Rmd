...
With its default settings, the learner will do nothing special: it learns a random label and creates constant predictions.
```{r 04-technical-004}
task = mlr_tasks$get("iris")
...
After training the learner, we can access the recorded log via the fields `log`, `warnings` and `errors`:
...
bmr = benchmark(expand_grid("iris", list(learner, "classif.rpart"), "cv"))
...
```{r 04-technical-031, R.options=list(width = 120)}
as.data.table(parA)
```
...
```{r 04-technical-035, R.options=list(width = 120)}
as.data.table(ps)
```
...
```{r 04-technical-048, echo = FALSE, out.width="45%", fig.show = "hold", fig.width = 4, fig.height = 4}
par(mar=c(4, 4, 2, 1))
