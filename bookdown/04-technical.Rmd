...
This chapter give an overview about technical details of the `r mlr_pkg("mlr3")` framework.

At first, some details about parallelization and the usage of the `r cran_pkg("future")` is given.
...
For the next example, we opt to signal an error during the train step and then try to train the learner on the `r ref("mlr_tasks_spam", text = "spam")` data set:
```{r, error = TRUE}
learner$param_set$values = list(error_train = TRUE)
learner$train("spam")
```
If this would be a regular learner, we could now start debugging with `r ref("traceback()")` (or create a [MRE](https://stackoverflow.com/help/minimal-reproducible-example) to fill a bug report).
...
With encapsulation, exceptions do not stop the program flow and all output is logged to the learner instead of just printed to the console.
Each `r ref("Learner")` has a field `encapsulate` to control how the train or predict step are executed.
One way to encapsulate the execution is provided by the package `r cran_pkg("evaluate")`, connected via the function `r ref("encapsulate()")` in `r mlr_pkg("mlr3misc")`:
...
learner$train(task)
```
After training the learner, we can access its log or directly access all recorded warnings and errors:
```{r}
learner$log
learner$warnings
learner$errors
...
Another possibility to encapsulate is by running the steps via the `r cran_pkg("callr")` package.
...
To handle the missing predictions in a graceful way during `r ref("resample()")` or `r ref("benchmark()")`, we next introduce fallback learners.

### Fallback learners

Fallback learners have the purpose to be able to continue with the computation in cases where a `r ref("Learner")` or a `r ref("Measure")` are misbehaving in some sense.
...
* The learner fails to predict for some or all observations.
  A typical case are new factor levels in the test data.
...
If the learner fails, we rely on a second learner to generate predictions: the fallback learner.

In this example, in addition to the debug learner, we also fit a simple featureless learner (predicting the majority class in each resampling split).
So whenever the debug learner fails (which is every time) and encapsulation in enabled, the featureless learner internally drops in to generate predictions:
...
learner$train(task)
```
Note that the log contains the captured error, and although we don't have a model, we can get predictions:
```{r}
learner$errors
learner$model
...
prediction$score()
```
While the fallback learner is of limited use for this stepwise train-predict procedure, it allows us to create scores to aggregate during resampling or benchmarking.
In the following snippet we compare the previously created debug learner with a simple classification tree.
But as the debug learner fails to fit a model every time, we are basically comparing the fallback learner of the debug learner with the classification tree:
```{r}
bmr = benchmark(expand_grid("spam", list(learner, "classif.rpart"), "cv3"))
aggr = bmr$aggregate(errors = TRUE)
aggr
```
To further investigate the errors, we can extract a `r ref("ResampleResult")`:
```{r}
...
Another problem emerges if a learner returns the prediction partially, e.g., if a learner predicts `NA` as class label for some observations.
...
When comparing the performance of these two algorithms, it is obviously not fair to just ignore the missing values and average only over the "easy-to-predict" observations for algorithm B.
Algorithm B would then easily outperform algorithm A which also tries to predict observations near the decision boundary.
On the other hand, it is also not feasible to exclude all observations from the test set where at least one algorithm failed to predict a label.
Instead, we proceed by imputing all missing predictions with something naive, e.g., the majority class.
And as the majority class may depend on the resampling split (or we opt for some other arbitrary baseline), it is best to just train a second learner on the same resampling split.
