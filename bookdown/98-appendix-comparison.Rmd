...
impute = po("impute")
filter = po("filter", filter = FilterVariance$new(), param_vals = list(filter.nfeat = 2L))
rpart = po("learner", lrn("classif.rpart"))

# Assemble the Pipeline
pipeline = impute %>>% filter %>>% rpart
# And convert to a 'GraphLearner'
learner = as_learner(id = "Pipeline", pipeline)
```
...
copy = po("copy", 2)
pca = po("pca")
selection = po("filter", filter = FilterVariance$new())
union = po("featureunion", 2)
svm = po("learner", lrn("classif.svm", param_vals = list(kernel = "linear")))
...
resampling = rsmp("cv", param_vals = list(folds = 5L))
