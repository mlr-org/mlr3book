...
op1 = PipeOpScale$new()
op2 = PipeOpPCA$new()
op3 = PipeOpLearner$new(learner = lrn("classif.rpart"))
```
...
rpart = PipeOpLearner$new(lrn("classif.rpart"))
...
```{r 98-appendix-comparison-010-2, eval = FALSE}
library("mlr3verse")
iris = tsk("iris")
...
svm = PipeOpLearner$new(lrn("classif.svm", kernel = "linear"))
...
# Create the task
tsk = as_task_classif(credit_data, target = "Status")

# Build up the Pipeline:
g = PipeOpImputeSample$new(id = "impute") %>>%
  PipeOpEncode$new(param_vals = list(method = "one-hot")) %>>%
  PipeOpScale$new() %>>%
  PipeOpLearner$new(lrn("classif.ranger", num.trees = 200, mtry = 12))
